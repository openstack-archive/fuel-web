...
.. note:: Any package specified in bootstrap building procedure
    must be listed in the requirements-rpms.txt file explicitly.
...
* you create a request to add these packages to Fuel requirements-*.txt
  ...
  You receive +1 vote from Fuel CI in case if these packages already exist
  on either Fuel internal mirrors or upstream mirrors for respective OS
