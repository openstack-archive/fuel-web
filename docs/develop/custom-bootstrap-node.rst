...
#. Repack the CentOS disk driver:
   ...
   ::
      ...
      upack the dd-src folder
      tar -xzvf dd-src.tar.gz
      find dd-src/ | cpio --quite -o -H newc | gzip -9 > /tmp/initrd_update.img
...
#. Modify it as you need. For example, copy files or modify the scripts:
   ...
   ::
      ...
      echo "depmode -a; modprobe hpvsa" > etc/modprobe.d/hpvsa.conf
...
#. Copy the new initramfs image into the bootstrap folder:
   ...
   ::
      ...
      dockerctl copy /var/www/nailgun/bootstrap/initramfs.img /vacobbler:/var/lib/tftpboot/images/bootstrap/initramfs.img
