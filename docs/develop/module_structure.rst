...
As a Puppet modules developers, we are tending to collaborate with Puppet
Openstack community and contribute to upstream modules all of improvements,
fixes and customizations we are making to improve Fuel as well.
That implies what every contributor should follow Puppet DSL basics and 
`puppet-openstack dev docs
<https://wiki.openstack.org/wiki/Puppet-openstack#Developer_documentation>`_

The most common and general rule is: upstream modules should be modified only
with bugfixes and improvements everyone in the community could benefit from.
And appropriate patch should be proposed to the upstream project prior to
Fuel project.
...
In case there is no ``Modulefile`` in module's directory, contributor could
submit patch directly to this module in Fuel library.
Otherwise, he or she should submit patch to upstream module first, and once
merged or +2 recieved from core reviewer, backport them to Fuel library
as well. Note that the patch submitted for Fuel library should contain in commit
message an upstream commit ID or link to github pull-request
(if the module is not on stackforge) or link to corresponding gerrit patch.
