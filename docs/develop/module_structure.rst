...
After style checks, test suite is executing Python tests by using `py.test <http://pytest.org>`_ test runner.
`py.test` is looking for Python files whose names begin with 'test\_' and searches for the tests in them.
