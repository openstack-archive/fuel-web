...
As Puppet developers, we tend to collaborate with Puppet Openstack community.
As a result, we contribute to upstream modules all of improvements,
fixes and customizations we make to improve Fuel as well.
...
In case there is no ``Modulefile`` in module's directory, contributor may
...
message the upstream commit ID or link to github pull-request
