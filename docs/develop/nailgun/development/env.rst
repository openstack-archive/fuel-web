...
#. Nailgun unit tests use `Tox <http://testrun.org/tox/latest/>`_ for generating test
   environments. So, you don't need to install all Python packages required for project
   to run them, because Tox does it by itself.
...
#. Tox reuses previously created environment, so after doing some changes with package
