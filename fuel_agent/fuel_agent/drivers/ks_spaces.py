...
def _match_device(hu_disk, ks_disk):
...
class KsSpaces(object):
    def __init__(self, provision_data):
        if isinstance(provision_data, (str, unicode)):
            self.provision_data = json.loads(provision_data)
        else:
            self.provision_data = provision_data


        self.data = self.provision_data['ks_meta']['pm_data']['ks_spaces']
        self.kernel_params = \
            self.provision_data['ks_meta']['pm_data']['kernel_params']

        self._validate(self.data)

        # Getting actual disks which are available on this node
        ...
        self.hu_disks = hu.list_block_devices(disks=True)

        # getting ks disks
        disk_filter = lambda x: x['type'] == 'disk' and x['size'] > 0
        self.ks_disks = filter(disk_filter, self.data)

        # getting ks volume groups
        vg_filter = lambda x: x['type'] == 'vg'
        self.ks_vgs = filter(vg_filter, self.data)

        self.scheme = objects.Scheme()

    def _validate(self, scheme):
        ks_spaces_validator.validate(scheme)
    ...
    def _disk_dev(self, ks_disk):
        ...
        matched = [hu_disk['device'] for hu_disk in self.hu_disks
                   if _match_device(hu_disk, ks_disk)]
    ...
    def parse(self):
        for disk in enumerate(self.ks_disks):
            ...
            for volume in disk['volumes']:
                ...
                if volume['type'] == 'pv':
                    self.scheme.vg_attach_by_name(
                        pvname=prt.name, vgname=volume['vg'])

                if volume['type'] == 'raid':
                    if 'mount' in volume and volume['mount'] != 'none':
                        self.scheme.md_attach_by_mount(
                            device=prt.name, mount=volume['mount'],
                            fs_type=volume.get('file_system', 'xfs'),
                            fs_label=self._getlabel(volume.get('disk_label')))
        ...
        for vg in enumerate(self.ks_vgs):
            for volume in vg['volumes']:
                ...
                if volume['type'] == 'lv':
                    lv = self.scheme.add_lv(name=volume['name'],
                                            vgname=vg['id'],
                                            size=volume['size'])
