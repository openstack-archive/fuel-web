...
class Nailgun(object):
    ...
    def configdrive(self):
        ...
        configdrive = objects.ConfigDrive()

        configdrive.boothook = {
            'udevrules': data['kernel_options']['udevrules'],
            'admin_mac': data['kernel_options']['netcfg/choose_interface'],
            'server': data['ks_meta']['mco_host']
        }

        configdrive.set_puppet(master=data['ks_meta']['puppet_master'])
        configdrive.set_mcollective(
        ...
        configdrive.set_common(
            ssh_auth_key=data['ks_meta']['auth_key'],
            hostname=data['hostname'],
            fqdn=data['hostname'],
            name_servers=data['name_servers'],
            search_domain=data['name_servers_search'],
            master_node=data['ks_meta']['master_node'],
            master_url='http:/%s:8000/api' % self.data['master_node'],
            udevrules=data['kernel_options']['udevrules'],
            admin_mac=data['kernel_options']['netcfg/choose_interface'],
