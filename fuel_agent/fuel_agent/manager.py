        ...
        if self.data_uri.startswith('http://'):
            ...
            try:
                ...
                if r.status_code == 200:
                    self.provision_data = r.json()
                else:
                    raise errors.ProvisionDataHttpError(
                        'Error while pulling json data over http from %s, '
                        'responce code %s' % (self.data_uri, r.status_code))
            except ValueError:
                raise errors.ProvisionDataMalformed(
                    'Error while decoding json datafrom %s' % self.data_uri)
            except errors.ProvisionDataHttpError:
                raise
            except Exception:
                raise errors.ProvisionDataHttpError(
