...
class Manager(object):
    ...
    def do_configdrive(self):
        ...
        utils.render_and_save(
            tmpl_dir, self.driver.configdrive_scheme.template_names('cloud_config'),
            self.driver.configdrive_scheme.template_data(), cc_output_path
        )
        utils.render_and_save(
            tmpl_dir, self.driver.configdrive_scheme.template_names('boothook'),
            self.driver.configdrive_scheme.template_data(), bh_output_path
        )
        utils.render_and_save(
            tmpl_dir, self.driver.configdrive_scheme.template_names('meta-data'),
    ...
    def umount_target(self, chroot):
        ...
        key = lambda x: len(x.mount.rstrip('/').split('/'))
        for fs in sorted(self.driver.partition_scheme.fss, key=key, reverse=True):
            if fs.mount == 'swap':
                continue
    ...
    def do_bootloader(self):
        ...
        with open(chroot + '/etc/udev/rules.d/70-persistent-net.rules',
            ...
            for mapping in self.driver.configdrive_scheme.common.udevrules.split(','):
