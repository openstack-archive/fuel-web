...
class Manager(object):
    ...
    def do_build_image(self):
        ...
        LOG.debug('*** Checking if image exists ***')
        if all([os.path.exists(i.uri.split('file://', 1)[1])
                for i in self.driver.image_scheme.images]):
        ...
        if not os.path.isdir(proc_path):
            os.makedirs(proc_path, mode=0o755)
        fu.mount_bind(proc_path)
