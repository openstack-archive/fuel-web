...
class BuildUtilsTestCase(testtools.TestCase):
    ...
    def test_get_free_loop_ok(self, mock_exec, mock_path, mock_mknod,
        ...
        expected_mock_path_calls = [mock.call('/dev/loop0'),
                                    mock.call('/dev/loop1'),
                                    mock.call('/dev/loop2'),
                                    mock.call('/dev/loop3'),
                                    mock.call('/dev/loop4'),
                                    ...
                                    mock.call('/dev/loop6'),
                                    mock.call('/dev/loop7')]
        expected_mock_mknod_calls = [
            mock.call('/dev/loop0', 25008, 'fake_dev'),
            mock.call('/dev/loop1', 25008, 'fake_dev'),
            mock.call('/dev/loop2', 25008, 'fake_dev'),
            mock.call('/dev/loop3', 25008, 'fake_dev'),
            mock.call('/dev/loop4', 25008, 'fake_dev'),
            mock.call('/dev/loop5', 25008, 'fake_dev'),
            mock.call('/dev/loop6', 25008, 'fake_dev'),
            mock.call('/dev/loop7', 25008, 'fake_dev')]
        expected_mock_mkdev_calls = [
            mock.call(CONF.loop_dev_major, 0),
            mock.call(CONF.loop_dev_major, 1),
            mock.call(CONF.loop_dev_major, 2),
            mock.call(CONF.loop_dev_major, 3),
            mock.call(CONF.loop_dev_major, 4),
            mock.call(CONF.loop_dev_major, 5),
            mock.call(CONF.loop_dev_major, 6),
            mock.call(CONF.loop_dev_major, 7)]
        self.assertEqual(expected_mock_mknod_calls, mock_mknod.call_args_list)
        self.assertEqual(expected_mock_mkdev_calls, mock_mkdev.call_args_list)
        self.assertEqual(expected_mock_path_calls,
                         mock_path.exists.call_args_list)

    @mock.patch.object(os, 'makedev')
    @mock.patch.object(os, 'mknod')
    @mock.patch.object(os, 'path')
    @mock.patch.object(utils, 'execute')
    def test_get_free_loop_too_many(self, mock_exec, mock_path, mock_mknod,
                                    mock_mkdev):
        mock_exec.return_value = ('', 'Error!!!')
        mock_path.exists.return_value = False
        mock_mkdev.return_value = 'fake_dev'
        self.assertRaises(errors.TooManyLoopDevices, bu.get_free_loop)

    @mock.patch('tempfile.NamedTemporaryFile')
    @mock.patch.object(utils, 'execute')
    def test_create_sparsed_tmp_file(self, mock_exec, mock_temp):
        tmp_file = mock.Mock()
        tmp_file.name = 'fake_name'
        mock_temp.return_value = tmp_file
        bu.create_sparsed_tmp_file('dir', 'suffix')
