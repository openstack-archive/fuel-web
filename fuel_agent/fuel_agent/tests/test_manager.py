...
class TestManager(test_base.BaseTestCase):
    ...
    def test_do_build_image(self, mock_os_path_exists, mock_sparse,
                            mock_free_loop, mock_attach, mock_makefs,
                            mock_mkdtemp, mock_mount_target, mock_serv_start,
                            mock_deboot, mock_apt_env, mock_pre_apt,
                            mock_apt_src, mock_apt_pref, mock_os_path_isdir,
                            mock_os_makedirs, mock_mount_bind, mock_run_apt,
                            mock_post_inst, mock_sign_proc, mock_umount_fs,
                            mock_umount_target, mock_detach_loop, mock_shrink,
                            mock_os_path_getsize, mock_md5, mock_containerize,
                            mock_shutil_move, mock_open, mock_sleep):
        ...
        mock_os_path_getsize.side_effect = [20, 2, 10, 1]
        mock_md5.side_effect = ['fakemd5_raw', 'fakemd5_raw_boot',
                                'fakemd5_gzip', 'fakemd5_gzip_boot']
        ...
        mock_mkdtemp.called_once_with(dir=CONF.image_build_dir,
                                      suffix=CONF.image_build_suffix)
        mock_mount_target.called_once_with(
            '/tmp/imgdir', treat_mtab=False, pseudo=False)
        self.assertEqual([mock.call('/tmp/imgdir')] * 2,
                         mock_serv_start.call_args_list)
        mock_deboot.called_once_with(uri='http://fakeubuntu', suite='trusty',
                                     chroot='/tmp/imgdir')
        mock_apt_env.called_once_with()
        mock_pre_apt.called_once_with('/tmp/imgdir')
        self.assertEqual([
        ...
        mock_os_path_isdir.called_once_with('/tmp/imgdir/proc')
        mock_mount_bind.called_once_with('/tmp/imgdir', '/proc')
        mock_run_apt.called_once_with('/tmp/imgdir',
                                      packages=['fakepackage1', 'fakepackage2'])
        mock_post_inst.called_once_with('/tmp/imgdir')
        self.assertEqual([mock.call('/tmp/imgdir', signal.SIGTERM),
                          mock.call('/tmp/imgdir', signal.SIGKILL)],
                         mock_sign_proc.call_args_list)
        mock_sleep.called_once_with(2)
        mock_umount_target.called_once_with('/tmp/imgdir', pseudo=False)
        ...
        mock_open.called_once_with('/fake/img.yaml')
        self.assertEqual(
