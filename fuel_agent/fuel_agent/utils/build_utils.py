...
def add_apt_source(chroot, name, url, suite, section):
    ...
    filename = 'fuel-image-%s.list' % name
    if section:
        entry = "deb %s %s %s\n" % (url, suite, section)
    else:
        entry = "deb %s %s\n" % (url, suite)
...
def add_apt_preference(chroot, name, priority, suite, section):
    ...
    filename = 'fuel-image-%s.pref' % name
        ...
        with open(os.path.join(chroot, pref_dir, filename), 'w') as f:
            ...
            if sections:
                for section in sections:
                    f.write('Pin: release a=%s,c=%s\n' % (suite, section))
            else:
                f.write('Pin: release a=%s\n' % suite)
            f.write('Pin-Priority: %s\n' % priority)


def run_pre_apt_get(chroot):
