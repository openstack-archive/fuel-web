...
def send_signal_to_chrooted_processes(chroot, signal, attempts=10,
                                      attempts_delay=2):
    """Sends signal to all processes, which are running inside of chroot.
    It tries several times. If at some point there are no running processes
    found, it returns True.

    :param chroot: Process root directory.
    :param signal: Which signal to send to processes (e.g. SIGKILL)
    ...
    return bool(get_running_processes())
