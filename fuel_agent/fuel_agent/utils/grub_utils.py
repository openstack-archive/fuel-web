...
def guess_grub(chroot=''):
    for grub in ('/sbin/grub', '/usr/sbin/grub'):
        ...
        if os.path.isfile(chroot + grub):
            ...
            return grub
    raise Exception('grub not found')
...
def guess_grub_install(chroot=''):
    for grub_install in ('/sbin/grub-install', '/sbin/grub2-install',
        ...
        if os.path.isfile(chroot + grub_install):
            ...
            return grub_install
    raise Exception('grub-install not found')
...
def guess_kernel(chroot=''):
    for filename in sorted(os.listdir(chroot + '/boot'), reverse=True):
        ...
        if filename.startswith('vmlinuz'):
            return filename
    raise Exception('Error while trying to find kernel: not found')
...
def guess_initrd(chroot=''):
    for filename in sorted(os.listdir(chroot + '/boot'), reverse=True):
        ...
        if filename.startswith('initramfs') or \
            ...
            return filename
    raise Exception('Error while trying to find initrd: not found')


def grub1_install(install_devices, boot_device, chroot=''):

    # Checking whether all install devices are disks
    if set(install_devices) & \
            set([d['device'] for d in hu.list_block_devices()]) != \
            set(install_devices):
        raise Exception(
            'Error while installing legacy grub: '
            'install device must be disk')
...
def grub1_mbr(install_device, boot_disk, boot_part, chroot=''):
    ...
    batch += 'root (hd1,{0})\n'.format(boot_part)
    batch += 'install /grub/stage1 d (hd0) /grub/stage2 p /grub/grub.conf\n'
    batch += 'quit'
    ...
    os.chmod('/tmp/grub.sh', 0755)
    utils.execute('{0} {1} /tmp/grub.sh'.format(
        'chroot' if chroot else '', chroot),
        run_as_root=True, check_exit_code=[0])


def grub1_cfg(kernel=None, initrd=None,
              kernel_params='root=LABEL=/', chroot=''):
...
def grub2_install(install_devices, chroot=''):
    ...
    for install_device in install_devices:
        utils.execute('chroot' if chroot else '', chroot,
                      grub_install, install_device,
                      run_as_root=True, check_exit_code=[0])


def grub2_cfg(kernel_params, chroot=''):
    grub_defaults = chroot + guess_grub2_default()
    ...
    with open(grub_defaults, 'wb') as f:
        f.write(new_content)
    utils.execute('chroot' if chroot else '', chroot,
                  guess_grub2_mkconfig(), '-o', guess_grub2_conf())
