...
class NailgunClient(object):
    ...
    def get_release_attributes(self, release_id):
        return self._request("get", "/api/releases/{0}/".format(release_id))

    def update_release_attributes(self, release_id, attrs):
        return self._request(
            "put", "/api/releases/{0}/".format(release_id), attrs)
    ...
    def get_releases(self):
        return self._request("get", "/api/releases/")
...
class FuelWebClient(object):
    ...
    def get_available_releases(self, operating_system=None):
        releases = self.client.get_releases()
        if operating_system is not None:
            ops = operating_system.lower()
            return filter(lambda x: x['operating_system'].lower() == ops,
                          releases)
        else:
            return releases
    ...
    def get_release_id(self, operating_system, release_version):
        for release in self.client.get_releases():
            if (release["operating_system"].lower() == operating_system.lower()
                    and release["version"] == release_version):
                return release["id"]

    def update_cluster_repos(self, cluster_id, settings=None):
        if settings is None:
            settings = {}
        ...
        logger.debug("Try to update cluster "
                     "with following attributes {0}".format(attributes))
        self.client.update_cluster_attributes(cluster_id, attributes)

    def update_default_repos(self, release_id, settings=None):
        if settings is None:
            settings = {}
        ...
        if 'repo_setup' in attributes['attributes_metadata']['editable']:
            repos_attr = (attributes['attributes_metadata']['editable']
                                    ['repo_setup']['repos'])
            repos_attr['value'] = utils.repo_merge(repos_attr['value'],
                                                   settings)
