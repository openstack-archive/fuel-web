...
class Settings(object):
    ...
    updates_destinations = {
        ...
        'ubuntu-baseos': r'/var/www/nailgun/{0}/ubuntu/{0}'.format(
            UBUNTU_CODENAME),
    }
    mirror_base = "http://mirror.fuel-infra.org/mos"
    default_mirrors = {
        'centos': '{0}/{1}/mos{2}/updates/'.format(mirror_base, CENTOS_VERSION,
                                                   FUEL_VER),
...
def repo_merge(list_a, list_b):
    ...
    result = OrderedDict()
    # to set repos from list_b as primary
    for repo in list_b:
...
def main():
    ...
    parser.add_option('-c', '--clear-upstream-repos',
                      ...
                      help="Clears upstream repos if ubuntu-baseos distro is "
                      "chosen. By default just replacing their's URIs")
