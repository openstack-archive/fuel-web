...
class BaseChecker(object):
    ...
    def save_get(self, url, auth=None):
        ...
        """
        try:
            result = requests.get(url, auth=auth, timeout=0.5)
...
class NailgunChecker(BaseChecker):
    ...
    checker_name = 'nailgun'

    def check(self):
        _, code = self.save_get(
...
class OSTFChecker(BaseChecker):
    ...
    def check(self):
        _, code = self.save_get('http://{host}:{port}/ostf/not_found'.format(
            **self.endpoints['ostf']))
...
class RabbitChecker(BaseChecker):
    ...
    checker_name = 'rabbitmq'

    def check(self):
        nodes, status_code = self.save_get(
...
class MCollectiveChecker(BaseChecker):
    ...
    checker_name = 'mcollective'

    def check(self):
        exchanges, code = self.save_get(
...
class NginxChecker(BaseChecker):
    ...
    def check(self):
        _, nailgun_code = self.save_get(
            'http://{host}:{port}/'.format(**self.endpoints['nginx_nailgun']))
        _, nginx_code = self.save_get(
...
class IntegrationCheckerPostgresqlNailgunNginx(BaseChecker):
    ...
    checker_name = 'integration_postgres_nailgun_nginx'

    def check(self):
        releases, code = self.save_get(
...
class IntegrationCheckerRabbitMQAstuteNailgun(BaseChecker):
    ...
    checker_name = 'integration_rabbitmq_astute_nailgun'
