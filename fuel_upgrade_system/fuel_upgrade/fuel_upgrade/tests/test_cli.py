...
@mock.patch('fuel_upgrade.cli.build_config', mock.MagicMock())
@mock.patch('fuel_upgrade.cli.get_keystone_token')
class TestAdminPassword(BaseTestCase):
    ...
    def test_use_password_arg(self, mget_token):
        password = '12345678'
        args = self.get_args(self.default_args + ['--password', password])
        run_upgrade(args)

        mget_token.assert_called_once_with(
            mock.ANY, password
        )

    @mock.patch('fuel_upgrade.cli.getpass')
    def test_ask_for_password(self, mgetpass, mget_token):
        password = '987654321'
