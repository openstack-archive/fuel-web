...
class TestConfigHidePasswords(BaseTestCase):
    ...
    def test_hide_data(self):
        self.assertEqual(
            sanitize(self.original_conf, ['password', 'token']),
            self.expected_conf
        )

    def test_original_config_unchanged(self):
        copy_conf = deepcopy(self.original_conf)
        sanitize(self.original_conf, ['password', 'token'])
        self.assertEqual(self.original_conf, copy_conf)

    def test_custom_mask(self):
        self.assertEqual(
            sanitize(self.original_conf, ['password', 'token'], mask='XXX'),
