...
class ReleaseAction(Action):
    ...
    def deployment_tasks(self, params):
        ...
        if params.download:
            ...
            print("Deployment tasks for release {0} uploaded from {1}.".format(
                release.id, dir_path))
    ...
    def sync_deployment_tasks(self, params):
        ...
        files = list(utils.find_files(params.dir, ('tasks.yaml',)))
        serialized_tasks = defaultdict(list)
        versions = [r['version'] for r in all_rels]

        #Group tasks for each release
        for file_name in files:
            for version in versions:
                if version in file_name:
                    serialized_tasks[version].extend(
                        self.serializer.read_from_file(file_name))
        ...
        for rel in all_rels:
            ...
            if data:
                ...
                print("Deployment tasks syncronized for release"
                      " {0} of version {1}".format(rel['id'], rel['version']))
            else:
                print("No tasks found for release {0} "
                      "of version {1}".format(rel['id'], rel['version']))
