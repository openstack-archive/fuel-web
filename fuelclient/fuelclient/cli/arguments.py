        ...
        if values:
            ...
            if input_macs:
                ...
                nodes_mac_to_id_map = dict(
                    (n["mac"], n["id"])
                    for n in server.get_request("nodes/")
                )
            ...
            setattr(namespace, self.dest, map(Node.get_by_id, only_ids))
...
class FuelVersionAction(argparse._VersionAction):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        parser.exit(message=APIServer().get_fuel_version())
...
def get_arg(name, flags=None, aliases=None, help=None, **kwargs):
    if "_" in name:
        name = name.replace("_", "-")
    args = ["--" + name, ]
    ...
    if aliases is not None:
        substitutions.update(
            {alias: args[0] for alias in aliases}
        )
    all_args = {
        "args": args,
        "params": {
...
def get_nst_arg(help_msg):
    return get_arg("net_segment_type",
                   flags=("--nst",),
                   action="store",
                   choices=("gre", "vlan"),
                   help=help_msg,
...
def get_create_arg(help_msg):
    return get_boolean_arg("create", flags=("-c", "--env-create"), help=help_msg)
...
def get_dir_arg(help_msg):
    return get_str_arg("dir", help=help_msg)
...
def get_release_arg(help_msg, required=False):
    return get_str_arg("release", flags=("--rel",), required=required, help=help_msg)
...
def get_task_arg(help_msg):
    return get_multinum_arg("task", flags=("--tid", "--task-id"), help=help_msg)
