...
class Plugins(base.BaseObject):
    ...
    def run_script(cls, cmd, cwd=None):
        ...
        for each in child.stdout:
            self.connection.print_debug(each.rstrip())
        child.wait()

        if exit_code != 0:
            raise error.GenericProcessingError(
                u'Shell command executed with "{0}" '
                'exit code: {1} '.format(exit_code, cmd))

        self.connection.print_debug(
            u'Command "{0}" successfully executed'.format(cmd))
    ...
    def install_plugin(cls, plugin_path, force=False):
        ...
        try:
            ...
            install_script = metadata.get('install_script', None)
