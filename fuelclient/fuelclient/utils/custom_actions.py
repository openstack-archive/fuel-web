...
class NetworkAdapterAction(argparse.Action):
    """Checks if network segmentation type argument is
    supplied in case when user has chosen 'neutron' value for
    network type argument
    """

    def __call__(self, parser, namespace, values, option_string=None):
        if getattr(namespace, 'net') == 'neutron':
            if not values:
                parser.error('--network-segmentation-type is required')
