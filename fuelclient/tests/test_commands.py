...
class TestEnvUpdate(BaseTestCommand):
    ...
    def setUp(self):
        self.fake_stdout = FakeStdout()
        self.cmd = environment.EnvUpdate(MyApp(self.fake_stdout), None)

    def test_env_update_successfully(self):
        cmd_args = ['1', 'test', 'multinode']
        self.verify_show_env_command(cmd_args,
                                     environment.EnvUpdate.columns_names,
                                     self.show_one_output_parser,
                                     self.return_data[0])
...
class TestEnvUpgrade(BaseTestCommand):
    cmd_args = ['1', '2']
    request_to_mock = 'put_request'
    return_value = {'id': 1}
    ...
    def setUp(self):
        self.fake_stdout = FakeStdout()
        self.cmd = environment.EnvUpgrade(MyApp(self.fake_stdout), None)
