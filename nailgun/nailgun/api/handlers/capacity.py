...
class CapacityLogCsvHandler(object):
    def _checksum(self, report):
        return \
            reduce(lambda x, y: x + y, map(ord, json.dumps(report)))
    ...
    def GET(self, log_id=None):
        ...
        csv_file.writerow(['Checksum', self._checksum(report)])
        ...
        csv_file.writerow(['Total number of nodes',
                           report['allocation_stats']['total']])
