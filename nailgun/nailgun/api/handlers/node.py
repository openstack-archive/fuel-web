...
class BaseNodeHandler(BaseHandler):
    ...
    def register_node(self, node_data):
        ...
        return web.created, node

    def update_node_state(self, node, nd, is_agent=False):
    ...
    def update_nodes_state(self, nodes_data, is_agent=False):
        ...
        nodes_to_update = [
            (self.get_node_by_data(nd), nd) for nd in nodes_data
        ]
        return [
            self.update_node_state(n, d)[1] for n, d in nodes_to_update if n
        ]
...
class NodeAgentHandler(BaseNodeHandler):
    ...
    def PUT(self):
        ...
        try:
            node_data = self.validator.validate_json(web.data())[0]
        except errors.InvalidData as e:
            raise web.badrequest(e.message)

        node = self.get_node_by_data(node_data)

        if node:
            try:
                node_data = self.validator.validate_collection_update(
                    web.data()
                )[0]
                # caching by simply hashing raw put data from node
                node_data['meta_checksum'] = hashlib.sha1(
                    web.data()
                ).hexdigest()
                status, node = self.update_node_state(node, node_data, True)
            except errors.InvalidData as e:
                raise web.badrequest(e.message)
        else:
            try:
