...
class NodeAgentHandler(BaseHandler):
    ...
    def PUT(self):
        ...
        if node.status in ('provisioning', 'error'):
            if nd.get('status', 'discover') == 'discover':
                logger.debug(
        ...
        meta = nd.get('meta', {})
        if meta and len(meta.get('disks', [])) == 0 and node.meta.get('disks'):
            logger.warning(
