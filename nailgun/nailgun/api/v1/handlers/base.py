...
class Scope(object):
    ...
    def __init__(self, limit=None, offset=None, order_by=None):
        ...
        self.limit = limit if limit is not None else self.get_limit()
        # dsutyagin: set offset to 0 if limit = 0 to allow
        # collection.scope() populate Content-Range header with 0-0/total
        self.offset = offset or self.get_offset() if self.limit != 0 else 0
        self.order_by = order_by or self.get_order_by()
