...
class Pagination(object):
    ...
    def strip_int(self, x):
        if x is not None:
            try:
                return int(x.strip()) if isinstance(x, basestring) else int(x)
            except:
                raise BaseHandler.http(400, 'Cannot convert "%s" to int' % x)
    ...
    def get_limit(self, limit):
        ...
        limit = limit if limit is not None else web.input(limit=None).limit
        return self.strip_int(limit)

    def get_offset(self, offset):
        offset = offset or web.input(offset=None).offset
        return self.strip_int(offset)

    def get_order_by(self, order_by):
        order_by = order_by or web.input(order_by=None).order_by
        if order_by:
            order_by = [s.strip() for s in order_by.strip().split(',')
                        if s.strip()]
    ...
    def __init__(self, limit=None, offset=None, order_by=None):
        ...
        # dsutyagin: set offset to 0 if limit = 0 to collection.scope() to
        # populate ContentRange structure accordingly,
        # to finally set Content-Range header to "0-0/total", indicating empty
        # response
        self.offset = self.get_offset(offset) if self.limit != 0 else 0
