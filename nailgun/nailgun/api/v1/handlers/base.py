...
class CollectionHandler(BaseHandler):
    ...
    def get_scope(self):
        ...
        If limit is set to 0, an emply responce will be given
    ...
    def get_scoped_query(self, scope=None, filter_by=None):
        """Return a filtered+paged query and content range tuple

        Return a scoped query and values to later set Content-Range header
        if the query represents a subset of collection's objects.
        If scope is not set, return query to all collection's objects
        (see NailgunCollection.scope)
        Allows getting object count via conent range by specifying
        scope['limit']=0
            ...
            if limits:
                if limits['valid']:
                    content_range = (limits['from'],
                                     limits['to'],
                                     limits['total'])
                    web.header('Content-Range', 'objects %s-%s/%s' %
    ...
    def GET(self):
        ...
        """
        query, content_range = self.get_scoped_query()
        if content_range:
