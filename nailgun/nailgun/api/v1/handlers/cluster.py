...
class VmwareAttributesHandler(BaseHandler):
    ...
    def GET(self, cluster_id):
        ...
        if not attributes:
            raise self.http(204, "No Vmware attributes found!")
    ...
    def PUT(self, cluster_id):
        ...
        data = self.checked_data(instance=cluster.vmware_attributes)
        objects.Cluster.update_vmware_attributes(cluster, data)
        attributes = objects.Cluster.get_vmware_attributes(cluster)
        attributes.editable.pop('metadata')
...
class VmwareAttributesDefaultsHandler(BaseHandler):
    ...
    def GET(self, cluster_id):
        ...
        if not attributes:
            raise self.http(204, "No default Vmware attributes found!")
