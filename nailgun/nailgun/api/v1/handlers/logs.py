...
class LogParser(object):
    ...
    def open(cls, filename, options):
        return cls._open_log_file(filename, open, options)

    @classmethod
    def _open_log_file(cls, filename, open_file_func, options):
        parser = None
        try:
            parser = cls(open_file_func(filename), **options)
        except IOError as e:
...
class GzipLogParser(LogParser):
    ...
    @classmethod
    def open(cls, filename, options):
        return cls._open_log_file(filename, gzip.open, options)
...
class LogrotatedLogParser(object):
    ...
    def parse(self, from_byte=-1, to_byte=0, max_entries=None):
        ...
        factory = LPSFactory.create_log_parsing_strategy(
            log_parsers, from_byte, to_byte, max_entries, self.fetch_older)

        data = factory.parse()

        total_logs_size = 0
        for _, parser in log_parsers:
            total_logs_size += parser.file_size
            parser.close()
...
class RecentlyAddedLPS(LogParsingStrategy):
    ...
    def parse(self):
        size = sum([p.file_size for _, p in self.log_parsers])
