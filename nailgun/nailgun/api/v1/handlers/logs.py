...
class GzipLogParser(LogParser):
    ...
    def open_file(self):
        return gzip.open(self.log_file)
...
def read_log(log_file=None, level=None, log_config={},
    ...
    n = 1
    while True:
        archived_log_file = '{0}.{1}.gz'.format(log_file, n)
        if not os.path.isfile(archived_log_file):
            break
        log_parsers.append(
            GzipLogParser(
                archived_log_file, fetch_older=fetch_older,
                max_entries=max_entries, log_config=log_config, regexp=regexp,
                level=level))
        n += 1
