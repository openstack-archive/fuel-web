...
class LogEntryCollectionHandler(BaseHandler):
    ...
    def GET(self):
        ...
        a user is not required to reload a page and can see log
        ...
        records that are currently loaded and s/he clicks on
        appropriate link to load more log entries.

        To cover these three cases `max_entries`, `from_byte`,
        `to_byte` and `fetch_older` parameters are used. Please note
        that not only these mentioned parameters are used, but all
        others don't distinguish mentioned use cases, they are common
        for all of them.

        In the first case `max_entries` is used. This parameter
        defines the maximum number of entries which is loaded on the
        page. So, for example, WebUI may request 100 entries and for
        this purpose it sends `max_entries` with value `100`. As the
        result it gets a response with `entries`, `from` and `to`
        values which mean from which and to which particular bytes the
        entries were found in the log file.

        Repeating requests are being sent in case of the second use
        case and each request includes recently mentioned `from` and
        `to` values. If `to` is equal (or greater) than a size of the
        ...
        those log entries that are located between `to` byte and the
        ...
        files that are rotated (as result of `logrotate` Linux
        tool). For example, a client may request to show most recent
        100 entries. The original log file has been just recently
        rotated, so on the server there are two files:

        - app.log (20 log entries)
        - app.log.1.gz (40 log entries)

        Current implementation treats these files like the one entire
        log file. So, if a client requests 25 most recent log entries
        (first use case) then log entries are fetched from the both
        `app.log` (all 20 log entries) and `app.log.1.gz` (five most
        recent log entries) log files. A client, in response, receives
        the following data:

        a) `from`, which equals to the length of `app.log` + the length
        of the 5 most recent log entries parsed from `app.log.1.gz`;
        b) `to`, which equals to the total length of log files involved
        in log parsing;
        c) `entries` with 25 most recent log entries.
