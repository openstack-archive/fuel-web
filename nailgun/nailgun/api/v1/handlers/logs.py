...
class LogParser(object):
    ...
    def file_size(self):
        """Get file size"""
        return os.stat(self.log_filename).st_size
...
class GzipLogParser(LogParser):
    ...
    @property
    def file_size(self):
        with open(self.log_fileobj.name) as f:
            f.seek(-4, 2)
...
class LogrotatedLogParser(object):
    ...
    def __init__(self, log_file, fetch_older, log_config, regexp, level):
        ...
        self.opened_fileobjs = []

        self.total_log_files_size = None
    ...
    def _make_log_parsers(self, filenames):
        ...
        """
        self.total_log_files_size = 0
        offset = 0
        log_parsers = []
        numbered_log_file = None
        for filename in filenames:
            if filename.endswith('.gz'):
                ...
                parser = self._make_plain_parser(numbered_log_file)
                if not parser:
                    # numbered log file doesn't exist, so archived log
                    # file is probably completed and we can use it
                    parser = self._make_gz_parser(filename)
            else:
                if filename != numbered_log_file:
                    parser = self._make_plain_parser(filename)
                else:
                    continue
            log_parsers.append((offset, parser, ))
            filesize = parser.file_size
            offset += filesize
            self.total_log_files_size += filesize

        log_parsers.reverse()

        return log_parsers

    def _make_gz_parser(self, filename):
        return self._make_log_parser(filename, GzipLogParser, gzip.open)

    def _make_plain_parser(self, filename):
        return self._make_log_parser(filename, LogParser, open)

    def _make_log_parser(self, filename, parser_class, file_open_func):
        parser = None
        try:
            f = file_open_func(filename)
            parser = parser_class(f, **self.parser_options)
            self.opened_fileobjs.append(f)
        except IOError as e:
    ...
    def __enter__(self):
        for i in range(10):  # retries 10 times
            filenames = sorted(glob.iglob(self.log_file + '*'))
            filenames.reverse()
            log_parsers = self._make_log_parsers(filenames)
            # validate that we are not ran into the raise condition,
            # when `logrotate` rotates a log file
            if (len(filenames) == len(log_parsers)
                    and all(f.log_filename in filenames
                            for _, f in log_parsers)):
                # everything is ok
                break
        else:
            log_parsers = []
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        self.log_parsers = []
        for fileobj in self.opened_fileobjs:
            fileobj.close()
...
class LogParsingStrategy(object):
    ...
    def parse(self):
        ...
        has_more = False
        with self.logrotated_log_parser as llp:
            for offset, parser in llp.get_log_parsers():
                try:
...
class FetchOlderLPS(LogParsingStrategy):
    ...
    def parse(self):
        with self.logrotated_log_parser:
            if self.from_byte == 0:
                return [], self.from_byte, False
