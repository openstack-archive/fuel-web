...
class LogEntryCollectionHandler(BaseHandler):
    ...
    def GET(self):
        ...
        * 500 (invalid regular expression in config)
        ...
        To cover these three cases `max_entries`, `from_byte`, `to_byte`
        and `fetch_older` parameters are used. Please note that not only
        these mentioned parameters are used, but all others doesn't
        distinguish mentioned use cases, they are common for all of
        them.
