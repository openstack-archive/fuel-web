...
class MasterNodeSettingsHandler(DBSingletonHandler):
    ...
    def _handle_stats_opt_in(self, mn_settings_data=None):
        """Creates task on stats user creation or removal

        :param mn_settings_data: dict with master node settings.
        Current data from DB will be used if mn_settings_data is None
        """
        must_send = self.single.must_send_stats(
            mn_settings_data=mn_settings_data)
        if must_send:
    ...
    def _perform_update(self, http_method):
        ...
        if new_opt_in is not None and old_opt_in != new_opt_in:
            self._handle_stats_opt_in(mn_settings_data=jsonutils.loads(result))
        return result
