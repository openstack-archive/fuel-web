...
class BaseNICsNamesHandler(BaseHandler):
    ...
    def _validate_node_params(self, params):
        ...
        for item in params.node_id:
            try:
                int(item)
            except ValueError:
                raise self.http(
                    400, "'node_id' must be integer, got '{0}'".format(item))
    ...
    def _check_nics_ids(self, data):
        # Check that NIC IDs specified in the data are valid and exist
        session = db()
        for nic in data:
            try:
                nic_id = int(nic['id'])
                ...
                raise self.http(
                    400, "'id' must be integer, got '{0}'".format(nic['id']))
            db_nic = session.query(NodeNICInterface).get(nic_id)
            if db_nic is None:
                raise self.http(
