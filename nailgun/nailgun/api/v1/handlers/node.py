...
class NodeCollectionHandler(CollectionHandler):
    ...
    def get_input_data(self, parameters):
        ...
        Retrieve input data from a request accordingly to the expected
        `parameters`.

        :param parameters: parameters which needs to be retrieved
        :type parameters: dict
        :returns: dict - input data
        """
        ...
        for key, value in six.iteritems(input_data):
            ...
            if value is None or value == []:
                continue
            if key in ('cluster_id', 'group_id') and value == '':
                value = None
            data[key] = value
        return data
