...
class OpenstackConfigExecuteHandler(BaseHandler):
    ...
    def PUT(self):
        ...
        :http: * 200 (OK)
               * 202 (Accepted)
        """
        data = self.checked_data(self.validator.validate_execute)
        ...
        for config in configs:
            refresh_on.update(config.configuration.keys())
        ...
        try:
            task = task_manager.execute(nodes, refreshable_tasks, configs)
