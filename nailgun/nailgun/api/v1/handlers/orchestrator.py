...
class BaseDeploySelectedNodes(SelectedNodesBase):
    ...
    def get_nodes(self, cluster):
        ...
        # all nodes sceduled for deployment are provisioned, hence
        ...
        if not_provisioned_nodes_ids:
            logger.warning("Nodes with ids {0} are not provisined yet for "
                           "cluster {1}."
                           .format(not_provisioned_nodes_ids,
                                   cluster.id)
                           )

            err_msg = ("Deployment operation is forbidden as nodes "
                       "with uids {0} are not provisioned yet."
                       .format(not_provisioned_nodes_ids))
            raise self.http(403, msg=err_msg)
...
class DeploySelectedNodes(BaseDeploySelectedNodes):
    ...
    def PUT(self, cluster_id):
        ...
        :http: * 200 (task successfully executed)
               * 202 (task scheduled for execution)
               * 400 (data validation failed)
               * 403 (deployment operation is forbidden)
               * 404 (cluster or nodes not found in db)
        """
        cluster = self.get_object_or_404(objects.Cluster, cluster_id)
        return self.handle_task(cluster)
