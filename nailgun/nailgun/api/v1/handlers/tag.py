...
class TagHandler(base.SingleHandler, TagMixIn):
    ...
    def _check_tag(self, obj_cls, obj, tag):
        if tag not in obj_cls.get_own_tags(obj):
            raise self.http(
                404,
                "Tag '{}' is not found for the {} {}".format(
                    tag, obj_cls.__name__.lower(), obj.id))

    @handle_errors
    @validate
    @serialize
    def GET(self, obj_type, obj_id, tag):
        """Retrieve tag
        ...
        self._check_tag(obj_cls, obj, tag)
        return TagSerializer.serialize_from_obj(obj_cls, obj, tag)

    @handle_errors
    @validate
    @serialize
    def PUT(self, obj_type, obj_id, tag):
        """Update tag
    ...
    @handle_errors
    def DELETE(self, obj_type, obj_id, tag):
        """Remove tag
...
class TagCollectionHandler(base.CollectionHandler, TagMixIn):
    ...
    def POST(self, obj_type, obj_id):
        ...
        obj_cls.update_tag(obj, data)
        raise self.http(
            201, TagSerializer.serialize_from_obj(obj_cls, obj, tag))
    ...
    def GET(self, obj_type, obj_id):
        obj_cls, obj = self._get_object_or_404(obj_type, obj_id)
        tags = six.iterkeys(obj_cls.get_tags_metadata(obj))
        return [TagSerializer.serialize_from_obj(obj_cls, obj, name)
                for name in tags]
