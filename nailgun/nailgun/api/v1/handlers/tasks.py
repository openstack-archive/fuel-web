...
class TaskCollectionHandler(CollectionHandler):
    ...
    def GET(self):
        ...
        """
        request_data = web.input(cluster_id=None, is_in_orchestrator=None)

        tasks = self.collection.eager(None, self.eager)
        if request_data.cluster_id is not None:
            tasks = tasks.get_by_cluster_id(request_data.cluster_id)
        if request_data.is_in_orchestrator is not None:
            tasks = tasks.filter_by(
                is_in_orchestrator=request_data.is_in_orchestrator)

        return self.collection.to_json(tasks)
