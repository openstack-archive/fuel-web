...
class BasicValidator(object):
    ...
    @classmethod
    def validate_release(cls, cluster):
        ...
        if objects.Release.is_granular_enabled(cluster.release):
            if not objects.Cluster.get_deployment_tasks(cluster):
                raise errors.NoDeploymentTasks(
                    "Deployment tasks not found for '{0}' release in the "
                    "database. Please upload them. If you're operating from "
                    "Fuel Master node, please check '/etc/puppet' "
