...
class AttributesValidator(BasicValidator):
    ...
    def _validate_custom_repos_format(cls, data):
        ...
        # save original regex string for error message below
        suite_regex_string = r'^[^/\s]+$|^(?P<directory>[^/]+)/{1}$'

        suite_regex = re.compile(suite_regex_string)
        for repo in repos_setup:
            if repo.get('suite'):
                m = re.search(suite_regex, repo['suite'])
                if not m:
                    raise errors.InvalidData(
                        'Incorrect description for custom repository with '
                        'name {0}. Suite (distribution) does not match '
                        'regular expression {1}'
                        .format(repo['name'], suite_regex_string)
                    )

                if m.group('directory') and repo.get('section'):
                    raise errors.InvalidData(
                        'Incorrect description for custom repository with '
