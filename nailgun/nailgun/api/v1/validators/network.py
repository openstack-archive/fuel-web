...
class NetAssignmentValidator(BasicValidator):
    ...
    @classmethod
    def _get_iface_by(cls, db_interfaces, default=None, **kwargs):
        data = list(objects.NICCollection.filter_by(db_interfaces, **kwargs))
        if data:
            return data[0]
        else:
            return default
    ...
    def _get_iface_by_id(cls, id_, db_interfaces, default=None):
        return cls._get_iface_by(db_interfaces, default, id=id_)

    @classmethod
    def _get_iface_by_name(cls, name, db_interfaces, default=None):
        return cls._get_iface_by(db_interfaces, default, name=name)
