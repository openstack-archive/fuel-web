...
class NetAssignmentValidator(BasicValidator):
    ...
    def verify_data_correctness(cls, node):
        ...
        bonded_eth_ids = set()
        pxe_iface = next((i for i in db_interfaces if i.pxe_interface), None)
        if not pxe_iface:
            raise errors.InvalidData(
                "Node '{0}': there is no pxe interface"
                " in DB".format(node['id']),
                log_message=True
            )
                ...
                if consts.NETWORKS.fuelweb_admin in iface_nets:
                    ...
                    if not pxe_iface_present:
                        raise errors.InvalidData(
                            ...
                            "interface '{2}'".format(
                                node['id'], iface['name'], pxe_iface.name),
                            log_message=True
                        )
