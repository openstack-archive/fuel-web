        ...
        if notation in [consts.NETWORK_NOTATION.cidr,
            ...
            if not cidr and not ng_db.cidr:
                raise errors.InvalidData(
                    ...
                    "{0}".format(net_id))
        if cluster and cluster.is_locked and cls._check_for_ip_conflicts(
...
class NetworkGroupValidator(NetworkConfigurationValidator):
    ...
    def validate_update(cls, data, **kwargs):
        ...
        cluster = db().query(Cluster).get(ng_db.nodegroup.cluster_id) \
            if ng_db.nodegroup else None
        return cls.validate_network_group(valid_data, ng_db, cluster)
    ...
    def validate_delete(cls, data, instance, force=False):
        if not instance.group_id:
            ...
            cluster = db().query(Cluster).get(instance.nodegroup.cluster_id)
            if cluster and cluster.is_locked:
                raise errors.InvalidData(
                    "Network configuration cannot be changed after deployment."
                )
