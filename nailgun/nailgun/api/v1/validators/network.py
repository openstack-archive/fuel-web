...
class NetworkConfigurationValidator(BasicValidator):
    ...
    @staticmethod
    def _validate_ip_ranges(ip_ranges, network_id, ng_db):
        # 'ip_ranges' must be present either in request or in DB
        if not ip_ranges and not ng_db.ip_ranges:
            raise errors.InvalidData(
                "No IP range was specified for network "
                "{0}".format(network_id))
        # If it's in DB don't validate
        elif ng_db.ip_ranges:
            return
    ...
    @staticmethod
    def _validate_cidr(cidr, network_id, ng_db):
        # 'cidr' must be present either in request or in DB
        if not cidr or not ng_db.cidr:
            raise errors.InvalidData(
    ...
    def validate_networks_update(cls, data):
        ...
        for network in d.get('networks'):
            ...
            cls._validate_cidr(cidr, network_id, ng_db)
            if notation == 'ip_ranges':
                cls._validate_ip_ranges(ip_ranges, network_id, ng_db)
