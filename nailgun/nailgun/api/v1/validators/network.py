...
class NetAssignmentValidator(BasicValidator):
    ...
    def _verify_iface_dpdk_properties(cls, iface, db_node):
        ...
        if db_iface is None:
            ...
            assert iface['type'] == consts.NETWORK_INTERFACE_TYPES.bond
            db_available = True
        ...
        enabled = iface_cls.dpdk_enabled(iface)

        # sanity checks
        available = iface['interface_properties'].get(
            'dpdk', {}).get('available')
