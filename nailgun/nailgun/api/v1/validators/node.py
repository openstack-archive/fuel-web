...
class NodeValidator(base.BasicValidator):
    ...
    def validate_update(cls, data, instance=None):
        ...
        if not instance:
            system_uuid = objects.Node.get_system_uuid(d)
            if not d.get('mac') and not d.get('id') and not system_uuid:
                raise errors.InvalidData(
                    ...
                    log_message=True)
            q = db().query(Node)
            n_by_mac_id = objects.Node.get_by_mac_or_uid(
                mac=d.get('mac'), node_uid=d.get('id'), query=q)
            n_by_s_uuid = objects.Node.get_by_system_uuid(d, query=q)
            instance = n_by_mac_id or n_by_s_uuid
            if not instance:
                raise errors.InvalidData(
                    'Cannot find node to update - no nodes found with mac: %s '
                    'or id: %s or system_uuid: %s' %
                    (d.get('mac'), d.get('id'), system_uuid))
            if n_by_mac_id and n_by_s_uuid and n_by_mac_id != n_by_s_uuid:
                ids1 = objects.Node.get_identification_attributes(n_by_mac_id)
                ids2 = objects.Node.get_identification_attributes(n_by_s_uuid)
                raise errors.InvalidData(
                    'Cannot update node %s - system uuid %s already assigned '
                    'to node %s' % (ids1[1], system_uuid, ids2[1]))
