...
class NodeValidator(base.BasicValidator):
    ...
    def validate_pending_roles(cls, data, node):
        ...
        if len(roles_set) != len(roles):
            raise errors.InvalidData(
                "pending_roles list for noe {0} contains "
        ...
        not_valid_roles = roles_set - set(available_roles)

        if not_valid_roles:
            raise errors.InvalidData(
                u"{0} are not valid roles for node {1} in environment {2}"
                .format(u", ".join(not_valid_roles), node.id, cluster.id),
                log_message=True
            )
