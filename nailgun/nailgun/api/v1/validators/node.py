...
class NodeValidator(base.BasicValidator):
    ...
    def validate_update(cls, data, instance=None):
        ...
        n_by_id = objects.Node.get_by_mac_or_uid(mac=n_id, query=q)
        n_by_s_uuid = objects.Node.get_by_system_uuid(d.get('meta'), query=q)
        _collision_check([n_by_mac, n_by_id, n_by_s_uuid])
        existent_node = n_by_mac or n_by_id or n_by_s_uuid
