...
class NodeValidator(BasicValidator):
    ...
    def validate_hostname(cls, hostname, instance):
        ...
        public_ssl_endpoint = cluster.attributes.editable[
            'public_ssl']['hostname']['value']

        if public_ssl_endpoint == hostname or \
            public_ssl_endpoint == objects.Node.generate_fqdn_by_hostname(
                hostname):
            raise errors.InvalidData(
                "New hostname '{0}' conflicts with public TLS endpoint".format(
                    hostname))
