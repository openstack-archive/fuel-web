...
class NodeValidator(base.BasicValidator):
    ...
    def validate_update(cls, data, instance=None):
        ...
        if not n_mac and not n_id and not n_sys_uuid:
            raise errors.InvalidData(
                'Cannot find node to update - none of the identification '
                "parameters specified (mac, id, meta['system']['uuid'])",
                log_message=True)
        n_by_mac = objects.Node.get_by_mac_or_uid(mac=n_mac)
        n_by_id = objects.Node.get_by_mac_or_uid(mac=n_id)
        n_by_sys_uuid = objects.Node.get_by_system_uuid(d.get('meta'))
        _collision_check([n_by_mac, n_by_id, n_by_sys_uuid])
        existent_node = n_by_mac or n_by_id or n_by_sys_uuid
