...
class BaseController(pecan.rest.RestController):
    ...
    def validate_request(self, request_type='POST'):
        ...
        if (
            request_type in ("GET", "DELETE") or
            getattr(self.__class__, 'validator', None) is None or
            resource_type == "single" and not self.validator.single_schema or
            resource_type == "collection" and not self.validator.collection_schema
        ):
    ...
    def get_objects_list_or_404(self, obj, ids=None):
        """Get list of objects
    ...
    def delete(self, obj_id):
        ...
        try:
            self.validator.validate_delete(None, obj)
...
class DBSingletonController(BaseController):
...
class BaseRemovedInController(BaseController):
    ...
    head = post = put = delete = get_one = get_all
