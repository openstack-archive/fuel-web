    ...
    def check_all_nodes(nodes, node_ids):
        ...
        if not_found_node_ids:
            raise errors.InvalidData(
                "Nodes with ids {0} were not found."
                .format(",".join(not_found_node_ids)),
                log_message=True
            )
...
class NodeAssignmentValidator(AssignmentValidator):
    ...
    def validate_collection_update(cls, data):
        ...
        received_node_ids = map(int, dict_data["assignment"].keys())
        ...
        release = db.query(Cluster).get(dict_data['cluster_id']).release
        for node_id in received_node_ids:
            cls.validate_roles(
                release,
                dict_data["assignment"][str(node_id)]
            )
        return dict_data
    ...
    def validate_roles(cls, release, roles):
        ...
        if not_valid_roles:
            raise errors.InvalidData(
                "{0} are not valid roles for node in {1} release"
