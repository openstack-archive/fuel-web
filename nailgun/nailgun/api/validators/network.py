...
class NetAssignmentValidator(BasicValidator):
        ...
        for iface in node['interfaces']:
            ...
            if iface['type'] == NETWORK_INTERFACE_TYPES.bond:
                ...
                for slave in iface['slaves']:
                    if 'id' not in slave and 'name' not in slave:
                        raise errors.InvalidData(
                            "Node '{0}', interface '{1}': each bond slave "
                            "must have either ID or Name".format(
                                node['id'],
    ...
    def verify_data_correctness(cls, node):
        ...
        if not network_group_ids:
            raise errors.InvalidData(
                "Node '{0}': there are no networks in DB".format(
                    node['id']),
                log_message=True
            )
                    ...
                    if iface_id:
                        ...
                        raise errors.InvalidData(
                            ...
                            "for bond '{2}' in DB".format(
                                node['id'],
                                slave.get('id') or slave.get('name'),
                                iface.get('id') or iface.get('name')),
                            log_message=True
                        )
        ...
        if network_group_ids:
            str_ng_ids = ["'" + str(id) + "'" for id in network_group_ids]
            raise errors.InvalidData(
