...
class NodeValidator(BasicValidator):
    ...
    def validate(cls, data):
        ...
        if cls.is_node_exists_in_db(data):
    ...
    @classmethod
    def is_node_exists_in_db(cls, data):
        mac = data['mac']
        q = db().query(Node)

        return q.filter(Node.mac == mac).first() \
            or q.join(NodeNICInterface, Node.nic_interfaces).filter(
                NodeNICInterface.mac == mac
            ).first()
