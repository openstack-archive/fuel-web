...
def upgrade():
    ...
    sa.Column('type', sa.Enum('nailgun', 'fuel', name='plugin_type'), nullable=False),
    ...
    sa.Column('state', sa.Enum('not_available', 'downloading', 'error', 'available', name='release_state'), nullable=False),
    ...
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('mode', sa.Enum('multinode', 'ha_full', 'ha_compact', name='cluster_mode'), nullable=False),
    sa.Column('status', sa.Enum('new', 'deployment', 'operational', 'error', 'remove', name='cluster_status'), nullable=False),
    sa.Column('net_provider', sa.Enum('nova_network', 'neutron', name='net_provider'), nullable=False),
    sa.Column('net_l23_provider', sa.Enum('ovs', name='net_l23_provider'), nullable=False),
    sa.Column('net_segment_type', sa.Enum('none', 'vlan', 'gre', name='net_segment_type'), nullable=False),
    sa.Column('net_manager', sa.Enum('FlatDHCPManager', 'VlanManager', name='cluster_net_manager'), nullable=False),
    sa.Column('grouping', sa.Enum('roles', 'hardware', 'both', name='cluster_grouping'), nullable=False),
    ...
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('name', sa.Enum('fuelweb_admin', 'storage', 'management', 'public', 'floating', 'fixed', 'private', name='network_group_name'), nullable=False),
    ...
    sa.Column('status', sa.Enum('ready', 'discover', 'provisioning', 'provisioned', 'deploying', 'error', name='node_status'), nullable=False),
    ...
    sa.Column('error_type', sa.Enum('deploy', 'provision', 'deletion', name='node_error_type'), nullable=True),
    ...
    sa.Column('name', sa.Enum('super', 'deploy', 'deployment', 'provision', 'node_deletion', 'cluster_deletion', 'check_before_deployment', 'check_networks', 'verify_networks', 'check_dhcp', 'verify_network_connectivity', 'install_plugin', 'update_plugin', 'delete_plugin', 'redhat_setup', 'redhat_check_credentials', 'redhat_check_licenses', 'redhat_download_release', 'redhat_update_cobbler_profile', 'dump', 'capacity_log', name='task_name'), nullable=False),
    sa.Column('message', sa.Text(), nullable=True),
    sa.Column('status', sa.Enum('ready', 'running', 'error', name='task_status'), nullable=False),
    sa.Column('progress', sa.Integer(), nullable=True),
    ...
    op.create_table('neutron_configs',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('cluster_id', sa.Integer(), nullable=True),
    sa.Column('parameters', JSON(), nullable=True),
    ...
    sa.Column('segmentation_type', sa.Enum('vlan', 'gre', name='segmentation_type'), nullable=False),
    ...
    sa.ForeignKeyConstraint(['network'], ['network_groups.id'], ondelete='CASCADE'),
    ...
    sa.ForeignKeyConstraint(['network_group_id'], ['network_groups.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    ...
    sa.Column('topic', sa.Enum('discover', 'done', 'error', 'warning', name='notif_topic'), nullable=False),
    sa.Column('message', sa.Text(), nullable=True),
    sa.Column('status', sa.Enum('read', 'unread', name='notif_status'), nullable=False),
    ...
    sa.Column('cluster_id', sa.Integer(), nullable=True),
    sa.Column('node_id', sa.Integer(), nullable=True),
    sa.Column('name', sa.Enum('networks', 'attributes', 'disks', name='possible_changes'), nullable=False),
    sa.ForeignKeyConstraint(['cluster_id'], ['clusters.id'], ),
    sa.ForeignKeyConstraint(['node_id'], ['nodes.id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_table('net_assignments',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('network_id', sa.Integer(), nullable=False),
    sa.Column('interface_id', sa.Integer(), nullable=False),
    sa.ForeignKeyConstraint(['interface_id'], ['node_nic_interfaces.id'], ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['network_id'], ['network_groups.id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_table('allowed_networks',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('network_id', sa.Integer(), nullable=False),
    sa.Column('interface_id', sa.Integer(), nullable=False),
    sa.ForeignKeyConstraint(['interface_id'], ['node_nic_interfaces.id'], ondelete='CASCADE'),
