...
def downgrade():
    ...
    downgrade_node_bind_attributes()
...
default_release_bond_attributes = {
    ...
    'mtu': {
        'value': {'type': 'text', 'weight': 10, 'value': '', 'label': 'MTU'},
...
def upgrade_node_bind_attributes():
    def _get_offloading_modes(slaves_offloading_modes_values):
        intersected_modes = six.moves.reduce(
            lambda x, y: set(x.keys()).intersection(y.keys()),
            slaves_offloading_modes_values)
        result = {}
        for slave_offloading_modes in slaves_offloading_modes_values:
            result = {k: result.get(k, True) and slave_offloading_modes[k]
    ...
    for result in connection.execute(bond_interface_query):
        ...
        attributes['mtu']['value']['value'] = interface_properties['mtu']
        attributes['offloading']['disable']['value'] = \
            interface_properties['disable_offloading']
        attributes['dpdk']['enabled']['value'] = \
            interface_properties['dpdk']['enabled']

        attributes['type__']['value'] = bond_properties['type__']
        attributes['mode']['value']['value'] = \
            bond_properties.get('mode') or result['mode']
        ...
        #                  node_bond_interfaces table or generated on backend
        #                  from slaves offloading modes each time
        slaves_offloading_modes_flat_values = [
            jsonutils.loads(r['attributes'])['offloading']['modes']['value']
...
def downgrade_node_bind_attributes():
    bond_interface_query = sa.sql.text(
    ...
    for id, attributes in connection.execute(bond_interface_query):
        ...
        interface_properties = {
            ...
            'dpdk': {
                # TODO(ekosareva): fix available
                'available': attributes['dpdk']['enabled']['value'],
