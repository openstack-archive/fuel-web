...
def upgrade_data():
    ...
    def upgrade_release_attributes_50_to_51(attrs_meta):
        ...
        def depends_to_restrictions(depends):
            for cond in depends:
                expr = cond.keys()[0]
                restrictions.append(expr + " != " + str(cond[expr]))
        ...
        for _, role in roles_meta.iteritems():
            if role.get('depends'):
                for depend in role['depends']:
                    if depend.get('condition'):
                        cond = depend['condition']
                        expr = cond.keys()[0]
                        depend['condition'] = expr + " == " + str(cond[expr])
    ...
    #     autoflush=True,
    #     autocommit=False,
    #     bind=create_engine(db_str, client_encoding='utf8')
    # )
    # print "session"
    # session = Session()
    #
    # print "session.query"
    # for release in session.query(models.Release):
    #     print str(release)
    #     upgrade_release_attributes_50_to_51(release.attributes_metadata)
    #     upgrade_release_roles_50_to_51(release.roles_metadata)
