...
def remove_mellanox_data():
    ...
    for release_id, attributes_metadata, networks_metadata \
        ...
        attrs.get('editable', {}).pop('neutron_mellanox')
        attrs.get('editable', {}).get('storage', {}).pop('iser')
        net = jsonutils.loads(networks_metadata)
        for availability in net.get('bonding', {}).get('availability', {}):
            if 'linux' in availability:
