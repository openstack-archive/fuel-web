...
def upgrade_node_roles_metadata():
    ...
    for id, roles_metadata in connection.execute(select_query):
        ...
        for role_name, role_info in six.iteritems(roles_metadata):
            if role_name in role_categories:
                role_info['category'] = role_categories[role_name]

        connection.execute(
            update_query,
            id=id,
            roles=jsonutils.dumps(roles_metadata),
        )
