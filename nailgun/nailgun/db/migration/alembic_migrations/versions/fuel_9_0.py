...
def update_vips_from_network_roles():
    ...
    vip_name_to_vip_data = {}

    # get namespaces from plugins
    vip_name_to_vip_data.update(
    ...
    ip_addrs_insert = sa.sql.text(
        "INSERT INTO ip_addrs (ip_addr, vip_namespace, vip_name) "
        "VALUES (:ip_addr, :vip_namespace, :vip_name)"
    )
    ...
    for vip_name in vip_name_to_vip_data:

        namespace = vip_name_to_vip_data[vip_name].get('namespace')
        ip_addr = vip_name_to_vip_data[vip_name].get('ip_addr')

        if vip_name in existing_names_to_id:
