...
class NoCacheQuery(Query):
    ...
    def delete(self, *args, **kwargs):
        ...
        For excepting deadlocks we should delete rows in certain order.
        PostgreSql ctid is used as workaround
        ...
        query_select = sql.expression.select(
            columns=['ctid'], whereclause=self.whereclause)
        query_select = query_select.order_by('id')
        ctids = [row[0] for row in db().execute(query_select).fetchall()]

        from_tables = [t.name for t in query_select.froms]
        query_delete = sql.text('DELETE FROM {0} WHERE ctid IN (:ctids)'.
                                format(','.join(from_tables)))
        result = db().execute(query_delete, {'ctids': ','.join(ctids)})
