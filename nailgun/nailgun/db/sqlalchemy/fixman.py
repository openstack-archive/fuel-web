...
def get_fixture_for_release(default_fxtr_name, ext, release=None):
...
def load_fake_deployment_tasks(rel_version=None, apply_to_db=True,
                               commit=True):
    """Load fake deployment tasks

    Some fixtures can be reused for different releases. This is controlled
    by mapping inside the function.

    :param apply_to_db: if True applying to all releases in db
    :param commit: boolean
    """

    # fixture for particular release might be suitable for other releases
    # (e.g. when there is no significant changes in corresponding components
    # of different versions)
    compatible_releases = {
        'newton-10.0': 'mitaka-9.0'
    }

    if rel_version is not None:
        rel_version = (compatible_releases[rel_version]
                       if compatible_releases.get(rel_version)
                       else rel_version)

    fxtr_path = get_fixture_for_release(
        'deployment_tasks', 'yaml', rel_version)
        ...
        for prop in model.__mapper__.iterate_properties:
            if isinstance(prop, sqlalchemy.orm.ColumnProperty):
                ...
                if not isinstance(value, (
                        list, dict, int, float, bool) + six.string_types):
                    value = ""
