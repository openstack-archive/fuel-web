...
class DeploymentGraph(Base):
    ...
    id = Column(
        Integer,
        primary_key=True)
    verbose_name = Column(  # to keep work with custom graphs convenient
        String(consts.DEPLOYMENT_GRAPH_NAME_MAX_LEN),
        nullable=True)


class DeploymentGraphTasks(Base):
    __tablename__ = 'deployment_graph_tasks'
    __table_args__ = (
        UniqueConstraint(
            'name',
            'deployment_graph_id',
            name='_name_deployment_graph_id_uc'),
    )
    id = Column(
        Integer,
        primary_key=True)
    ...
    deployment_graph_id = Column(
        ...
        nullable=False)
    type = Column(
        Enum(*consts.ORCHESTRATOR_TASK_TYPES, name='type'),
        nullable=False)
    groups = Column(
        psql.ARRAY(String(consts.DEPLOYMENT_TASK_GROUP_MAX_LEN)),
        default=[],
        nullable=False)
...
class ClusterPluginDeploymentGraph(Base):
    ...
    __tablename__ = 'cluster_plugin_deployment_graphs'
    __table_args__ = (
        UniqueConstraint(
            'type',
            'cluster_plugin_id',
