...
def delete_orphan_graphs(session, ctx):
    ...
    if orphans_possible:
        ...
        orphan_graphs_query = session.query(DeploymentGraph.id).filter(sa.and_(
            ~DeploymentGraph.clusters.any(),
            ~DeploymentGraph.releases.any(),
            ~DeploymentGraph.plugins.any(),
        )).subquery()
        # todo(ikutukov): log something about deleting orphan graphs
        session.query(DeploymentGraph).filter(
            DeploymentGraph.id.in_(orphan_graphs_query)
