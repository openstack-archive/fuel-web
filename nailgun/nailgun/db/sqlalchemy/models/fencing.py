...
class FencingConfiguration(Base):
    ...
    @property
    def primitives(self):
        return [p.name for p in self.primitive_list]

    @primitives.setter
    def primitives(self, new_primitives):
        db().query(FencingPrimitive).filter_by(
            configuration_id=self.id).delete()
...
class FencingMetadata(Base):
    __tablename__ = 'fencing_metadata'

    id = Column(
        Integer,
        primary_key=True
    )
    primitives_ui_parameters = Column(
        JSON,
        default={}
    )
    primitives_all_parameters = Column(
        JSON,
        default={}
    )
