...
class Node(Base):
    ...
    mac = Column(LowercaseString(consts.MAC_FIELD_SIZE), nullable=False, unique=True)
    ip = Column(String(15))
...
class NodeNICInterface(Base):
    ...
    mac = Column(LowercaseString(consts.MAC_FIELD_SIZE), nullable=False)
...
class NodeBondInterface(Base):
    ...
    mac = Column(LowercaseString(consts.MAC_FIELD_SIZE))
    assigned_networks_list = relationship(
        "NetworkGroup",
