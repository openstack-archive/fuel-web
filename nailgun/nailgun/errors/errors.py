...
class FailedProvisioning(DeploymentException):
...
class NetworkTemplateMissingRoles(NetworkException):
    message = "Roles are missing from network template"


class NetworkTemplateMissingNetRoles(NetworkException):
    message = "Network roles are missing"


class NetworkTemplateMissingNetworkGroup(NetworkException):
...
class TaskBaseDeploymentNotAllowed(UnhandledException):
    message = "The task-based deployment is not allowed"


class NoChanges(UnhandledException):
