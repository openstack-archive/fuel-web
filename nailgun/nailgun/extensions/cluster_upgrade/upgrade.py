...
#    )
    ...
    @classmethod
    def clone_ips(cls, new_cluster_id, orig_cluster, data):
        new_cluster = objects.Cluster.get_by_uid(new_cluster_id)
        orig_cluster = objects.Cluster.get_by_uid(
            data["original_cluster_id"])

        new_controllers = objects.Cluster.get_nodes_by_role(new_cluster,
                                                            'controller')
        orig_controllers = objects.Cluster.get_nodes_by_role(orig_cluster,
                                                             'controller')

        cls.assign_ips(new_cluster, new_controllers)
