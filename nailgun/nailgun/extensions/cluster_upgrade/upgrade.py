...
class UpgradeHelper(object):
    ...
    @classmethod
    def assign_node_to_cluster(cls, dst_cluster, node):
...
#        node.assgin_group()
        ...
        src_manager.set_nic_assignment_netgroups_ids(
            node.node, netgroups_id_mapping)

        # change bond_assignment
        src_manager.set_bond_assignment_netgroups_ids(
            node.node, netgroups_id_mapping)
...
#        node.add_pending_change(consts.CLUSTER_CHANGES.interfaces)
    ...
    @classmethod
    def get_netgroups_id_mapping(self, src_cluster, dst_cluster):
