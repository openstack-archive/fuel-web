...
class NetworkCheck(object):
    ...
    def neutron_check_l3_addresses_not_match_subnet_and_broadcast(self):
        ...
        if objects.Cluster.is_component_enabled(self.cluster, 'ironic'):
            ...
            if any([self.net_man.is_range_intersection(ironic_net_range,
                                                       bare_range)
                    for bare_range in bare_net_ranges]):
                self.err_msgs.append(
                    u"Neutron L3 address intersection between Ironic ranges "
                    u"for service nodes and baremetal nodes."
                )
                self.result.append({"ids": [],
                                    "name": ["baremetal"],
                                    "errors": ["ip_ranges"]})
