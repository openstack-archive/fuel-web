...
class NetworkManager(object):
    ...
    def __set_interface_attributes(cls, interface, interface_attrs):
        ...
        if not interface_properties:
            interface_properties = \
                objects.NIC.get_default_interface_properties()
        ...
        interface.meta = {
            ...
            'sriov': {
                ...
                'pci_id': interface_properties.get(
                    'sriov').get('pci_id', ''),
                'sriov_totalvfs': interface_properties.get(
    ...
    def get_iface_properties(cls, iface):
        properties = {}
        if iface.attributes.get('mtu', {}).get('mtu_value', {}).get('value'):
            properties['mtu'] = iface.attributes['mtu']['mtu_value']['value']
        if iface.attributes.get('offloading', {}).get(
                'disable_offloading', {}).get('value'):
            properties['vendor_specific'] = {
                'disable_offloading':
                iface.attributes['offloading']['disable_offloading']['value']
            }
