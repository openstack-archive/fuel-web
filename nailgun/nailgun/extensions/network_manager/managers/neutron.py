...
class NeutronManager(NetworkManager):
    ...
    def dpdk_nics(cls, node):
        """Returns generator with dpdk interfaces

        :param node: Node SQLAlchemy model
        :type node: models.Node
        :return: generator list[models.NodeNICInterface] - DPDK NICs
        """
...
class NeutronManager70(
    ...
    def _node_template_transformations(cls, node):
        ...
        for tmpl_name in template_names:
            yield template[tmpl_name]['transformations']
    ...
    def dpdk_enabled_for_node(cls, node):
        if node.network_template:
            ...
            for transformation in cls._node_template_transformations(node):
                if next(cls.dpdk_nics_names(
                        transformations, bridge=endpoint_name), None):
                    return True
            return False
        else:
