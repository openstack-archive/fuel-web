...
class NetAssignmentValidator(BasicValidator):
    ...
    def _verify_sriov_properties(cls, iface, data, db_node):
        ...
        if sriov_new['sriov_enabled']['value']:
        ...
        if (not sriov_meta['available'] and
                sriov_new['sriov_enabled']['value']):
            raise errors.InvalidData(
                "Node '{0}' interface '{1}': SR-IOV cannot be enabled as it is"
                " not available".format(db_node.id, iface.name),
                log_message=True
            )
        if (not sriov_new['sriov_numvfs']['value'] and
                sriov_new['sriov_enabled']['value']):
            raise errors.InvalidData(
        ...
        if (sriov_meta['sriov_totalvfs'] <
                sriov_new['sriov_numvfs']['value']):
            raise errors.InvalidData(
                "Node '{0}' interface '{1}': '{2}' virtual functions was"
                " requested but just '{3}' are available".format(
                    db_node.id, iface.name, sriov_new['sriov_numvfs']['value'],
                    sriov_db['sriov_numvfs']['max']),
                log_message=True
            )
        if (sriov_new['sriov_enabled']['value'] and
    ...
    def _verify_iface_dpdk_properties(cls, iface, db_interfaces,
        ...
        enabled = attributes.get('dpdk', {}).get(
            'dpdk_enabled', {}).get('value', False)
        ...
        mtu = attributes.get('mtu', {}).get('mtu_value', {}).get('value')
        ...
        for iface in interfaces:
            ...
            if iface['type'] == consts.NETWORK_INTERFACE_TYPES.ether:
                ...
                for slave in iface['slaves']:
                    ...
                    if iface_props.get('sriov', {}).get(
                            'sriov_enabled').get('value'):
                        raise errors.InvalidData(
