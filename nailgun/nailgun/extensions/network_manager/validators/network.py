    ...
    def validate_network_group(cls, ng_data, ng_db, cluster,
        ...
        if use_gateway:
            if gateway not in IPNetwork(cidr):
                raise errors.InvalidData("Gateway address does not"
                                         " belong to the network {0}."
                                         .format(ng_db['name']))
            else:
                if nm.check_ips_belong_to_ranges([gateway], ranges):
                    raise errors.InvalidData(
                        "Address intersection between {0} gateway and IP range"
...
class NetworkGroupValidator(NetworkConfigurationValidator):
    ...
    def validate_update(cls, data, **kwargs):
        ...
        if not ng_db.group_id:
            allow_fields = ["gateway", "ip_ranges", "id"]
            for field in d:
                if field == "id" and d[field] != ng_db.id:
                    raise errors.InvalidData(
                        "In Default Admin-pxe network you can change "
                        "only gateway and ip_ranges. Not {0}/{1}/{2}".format(
                            field, d, ng_db.id))
                if field not in allow_fields:
                    raise errors.InvalidData(
                        "In Default Admin-pxe network you can change "
                        "only gateway and ip_ranges. Not {0}".format(field))
