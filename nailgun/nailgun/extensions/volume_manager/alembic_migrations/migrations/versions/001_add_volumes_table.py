...
def migrate_data_from_core(connection):
    ...
    select_query = text(
        ...
        'AND extension_version=:extension_version'.format(
    ...
    for volumes_data in connection.execute(
            select_query,
            extension_name=ext_name,
            extension_version=ext_version):

        buffer_record_id = volumes_data[0]
        volumes_parsed = jsonutils.loads(volumes_data[1])
