...
class DisksFormatConvertor(object):
    ...
    def format_disks_to_simple(cls, full):
        ...
        for disk in disks_full_format:
            reserved_size = cls.calculate_service_partitions_size(disk)
    ...
    @classmethod
    def calculate_service_partitions_size(cls, disk):
        service_partitions = filter(is_service, disk['volumes'])
        result = sum([partition.get('size', 0)
                      for partition in service_partitions])
        result += disk.get('reserved_boot_space', 0)
        return result
...
class Disk(object):
    ...
    def create_service_partitions(self):
        ...
        if self.bootable:
            ...
            self.get_size(self.reserved_boot_space)
    ...
    @property
    def reserved_boot_space(self):
        boot_size = self.call_generator('calc_boot_size')
        boot_records_size = self.call_generator('calc_boot_records_size')
        return boot_size + boot_records_size
    ...
    def wipe_boot_volumes(self):
        existing_boot_partion = filter(
            lambda volume: volume.get('mount') == '/boot', self.volumes)
        for part in existing_boot_partion:
            self.volumes.remove(part)

        existing_boot_record = filter(
            lambda volume: volume.get('type') == 'boot', self.volumes)
        for rec in existing_boot_record:
            self.volumes.remove(rec)
        self.free_space += self.reserved_boot_space
    ...
    def render(self):
        ...
        if not self.bootable:
            rendered['reserved_boot_space'] = self.reserved_boot_space
        return rendered
    ...
    def update_bootable_flags(self, disks):
        ...
        for disk in disks:
            ...
            if disk_to_update.bootable != disk.get('bootable', False):
                ...
                # Update Disk.volumes
                disk_to_update.wipe_boot_volumes()
                if disk_to_update.bootable:
