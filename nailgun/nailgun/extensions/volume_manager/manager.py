...
class VolumeManager(object):
    ...
    def _get_root_disk(self):
        """Check given disk contains root partition"""
        for disk in self.disks:
            mounts = []
            mounts.extend([volume.get('mount') for volume in disk.volumes])
    ...
    def pick_default_bootable_disk(self):
        """Default strategy to pick a bootable disk"""
        def _upd_serialized_volumes():
            """Update self.volumes for consistency"""
            for disk in self.disks:
                for idx, volume in enumerate(self.volumes):
                    if volume.get('id') == disk.id:
        ...
        for rule in self.rule_to_pick_boot:
            ...
            if regex.match(root_disk.name):
                root_disk.bootable = True
                _upd_serialized_volumes()
                return
