...
class VolumeManager(object):
    ...
    def find_existing_disk(cls, disk_info, volumes):
        ...
        existing_disk = list(six.moves.filter(
            # Here we perform strict match for cover case with same ids for
            # different disks.
            lambda disk: disk_info_composite_id == build_disk_id_by_keys(
        ...
        existing_disk = list(six.moves.filter(
            lambda disk: disk_info_id_only & build_disk_id_by_keys(
        ...
        existing_disk = list(six.moves.filter(
            lambda disk: disk_info_path_only & build_disk_id_by_keys(
                disk, keys=('id',)),
            disks
        ))
