...
class TestExtension(BaseTestCase):
    ...
    def test_on_node_deletion(self, obj_call_mock):
        node_mock = mock.MagicMock(id=1)
        VolumeManagerExtension.on_node_deletion(node_mock)
        obj_call_mock.assert_called_once_with([1])

    def test_on_node_collection_deletion(self, obj_call_mock):
        ids = [1, 2, 3]
        VolumeManagerExtension.on_node_collection_deletion(ids)
