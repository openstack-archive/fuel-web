...
def create_node(pk):
    ...
    new_node["fields"]["meta"]["system"]["version"] = "".join(
        [str(random.randint(0, 9)) for _ in six.range(10)]
    )
    ...
    new_node["fields"]["meta"]["cpu"]["spec"] = [
        proc.copy() for i in six.range(total_proc)
    ]
    ...
    total = 0
    for i in six.range(random.randrange(2, 9)):
...
if __name__ == "__main__":
    ...
    res = []
    for i in six.range(args.number):
        res.append(create_node(i + 1))
