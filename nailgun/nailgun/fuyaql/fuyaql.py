...
class Fuyaql(object):
    ...
    def evaluate(self, yaql_expression):
        ...
        :return: result of evaluation as a string, empty string in case of
        empty cluster
        """
        if not self.cluster:
            print("Cluster needs to be loaded first, call ':use cluster' for it")
            return ""
        try:
    ...
    def use_old_context_from_task(self, task_id):
        ...
        """
        if not self.cluster:
            print("No cluster loaded, so there are no tasks for it")
            return False
    ...
    def show_nodes(self):
        ...
        nodes_ids = {}
        if not self.cluster:
            print("No cluster loaded, can't show nodes list from nowhere")
            return
    ...
    def use_node(self, node_id):
        ...
        """
        if not self.cluster:
            print("No cluster loaded, so there are no any node")
            return
