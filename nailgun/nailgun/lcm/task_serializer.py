...
class DeploymentTaskSerializer(object):
    ...
    @classmethod
    def finalize(cls, task, excludes=None):
        """Gets rid off extra attributes.

        :param task: the serialized task
        :param excludes: the list of fields to exclude
        """
        fields = cls.fields ^ excludes if excludes else cls.fields
...
class NoopTaskSerializer(DeploymentTaskSerializer):
    ...
    def serialize(self, node_id):
        task = self.finalize(self.task_template, {'parameters'})
