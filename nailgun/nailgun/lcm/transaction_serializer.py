...
def _dispy_setup_computation_node(context_file, settings_data_file):
    ...
    # If paths to files passed as absolute paths we should use only they
    # basenames for building correct path to stored on the dispy node files
    cwd = os.getcwd()
    local_context_file = os.path.join(
        cwd, os.path.basename(context_file))
    local_settings_data_file = os.path.join(
        cwd, os.path.basename(settings_data_file))
