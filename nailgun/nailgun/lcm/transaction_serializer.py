...
def _rpc_serialize_task_for_node(context, node_id_task):
    try:
        from nailgun.lcm.task_serializer import TasksSerializersFactory
        from nailgun.logger import logger
        logger.debug("RPC task serialization initiated")
        node_id, task = node_id_task
        logger.debug("RPC task for node_id: %s task: %s", node_id, task)
        logger.debug("RPC task factory creating")
        factory = TasksSerializersFactory(context)
        logger.debug("RPC task serializer creation")
        task_serializer = factory.create_serializer(task)
        logger.debug("RPC task serialization initiated")
        serialized = task_serializer.serialize(node_id)
        logger.debug("RPC serialized task: %s", serialized)
        return node_id, serialized, None
    except Exception as e:
        logger.exception("RPC task serialization failed: %s", e)
...
class DistributedProcessingPolicy(object):
    ...
    def _create_job_cluster(self):
        ...
        # Adding cluster nodes to the JobCluster nodes
        ...
        # Adding bootstrap nodes to the JobCluster nodes
    ...
    def execute(self, context, _, tasks):
        ...
        for task in tasks:
            ...
            try:
                logger.debug("Creating job for task: '%s'", task_id)
                job = job_cluster.submit(context, task)
                self.jobs_cond.acquire()
                job.id = task_id
                # there is a chance the job may have finished and job_callback
                # called by this time, so put it in 'pending_jobs' only if job
                # is pending
                if job.status in (dispy.DispyJob.Created,
                                  dispy.DispyJob.Running):
                    self.pending_jobs[task_id] = job
                    logger.debug("Job '%s' submitted. Pending jobs count: %s",
                                 job.id, len(self.pending_jobs))
                ...
                logger.exception("Task processing failed: %s", task_data['id'])
                raise errors.DeploymentException(e)
