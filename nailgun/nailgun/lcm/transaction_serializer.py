...
def _distributed_serialize_tasks_for_node(formatter_contexts_idx,
    ...
    try:
        ...
        for k in formatter_contexts_idx.keys():
...
class DistributedProcessingPolicy(object):
    ...
    def _scatter_data(self, job_cluster, context, workers):
        ...
        scattered = job_cluster.scatter(shared_data, workers=workers)
    ...
    def _get_allowed_nodes_statuses(self):
        ...
        logger.debug("Getting allowed nodes statuses to use as serialization "
                     "workers for cluster %s", self.transaction.cluster.id)
        attrs = objects.Cluster.get_editable_attributes(
            self.transaction.cluster)
        check_fields = {
        ...
        for field in check_fields:
            if attrs.get('common', {}).get(field, {}).get('value'):
                statuses.add(check_fields[field])

        logger.debug("Allowed nodes statuses to use as serialization workers "
                     "for cluster %s are: %s",
                     self.transaction.cluster.id, statuses)
        return statuses

    def _get_allowed_nodes(self):
        ...
        :return: generator of nodes allowed for distributed serialization
        """
        logger.debug("Getting allowed nodes to use as serialization workers "
                     "for cluster %s", self.transaction.cluster.id)
        allowed_statuses = self._get_allowed_nodes_statuses()
        for node in self.transaction.cluster.nodes:
            if node.online and node.status in allowed_statuses:
                yield node

    def _get_allowed_nodes_ips(self):
        result = set(n.ip for n in self._get_allowed_nodes())
        result.add(settings.MASTER_IP)
        logger.debug("Nodes allowed to use as serialization workers for "
                     "cluster %s are: %s", self.transaction.cluster.id,
                     result)
        return result
    ...
    def _get_allowed_workers(self, job_cluster, allowed_ips):
        ...
        """
        logger.debug("Getting allowed workers for cluster %s",
                     self.transaction.cluster.id)
        workers = {}
        ...
        for worker_addr in six.iterkeys(info['workers']):
            ...
            try:
                pool = workers[ip]
                pool.append(ip_port)
                logger.debug("Worker %s is allowed for cluster: %s",
                             ip, self.transaction.cluster.id)
            except KeyError:
                workers[ip] = [ip_port]
    ...
    def execute(self, context, _, tasks):
        ...
        allowed_ips = self._get_allowed_nodes_ips()
