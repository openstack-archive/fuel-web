...
class NetworkCheck(object):
    ...
    def __init__(self, task, data):
        ...
        parameters = {'neutron': 'neutron_parameters',
                      'nova_network': 'nova_network_parameters'}
        self.network_config = serializer[self.net_provider].\
            serialize_network_params(self.cluster)
        self.network_config.update(data.get(parameters[self.net_provider], {}))
    ...
    def check_untagged_intersection(self):
        ...
        """
        netw_tag = lambda n: self.network_config[n['ext_vlan_tag']] \
            if n.get('ext_vlan_tag') else n['vlan_start']
        untagged_nets = set([n['name'] for n in self.networks
                            if netw_tag(n) is None])
