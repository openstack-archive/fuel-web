...
class NetworkCheck(object):
    ...
    def check_network_template(self):
        """Check for network template

        If network template is uploaded for custer it should contain all
        ...
        for role in assigned_roles:
            ...
            for node_group in node_groups:
                if role not in template['adv_net_template']\
                    ...
                    self.err_msgs.append(
                        "Node role '{0}' is assigned for some nodes in group"
                        " '{1}', but not found in network template for this "
                        "node group".format(role, node_group.name))
                    self.result.append({"ids": [],
                                        "errors": ["network_template"]})
        self.expose_error_messages()
