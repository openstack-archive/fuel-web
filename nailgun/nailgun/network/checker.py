    ...
    def check_vlan_ids_intersection(self):
        ...
        if self.cluster.network_config.configuration_template is None:
            ...
            nodes_bonds_networks = db().query(
                ...
                models.Node.cluster_id == self.cluster.id,
                models.NodeBondInterface.node_id == models.Node.id,
                models.NetworkBondAssignment.bond_id ==
                models.NodeBondInterface.id,
                models.NetworkBondAssignment.network_id ==
                models.NetworkGroup.id,
            )
            nodes_networks = nodes_nics_networks.union(
            ...
            for node_name, data in groupby(nodes_networks, lambda x: x[0]):
                for if_name, nic_nets in groupby(data, lambda x: x[1]):
                    net_ids = [net_id
                               ...
                               if tagged_nets.get(net_id, None) is not None]
                    if len(net_ids) < 2:
                    ...
                    if len(set(vlan_ids)) < len(vlan_ids):
                        ...
                        net_by_if = defaultdict(list)
                        for idx in range(len(vlan_ids)):
                            net_by_if[vlan_ids[idx]].append(net_ids[idx])
                        net_names = []
                        for net_ids in six.itervalues(net_by_if):
                            if len(net_ids) > 1:
                                net_names.extend(net['name']
                                                 for net in self.networks
                                                 if net['id'] in net_ids)
                        err_msg = u"Node {0}, interface {1}: {2} networks " \
                            u"use the same VLAN tags. Different VLAN tags " \
                            u"should be assigned to the networks on the same" \
                            u" interface.".format(
                                node_name, if_name, ", ".join(net_names))
                        raise errors.NetworkCheckError(err_msg)
