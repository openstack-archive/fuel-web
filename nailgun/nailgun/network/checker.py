...
class NetworkCheck(object):
    ...
    def neutron_check_network_address_spaces_intersection(self):
        ...
        for fl_range in self.network_config['floating_ranges']:
            fl_ip_range = netaddr.IPRange(fl_range[0], fl_range[1])
            fl_in_public = any(True for x in public_cidrs if fl_ip_range in x)
            if not fl_in_public:
                self.err_msgs.append(
        ...
        for public in publics:
            ...
            ranges = [netaddr.IPRange(v[0], v[1])
                      for v in public['ip_ranges']]
            for fl_ip_range in fl_ip_ranges:
                if fl_ip_range in public_cidr:
                    ranges += [fl_ip_range]
