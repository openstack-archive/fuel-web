...
class NetworkManager(object):
    ...
    @classmethod
    def add_vip(
            cls, network=None, ip_addr=None, vip_info=None, nodegroup=None):
        """Add VIP to DB.
    ...
    def _get_assigned_vips_for_net_groups(cls, cluster):
        ...
        vips = db.query(IPAddr).join(IPAddr.network_data).filter(*clause)
        return vips
...
class AllocateVIPs80Mixin(object):
    ...
    def add_vip(
        ...
        vip = super(AllocateVIPs80Mixin, cls).add_vip(
            network=network,
            ip_addr=ip_addr,
            vip_info=vip_info,
            nodegroup=nodegroup)
