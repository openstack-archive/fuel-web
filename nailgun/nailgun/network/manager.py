...
class NetworkManager(object):
    ...
    def _get_assigned_vip(cls, nodegroup, network_name, vip_name):
        ...
        :returns: IPAddr SQLAlchemy object of a VIP that matches
                  specified criterias.
    ...
    @classmethod
    def _assign_vip(cls, nodegroup, network_name, vip_name, namespace=None):
        ...
        :type namespace: str
        :returns: assigned VIP (IPAddr SQLAlchemy object)
        :raises: Exception
    ...
    def assign_vips_for_net_groups(cls, cluster):
        """Calls cls._assign_vip for all of cluster's network_groups.
        ...
        for ng in cluster.network_groups:
            for vip_name in ng.meta.get('vips', ()):
                ...
                result[key] = cls._assign_vip(nodegroup,
                                              ng.name, vip_name).ip_addr
    ...
    def get_end_point_ip(cls, cluster_id):
        ...
        if cluster_db.is_ha_mode:
            ...
            ip = cls._assign_vip(nodegroup, "public").ip_addr
...
class AllocateVIPs70Mixin(object):
    ...
    def _build_advanced_vip_info(cls, vip_info, role, address, namespace):
        """Create dict with information about vip.

        :param vip_info: information about vip from releases table
        :type vip_info: dict
        :param role: node role
        :type role: str
        :param address: vip address
        :type address: str
        :param namespace: vip namespace
        :type namespace: str
        """
    ...
    def get_end_point_ip(cls, cluster_id):
        ...
        return cls._assign_vip(node_group,
                               net_group, vip_name='public').ip_addr
    ...
    @classmethod
    def assign_vips_for_net_groups_for_api(cls, cluster, allocate=True):
        """Calls cls._assign_vip for all vips in network roles.
...
class AllocateVIPs80Mixin(object):
    ...
    @classmethod
    def _build_advanced_vip_info(cls, vip_info, role, address, namespace):
        "See AllocateVIPs70Mixin._build_advanced_vip_info."
