...
class NetworkManager(object):
    ...
    def assign_public_network_on_node(cls, cluster, node):
        public_net = next(net for net in chain.from_iterable((ng.networks
    ...
    def unassign_public_network_on_node(cls, cluster, node):
        ...
        for iface in ifaces:
            try:
                iface.assigned_networks_list.remove(public_net)
            except ValueError:
                pass
    ...
    def update_public_networks(cls, cluster):
        """Assign or unassing public network for node in cluster.
