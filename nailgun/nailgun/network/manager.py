...
class NetworkManager(object):
    ...
    def get_default_networks_assignment(cls, node):
        ...
        for i, nic in enumerate(node.interfaces):
            if nic.type == consts.NETWORK_INTERFACE_TYPES.ether:
                nic_dict = {
                    "id": nic.id,
                    "name": nic.name,
                    "mac": nic.mac,
            ...
            elif nic.type == consts.NETWORK_INTERFACE_TYPES.bond:
                nic_dict = {
                    "name": nic.name,
                    "mode": nic.mode,
                    "type": nic.type,
                    "slaves": [{'name': n.name} for n in nic.slaves]
                }

            if nic.type == consts.NETWORK_INTERFACE_TYPES.ether and nic.bond:
                pass
            elif to_assign_ids:
        ...
        if to_assign_ids:
            ...
            logger.warn("Cannot assign all networks appropriately for"
                " node %r. Set all unassigned networks to the interface %r" %
                (node.name, nics[0].name))
