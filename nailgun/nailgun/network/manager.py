...
class NetworkManager(object):
    ...
    def create_networks(cls, nw_group):
        ...
        if nw_group.vlan_start is not None:
            for n in xrange(nw_group.vlan_start,
                            nw_group.vlan_start + nw_group.amount):
                ng_by_vlan = db().query(NetworkGroup).filter(
                    n >= NetworkGroup.vlan_start,
                    n < NetworkGroup.vlan_start + NetworkGroup.amount,
                    not_(NetworkGroup.id == nw_group.id))
                if ng_by_vlan:
                    logger.warning("Intersection with existing vlan_id: %s", n)
        db().commit()
