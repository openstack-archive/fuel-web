...
class NetworkManager(object):
    ...
    @classmethod
    def _get_ips_except_admin(cls, node_id=None, node=None,
                              network_id=None, joined=False):
        """Receives IP addresses for node or network except Admin Network.

        :param node_id: Node database ID. If node_id is None and node is
        provided node.id will be used as node_id value
        :type  node_id: int
    ...
    def get_default_interfaces_configuration(cls, node):
        ...
        ng_wo_admin_ids = ng_ids.symmetric_difference(
            [cls.get_admin_network_group(node.id).id]
        )
    ...
    def get_admin_ips_for_interfaces(cls, node):
        ...
        admin_net_id = cls.get_admin_network_group(node).id
        admin_ips = set([
