...
class NetworkManager(object):
    ...
    def get_iface_properties(cls, iface):
        ...
        if iface.offload_modes:
            modified_offload_modes = \
                cls.__get_modified_offload_modes(iface.offload_modes)
    ...
    @classmethod
    def __get_modified_offload_modes(cls, offload_modes):
        result = dict()
        for mode in offload_modes:
            if mode['state'] is not None:
                result[mode['name']] = mode['state']
            if mode['sub'] and mode['state']:
                result.update(cls.__get_modified_offload_modes(mode['sub']))
        return result
