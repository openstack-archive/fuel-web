...
class NetworkManager(object):
    ...
    def get_node_network_by_netname(cls, node, netname):
        ...
        # get_node_networks includes the admin net but without IP information
        # so we replace it with the node's admin network (with IP information)
        networks[-1] = cls._get_admin_node_network(node.id)
        return filter(
    ...
    def get_node_networks(cls, node):
        ...
        network_data.append(cls._get_admin_network(node.id))
