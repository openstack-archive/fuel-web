...
class NetworkManager(object):
    ...
    def assign_vip(cls, nodegroup, network_name, vip_name, namespace=None):
        ...
        vip_obj = IPAddr(network=network.id, ip_addr=vip,
...
class AllocateVIPs70Mixin(object):
    ...
    def _assign_vips_for_net_groups(cls, cluster):
        ...
        for nodegroup, net_group, vip_name, role, vip_infox\
                in net_role_vip_mappings:

            vip_addr = cls.assign_vip(nodegroup, net_group, vip_name, vip_infox.get('namespace'))

            vip_info = cls._build_vip_info(vip_infox, vip_addr)
