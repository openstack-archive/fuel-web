...
class NetworkManager(object):
    ...
    def add_vip(cls, **kwargs):
        ...
        vip = kwargs.get('vip')
        vip_info = jsonutils.dumps(kwargs.get('vip_info'))
        ip_addr = IPAddr(network=network.id, ip_addr=vip, vip_info=vip_info)
        return ip_addr
    ...
    def assign_vip(cls, nodegroup, network_name,
        ...
        vip_info_clause = text(
            """vip_info->>'name'=:vip_name""", vip_name=vip_name)
        cluster_vip = db().query(IPAddr).filter_by(
            network=network.id,
            node=None
        ).filter(
            Select(whereclause=vip_info_clause)
        ).first()
    ...
    def get_assigned_ips_by_network_id(cls, network_id):
        ...
        return [
            ...
            db().query(IPAddr.ip_addr).filter(
                ...
                or_(IPAddr.node.isnot(None), IPAddr.vip_info.contains('name'))
            )
        ]
...
class AllocateVIPs80Mixin(object):
    ...
    def add_vip(cls, **kwargs):
        ...
        network = kwargs.get('network')
        vip = kwargs.get('vip')
        vip_name = kwargs.get('vip_info')['name']
        vip_info = objects.Cluster.get_vip_info(nodegroup.cluster, vip_name)
        ip_addr = IPAddr(network=network.id, ip_addr=vip, vip_info=vip_info)
        return ip_addr
