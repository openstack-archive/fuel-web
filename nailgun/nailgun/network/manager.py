...
class NetworkManager(object):
    ...
    def reusable_ip_address(cls, node, network):
        ...
        if node.ip and cls.check_ip_belongs_to_net(node.ip, network):
            return objects.IPAddress.create(
                {'network': network.id,
                 'ip_addr': node.ip,
                 'node': node.id
                 })
    ...
    def assign_admin_ips(cls, nodes):
        ...
        for node in nodes:
            ...
            node_admin_ips = objects.Node.get_network_ips_count(
    ...
    def get_node_networks_ips(cls, node):
        return objects.Node.get_networks_ips(node)

    @classmethod
    def set_node_networks_ips(cls, node, ips_by_network_name):
        objects.Node.set_networks_ips(node, ips_by_network_name)
    ...
    def assign_ips(cls, cluster, nodes, network_name):
        ...
        for node in nodes:
            ...
            if network_name == consts.NETWORKS.fuelweb_admin:
                network = objects.NetworkGroup.get_admin_network_group(node.id)
            else:
                network = \
                    objects.NetworkGroup.get_from_node_group_by_name(
                        group_id, network_name)
    ...
    def _get_ips_except_admin(cls, node_id=None,
        ...
        """
        return objects.IPAddress.get_ips_except_admin(
            node_id, network_id, joined)
    ...
    def get_admin_ips_for_interfaces(cls, node):
        ...
        admin_ips = objects.Node.get_admin_ip(node, admin_net_id)
    ...
    def get_admin_networks(cls, cluster_nodegroup_info=False):
        admin_dbs = objects.NetworkGroupCollection.filter_by(
            None, name=consts.NETWORKS.fuelweb_admin)
        result = []
...
class AssignIPs70Mixin(object):
    ...
    def prepare_for_deployment(cls, cluster, nodes):
        ...
        query = objects.Cluster.get_network_groups_by_node_group(cluster.id)
