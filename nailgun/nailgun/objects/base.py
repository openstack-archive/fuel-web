...
class NailgunCollection(object):
    ...
    def scope(cls, scope=None, filter_by=None):
        ...
        query_full = query
        if scope:
            if scope.limit > 0 or scope.limit is None:
                if scope.order_by:
            ...
            if scope.offset or scope.limit is not None:
                ...
                selected = query.count() if scope.limit != 0 else 0
                # first element index=1
                first = scope.offset + 1 if scope.offset else 1
                if selected == 0 or scope.limit == 0:
                    ...
                    last = total
                valid = selected > 0 or scope.limit == 0 or total == 0
