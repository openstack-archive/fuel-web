...
class Cluster(NailgunObject):
    ...
    def get_default_group(cls, instance):
        if not instance.node_groups:
            return cls.create_default_group(instance)
        return [g for g in instance.node_groups
                if g.name == consts.NODE_GROUPS_NAMES.default][0]

    @classmethod
    def create_default_group(cls, instance):
        ng = models.NodeGroup(cluster_id=instance.id,
                              name=consts.NODE_GROUPS_NAMES.default)
        db.add(ng)
        db().flush()
        instance.node_groups.append(ng)
        return instance.node_groups[0]
