...
class Cluster(NailgunObject):
    ...
    def get_network_manager(cls, instance=None):
        ...
        if instance.net_provider == 'neutron':
            version = extract_env_version(instance.release.version)
            if StrictVersion(version) >= StrictVersion'7.0':
...
class VmwareAttributes(NailgunObject):
    ...
    from distutils.version import StrictVersion
    from distutils.version import StrictVersion
    from distutils.version import StrictVersionmodel = models.VmwareAttributes
