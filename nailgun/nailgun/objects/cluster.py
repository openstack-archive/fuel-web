...
class Cluster(NailgunObject):
    ...
    def get_cluster_attributes_by_components(cls, components, release_id):
        ...
        for component in selected_release_components:
            for bind_exp in component['bind']:
                if isinstance(bind_exp, six.string_types):
                    _update_attributes_dict_by_binds_exp(bind_exp, True)
                elif isinstance(bind_exp, dict):
                    for bind_item, value in bind_exp.items():
                        _update_attributes_dict_by_binds_exp(bind_item, value)
