...
class Cluster(NailgunObject):
    ...
    def get_cluster_tasks(cls, instance):
        ...
        exp_models = cls.get_expression_models(instance)
        for role, tasks in cls.get_base_tasks(instance):
            for task in tasks:
                if 'condition' in task:
                    if Expression(task['condition'], exp_models).evaluate():
    ...
    def get_base_tasks_configs(cls, instance):
        ...
        config_dir = os.path.join(settings.TASK_DIR, version, '*.yaml')
    ...
    @classmethod
    def is_task_based(self, instance):
        configs = self.get_base_tasks_configs(instance)
        return configs and 'experimental' in settings.VERSION['feature_groups']
