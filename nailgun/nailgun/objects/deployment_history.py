...
class DeploymentHistoryCollection(NailgunCollection):
    ...
    @classmethod
    def get_by_transaction_id_and_node_ids_and_statuses(
            cls, transaction_id, node_ids=None, statuses=None):
        history_query = cls.get_by_transaction_id(transaction_id)
        if node_ids and statuses:
            h_q = cls.filter_by_list(history_query, 'node_id', list(node_ids))
            return cls.filter_by_list(h_q, 'status', list(statuses))
        elif node_ids and not statuses:
            return cls.filter_by_list(history_query, 'node_id',
                                      list(node_ids))
        elif statuses and not node_ids:
            return cls.filter_by_list(history_query, 'status',
                                      list(statuses))
