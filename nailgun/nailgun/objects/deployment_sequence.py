...
class DeploymentSequence(NailgunObject):
    ...
    @classmethod
    def get_by_name(cls, name, fail_if_not_found=False, lock_for_update=False):
        ...
        q = db().query(cls.model).filter_by(name=name)
        if lock_for_update:
            q = q.order_by('id')
        ...
        if not res and fail_if_not_found:
            raise errors.ObjectNotFound(
                "Sequence with name='{0}' is not found in DB".format(name)
            )
        return res
