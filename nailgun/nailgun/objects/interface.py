...
class NIC(DPDKMixin, NailgunObject):
    ...
    def get_dpdk_queues_count(cls, instance):
        dpdk_cpu_pining = utils.get_in(instance.node.attributes,
                                       'cpu_pinning', 'dpdk', 'value') or 0
        max_queues = utils.get_in(instance.meta, 'interface_properties',
                                  'max_queues') or 0
        return max_queues if dpdk_cpu_pining > max_queues else dpdk_cpu_pining
