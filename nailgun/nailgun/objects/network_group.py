        ...
        if notation:
            ...
            try:
                if notation == 'cidr':
                    ...
                    if instance.meta.get('use_gateway'):
                        gw = cidr_gw
                    else:
                        ip_range.first = cidr_gw
                elif notation == 'ip_ranges' and instance.meta.get("ip_range"):
                    ip_range = models.IPAddrRange(
                        first=instance.meta["ip_range"][0],
                        last=instance.meta["ip_range"][1]
                    )
                    gw = instance.meta.get('gateway') or cidr_gw \
                        if instance.meta.get('use_gateway') else None
                else:
            ...
            instance.ip_ranges.append(ip_range)
            db().flush()
