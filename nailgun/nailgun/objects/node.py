...
class NodeAttributes(object):
    ...
    def _safe_get_hugepages(cls, node, attributes=None):
        ...
        hugepages = attributes['hugepages']
    ...
    def _safe_get_cpu_pinning(cls, node, attributes=None):
        if not attributes:
            attributes = Node.get_attributes(node)
        cpu_pinning = attributes['cpu_pinning']
    ...
    def is_nova_cpu_pinning_enabled(cls, node):
        return bool(cls._safe_get_cpu_pinning(node)['nova']['value'])
    ...
    def is_nova_hugepages_enabled(cls, node):
        nova_hugepages = cls._safe_get_hugepages(node)['nova']['value']
    ...
    def dpdk_hugepages_attrs(cls, node):
        ...
        4 NUMA nodes)
        """
        dpdk_memory = cls._safe_get_hugepages(node)['dpdk']['value']
