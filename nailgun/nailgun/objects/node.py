...
class Node(NailgunObject):
    ...
    def update_tags(cls, instance, new_roles):
        ...
        for assoc in instance.tags:
            if assoc.tag.tag not in old_role_tags:
                tags.append(assoc)

        instance.tags = tags

        # Assign tags for new role(s)
        role_tags = TagCollection.filter_by_list(
            None, 'tag', new_tags - current_tags
        ).filter(
            ((models.Tag.owner_id == instance.cluster.release.id) &
             (models.Tag.owner_type == 'release')) |
            ((models.Tag.owner_id == instance.cluster.id) &
