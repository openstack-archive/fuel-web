...
class Node(NailgunObject):
    ...
    def create(cls, data):
        ...
        if new_node.meta and new_node.meta.get('interfaces'):
            NetworkManager.update_interfaces_info(new_node)
    ...
    def create_attributes(cls, instance):
        new_attributes = models.NodeAttributes(node_id=instance.id)
    ...
    def update(cls, instance, data):
        ...
        # calculating flags
        roles_changed = (
            'roles' in data and set(data['roles']) != set(instance.roles)
        )
        pending_roles_changed = (
            'pending_roles' in data and
            set(data['pending_roles']) != set(instance.pending_roles),
        )
