...
class Node(NailgunObject):
    ...
    def is_interfaces_configuration_locked(cls, instance, is_agent=False):
        """Returns true if update of network configuration not allowed.

        Configuration update is allowed for nodes in 'error' and 'discover'
        states except the discover failure.

        We are allowing configuration of the stopped node to be updated
        by API but not nailgun-agent.
