...
class Node(NailgunObject):
    ...
    def distribute_node_cpus(cls, instance):
        ...
        be allocated). Since using of float lead to inaccuracy all calculation
        will be done in integer numbers. It means that

                           float_result = xi * (M / N) + carry
                           carry = float_result - math.floor(float_result)
        ...
        where / - div (integer division), % - modulo operator
        ...
        for numa_node in instance.meta['numa_topology']['numa_nodes']:
            if not total_required_cpus:
                break
            cpus_number = min(len(numa_node['cpus']), total_required_cpus)
            ...
            for component in components:
                ...
                cpus_to_allocate, carry = divmod(numerator,
                                                 total_required_cpus)
                cpus_to_allocate = min(cpus_to_allocate,
        ...
        for component in components:
            ...
            if component['required_cpus']:
                # This should be quite impossible, but better to make sure
