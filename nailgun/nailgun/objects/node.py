...
class Node(NailgunObject):
    ...
    def interpret_disks(disks):
        mapping = {}
        result = {}
        for device in disks['dmsetup_info']:
            print device
            device['DM_BLKDEVS_USED'] = device['DM_BLKDEVS_USED'].split(',')
            device['DM_BLKDEVNAME'] = device['DM_BLKDEVNAME'].strip()
            ...
            disk['disk'] = ''
            disk['extra'] = ''
            disk['mpath'] = sorted(device['DM_BLKDEVS_USED'])
        ...
        for block in disks['blocks']:
            ...
            disk['removable'] = int(block['removable'])
