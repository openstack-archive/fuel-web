...
class Node(NailgunObject):
    ...
    @classmethod
    def get_identification_attributes(cls, instance):
        """Get node mac, id and system_uuid"""
        system_uuid = None
        if getattr(instance, 'meta'):
            system_uuid = instance.meta.get('system', {}).get('uuid')
        return tuple([instance.mac, instance.id, system_uuid])
    ...
    @classmethod
    def get_by_mac_or_uid(cls, mac=None, node_uid=None, query=None):
    ...
    @classmethod
    def get_by_meta(cls, meta, query=None):
        """Find node by mac, id or system uuid.

        :param meta: dict with node metadata
        :returns: Node instance
        """
        node = (cls.get_by_mac_or_uid(mac=meta.get('mac'),
                                      node_uid=meta.get('id'), query=query) or
                cls.get_by_system_uuid(meta, query=query))
        return node

    @classmethod
    def get_system_uuid(cls, meta):
        """Get system uuid from provided metadata

        :param meta: dict with node metadata
        :returns: uuid string
        """
        if type(meta) is dict:
            return meta.get('system', {}).get('uuid')

    @classmethod
    def get_by_system_uuid(cls, meta, query=None):
        ...
        uuid = cls.get_system_uuid(meta.get('meta'))
        if uuid:
            for n in query or db().query(cls.model):
                if uuid == cls.get_system_uuid(n.meta):
                    return n
