...
class Node(NailgunObject):
    ...
    def generate_fqdn_by_hostname(cls, hostname,
        ...
        return u"{instance_name}.{dns_domain}" \
            .format(instance_name=hostname, dns_domain=dns_domain)
    ...
    def get_node_fqdn(cls, instance):
        if instance.cluster:
            ...
            dns_domain = settings.DNS_DOMAIN
        return cls.generate_fqdn_by_hostname(instance.hostname, dns_domain)
    ...
    def get_restrictions_models(cls, instance):
        ...
        cluster = instance.cluster
        models = {
            'settings': Cluster.get_editable_attributes(cluster),
            'cluster': cluster,
            'version': settings.VERSION,
            'networking_parameters': cluster.network_config,
            'node_attributes': cls.get_attributes(instance)
        }
        return models
