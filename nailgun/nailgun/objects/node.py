...
class Node(NailgunObject):
    ...
    def update_tags(cls, instance, new_roles):
        ...
        current_tags = set()
        # Delete all tags associated with current roles keeping any custom
        # tags in place.
        ...
        for tag in instance.tags:
            if tag.tag in current_tags:
                to_be_deleted.append(tag.id)
        Tag.bulk_delete(to_be_deleted)

        # Add tags for all new roles
        for role in new_roles:
            new_tags = roles_metadata.get(role, {}).get('tags', [])
            for tag in new_tags:
