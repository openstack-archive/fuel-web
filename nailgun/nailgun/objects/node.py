...
class Node(NailgunObject):
    ...
    def apply_network_template(cls, instance, template):
        ...
        if node_group not in template['adv_net_template']:
            node_group = 'default'

        node_name = cls.make_slave_name(instance)
        nic_mapping = template['adv_net_template'][node_group]['nic_mapping']
        ...
        template_object = env.from_string(jsonutils.dumps(template))
        node_template = template_object.render(nic_mapping)
        parsed_template = jsonutils.loads(node_template)['adv_net_template']
