...
class OpenstackConfig(NailgunObject):
    ...
    def find_config(cls, **filters):
        ...
        """
        query = cls.find_configs(**filters)
        return query.first()

    @classmethod
    def find_configs(cls, **filters):
        """Returns query for list of configurations for specified filters.

        Example:
            OpenstackConfig.find_configs(cluster_id=10, node_id=12)
        """
        return db().query(cls.model).filter_by(**filters)
    ...
    def find_configs_for_nodes(cls, cluster, nodes):
        ...
        """
        configs_query = cls.find_configs(
