    ...
    def get_all_by_release(cls, release_id):
        ...
        for db_plugin in cls.all():
            for plugin_release in db_plugin.releases:
                plugin_release_os = plugin_release.get('os')
                plugin_release_version = plugin_release.get('version')
                if (release_os == plugin_release_os and
                        release_version == plugin_release_version):
                    if db_plugin not in releated_plugins:
                        releated_plugins.append(db_plugin)

        return releated_plugins
