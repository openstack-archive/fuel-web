...
class TagCollection(NailgunCollection):
    ...
    @classmethod
    def get_node_tags_ids(cls, node_id):
        return cls.get_node_tags(node_id).with_entities(
            models.NodeTag.tag_id
        )

    @classmethod
    def get_node_tags_ids_in_range(cls, node_id, tag_ids):
        return cls.get_node_tags_ids(node_id).filter(
            models.NodeTag.tag_id.in_(tag_ids)
        )

    @classmethod
    def get_tag_nodes(cls, tag_id):
    ...
    def get_cluster_nm_tags(cls, cluster, **kwargs):
        return cls.get_cluster_nm_tags_query(cluster).filter_by(**kwargs)
