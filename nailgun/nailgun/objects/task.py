...
class Task(NailgunObject):
    ...
    def __update_cluster_status(cls, cluster, status, expected_node_status):
        ...
        if expected_node_status is not None:
            ...
            if remaining > 0:
                logger.debug("Detect that cluster '%s' is underway.",
                             cluster.id)
                status = consts.CLUSTER_STATUSES.underway
    ...
    def _update_cluster_data(cls, instance):
        ...
        if instance.name == consts.TASK_NAMES.deploy:
            ...
            elif instance.status == consts.CLUSTER_STATUSES.error and \
        ...
        elif instance.name == consts.TASK_NAMES.deployment and \
                instance.status == consts.TASK_STATUSES.error:
            cls.__update_cluster_status(
                cluster, consts.CLUSTER_STATUSES.error, None
            )
            ...
            if instance.status == consts.TASK_STATUSES.ready:
                cls.__update_cluster_status(
                    cluster,
                    consts.CLUSTER_STATUSES.provisioned,
