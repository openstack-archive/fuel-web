...
class TasksHistory(NailgunObject):
    ...
    def update(cls, deployment_task_id, node_id, task_name, status, summary):
        ...
        if not task_history.time_start and \
                task_history.status != TASK_STATUSES.pending:
            task_history.time_start = datetime.utcnow()
        if task_history.status in [TASK_STATUSES.ready, TASK_STATUSES.error] \
                and not task_history.time_end:
            task_history.time_end = datetime.utcnow()
