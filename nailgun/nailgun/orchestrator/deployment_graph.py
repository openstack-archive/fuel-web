...
class DeploymentGraph(nx.DiGraph):
    ...
    def traverse_from_start(self, start):
        ...
        # A->B, B->C, B->D, C->E
        all_tasks.update(nx.dfs_postorder_nodes(self, start))

        # get all graph nodes connected to stages after current one
        all_tasks.update(self.traverse_stages(consts.STAGES[stage_pos + 1:]))
    ...
    def traverse_to_end(self, end):
        ...
        all_tasks.update(self.traverse_stages(consts.STAGES[:stage_pos]))
        ...
        all_tasks.update(nx.dfs_postorder_nodes(self.reverse(), end))
