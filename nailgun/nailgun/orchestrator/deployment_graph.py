...
class DeploymentGraph(nx.DiGraph):
    ...
    def get_groups_subgraph(self):
        ...
        for k, v in six.iteritems(self.node):
            if k == 'deploy_start':
                print 'START'
                print v
                print self['deploy_start']
            if k == 'deploy_end':
                print 'END'
                print v
                print self['deploy_end']
    ...
    def _check_if_task_exists(self, node_key, node_value):
        if not node_value.get('id'):
            print self['deploy_end']
            print self['deploy_start']
            neighbors = self.neighbors(node_key)
            print neighbors
            raise errors.ObjectNotFound(
                "Task '{task_id}' can't be in requires|required_for|groups|tasks"
                " for {tasks} because doesn't exists in graph".format(
