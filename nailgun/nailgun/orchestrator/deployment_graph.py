...
class DeploymentGraph(nx.DiGraph):
    ...
    def assign_parallel_nodes(self, priority, nodes):
        """This is possible that same node have 2 or more roles that can be
    ...
    def add_priorities(self, nodes):
        ...
        while current_roles:
            current_nodes = self.group_nodes_by_roles(nodes, current_roles)
            one_by_one = []
            parallel = []
            for r in current_nodes.keys():
                role = self.node[r]
