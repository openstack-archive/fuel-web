...
class DeploymentHASerializer90(DeploymentHASerializer80):
    ...
    def serialize_nodes(self, nodes):
        ...
        return self.generate_cpu_pinning(nodes, serialized_nodes)
    ...
    def generate_cpu_pinning(cls, nodes, serialized_nodes):
        ...
        for node, serialized_node in zip(nodes, serialized_nodes):
            if node.replaced_deployment_info:
                continue
            cpu_pinning = cls._generate_cpu_pinning_for_node(
                node, nova_pinning_enabled)
            result.append(
                utils.dict_merge(serialized_node, cpu_pinning))
        return result
    ...
    def _is_nova_cpu_pinning_enabled(nodes):
        ...
        for node in nodes:
            try:
                node_pinning = objects.Node.node_cpu_pinning_info(node)
            except KeyError:
                continue
            cpu_pinning = dict((item['name'], item)
                               for item in node_pinning['components'])
            if cpu_pinning['nova']['required_cpus']:
                return True
        return False

    @classmethod
    def _generate_cpu_pinning_for_node(cls, node, nova_pinning_enabled):
        cpu_pinning = objects.Node.distribute_node_cpus(node)
        cpu_pinning = dict((item['name'], item)
                           for item in cpu_pinning['components'])
        data = {}
        data.update(cls._generate_nova_cpu_pinning(
            cpu_pinning['nova'], nova_pinning_enabled))
        data.update(cls._generate_dpdk_cpu_pinning(cpu_pinning['dpdk']))

        return data

    @staticmethod
    def _generate_nova_cpu_pinning(cpu_pinning, pinning_enabled):
        if pinning_enabled:
            return {'nova': {
                'enable_cpu_pinning': True,
                'cpu_pinning': cpu_pinning['cpus'],
            }}
        else:
            return {'nova': {
    ...
    @staticmethod
    def _generate_dpdk_cpu_pinning(cpu_pinning):
        cpus = cpu_pinning['cpus']
        if not cpus:
            return {}
