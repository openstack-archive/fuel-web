...
class DeploymentMultinodeSerializer(object):
    ...
    def get_common_attrs(cls, cluster):
        ...
        release = Release.get_by_uid(cluster.pending_release_id) \
            if cluster.status == 'update' else cluster.release
        attrs['deployment_mode'] = cluster.mode
        attrs['deployment_id'] = cluster.id
        attrs['repo_metadata'] = release.repo_metadata
        attrs['puppet_modules_source'] = release.pp_modules_source
        attrs['puppet_manifests_source'] = release.pp_manifests_source
        attrs['openstack_version'] = release.openstack_version
        attrs['api_version'] = release.api_version
        attrs['fuel_version'] = release.fuel_version
...
class NeutronNetworkDeploymentSerializer(NetworkDeploymentSerializer):
    ...
    def generate_network_scheme(cls, node):
        ...
        use_vlan_splinters = \
            node.cluster.attributes.editable.get(
                'vlan_splinters', {}
            ).get('value')
