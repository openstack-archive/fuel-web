...
class DeploymentMultinodeSerializer(DeploymentSerializer):
    ...
    def __init__(self, tasks_graph=None):
        if tasks_graph is not None:
            self.tasks_serializer = GraphBasedTasksSerializer(tasks_graph)
        else:
            self.tasks_serializer = NullTasksSerializer
...
def serialize(cluster, nodes, tasks_graph=None, ignore_customized=False):
    ...
    serializer = get_serializer_for_cluster(cluster)(tasks_graph)
