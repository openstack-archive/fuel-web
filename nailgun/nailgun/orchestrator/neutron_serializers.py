...
class NeutronNetworkDeploymentSerializer70(
    ...
    @classmethod
    def get_network_roles_mapping(cls, node, to_interfaces=False):
        """Returns network roles mapping either to interfaces or to
        IP addresses.

        :param node: instance of db.sqlalchemy.models.node.Node
        :param to_interfaces: return mapping either to interfaces or to
        IP addresses (default False)
        :return: dict of network roles mapping
        """
        nm = Cluster.get_network_manager(node.cluster)
        if to_interfaces:
            mapping = nm.default_network_to_endpoint_mapping
        else:
            mapping = nm.get_network_to_ip_mapping(node)
        ...
        # Special case for 'neutron/*' roles:
    ...
    def generate_network_scheme(cls, node):
        ...
        mapping = cls.get_network_roles_mapping(node, to_interfaces=True)
    ...
    def generate_network_metadata(cls, cluster):
        ...
        for node in Cluster.get_nodes_not_for_deletion(cluster):
            ...
            network_roles = cls.get_network_roles_mapping(node)
