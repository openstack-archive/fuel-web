...
class NeutronNetworkTemplateSerializer70(
    ...
    def get_template_for_node(cls, node):
        config_template = node.cluster.network_config.configuration_template
        ...
        for t in template['adv_net_template']:
            if t['node_group'] == 'default':
                default_nic_mapping = t['nic_mapping']
            if t['node_group'] == node.group_id:
                nic_mapping = t['nic_mapping']
        else:
            nic_mapping = default_nic_mapping
        ...
        config_template = config_template.replace("<<", "{").replace(">>","}")
        template = yaml.load(config_template.format(**nic_mapping))
