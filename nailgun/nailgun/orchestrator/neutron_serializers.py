...
class NeutronNetworkDeploymentSerializer70(
    ...
    @classmethod
    def is_valid_bridge_name(cls, name):
        if name in list(consts.DEFAULT_BRIDGES_NAMES):
            return False
        if not re.match('^br-[0-9a-z\-]{0,11}[0-9a-z]$', name):
            return False
        return True
    ...
    def get_node_non_default_bridge_mapping(cls, node):
        ...
        for net in cls.get_node_non_default_networks(node):
            mapping[net['name']] = cls.get_bridge_name(net['name'])
        for k1, v1 in mapping.iteritems():
            for k2, v2 in mapping.iteritems():
                duplicates = 0
                while ((mapping[k1] == mapping[k2] and k1 != k2) or
                        not cls.is_valid_bridge_name(mapping[k2])):
                    duplicates += 1
                    mapping[k2] = cls.get_bridge_name(mapping[k2], duplicates)
        return mapping
