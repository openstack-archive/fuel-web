...
class SriovSerializerMixin(object):

    @classmethod
    def neutron_attrs(cls, cluster):
        attrs = super(SriovSerializerMixin, cls).neutron_attrs(cluster)
        pci_ids = set()
        for n in cluster.nodes:
            for nic in n.nic_interfaces:
                if objects.NIC.get_sriov_enabled(nic):
    ...
    def generate_transformations(cls, node, nm, nets_by_ifaces, is_public,
        ...
        for iface in node.nic_interfaces:
            ...
            if (not iface.bond and iface.name not in nets_by_ifaces and
                    objects.NIC.get_sriov_enabled(iface)):
