...
def _execute_pipeline(data, cluster, nodes, ignore_customized):
    ...
    if ignore_customized:
        ...
        temp_nodes = copy.deepcopy(data['nodes'])
        ...
        fire_callback_on_provisioning_data_serialization(
            data, cluster=cluster, nodes=nodes_without_customized.values())

        # customized nodes
        data['nodes'].extend(six.moves.filterfalse(keyfunc, temp_nodes))
