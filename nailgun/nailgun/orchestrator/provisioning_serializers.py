...
class ProvisioningSerializer61(ProvisioningSerializer):
    ...
    def serialize_pre_provision_tasks(cls, cluster):
        ...
        if is_build_images:
            packages = cls._make_privisioning_package_list(attrs['provision'])
            tasks.append(
    ...
    @classmethod
    def _make_privisioning_package_list(cls, provision_data):
        packages = provision_data.get('packages', '')
        return filter(bool, [s.strip() for s in packages.split('\n')])
