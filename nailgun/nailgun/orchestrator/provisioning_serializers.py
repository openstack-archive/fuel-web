...
class ProvisioningSerializer(object):
    ...
    def serialize_node(cls, cluster_attrs, node):
        ...
        if mellanox_data:
            ...
            k_params = serialized_node['ks_meta']['pm_data']['kernel_params']
            if (mellanox_data['plugin'] in ('ethernet', 'drivers_only') and
                'intel_iommu=' not in k_params):
                    k_params += ' intel_iommu=on'
