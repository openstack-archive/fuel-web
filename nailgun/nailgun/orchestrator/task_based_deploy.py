...
class TaskProcessor(object):
    ...
    def process_tasks(self, origin_task, serialized_tasks):
        ...
        if len(tail) < 2:
            ...
            # do nothing
            if len(tail) > 0:
                yield self.convert_tasks(tail.pop(), origin_task)
            return

        # it is chain of tasks, need to properly handle them
        logger.debug("the chain detected: %s", origin_task['id'])
        task = self.convert_first_task(tail.popleft(), origin_task)
        ...
        for n in itertools.count(1):
            try:
                ...
                break
            ntask = self.convert_to_chain_task(tail.popleft(), origin_task, n)
            # link current task with previous
            self.link_tasks(task, ntask)
            task = ntask
            yield task.copy()

        ntask = self.convert_last_task(tail.pop(), origin_task)
        # link this task with previous in chain
        self.link_tasks(task, ntask)
        yield ntask

    def convert_first_task(self, serialized, origin):
    ...
    def convert_to_chain_task(self, serialized, origin, num):
        """Make the first task in chain.
    ...
    def convert_last_task(self, serialized, origin):
    ...
    def convert_tasks(self, serialized, origin, task_id=None, attrs=None):
