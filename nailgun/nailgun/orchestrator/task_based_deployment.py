...
class TaskProcessor(object):
    ...
    def process_tasks(self, origin_task, serialized_tasks):
        ...
        while True:
            ...
            task = ntask
            task['id'] = self.get_task_id(chain_id, num)
            yield ntask
            num += 1
    ...
    def make_task(self, origin_task, task_id, serialized_task):
    ...
    def add_requires(task, ntask):
        if ntask['uids'] == task['uids']:
            task['requires'] = [task['id']]
        else:
            requires_ex = ntask.setdefault('requires-ex', [])
            for node_id in task['uids']:
                requires_ex.append(
...
class TasksSerializer(object):
        ...
        for node_id, tasks in six.iteritems(self.tasks_per_node):
            for task in six.itervalues(tasks):
                ...
                task['required_for'].extend(
                    self.expand_cross_dependencies(
                        node_id, task.pop('cross-depends-for', None)
                    )
                )
                task['requires'].extend(task.pop('requires-ex', ()))
    ...
    def expand_cross_dependencies(self, node_id, dependencies):
        ...
        for dep in dependencies:
            ...
            logger.debug(
                ...
                roles, dep.get('policy'), ', '.join(node_ids)
            )
    ...
    def resolve_relation(self, pattern, node_ids):
