...
class TasksSerializer(object):
    ...
    def process_task(self, task, nodes, resolver_factory, skip=False):
        ...
        for astute_task in self.task_processor.process_tasks(
            ...
            for node_id in astute_task.pop('uids', ()):
                ...
                # overwrite if existed task skipped and new is not.
                if self.need_overwrite_task(node_tasks, astute_task):
    ...
    @classmethod
    def need_overwrite_task(cls, tasks, task):
        """Checks that task shall overwrite existed one.
