...
class RoleResolvePolicy(object):
    @abc.abstractmethod
...
class SimpleRoleResolvePolicy(RoleResolvePolicy):
...
class StandartConfigRolesHook(ExpressionBasedTask):
    ...
    def __init__(self, task, cluster, nodes, roles_resolver=None):
        super(StandartConfigRolesHook, self).__init__(task, cluster)
        self.nodes = nodes
        if roles_resolver is None:
            self.role_resolver = SimpleRoleResolvePolicy(nodes)
        else:
            self.role_resolver = roles_resolver
...
class CreateVMsOnCompute(GenericRolesHook):
    ...
    def __init__(self, *args, **kwargs):
        super(CreateVMsOnCompute, self).__init__(*args, **kwargs)
...
class UploadConfiguration(GenericRolesHook):
    ...
    def __init__(self, task, cluster, nodes, configs=None, roles_resolver=None):
        super(UploadConfiguration, self).__init__(
            task, cluster, nodes, roles_resolver=roles_resolver
        )
