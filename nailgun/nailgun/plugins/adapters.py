...
class PluginAdapterBase(object):
    ...
    def repo_url(self, cluster):
        ...
        return six.moves.urllib.parse.urlparse(
            repo_base,
            release_info['repository_path']
        )
...
class PluginAdapterV3(PluginAdapterV2):
    ...
    def _process_deployment_tasks(self, data):
        dg = DeploymentGraph.get_for_model(self.plugin)
        if dg:
            DeploymentGraph.update(dg, {'tasks': data})
        else:
            DeploymentGraph.create_for_model({'tasks': data}, self.plugin)
