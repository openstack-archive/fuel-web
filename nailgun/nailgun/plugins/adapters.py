...
class PluginAdapterV5(PluginAdapterV4):
    ...
    def attributes_processors(self):
        ...
        ap.update({
            'releases': self._process_configurations,
    ...
    def _create_release_from_configuration(self, configuration):
        ...
        # apply base template
        base_release = configuration.pop('base_release')
        if base_release:
        ...
        graphs_by_type = {
            consts.DEFAULT_DEPLOYMENT_GRAPH_TYPE:
                configuration.pop('deployment_tasks')
        }
        graphs_list = configuration.pop('graphs')
        for graph in graphs_list:
            graphs_by_type[graph.pop('type')] = graph

        release_obj = nailgun.objects.Release.create(configuration)

        for graph_type, graph_data in six.iteritems():
            nailgun.objects.DeploymentGraph.create_for_model(
                graph_data, release_obj, graph_type)

    def _process_configurations(self, configurations_records):
        """Split new release records from old-style release-deps records.

        :param configurations_records: list of plugins and releases data
        :type configurations_records: list

        :return: configurations that are extending existing
        :rtype: list
        """
        extend_configurations = []
        for configuration in configurations_records:
            is_basic_configuration = configuration.get('is_release', False)
            if is_basic_configuration:
                self._create_release_from_configuration(configuration)
            else:
                extend_configurations.append(configuration)

        return extend_configurations
