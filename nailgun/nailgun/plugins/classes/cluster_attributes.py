...
class ClusterAttributesPlugin(NailgunPlugin):
    ...
    def upload_plugin_attributes(self, cluster):
        ...
        """
        # cluster_attrs = cluster.attributes
        if self.config and cluster.release.version in self.plugin_versions:
            attrs = self.config.get("attributes", {})
            attrs.update(self.metadata)
            # cluster_attrs.editable.update({self.plugin_name: attrs})
            # with self.storage as storage:
            #     storage.add_record(
            #         "cluster_attribute",
            #         {"attributes": attrs,
            #          "cluster_id": cluster.id})
            return {self.plugin_name: attrs}
        return {}

    def process_cluster_attributes(self, cluster, cluster_attrs):
        custom_attrs = cluster_attrs.get(self.plugin_name, {})
        # with self.storage as storage:
    ...
    def metadata(self):
        return {u'metadata': {u'enabled': False, u'toggleable': True,
                              u'weight': 70, u'label': self.plugin_name}}

    def get_cluster_tasks(self, cluster):
        plugin_version = '{0}-{1}'.format(self.plugin.name,
                                          self.plugin.version)
        ...
        if self.plugin in cluster.plugins:
            return task_data
        return []
    ...
    def plugin_versions(self):
        if not self.plugin.releases:
            return []
        return [rel['version'] for rel in self.plugin.releases]
