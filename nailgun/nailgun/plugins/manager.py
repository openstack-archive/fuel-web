...
class PluginManager(object):
    ...
    def process_cluster_attributes(cls, cluster, attrs):
        ...
        for key, attr in six.iteritems(attrs):
            ...
            enabled = attr['metadata'].pop('enabled', False)
            del attr['metadata']['plugin_id']
            del attr['metadata']['label']
            PluginCollection.set_attributes(
                plugin.id, cluster.id, enabled, attr
            )
    ...
    def get_plugins_attributes(cls, cluster):
        ...
        newest_plugins = [p.id for p in PluginCollection.all_newest()]
        plugins_attributes = {}
        for pid, name, title, enabled, attributes\
            ...
            plugins_attributes.update({name: attributes})
