...
class PluginManager(object):
    ...
    def get_plugins_deployment_tasks(cls, cluster):
        ...
        processed_tasks = {}
        for plugin_db in map(wrap_plugin, cluster.plugins):
            depl_tasks = plugin_db.deployment_tasks

            for t_id in (t['id'] for t in depl_tasks):
                if t_id in processed_tasks:
                    raise errors.PluginsTasksOverlapping(
                        ...
                        'id {2}'
                        .format(plugin_db.full_name, processed_tasks[t_id],
                                t_id)
                    )

                processed_tasks[t_id] = plugin_db.full_name
