...
class PluginManager(object):
    ...
    def add_plugin_attributes_for_bond(cls, bond):
        ...
        NodeBondInterfaceClusterPlugin.\
            add_cluster_plugin_for_node_bond(bond)

    @classmethod
    def update_bond_attributes(cls, bond, attributes):
        plugins = {}
        for k in list(attributes):
            if cls.is_plugin_data(attributes[k]):
        ...
        for plugin in six.itervalues(plugins):
            NodeBondInterfaceClusterPlugin.\
                set_attributes(
                    plugin['metadata']['bond_plugin_id'],
                    plugin['attributes']
                )
    ...
    def update_nic_attributes(cls, attributes):
        for k in list(attributes):
            nic_plugin_id = attributes[k].pop('nic_plugin_id', None)
            if nic_plugin_id:
                NodeNICInterfaceClusterPlugin.\
                    set_attributes(nic_plugin_id, {k: attributes.pop(k)})
