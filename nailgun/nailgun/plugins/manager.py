...
class PluginManager(object):
    ...
    def _install_or_update_or_delete_plugins(cls):
        ...
        for plugin_dir in os.listdir(settings.PLUGINS_PATH):
            if plugin_dir in installed_plugins.keys:
                plugin_adapter = wrap_plugin(installed_plugins[plugin_dir])
                metadata = plugin_adapter.get_metadata()
                Plugin.update(installed_plugins[plugin_dir], metadata)
                installed_plugins.pop(plugin_dir, None)
            else:
        ...
        for deleted_plugin in installed_plugins.values():
            if cls._is_plugin_deletion_enabled(deleted_plugin):
                Plugin.delete(deleted_plugin)
            else:
                logger.error("cannot delete plugin {0} from DB because it "
                             "is still used at least in one cluster, but "
                             "it is already deleted on filesystem. Please "
                             "reinstall it using package or disable it"
                             "in every cluster"
                             .format(deleted_plugin.name))
    ...
    @classmethod
    def _is_plugin_deletion_enabled(cls, plugin):
