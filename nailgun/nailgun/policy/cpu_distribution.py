...
def _make_dpdk_chunks(cpus_consumer, dpdk_info, nics_numas):
    if not dpdk_info:
        return []
    if nics_numas and not dpdk_info['required_cpus']:
        raise ValueError(
            "CPU pinning is not configured for DPDK interfaces."
            " At least {0} CPU should be pinned."
            .format(len(nics_numas) + OVS_CORE_CPUS))
    if not dpdk_info['required_cpus']:
        return []
    if not nics_numas:
        raise ValueError(
            "DPDK CPUs distribution error: there is no"
            " any configured DPDK interfaces")

    required = dpdk_info['required_cpus']
    # currently there is requirements that ovs_core_mask
    # should contain 1 CPU
    core_chunk = {
        'required': OVS_CORE_CPUS,
        'rules': [
            Rule('ovs_core_mask', numa_id=nics_numas[0],
                 min_cpus=1, max_cpus=OVS_CORE_CPUS)
        ]
    }
...
def _make_components_chunks(cpus_consumer, components):
    ...
    chunk['rules'].sort(key=operator.attrgetter('component_name'))
    return [chunk]
