...
def distribute_node_cpus(node):
    components = sorted(six.itervalues(
                            node_cpu_pinning_info(node)['components']),
    ...
    for _, group in itertools.groupby(grouped_components, keyfunc):
        distributor = CpuDistributorForGroup(sorted(group, key=operator.itemgetter('required_cpus'), reverse=True))
        while True:
            if not current_cpus:
