...
class NailgunReceiver(object):
    ...
    def _check_repos_connectivity(cls, resp_kwargs, failed_nodes_msg,
                                  failed_repos_msg, err_msg):
        """Process response data for check_repository* tasks

        Response data is the same for check_repository and
        check_repository_with_setup tasks. The only difference
        is error message to be supplied for them. So purpose
        of this method is unification of the data processing
        ...
        if failed_nodes:
            msg = failed_nodes_msg.format(', '.join(failed_nodes))
        if failed_repos:
            msg += failed_repos_msg.format(', '.join(failed_repos))
        if msg and err_msg:
            msg += err_msg
