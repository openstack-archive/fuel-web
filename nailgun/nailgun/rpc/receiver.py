    ...
    def multicast_verification_resp(cls, **kwargs):
        ...
        if task.status == TASK_STATUSES.error:
            ...
            if not_received_nodes:
                msg = (u'Some nodes does'
                       u'not answered: {0}').format(list(not_received_nodes))
                errors.append(msg)
        ...
        db().flush()
        logger.debug(u'Multicast verification message %s', error_msg)
        TaskHelper.update_verify_networks(
            task_uuid, task.status,
            task.progress, error_msg, task.result)
