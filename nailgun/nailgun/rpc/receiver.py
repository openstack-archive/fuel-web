...
class NailgunReceiver(object):
    ...
    def check_repositories_with_setup(cls, **kwargs):
        ...
        nodes = objects.NodeCollection.filter_by_list(
                None, 'id', nodes.keys(), order_by='id')

        failed_nodes = []
        failed_repos = set()
        for node in nodes:
            node_response = response[node.uid]
            if node_response['exit code'] != 0:
                if 'failed_urls' in node_response['stdout']:
                    failed_repos.update(
                        node_response['stdout']['failed_urls'])
