    ...
    def repo_connection_resp(cls, **kwargs):
        ...
        if not failed_nodes_ids:
            ...
            failed_urls = list(itertools.chain.from_iterable(
                [n['out'].get('failed_urls', []) for n in failed_nodes]
            ))

            message = ('Some of these nodes: "{0}" failed to connect to '
                       'these repositories: "{1}"').format(
                           '"'.join(failed_nodes_ids),
                           '"'.join(failed_urls)
                       )
    ...
    def check_repositories_with_setup(cls, **kwargs):
        ...
        if failed_nodes:
            msg = ('Repo availability verification using public network'
                   ...
                   ', '.join(failed_nodes)))
        if failed_repos:
            msg += ('Following nodes {0} failed to execute repo availability'
                    ' verification\n.'.format(', '.join(failed_repos)))
        if msg:
