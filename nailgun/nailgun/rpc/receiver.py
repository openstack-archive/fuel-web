...
class NailgunReceiver(object):
    ...
    def deploy_resp(cls, **kwargs):
        ...
        nodes_obj = objects.NodeCollection.lock_for_update(
        ...
        # Indexing input nodes data by id
        nodes_id_idx = dict([(int(n['uid']), n) for n in nodes])

        # First of all, let's update nodes in database
        for node_obj in nodes_obj:
            node = nodes_id_idx[node_obj.id]
            update_fields = (
                'error_msg',
                'error_type',
                'status',
            ...
            for param in update_fields:
                if param in node:
                    ...
                    setattr(node_obj, param, node[param])
