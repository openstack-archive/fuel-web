...
class NailgunReceiver(object):
    ...
    def reset_environment_resp(cls, **kwargs):
        logger.info(
            "RPC method reset_environment_resp received",
            jsonutils.dumps(kwargs)
        )
        logger.debug(
            jsonutils.dumps(kwargs)
    ...
    def remove_keys_resp(cls, **kwargs):
        logger.info(
            "RPC method remove_keys_resp received",
            jsonutils.dumps(kwargs)
        )
        logger.debug(
            jsonutils.dumps(kwargs)
        )
        message = u"Keys was removed from environment {0}"
        cls._reset_resp(message, **kwargs)

    @classmethod
    def remove_ironic_bootstrap_resp(cls, **kwargs):
        logger.info(
            "RPC method remove_ironic_bootstrap_resp received",
            jsonutils.dumps(kwargs)
        )
        logger.debug(
            jsonutils.dumps(kwargs)
