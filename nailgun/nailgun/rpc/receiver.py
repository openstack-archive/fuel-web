...
class NailgunReceiver(object):
    ...
    def transaction_resp(cls, **kwargs):
        ...
        node_params_map.pop(consts.MASTER_NODE_UID, None)
        node_params_map.pop('None', None)
        ...
        # the next one
        if transaction.parent and status in (consts.TASK_STATUSES.ready, ):
            manager = transactions.TransactionsManager(transaction.cluster.id)
            manager.continue_(transaction.parent)
