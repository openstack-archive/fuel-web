...
class OpenStackInfoCollector(object):
    ...
    def __init__(self, cluster, cluster_nodes):
        proxy_port = 8888
        proxy_host = self.get_online_controllers(cluster_nodes)[0].ip
        ...
        self.auth_url = "http://{0}:5000/v2.0/".format(auth_host)

        self.initialize_clients(os_user, os_password, os_tenant)
    ...
    def get_online_controllers(self, nodes):
        return filter(
            lambda node: "controller" in node.roles and node.online is True,
            nodes
        )

    def get_host_for_auth(self, cluster, nodes):
        if "ha" not in cluster.mode:
            return self.get_online_controllers(nodes)[0].ip

        node_id = nodes[0].id
        admin_network_group = \
            manager.NetworkManager.get_admin_network_group(node_id=node_id)

        host = manager.NetworkManager.get_free_ips(admin_network_group)[0]
