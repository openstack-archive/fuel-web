...
def get_info_from_os_resource_manager(resource_manager, attrs_path):
    ...
    for inst in instances_list:
        ...
        for attr_name, attr_path in six.iteritems(attrs_path):
            ...
            inst_details[attr_name] = _get_attr_by_path(
                obj_dict, attr_path
            )
...
def _get_attr_by_path(obj_dict, attr_path):
    value = obj_dict.get(attr_path[0])

    if isinstance(value, dict):
        return _get_attr_by_path(value, attr_path[1:])
