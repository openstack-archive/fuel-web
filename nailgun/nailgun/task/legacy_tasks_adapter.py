...
def _join_groups(groups, sync_points):
    for group in groups.values():
        for req in group.get('requires', ()):
            if req in groups:
                group.setdefault('cross_depends', []).append({
                    'name': TASK_END_TEMPLATE.format(req),
                    'role': _get_role(groups[req])
                })
            elif req in sync_points:
                group.setdefault('cross_depends', []).append(
                    {'name': req, 'role': None}
                )
        ...
        for req in group.get('required_for', ()):
            if req in groups:
                groups[req].setdefault('cross_depends', []).append({
                    'name': TASK_END_TEMPLATE.format(group['id']),
                    'role': _get_role(group)
                })
            elif req in sync_points:
                group.setdefault('cross_depended_by', []).append(
                    {'name': req, 'role': None}
                )
...
def _get_group_start(group):
    return {
        ...
        'cross_depends': group.get('cross_depends', []),
        'cross_depended_by': [{
...
def _get_group_end(group):
    return {
        ...
        'version': consts.TASK_CROSS_DEPENDENCY,
        'roles': _get_role(group),
        'cross_depended_by': group.get('cross_depended_by', [])
    }
...
def adapt_legacy_tasks(deployment_tasks, legacy_plugin_tasks, role_resolver):
    ...
    _join_groups(groups, sync_points)
    ...
    for stage, tasks in tasks_per_stage:
        ...
        neighbors = sync_points.neighbors(sync_point_name)
        if neighbors:
            logger.debug(
                'The next stage is found for %s: %s',
                sync_point_name, neighbors[0]
            )
            cross_depended_by = [{'name': neighbors[0], 'role': None}]
        else:
            logger.debug(
