...
class ApplyChangesTaskManager(TaskManager):
    ...
    def _execute_async_content(self, supertask, deployment_tasks=None,
        ...
        if nodes_to_deployment:
            nodes_to_deploy = [node for node in nodes_to_deploy if node.id in nodes_to_deployment]
            nodes_to_provision = [node for node in nodes_to_provision if node.id in nodes_to_deployment]
