...
class TaskManager(object):
    ...
    def check_running_task(self, task_names):
        ...
        cluster = hasattr(self, 'cluster')
        if cluster:
            cluster_id = self.cluster.id
        else:
            # use empty string here to comply with get_cluster_by_id
            # implementation
            cluster_id = ''
...
class BaseDeploymentTaskManager(TaskManager):
    ...
    def get_deployment_transaction_name(self, dry_run, noop):
...
class StopDeploymentTaskManager(TaskManager):
    ...
    def execute(self, **kwargs):
        ...
        deployment_task = objects.TaskCollection.get_cluster_tasks(
            self.cluster.id, names=(consts.TASK_NAMES.deployment))
