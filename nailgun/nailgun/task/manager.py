...
class ApplyChangesTaskManager(TaskManager, DeploymentCheckMixin):
    ...
    def get_update_info_task(self):
        if objects.Release.is_lcm_supported(self.cluster.release):
            return None
        return tasks.UpdateNodesInfoTask
    ...
    def _execute_async_content(self, supertask, deployment_tasks=None,
        ...
        deployment_task = self.get_deployment_task()
        update_info_task = self.get_update_info_task()

        deployment_message = None
        if (nodes_to_deploy or affected_nodes or
                (nodes_to_delete and not update_info_task)):
            if nodes_to_deploy:
        ...
        # deployment.
        if nodes_to_delete and not nodes_to_deploy and update_info_task:
            logger.debug(
            ...
            task_message = update_info_task.message(task_deployment)
