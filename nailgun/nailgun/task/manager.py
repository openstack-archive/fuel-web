...
class ApplyChangesTaskManager(BaseDeploymentTaskManager, DeploymentCheckMixin):
    ...
    def execute(self, nodes_to_provision_deploy=None, deployment_tasks=None,
        ...
        try:
            self.check_running_task(delete_obsolete=objects.Task.delete)
...
class ClusterDeletionManager(TaskManager):
    ...
    def execute(self, **kwargs):
        try:
            ...
            raise errors.DeploymentAlreadyStarted(
                "Can't delete environment '{0}' when "
                "running deployment task exists.".format(
                    self.cluster.id
                )
            )
...
class DumpTaskManager(TaskManager):
    ...
    def execute(self, conf=None, **kwargs):
        ...
        self.check_running_task(consts.TASK_NAMES.dump, objects.Task.delete)
...
class GenerateCapacityLogTaskManager(TaskManager):
    ...
    def execute(self, **kwargs):
        ...
        self.check_running_task(
            consts.TASK_NAMES.capacity_log, objects.Task.delete
        )
