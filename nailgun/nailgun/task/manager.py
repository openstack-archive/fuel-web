    ...
    def execute(self):
        ...
        if nodes_to_delete:
            ...
            self._call_silently(
                task_deletion,
                tasks.DeletionTask,
                tasks.DeletionTask.get_nodes_for_cluster(self.cluster))
            # we should have task committed for processing in other threads
            db().commit()
        ...
        for node in nodes:
            if node.cluster_id != self.cluster_id:
                raise errors.InvalidData(
                    "Invalid data -- node's cluster_id {0} does not match "
                    "cluster's id {1}".format(node.cluster_id, self.cluster_id)
                )
