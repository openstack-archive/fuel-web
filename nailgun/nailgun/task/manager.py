...
class TaskManager(object):
    ...
    def _lock_cluster_to_run_unique_task(self, task_name):
        ...
        This method allow to resolve concurrency problems creating cluster-
        level lock for active task of given type.

        .check_running_task method provides close functionality but not locking
        cluster and cleaning task run history.

        .check_no_running_deployment only checking task not cleaning history
        and not locking cluster.
        ...
        existing_task = objects.TaskCollection.filter_by(
            None,
            cluster_id=cluster.id,
            name=task_name
        )
        existing_task = objects.TaskCollection.order_by(
            existing_task, 'id'
        ).first()
            ...
            if existing_task.status in (
                ...
                raise errors.StopAlreadyRunning(
                    "{0} task is already {1}".format(
                        task_name, consts.TASK_STATUSES.running)
                )
        return self.cluster
...
class ProvisioningTaskManager(TaskManager):
    ...
    def execute(self, nodes_to_provision, **kwargs):
        ...
        self._lock_cluster_to_run_unique_task(consts.TASK_NAMES.provision)
        ...
        mule.call_task_manager_async(
            ...
            self.cluster.id,
            task_provision.id,
            nodes_to_provision=nodes_to_provision,
            **kwargs
        )

        return task_provision

    def _execute_async(self, task_provision_id, nodes_to_provision, **kwargs):
