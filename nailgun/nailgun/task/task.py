...
class BaseNetworkVerification(object):
    ...
    def _filter_nodes_for_templates(cls, nodes):
        """Removes unique VLANs and networks with empty VLANs.

        If VLAN is set only on the single node, it is not passed
        to the verificator, since it will cause verification falure.
        Interfaces with empty VLAN list are not passed to the verificator.
        """
    ...
    def get_message_body(self):
        ...
        if self.task.cluster.network_config.configuration_template:
            self._filter_nodes_for_templates(nodes)
