...
class CheckBeforeDeploymentTask(object):
    ...
    def _validate_network_template(cls, task):
        ...
        if not getattr(cluster.network_config, 'configuration_template', None):
            return
        ...
        network_roles = set()
        for network_template in template['templates'].values():
            network_roles.update(network_template['roles'])
