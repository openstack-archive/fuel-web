    ...
    def get_config(cls, cluster):
        ...
        for public in public_networks:
            ...
            nodes = objects.NodeCollection.filter_by(
                None, group_id=group_id, status=consts.NODE_STATUSES.discover)
            if all_public:
                public_nodes = nodes
            else:
                for n in nodes:
            ...
            free_ips = cls.get_free_ips(public, len(public_nodes))
            ...
            for node, ip in zip(public_nodes, free_ips):
                ...
                node_config = {
                    'addr': '{0}/{0}'.format(ip, mask),
                    'gateway': public.gateway,
                    'vlan': public.vlan_start or None,
                    'iface': iface.name,
                    'urls': urls,
                    'uid': node.uid}
