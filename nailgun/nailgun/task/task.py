...
from nailgun.rpc.json_rpc_client import JSONRPCClient
...
#   those which are prepared for removal.
    ...
    def message(cls, task, nodes):
        ...
        return JSONRPCClient.make_orch_message(
            'deploy',
            'deploy_resp',
            {
                'task_uuid': task.uuid,
...
class DeletionTask(object):
    ...
    def execute(self, task, respond_to='remove_nodes_resp'):
        ...
        msg_delete = JSONRPCClient.make_orch_message(
            'remove_nodes',
            respond_to,
            {
                'task_uuid': task.uuid,
...
class VerifyNetworksTask(object):
    ...
    def _message(cls, task, data):
        ...
        message = JSONRPCClient.make_orch_message(
            task.name,
