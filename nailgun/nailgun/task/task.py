...
class BaseNetworkVerification(object):
    ...
    def get_ifaces_on_undeployed_node(self, node, node_json, is_public):
        ...
        for iface in node.nic_interfaces:
            ...
            for ng in assigned_networks:
                ...
                if ng.group_id is None:
                    vlans.append(0)
                    continue
                if ng.name == 'public' and not is_public:
                    continue
    ...
    def get_ifaces_on_deployed_node(self, node, node_json, is_public):
    ...
    def get_message_body(self):
        ...
        for node in self.task.cluster.nodes:
            ...
            is_public = node.id in nodes_w_public
            # Check bonds on deployed nodes and check bonds slave NICs on
            # undeployed ones.
            if node.status == consts.NODE_STATUSES.ready:
                self.get_ifaces_on_deployed_node(node, node_json, is_public)
            else:
                self.get_ifaces_on_undeployed_node(node, node_json, is_public)
