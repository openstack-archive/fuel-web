...
class DeploymentTask(BaseDeploymentTask):
    ...
    @classmethod
    def merge_stage_tasks(cls, tasks_on_modified, tasks_on_affected):
        ...
        for task in tasks_on_affected:
            if 'id' not in task or task['id'] not in tasks_on_modified:
                continue
        ...
        tasks_on_affected.extend(task for task in tasks_on_modified
                                 if 'id' not in task)
    ...
    def granular_deploy(cls, transaction, tasks, nodes,
        ...
        if affected_nodes:
            ...
            pre_deployment = cls.merge_stage_tasks(pre_deployment,
                                                   pre_deployment_affected)
            post_deployment = cls.merge_stage_tasks(post_deployment,
