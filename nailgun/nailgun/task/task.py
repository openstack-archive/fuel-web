...
class CheckNetworksTask(object):
        ...
        for ng in networks:
            ...
            if not ng_db:
                ...
                if ng.get('cidr'):
                    ...
                    if NetworkManager().is_range_intersection(fnet,
                                                              admin_range):
                ...
                if 'ip_ranges' in ng:
                    for k, v in enumerate(ng['ip_ranges']):
                        ...
                        if NetworkManager().is_range_intersection(admin_range,
                                                                  ip_range):
                            net_errors.append("cidr")
                            err_msgs.append(
    ...
    def neutron_check_config(cls, task, data):
        ...
        tagged_nets = {n["name"]: n["vlan_start"] for n in filter(
        ...
        for ng1 in networks:
            ...
            for ng2 in ngs:
                if ng1.get('cidr') and ng2.get('cidr'):
                    ...
                    if NetworkManager().is_cidr_intersection(cidr1, cidr2):
                        net_errors.append("cidr")
                        err_msgs.append(
        ...
        if public.get('cidr') and public.get('gateway'):
            ...
            fl_range = ext_l3['floating']
            if netaddr.IPAddress(fl_range[0]) not in cidr or netaddr.IPAddress(
                    fl_range[1]) not in cidr:
        ...
        if err_msgs:
            task.result = {"id": int(public["id"]), "errors": net_errors}
            db().add(task)
            db().commit()
            full_err_msg = "\n".join(err_msgs)
            raise errors.NetworkCheckError(full_err_msg, add_client=False)
