...
class NodeVisitor(object):
    ...
    def accept(self, node):
        ...
        :return: True if node accepted and no further processing is needed
        """

class TaskFactory(object):


class DeletedNodes(NodeVisitor):
    priority = 1

    def accept(self, node):
        if node.pending_deletion or node.needs_redeletion:
            self.nodes.append(node)
            return True


class AllClusterNodesVisitor(NodeVisitor):
    priority = 10
...
class ProvisionVisitor(NodeVisitor):
    def accept(self, node):
        if node.pending_addition or node.needs_reprovision:
            self.nodes.append(node)


class DeploymentVisitor(ProvisionVisitor):
    def accept(self, node):
        if node.pending_addition or node.needs_redeploy:
            self.nodes.append(node)


class DeployVisitor(NodeVisitor):
    def accept(self, node):


class ClusterTransaction(object):
    visitors = []

    def __init__(self, cluster_id):
        self.cluster = objects.Cluster.get_by_uid(cluster_id)

    def execute(self, node_ids=None, **kwargs):
        logger.info("Start transaction: %s on '%s'.", self.cluster.name)

        all_nodes = objects.NodeCollection.filter_by(
            objects.NodeCollection.all(), cluster_id=self.cluster.id
        )
        if node_ids is not None:
            all_nodes = objects.NodeCollection.filter_by_list(
                all_nodes, node_ids
            )
        ...
        supertask = Task(name=self.task_name, cluster=self.cluster)
