    ...
    def stop_deployment(self, expect_http=202):
        if self.clusters:
            resp = self.app.put(
                ...
                expect_errors=True,
                headers=self.default_headers)
            # TODO(pkaminski): this is a hack, remove it after solving issue
            # with wrong status in test_verify_networks_task_manager ->
            # test_verify_networks_after_stop
            if isinstance(expect_http, list):
