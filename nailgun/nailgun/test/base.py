...
class EnvironmentManager(object):
    ...
    def create_nodes_w_interfaces_count(self,
        ...
        """
        nodes = []
        for i in six.moves.range(nodes_count):
            ...
            if_list = [
                ...
                for i in six.moves.range(if_count)]
            if_list[0]['pxe'] = True
    ...
    def generate_random_mac(self):
        mac = [randint(0x00, 0x7f) for _ in six.moves.range(6)]
...
def datadiff(data1, data2, branch, p=True):
    def iterator(data1, data2):
        if isinstance(data1, (list,)) and isinstance(data2, (list,)):
            return six.moves.range(max(len(data1), len(data2)))
