...
class TestAttributesWithPlugins(BaseIntegrationTest):
    ...
    def test_change_plugins_attributes(self):
        ...
        self.assertEqual(200, resp.status_code)
        attrs = objects.Cluster.get_attributes(self.cluster)
        self.assertIn('testing_plugin', attrs.editable)
        self.assertFalse(
            attrs.editable['testing_plugin']['metadata']['enabled'])
        self.assertEqual(1, attrs.editable['testing_plugin']['value'])
