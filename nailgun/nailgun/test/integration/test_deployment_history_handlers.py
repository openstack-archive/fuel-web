...
class TestDeploymentHistoryHandlers(BaseIntegrationTest):
    ...
    def test_history_collection_handler(self, tasks_mock, rpc_mock):
    ...
    @mock.patch('nailgun.task.task.rpc.cast')
    @mock.patch('objects.Cluster.get_deployment_tasks')
    def test_history_task_handler(self, tasks_mock, rpc_mock):
        ...
        response = self.app.get(
            reverse(
                'DeploymentHistoryTaskCollectionHandler',
                kwargs={
                    'transaction_id': deployment_task.id,
                    'deployment_task_name': 'test'
                }
            ),
            headers=self.default_headers
        )
    ...
    @mock.patch('nailgun.task.task.rpc.cast')
    @mock.patch('objects.Cluster.get_deployment_tasks')
    def test_history_task_handler_not_found(self, tasks_mock, rpc_mock):
        tasks_mock.return_value = self.test_tasks
        ...
        response = self.app.get(
            reverse(
                ...
                kwargs={
                    'transaction_id': deployment_task.id,
                    'deployment_task_name': 'NOSUCHTASK'
                }
            ),
            headers=self.default_headers,
            expect_errors=True
    ...
    @mock.patch('nailgun.task.task.rpc.cast')
    @mock.patch('objects.Cluster.get_deployment_tasks')
    def test_history_task_handler_no_snapshot_fall(self, tasks_mock, rpc_mock):
