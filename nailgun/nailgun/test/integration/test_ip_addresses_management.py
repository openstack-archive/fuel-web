...
class BaseIPAddrTest(BaseNetworkManagerTest):
    def setUp(self):
        ...
        vips = self._create_ip_addrs_by_rules(
            self.cluster,
            self.vips_to_create)
        self.vip_ids = [v.get('id') for v in vips]
        self.expected_vips = [
...
class TestIPAddrList(BaseIPAddrTest):
    ...
    def test_update(self):
        ...
        self.assertEqual(200, resp.status_code)
        for record_id, updated_record in enumerate(resp.json_body):
            for k, v in new_data[record_id].items():
                self.assertEqual(v, updated_record[k])
