...
class TestNetworkManager(BaseNetworkManagerTest):
    ...
    def test_get_node_networks_ips(self):
        ...
        node_net_ips = dict([(ip.network_data.name, ip.ip_addr) for ip in node.ip_addrs])
    ...
    def test_set_node_networks_ips(self):
        ...
        node = self.env.create_node(cluster_id=cluster.id)
        self.env.network_manager.assign_ips([node], consts.NETWORKS.management)
        node_net_ips = dict([(net.name, self.env.network_manager.get_free_ips(net)[0])
    ...
    def test_set_node_netgroups_ids(self):
        ...
        node = self.env.create_node(cluster_id=cluster.id)
        self.env.network_manager.assign_ips([node], consts.NETWORKS.management)
        admin_ng_id = self.env.network_manager.get_admin_network_group_id(node.id)
    ...
    def test_set_nic_assignment_netgroups_ids(self):
        ...
        self.env.network_manager.assign_ips([node], consts.NETWORKS.management)
        admin_ng_id = self.env.network_manager.get_admin_network_group_id(node.id)
        nic_ng_ids = dict([(net.id, admin_ng_id) for iface in node.nic_interfaces
                           for net in iface.assigned_networks_list])
        self.env.network_manager.set_nic_assignment_netgroups_ids(node, nic_ng_ids)
    ...
    def test_set_bond_assignment_netgroups_ids(self):
        ...
        admin_ng_id = self.env.network_manager.get_admin_network_group_id(node.id)
        bond_ng_ids = dict([(net.id, admin_ng_id) for iface in node.bond_interfaces
                            for net in iface.assigned_networks_list])
        self.env.network_manager.set_bond_assignment_netgroups_ids(node, bond_ng_ids)
        self.db.refresh(node)
