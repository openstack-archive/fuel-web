...
class TestNetworkManager(BaseIntegrationTest):
    ...
    def _create_ip_addrs_by_rules(self, cluster, rules):
        ...
        for net_group in cluster.network_groups:
            ...
            for vip_type, ip_addr in vips_by_types.iteritems():
                ip = IPAddr(
    ...
    def test_get_assigned_vips_with_return_objects(self):
        vips_to_create = {
            'management': {
                'haproxy': '192.168.0.1',
                'vrouter': '192.168.0.2',
        ...
        cluster = self.env.create_cluster(api=False)
        expected_vips = self._create_ip_addrs_by_rules(cluster, vips_to_create)
        self.db.commit()
        assigned_vips = self.env.network_manager.get_assigned_vips(
            cluster, return_objects=True)
        call_itervalues = operator.methodcaller("itervalues")
        vips = itertools.chain.from_iterable(
            itertools.imap(call_itervalues, assigned_vips.itervalues()))
        self.assertEqual(set(expected_vips), set(vips))
