...
def _convert_vips(vips):
    """Returns VIPs in a form that is easy to compare on equality.

    This function is used in tests that are related to getting and
    assigning VIPs.

    :param vips: A of dicts that represents VIPs.
    :returns: A two level dict with the next keys mapping
              network_name:vip_name = ip_addr.
    """
    vips = deepcopy(vips)
    for net in vips:
...
class TestNetworkManager(BaseIntegrationTest):
    ...
    def test_assign_admin_ip_multiple_groups(self):
        ...
        for n in self.env.nodes:
            admin_net = objects.NetworkGroup.get_admin_network_group(n)
    ...
    def test_set_netgroups_ids(self):
        ...
        admin_ng_id = objects.NetworkGroup.get_admin_network_group(node).id
