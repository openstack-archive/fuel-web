...
class TestNovaHandlers(TestNetworkChecking):
    ...
    def test_checker_message_format(self):
        ...
        def expose_bad_message_ids(inst):
            if len(inst.result) > 0:
                inst.result[0]["ids"] = 0
            return orig_expose(inst)

        def expose_bad_message_errors(inst):
            if len(inst.result) > 0:
                inst.result[0]["errors"] = 0
            return orig_expose(inst)

        def expose_bad_messages_errors_values(inst):
            if len(inst.result) > 0:
                inst.result[0]["errors"] = ["not-a-net-group-field"]
            return orig_expose(inst)

        def check_wrong_message_format(new_func, format_msg):
            msg = None
            with patch.object(NetworkCheck, 'expose_error_messages', new_func):
                try:
                    self.update_nova_networks_w_error(self.cluster.id,
                                                      self.nets)
        ...
        check_wrong_message_format(expose_bad_message_ids,
                                   "0 is not an instance of <type 'list'>")

        check_wrong_message_format(expose_bad_message_errors,
                                   "0 is not an instance of <type 'list'>")

        check_wrong_message_format(expose_bad_messages_errors_values,
                                   "u'not-a-net-group-field' not found in [")
