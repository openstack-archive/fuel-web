...
class TestNovaHandlers(BaseIntegrationTest):
    ...
    def test_network_checking_fails_if_public_gateway_not_in_cidr(self):
        ...
        self.assertEquals(
            task['message'],
            "Public gateway is not in public CIDR."
        )
    ...
    def test_network_checking_failed_if_private_paired_w_other_network(self):
        ...
        for iface in ifaces:
            ...
            if priv_net:
                priv_if = iface
                break
        priv_if["assigned_networks"].remove(priv_net[0])
        iface = [iface for iface in ifaces[1:]
                 if iface != priv_if][0]
        iface["assigned_networks"].append(priv_net[0])
