...
class TestNovaHandlers(BaseIntegrationTest):
    ...
    def test_network_checking_fails_if_amount_flatdhcp(self):
        self.nets['networks'][-2]["amount"] = 2
        self.nets['networks'][-2]["cidr"] = "10.0.0.0/23"
        resp = self.update_networks(self.cluster.id, self.nets,
                                    expect_errors=True)
        self.assertEquals(resp.status, 202)
        task = json.loads(resp.body)
        ...
        self.assertEquals(
            ...
            "while using FlatDHCP manager.".format(
                self.nets['networks'][-2]["name"]))

    def test_fails_if_netmask_for_public_network_not_set_or_not_valid(self):
        net_without_netmask = dict(self.nets)
        net_with_invalid_netmask = dict(self.nets)

        net_without_netmask['networks'][1]['netmask'] = None
        net_with_invalid_netmask['networks'][1]['netmask'] = '255.255.255.2'

        for networks in [net_without_netmask, net_with_invalid_netmask]:
            resp = self.update_networks(self.cluster.id, networks,
                                        expect_errors=True)
