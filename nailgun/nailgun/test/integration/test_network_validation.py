...
class TestNetworkChecking(BaseIntegrationTest):
    ...
    def check_result_format(self, task):
        if task.get('result'):
            ...
            except errors.NetworkCheckError as ex:
                task['format_error'] = ex.message
        return 'format_error' in task

    def set_cluster_changes_w_error(self, cluster_id, format_error=False):
        ...
        self.assertEquals(self.check_result_format(task), format_error)
        return task
