...
class TestNodeAssignment(BaseIntegrationTest):
    ...
    def test_reassign_role_to_deployed_node(self, _):
        ...
        # Update roles
        refresh_node = filter(lambda n: n.name == 'First', self.cluster.nodes)[0]
        assignment_data = [{"id": refresh_node.id, "roles": ['controller', 'cinder']}]
        self.app.post(
        ...
        self.app.put(reverse(
            'DeploySelectedNodes',
            kwargs={'cluster_id': self.cluster.id}), '{}', headers=self.default_headers)
