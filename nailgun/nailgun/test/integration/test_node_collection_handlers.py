...
class TestHandlers(BaseIntegrationTest):
    ...
    def test_node_create_with_non_unique_macs_in_meta(self):
        # create a node with non-unique macs except PXE
        meta = self.env.default_metadata()
        meta['interfaces'][0]['mac'] = 'de:ad:be:ef:00:00'
        for interface in meta['interfaces'][1:]:
            interface['mac'] = settings.NON_UNIQUE_MACS[0]
        self.env.create_node(api=False, meta=meta)
        # create another node with the same non-unique macs except PXE
        meta['interfaces'][0]['mac'] = 'de:ad:be:ef:00:01'
        resp = self.app.post(
            reverse('NodeCollectionHandler'),
