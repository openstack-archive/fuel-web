...
def make_orchestrator_uri(node_ids=None, tasks=None):
    rst = ''
    if node_ids:
        rst += 'nodes={0}'.format(','.join(node_ids))
    if rst:
        rst += '&'
    if tasks:
        rst += 'tasks={0}'.format(','.join(tasks))
...
class TestSerializedTasksHandler(BaseIntegrationTest):
    ...
    def get_serialized_tasks(self, cluster_id, nodes_ids=None, tasks=None):
        return self.app.get(
            reverse(
                "SerializedTasksHandler",
                kwargs={'cluster_id': cluster_id}) +
            make_orchestrator_uri(nodes_ids, tasks),
            expect_errors=True)
