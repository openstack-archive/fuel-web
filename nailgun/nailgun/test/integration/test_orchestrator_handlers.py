...
class TestSerializedTasksHandler(BaseIntegrationTest):
    ...
    def test_serialized_tasks_returned(self, _):
        ...
        resp = self.get_serialized_tasks(self.cluster.id, nodes_uids)
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(set(nodes_uids + ['null']), set(resp.json.keys()))
        expected_tasks = set(['netconfig', 'globals', 'deploy_legacy',
                              'upload_nodes_info', 'update_hosts'])
        for n in nodes_uids:
            received_tasks = set((t['id'] for t in resp.json[n]))
            self.assertEqual(received_tasks, expected_tasks)
        self.assertEqual(
            set((t['id'] for t in resp.json['null'])),
            set(['post_deployment_start', 'post_deployment_end',
