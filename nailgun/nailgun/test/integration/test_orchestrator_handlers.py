...
class TestDeploymentHandlerSkipTasks(BaseSelectedNodes):
    ...
    def assert_skiped_all_tasks(self, response, mocked_cast):
        self.assertEqual(response.status_code, 202)
        args, kwargs = mocked_cast.call_args
        deployed_uids = [n['uid'] for n in args[1]['args']['deployment_info']]
    ...
    def assert_only_tasks(self, response, mocked_cast):
        self.assertEqual(response.status_code, 202)
        args, kwargs = mocked_cast.call_args
        deployed_uids = [n['uid'] for n in args[1]['args']['deployment_info']]
        deployment_data = next(n for n in args[1]['args']['deployment_info'])
    ...
    def test_skip_tasks(self, mcast):
        action_url = reverse(
            ...
            kwargs={'cluster_id': self.cluster.id}) + \
            make_orchestrator_uri(self.node_uids)
        out = self.send_put(action_url, skip_tasks=self.tasks)
        self.assert_skiped_all_tasks(out, mcast)
    ...
    def test_use_only_certain_tasks(self, mcast):
        action_url = reverse(
            ...
            kwargs={'cluster_id': self.cluster.id}) + \
            make_orchestrator_uri(self.node_uids)
        out = self.send_put(action_url, only_tasks=self.tasks)
        self.assert_only_tasks(out, mcast)

    def test_fail_if_both_types_provided(self):
        action_url = reverse(
            'DeploySelectedNodes',
            kwargs={'cluster_id': self.cluster.id}) + \
            make_orchestrator_uri(self.node_uids)
        out = self.send_put(
            action_url, only_tasks=self.tasks, skip_tasks=self.tasks)
        self.assertEqual(out.status_code, 400)

    @patch('nailgun.task.task.rpc.cast')
    def test_skip_stage(self, mcast):
        action_url = reverse(
            'DeploySelectedNodes',
            kwargs={'cluster_id': self.cluster.id}) + \
            make_orchestrator_uri(self.node_uids)
        out = self.send_put(action_url, skip_tasks=['deploy'])
        self.assert_skiped_all_tasks(out, mcast)

    @patch('nailgun.task.task.rpc.cast')
    def test_execute_only_stage(self, mcast):
        action_url = reverse(
            'DeploySelectedNodes',
            kwargs={'cluster_id': self.cluster.id}) + \
            make_orchestrator_uri(self.node_uids)
        out = self.send_put(action_url, only_tasks=['deploy'])
        self.assert_only_tasks(out, mcast)
