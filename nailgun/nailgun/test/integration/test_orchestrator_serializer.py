...
class TestNovaFlatOrchestratorSerializer61(OrchestratorSerializerTestBase):

    def setUp(self):
        super(TestNovaFlatOrchestratorSerializer61, self).setUp()
    ...
    def test_network_schema(self):
        ...
        for node in facts:
            ...
            self.assertEqual(
                ...
                set(['storage', 'management', 'fw-admin', 'ex'])
            )
            self.assertEqual(
...
class TestNeutronOrchestratorSerializer61(OrchestratorSerializerTestBase):
    ...
    def test_vlan_schema(self):
        ...
        for node in facts:
            ...
            role_set = set(['storage', 'management', 'fw-admin', 'private'])
            if is_public:
                br_set.update(['br-ex', 'br-floating'])
                role_set.update(['ex', 'floating'])
            self.assertEqual(
                set(scheme['endpoints'].keys()),
                br_set
            )
            ...
            if not is_public:
                ...
                transformations = transformations[:3] + \
                    transformations[6:11] + transformations[12:]
            self.assertEqual(
                scheme['transformations'],
                transformations
            )
    ...
    def test_gre_schema(self):
        ...
        for node in facts:
            ...
            role_set = set(['storage', 'management', 'fw-admin', 'mesh'])
            if is_public:
                br_set.update(['br-ex', 'br-floating'])
                role_set.update(['ex', 'floating'])
            self.assertEqual(
            ...
            if not is_public:
                # exclude all 'br-ex' and 'br-floating' objects
                transformations = transformations[:3] + \
                    transformations[6:8] + transformations[9:]
            self.assertEqual(
                scheme['transformations'],
                transformations
            )
