    ...
    def test_routes_while_using_multiple_node_groups(self):
        ...
        for node in facts:
            ...
            for name, descr in six.iteritems(endpoints):
                # nailgun 7.0 doesn't set gateways for non-admin networks in new
                # nodegroupts. Nailgun's serializer doesn't generate routes if
                # no gateway is set. So the only route in this case is for
                # Admin network
                if name == 'br-fw-admin':
