...
class BasePluginTest(base.BaseIntegrationTest):
    ...
    def upload_plugin(self, file_name, force=False, expect_errors=False):
        path = os.path.join(self.env.fixture_dir, 'fake_plugins', file_name)
        resp = self.app.post(
            base.reverse('PluginUploadHandler'),
            collections.OrderedDict([('force', force)]),
            upload_files=[('uploaded', file_name, open(path, 'rb').read())],
            expect_errors=expect_errors
        )
        return resp
...
class TestPluginsApi(BasePluginTest):
    ...
    def setUp(self):
        ...
        self.plugin_1_old = 'fake_plugin-1.0.0.fp'
        self.plugin_1_new = 'fake_plugin-1.0.1.fp'
        self.plugin_2_old = 'fake_plugin-2.0-2.0.0-1.noarch.rpm'
        self.plugin_2_new = 'fake_plugin-2.0-2.0.1-1.noarch.rpm'
    ...
    def test_delete_plugin(self):
        ...
        with mock.patch.object(PackageV1Handler, 'remove'):
    ...
    def test_install_uploaded_plugin(self):
        with mock.patch.object(PackageV1Handler, 'install'):
            resp = self.upload_plugin(self.plugin_1_old)
            self.assertEqual(resp.json['action'], 'installed')

        with mock.patch.object(PackageV2Handler, 'get_metadata') as meta_mock:
            meta_mock.return_value = self.env.get_default_plugin_metadata(
                name='fake_plugin', version='2.0.0', package_version='4.0.0')
            with mock.patch.object(PackageV2Handler, 'install'):
                self.upload_plugin(self.plugin_2_old)
                self.assertEqual(resp.json['action'], 'installed')

    def test_reinstall_uploaded_plugin(self):
        ...
        resp = self.upload_plugin(self.plugin_1_old, expect_errors=True)
        self.assertEqual(resp.status_code, 409)
        with mock.patch.object(PackageV1Handler, 'reinstall'):
            resp = self.upload_plugin(self.plugin_1_old, force=True)
            self.assertEqual(resp.json['action'], 'reinstalled')

        with mock.patch.object(PackageV2Handler, 'get_metadata') as meta_mock:
            meta_mock.return_value = self.env.get_default_plugin_metadata(
                name='fake_plugin', version='2.0.0', package_version='4.0.0')
            resp = self.upload_plugin(self.plugin_2_old, expect_errors=True)
            self.assertEqual(resp.status_code, 409)
            with mock.patch.object(PackageV2Handler, 'reinstall'):
                resp = self.upload_plugin(self.plugin_2_old, force=True)
                self.assertEqual(resp.json['action'], 'reinstalled')

    def test_upgrade_uploaded_plugin(self):
        self.env.create_plugin(api=True, name='fake_plugin', version='1.0.0',
                               package_version='1.0.0')
        self.env.create_plugin(api=True, name='fake_plugin', version='2.0.0',
                               package_version='4.0.0')

        resp = self.upload_plugin(self.plugin_1_new, expect_errors=True)
        self.assertEqual(resp.status_code, 406)

        with mock.patch.object(PackageV2Handler, 'get_metadata') as meta_mock:
            meta_mock.return_value = self.env.get_default_plugin_metadata(
                name='fake_plugin', version='2.0.1', package_version='4.0.0')
            with mock.patch.object(PackageV2Handler, 'upgrade'):
                resp = self.upload_plugin(self.plugin_2_new)
                self.assertEqual(resp.json['action'], 'upgraded')

    def test_downgrade_uploaded_plugin(self):
        self.env.create_plugin(api=True, name='fake_plugin', version='1.0.1',
                               package_version='1.0.0')
        ...
        with mock.patch.object(PackageV2Handler, 'get_metadata') as meta_mock:
            ...
            resp = self.upload_plugin(self.plugin_2_old, expect_errors=True)
            self.assertEqual(resp.status_code, 409)
            with mock.patch.object(PackageV2Handler, 'downgrade'):
                resp = self.upload_plugin(self.plugin_2_old, force=True)
