...
class BasePluginTest(base.BaseIntegrationTest):
    ...
    def sync_plugins(self, sample=None, expect_errors=False, plugin_ids=[]):
        plugin_sync_data = sample or {'ids': plugin_ids}
        resp = self.app.post(
            base.reverse('PluginSyncHandler'),
            jsonutils.dumps(plugin_sync_data),
            headers=self.default_headers,
            expect_errors=expect_errors
        )
...
class TestPluginsApi(BasePluginTest):
    ...
    def test_sync_all_plugins(self):
        ...
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.json['ids'], plugin_ids)

    def test_sync_specific_plugins(self):
        plugin_ids = self._create_new_and_old_version_plugins_for_sync()
        specific_plugins_ids = plugin_ids[:1]

        resp = self.sync_plugins(plugin_ids=specific_plugins_ids)
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.json['ids'], specific_plugins_ids)

    def test_sync_failed_when_plugin_not_found(self):
        plugin_ids = self._create_new_and_old_version_plugins_for_sync()
        ids = [plugin_ids.pop() + 1]
        resp = self.sync_plugins(plugin_ids=ids, expect_errors=True)
