# -*- coding: utf-8 -*-

#    Copyright 2013 Mirantis, Inc.
#
#    Licensed under the Apache License, Version 2.0 (the "License"); you may
#    not use this file except in compliance with the License. You may obtain
#    a copy of the License at
#
#         http://www.apache.org/licenses/LICENSE-2.0
#
#    Unless required by applicable law or agreed to in writing, software
#    distributed under the License is distributed on an "AS IS" BASIS, WITHOUT
#    WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the
#    License for the specific language governing permissions and limitations
#    under the License.


import yaml

from nailgun import objects
from nailgun.test import base


class TestRoleApi(base.BaseIntegrationTest):

    ROLE = """
    name: my_role
    release_id: 'CHANGE THIS'
    meta:
        name: My Role
        description: Something goes here
    volumes:
        - id: os
          allocate_size: all
    """

    def setUp(self):
        super(TestRoleApi, self).setUp()
        self.release = self.env.create_release()
        self.role_data = yaml.load(self.ROLE)
        self.role_data['release_id'] = self.release.id

    def test_get_all_roles(self):

        all_roles = objects.RoleCollection.all()

        resp = self.app.get(
            base.reverse('RoleCollectionHandler'),
            headers=self.default_headers,
        )

        self.assertItemsEqual(
            [r.id for r in all_roles],
            [r['id'] for r in resp.json])

    def test_create_role(self):
        resp = self.env.create_role(self.role_data)
        self.assertEqual(resp.json['meta'], self.role_data['meta'])

    def test_update_role(self):
        changed_name = 'Another name'

        resp = self.env.create_role(self.role_data)

        data = resp.json
        data['meta']['name'] = changed_name

        resp = self.env.update_role(data['id'], data)
        self.assertEqual(resp.json['meta']['name'], changed_name)

    def test_create_role_wo_volumes(self):
        self.role_data['volumes'] = []
        resp = self.env.create_role(self.role_data, expect_errors=True)
        self.assertEqual(resp.status_code, 400)

    def test_delete_role(self):

        resp = self.env.create_role(self.role_data)
        delete_resp = self.env.delete_role(resp.json['id'])

        self.assertEqual(delete_resp.status_code, 204)

    def test_delete_assigned_role(self):
        role = self.env.create_role(self.role_data).json
        self.env.create(
            nodes_kwargs=[
                {'roles': [role['name']], 'pending_addition': True},
            ],
            cluster_kwargs={'release_id': role['release_id']},
        )

        delete_resp = self.env.delete_role(role['id'], expect_errors=True)
        self.assertEqual(delete_resp.status_code, 400)
