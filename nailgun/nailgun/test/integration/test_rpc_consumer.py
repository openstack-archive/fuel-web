...
class TestResetEnvironment(BaseReciverTestCase):
    ...
    def test_task_message(self):
        ...
        base_reset_environment_task = reset_environment.create_subtask(
            consts.TASK_NAMES.base_reset_environment
        )
        ...
        base_reset_environment_message = (
            u"Environment '{0}' was successfully reset".format(
                base_reset_environment_task.cluster.name or
                base_reset_environment_task.cluster_id
            )
        )
        ...
        reset_environment_message = (
            u'\n'.join(
                (base_reset_environment_message, remove_keys_message,
                 remove_ironic_bootstrap_message)
            )
        )

        reset_environment_resp = {
            'task_uuid': base_reset_environment_task.uuid,
            'status': consts.TASK_STATUSES.ready,
            'nodes': [
                {'uid': node.uid},
            ]
        ...
        self.assertEqual(base_reset_environment_message,
                         base_reset_environment_task.message)
