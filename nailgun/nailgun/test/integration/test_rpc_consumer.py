...
class TestResetEnvironment(BaseReciverTestCase):
    ...
    def test_task_message(self):
        ...
        reset_environment_task = Task(
            uuid=str(uuid.uuid4()),
            name=consts.TASK_NAMES.reset_environment,
            cluster_id=cluster.id)
        self.db.add(reset_environment_task)
        self.db.flush()

        remove_keys_task = reset_environment_task.create_subtask(
            consts.TASK_NAMES.remove_keys
        )

        remove_ironic_bootstrap_task = reset_environment_task.create_subtask(
        ...
        self.assertEqual(remove_keys_message,
                         remove_keys_task.message)
