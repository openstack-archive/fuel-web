...
class TestResetEnvironment(BaseReciverTestCase):
    ...
    def test_task_message(self):
        ...
        reset_environment_supertask = Task(
            uuid=str(uuid.uuid4()),
            name=consts.TASK_NAMES.reset_environment_supertask,
            cluster_id=cluster.id)
        self.db.add(reset_environment_supertask)
        self.db.flush()

        reset_environment_task = reset_environment_supertask.create_subtask(
            consts.TASK_NAMES.reset_environment
        )
        remove_keys_task = reset_environment_supertask.create_subtask(
            consts.TASK_NAMES.remove_keys
        )
        remove_ironic_bootstrap_task = reset_environment_supertask.create_subtask(
        ...
        reset_environment_message = (
            u"Environment '{0}' was successfully reset".format(
                reset_environment_task.cluster.name or
                reset_environment_task.cluster_id
            )
        )
        remove_keys_message = u"Keys was removed from environment {0}".format(
        ...
        reset_environment_supertask_message = (
            u'\n'.join(
                (reset_environment_message, remove_keys_message,
                 remove_ironic_bootstrap_message)
            )
        )

        reset_environment_resp = {
            'task_uuid': reset_environment_task.uuid,
            'status': consts.TASK_STATUSES.ready,
            'nodes': [
                {'uid': node.uid},
            ]
        ...
        self.assertEqual(reset_environment_message,
                         reset_environment_task.message)
        ...
        self.assertEqual(reset_environment_supertask_message,
                         reset_environment_supertask.message)
