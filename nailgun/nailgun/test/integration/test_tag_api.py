...
class TestTagApi(base.BaseIntegrationTest):
    ...
    def test_failed_create_tag_with_id(self):
        owner, owner_id = consts.TAG_OWNER_TYPES.cluster, self.cluster.id
        self.tag_data['id'] = 10
        resp = self.env.create_tag(self.owner_map[owner],
                                   owner_id,
        ...
        self.assertIn("is not supported for create", resp.body)

    def test_failed_update_tag_with_other_id(self):
        owner, owner_id = consts.TAG_OWNER_TYPES.cluster, self.cluster.id
        resp = self.env.create_tag(self.owner_map[owner],
                                   owner_id,
                                   self.tag_data)
        ...
        self.tag_data['id'] = next(x for x in xrange(1, 100)
                                   if x != resp.json['id'])
        resp = self.env.update_tag(resp.json['id'],
                                   self.tag_data,
                                   expect_errors=True)
        self.assertEqual(resp.status_code, 400)
        self.assertIn("is not equal with old tag id", resp.body)
