...
class TestTasksLogging(BaseIntegrationTest):
    ...
    def test_update_task_logging_on_deployment(self):
        ...
        # To exclude raise condition errors in the tests we only simulate
        # running process of deployment
        TaskHelper.update_action_log(deploy)
        deploy.status = consts.TASK_STATUSES.running
        deploy.progress = 42
        action_log = objects.ActionLog.get_by_kwargs(task_uuid=deploy.uuid,
                                                     action_name=deploy.name)
