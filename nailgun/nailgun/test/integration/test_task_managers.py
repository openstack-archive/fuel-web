...
class TestUpdateDnsmasqTaskManagers(BaseIntegrationTest):
    ...
    def setUp(self):
        ...
        self.cl = self.db.query(models.Cluster).get(cluster['id'])

    def change_ip_range(self, net_name='fuelweb_admin', status='ready'):
        data = self.env.neutron_networks_get(self.cl['id']).json_body
        ...
        resp = self.env.neutron_networks_put(self.cl['id'], data)
        self.assertEqual(resp.json_body['status'], status)
    ...
    def test_update_admin_failed_while_previous_in_progress(self, mocked_rpc):
        ...
        # in progress
        self.change_ip_range(status='error')
        # no more calls were made
        self.assertEqual(mocked_rpc.call_count, 1)
