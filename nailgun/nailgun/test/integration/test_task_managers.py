...
class TestTaskManagers(BaseIntegrationTest):
    ...
    @fake_tasks(override_state={"progress": 100, "status": "ready"})
    def test_settings_saved_in_transaction(self):
        self.env.create(
            nodes_kwargs=[
                {"pending_addition": True},
                {"pending_deletion": True,
    ...
    @fake_tasks(override_state={"progress": 100, "status": "ready"})
    def test_deployment_info_saves_in_transaction(self):
        self.env.create(
            nodes_kwargs=[
                {"pending_addition": True},
                {"pending_deletion": True,
        ...
        node_ids = [n.uid for n in cluster.nodes]
        # the one node has been deleted
        self.assertEqual(1, len(node_ids))
        # information about master node should be in deployment info
        node_ids.append(consts.MASTER_NODE_UID)
