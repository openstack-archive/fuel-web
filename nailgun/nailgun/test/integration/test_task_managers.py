...
class TestRexecuteOnDeployment(BaseIntegrationTest):
    ...
    def test_reexecute_on_pre_deployment(self, mocked_rpc):
        ...
        ensure_tasks = set(['pre-deployment-1'])
        for t in tasks:
            for ensure_task in ensure_tasks:
                if t['id'] != ensure_task:
                    continue

                self.assertTrue(computes.issubset(t['uids']))
