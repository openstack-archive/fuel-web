...
class TestPostDeploymentReexecuteTasksOrder(BaseIntegrationTest):
    ...
    def test_reexecute_tasks_order(self, mocked_rpc):
        ...
        # Checking reexecute tasks added to the end of deployment tasks list
        # and their priorities
        base_tasks = list(filter(lambda x: x['id'] == 'base', tasks))
        main_ctl = list(filter(lambda x: x['id'] == 'main-controller', tasks))
        main_comp = list(filter(lambda x: x['id'] == 'main-compute', tasks))
        base_depl_priority = base_tasks[0]['priority']
        base_reex_priority = base_tasks[1]['priority']
        ctl_priority = main_ctl[0]['priority']
        comp_priority = main_comp[0]['priority']

        self.assertEqual(2, len(base_tasks))
        self.assertTrue(base_depl_priority < comp_priority)
        self.assertTrue(comp_priority < base_reex_priority)
        self.assertTrue(base_reex_priority < ctl_priority)
