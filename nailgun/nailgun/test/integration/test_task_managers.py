...
class TestPluginDeploymentTasksMixing(BaseIntegrationTest):
    ...
    def _create_plugin(self, **plugin_kwargs):
        default_plugin_kwargs = {
            'deployment_tasks': self._create_deployment_tasks(
                **plugin_kwargs.pop('deployment_task', {})),
            'releases': [
                {
        ...
        plugin_kwargs.update(default_plugin_kwargs)
    ...
    def _create_deployment_tasks(self, **deployment_task_kwargs):
        default_task_kwargs = {
            'type': 'puppet',
            'groups': ['primary-controller']
        }

        deployment_task_kwargs.update(default_task_kwargs)

        deployment_tasks = self.env.get_default_plugin_deployment_tasks(
            **deployment_task_kwargs
        )

        del(deployment_tasks[0]['role'])
    ...
    def test_plugin_depl_tasks_proper_injections(self, cast_mock):
        self.prepare_plugins_for_cluster(
            ...
            [
                ...
                {
                    'name': 'after_second_rel_task',
                    'deployment_task': {
                        'id': 'after-second-rel-task',
                        'requires': ['second-fake-depl-task'],
                        'required_for': ['deploy_end'],
                        'parameters': {
                            'puppet_manifest': 'after-second-rel-task',
                            'puppet_modules': 'test',
                            'timeout': 0
                        }
                    }
        ...
        expected_names = ('between_rel_tasks', 'after_second_rel_task')
        diagnostic_names = [
            t['diagnostic_name'].split('-')[0] for t in predeployment_tasks]

        for name in expected_names:
            self.assertIn(name, diagnostic_names)
