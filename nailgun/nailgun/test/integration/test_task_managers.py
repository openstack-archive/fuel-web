...
class TestPluginDeploymentTasksMixing(BaseIntegrationTest):
    ...
    def test_plugin_depl_task_in_pre_depl(self):
        ...
        # check that serialized task list for pre-deployment stage
        # is not empty
        self.assertTrue(pre_deployment)
        # check that processed task is supplied by test
        task_name = pre_deployment[0]['diagnostic_name'].split('-')[0]
        self.assertEqual(task_name, 'pre_depl_plugin_task')
