...
from nailgun.middleware.profiler import ProfilerMiddleware
...
@unittest.skipUnless(settings.PERFORMANCE_PROFILING_TESTS,
...
class BaseLoadTestCase(BaseTestCase):
    ...
    def tearDown(self):
        ...
        test_path = os.path.join(
            ...
            '{exec_time}{test_name}'.format(
                exec_time=exec_time,
                test_name=self.__str__().split()[0]))
        shutil.copytree(settings.LOAD_TESTS_PATHS['last_load_test'], test_path)
        shutil.rmtree(settings.LOAD_TESTS_PATHS['last_load_test'])
