...
def pytest_sessionfinish(session, exitstatus):
    if settings.PERFORMANCE_PROFILING_TESTS == 1:
        creator = PerformanceDataCsvCreator()
        creator.read_data()
        creator.save_data()


class PerformanceDataCsvCreator(object):
    NEW_REPORT_FILE_NAME = 'test_report.csv'
    OLD_REPORT_FILE_NAME = 'test_report.csv'
    ...
    def _read_historic_data(self):
        if self.has_historic_data:
            with open(self.OLD_REPORT_FILE_NAME, 'rb') as report:
                ...
                first_row = hist_data_reader.next()
                self.column_names = self.column_names + \
                    [name for name in hist_data_reader.fieldnames if not
                        name == self.DATE_FIELD_NAME]

                self.report_data.append(first_row)
                for row in hist_data_reader:
    ...
    def _flatten_json_dictionary(self, data):
        return [
            item for sublist in [
                sub[1].items() for sub in data.items()
            ] for item in sublist
        ]
