...
class TestFuyaqlController(base.BaseUnitTest):
    ...
    def test_set_task_without_task_id(self, obj_mock):
    ...
    def test_set_task_when_task_is_not_false(self, obj_mock):
    ...
    def test_set_task_when_get_by_uid_returns_false(self, obj_mock):
        self.controller._cluster = 'cluster'
        obj_mock.Transaction.get_by_uid.return_value = False
        with mock.patch.object(self.controller, '_set_task') as s_task:
            self.assertFalse(self.controller.set_task(self.controller.EXPECTED,
                                                      1))
        s_task.assert_not_called()
...
class TestFuyaqlInterpreter(base.BaseUnitTest):
    ...
    def test_execute_command_with_invalid_arguments(self, _):
        r = self.interpreter.execute_command(":use cluster")
        self.assertEqual('Not enough arguments for a command were given.', r)
