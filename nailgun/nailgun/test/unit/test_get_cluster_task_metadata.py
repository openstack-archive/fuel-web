...
ZABBIX_TASKS = {'zabbix': [{'description': 'Install zabbix server',
                            'name': 'zabbix::deploy',
                            'priority': 20}],
                'compute': [{'description': 'Install zabbix agent',
                             'name': 'zabbix::agent',
                             'priority': 10,
                             'condition': '"zabbix" in artifacts:roles'}],
                'controller': [{'description': 'Install zabbix agent',
                                'name': 'zabbix::agent',
                                'priority': 10,
                                'condition': '"zabbix" in artifacts:roles'}],
                }
...
class TestGetTaskMetadata(base.BaseTestCase):
    ...
    def test_get_tasks_for_cluster(self, mglob):
        mglob.return_value = ['2014.1.1-5.1/base.yaml',
                              '2014.1.1-5.1/vcenter.yaml',
                              '2014.1.1-5.1/xen.yaml']
        glob_pattern = os.path.join(
            settings.TASK_DIR, self.cluster.release.version, '*.yaml')
        task_metadata = objects.Cluster.get_cluster_tasks(self.cluster)
        mglob.assert_called_once_with(glob_pattern)
...
@mock.patch('nailgun.objects.cluster.glob.glob')
@mock.patch('__builtin__.open', mock.MagicMock(side_effect=get_config))
class TestConditionalTasksBasedOnClusterRoles(base.BaseTestCase):

    def setUp(self):
        super(TestConditionalTasksBasedOnClusterRoles, self).setUp()
        self.env.create()
        self.cluster = self.env.clusters[0]

    @mock.patch('nailgun.objects.cluster.Cluster.get_cluster_artifacts')
    def test_condition_based_existing_role(self, martifacts, mglob):
        martifacts.return_value = {
            'artifacts': {'roles': ['zabbix', 'compute', 'controller']}}
        mglob.return_value = ['2014.1.1-5.1/zabbix.yaml']
        glob_pattern = os.path.join(
            settings.TASK_DIR, self.cluster.release.version, '*.yaml')
        task_metadata = objects.Cluster.get_cluster_tasks(self.cluster)
        mglob.assert_called_once_with(glob_pattern)
        self.assertEqual(task_metadata, ZABBIX_TASKS)
