...
class TestTransactionSerializer(BaseTestCase):
    ...
    def test_distributed_serialization(self, save_settings, save_context):
        with mock.patch(
            ...
            job = mock.Mock()
            job.result = ('1', {"id": "task1", "type": "skipped"}), None
    ...
    @mock.patch('nailgun.lcm.transaction_serializer.os.getcwd',
                return_value=tempfile.gettempdir())
    def test_distributed_serialization_serialize_task(self, _):
        task = {
            'id': 'task1', 'roles': ['controller'],
            'type': 'puppet', 'version': '2.0.0',
            'parameters': {
    ...
    @mock.patch('nailgun.lcm.transaction_serializer.os.getcwd',
                return_value=tempfile.gettempdir())
    def test_distributed_serialization_serialize_task_failure(self, _):
        task = {
            'id': 'task1', 'roles': ['controller'],
            'type': 'puppet', 'version': '2.0.0',
            'parameters': {
    ...
    def test_distributed_serialization_resubmit(self, _):
        policy = lcm.transaction_serializer.DistributedProcessingPolicy(
            self.transaction)

        tasks = [('1', {'id': 'task1', 'roles': ['controller'],
                        'type': 'puppet', 'version': '2.0.0'})]
        success_job = mock.Mock()
        success_job.status = dispy.DispyJob.Finished
        success_job.result = tasks[0], None
        ...
        for status in policy.resubmit_statuses:
            ...
            submit = mock.Mock()
            submit.side_effect = [job, success_job]
