...
class TestTransactionSerializer(BaseTestCase):
    ...
    def test_distributed_serialization(self):
        with mock.patch(
            ...
            with mock.patch(
                    'nailgun.lcm.transaction_serializer.'
    ...
    @mock.patch('nailgun.lcm.transaction_serializer.settings.LCM_DS_NODES',
                new=['xx'])
    def test_distributed_serialization_nodes_list_set_in_config(self):
        policy = lcm.transaction_serializer.DistributedProcessingPolicy(
            self.transaction)
    ...
    @mock.patch('nailgun.lcm.transaction_serializer._load_settings')
    @mock.patch('nailgun.lcm.transaction_serializer._load_context')
    def test_distributed_serialization_rpc_serialize_task(
            self, load_context, load_settings):
        task = {
            'id': 'task1', 'roles': ['controller'],
            'type': 'puppet', 'version': '2.0.0',
            'parameters': {
        ...
        load_settings.return_value = {'a': 'b'}
        # Checking node 1 serialization
        (node_id, serialized), __ = lcm.transaction_serializer.\
            _rpc_serialize_task_for_node(self.transaction.id,
    ...
    @mock.patch('nailgun.lcm.transaction_serializer._load_settings')
    @mock.patch('nailgun.lcm.transaction_serializer._load_context')
    def test_distributed_serialization_rpc_serialize_task_failure(
            self, load_context, load_settings):
        task = {
            'id': 'task1', 'roles': ['controller'],
            'type': 'puppet', 'version': '2.0.0',
