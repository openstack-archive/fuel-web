...
from nailgun.utils.logs import LogParser
...
class BaseTestLPS(BaseIntegrationTest):

    def setUp(self):
        super(BaseTestLPS, self).setUp()
        self.log_dir = tempfile.mkdtemp()
        self.local_log_file = os.path.join(self.log_dir, 'nailgun.log')
        regexp = (r'^(?P<date>\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2}):'
                  '(?P<level>\w+):(?P<text>.+)$')
    ...
    def tearDown(self):
        shutil.rmtree(self.log_dir)
        self.patcher.stop()
        super(BaseTestLPS, self).tearDown()
...
class TestRecentlyAddedLPS(BaseTestLPS):
    ...
    def test_empty_entries_on_repetitive_requests(self):
        self.env.create_cluster(api=False)
        entries = self._create_and_populate_logfile(100)
        expected_entries = list(reversed(entries))[:10]
        total_len = len(
        ...
        self.assertEqual(resp.status_code, 200)
        response = resp.json_body
        self.assertFalse(len(response['entries']))
        self.assertEqual(response['from'], from_byte)
        self.assertEqual(response['to'], total_len)
        self.assertFalse(response['has_more'])


class TestFetchOlderLPS(BaseTestLPS):
    ...
    def test_fetch_with_zero_from_byte(self):
        self.env.create_cluster(api=False)
        entries = self._create_and_populate_logfile(10)
        total_len = len(
            ''.join(map(self._format_log_entry, entries)))
