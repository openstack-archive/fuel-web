...
class TestCephAttributesMigration(base.BaseAlembicMigrationTest):
    ...
    def test_ceph_release_attrs(self):
        ceph_opts = ['fsid', 'mon_key', 'admin_key', 'bootstrap_osd_key',
                     'radosgw_key']
        ...
        results = db.execute(
            ...
            .where(releases.c.state != 'unavailable')
        ).fetchall()

        for release_attrs_row in results:
            release_attrs = jsonutils.loads(release_attrs_row[0])
            for ceph_opt in ceph_opts:
                self.assertIn(ceph_opt, release_attrs['editable']['storage'])
