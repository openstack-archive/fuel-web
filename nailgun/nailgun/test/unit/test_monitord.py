...
class MonitordTest(BaseIntegrationTest):
    def setUp(self):
        super(MonitordTest, self).setUp()

        self.temp_file = tempfile.NamedTemporaryFile()
        settings.MONITORD['states_file'] = self.temp_file.name
    ...
    def test_disk_space_monitor(self):
        ...
        notis = objects.NotificationCollection.all()
        noti = objects.NotificationCollection.order_by(notis, '-id')[0]
