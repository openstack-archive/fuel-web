...
class TestMutableList(TestMutableListBase):
    ...
    def test_append(self, m_changed):
        ...
        m_changed.assert_called_once_with()
        self.assertEqual(['element1', 'element2', 'element'], self.m_list)

    def test_extend(self, m_changed):
        self.m_list.extend(('element3', 'element4', 'element5'))
        m_changed.assert_called_once_with()
        self.assertEqual(['element1', 'element2',
                          'element3', 'element4', 'element5'], self.m_list)
    ...
    def test_set_slice_integration(self, m_changed):
        ...
        m_changed.assert_called_once_with()
        self.assertEqual(['new_element'], self.m_list)

        m_changed.reset_mock()
        self.m_list[:] = ["again_new_element"]
        m_changed.assert_called_once_with()
        self.assertEqual(['again_new_element'], self.m_list)
