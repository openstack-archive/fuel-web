...
class TestNetworkCheck(BaseIntegrationTest):
    ...
    @patch.object(helpers, 'db')
    @patch('objects.Cluster.get_assigned_roles', return_value=['controller',
                                                               'compute'])
    def test_check_network_template(self,
                                    mocked_get_assigned_roles,
                                    mocked_db):
        cluster = self.env.create(
            nodes_kwargs=[
                {'roles': ['controller'], 'pending_addition': True},
        ...
        cluster_db = self.db.query(Cluster).get(cluster['id'])
        fake_template = self.env.read_fixtures(['network_template'])[0]
