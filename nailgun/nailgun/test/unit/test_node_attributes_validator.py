...
class BaseNodeAttributeValidatorTest(base.BaseTestCase):
    def setUp(self):
        ...
        self.node = mock.Mock(meta=meta, attributes=attributes, id=1)
...
class TestNodeAttributesValidatorHugepages(BaseNodeAttributeValidatorTest):
    ...
    def test_valid_hugepages_non_dpdk(self, m_dpdk_nics, m_dpdk_enabled):
        # This test validates that dpdk hugepages value does not impact
        # on hugepages distribution in non dpdk case
        data = {
            'hugepages': {
                'nova': {
                    'value': {
