...
class TestNodeGroups(BaseIntegrationTest):
    ...
    def test_nodegroup_rename(self):
        ...
        resp = self.app.put(
            reverse('NodeGroupHandler', kwargs={'obj_id': resp.json_body['id']}),
            json.dumps({'cluster_id': self.cluster['id'], 'name': 'test_ng_2'}),
            headers=self.default_headers,
            expect_errors=False
        )
    ...
    def test_nodegroup_rename_using_existing_name(self):
        ...
        with self.assertRaisesRegexp(AppError, msg):
            resp = self.app.put(
                reverse('NodeGroupHandler',
                    kwargs={'obj_id': new_ng_resp.json_body['id']}),
                json.dumps({'cluster_id': self.cluster['id'],
                    'name': nodegroup_name}),
                headers=self.default_headers,
                expect_errors=False
            )
