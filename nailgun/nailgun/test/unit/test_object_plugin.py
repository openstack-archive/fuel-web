...
class TestPluginCollection(base.BaseTestCase):
    ...
    def test_get_by_uids(self):
        ...
        # Returns all plugins if there is no received ids
        plugins = PluginCollection.get_by_uids()
        self.assertEqual(len(list(plugins)), 5)
        self.assertListEqual(
            [plugin.id for plugin in plugins], plugin_ids)
        # Returns specific plugins
        ids = plugin_ids[:2]
