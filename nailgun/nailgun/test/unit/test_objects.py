...
class TestNodeObject(BaseIntegrationTest):
    ...
    def test_update_tags(self):
        self.env.create(
            ...
            nodes_kwargs=[{'role': 'controller'}])
        node = self.env.nodes[0]
        controller_tags = ['controller', 'database', 'keystone', 'neutron',
                           'rabbitmq']
        self.assertEqual(controller_tags, objects.Node.all_tags(node))

        objects.Node.update_roles(node, ['controller', 'cinder'])
        controller_tags.append('cinder')
        self.assertItemsEqual(
            controller_tags, objects.Node.all_tags(node)
        )
