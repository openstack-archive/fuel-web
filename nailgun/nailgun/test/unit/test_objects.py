...
class TestObjects(BaseIntegrationTest):
    ...
    def test_to_list_iterable_empty(self):
        ...
        self.assertEqual(nodes_count, len(self.env.nodes))
        nodes_db = objects.NodeCollection.to_list([])
        self.assertEqual(0, len(nodes_db))
...
class TestNodeObject(BaseIntegrationTest):
    ...
    @mock.patch.object(objects.Cluster, 'get_restrictions_models')
    def test_get_restrictions_models(self, get_cluster_restriction_models):
        mocked_cluster_models = {
            'settings': 'some_settings',
            'cluster': 'some_cluster_settings',
            'version': 'version-1',
            'networking_parameters': 'some_network_parameters'
        }
        mocked_node_attributes = {
            'plugin_section_a': 'some_attributes',
            'cpu_pinning': {}
        }
        get_cluster_restriction_models.return_value = mocked_cluster_models
        with mock.patch.object(objects.Node, 'get_attributes',
            ...
            expected_models = {'node_attributes': mocked_node_attributes}
            expected_models.update(mocked_cluster_models)
