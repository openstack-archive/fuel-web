...
class BasePackageManagerTestCase(base.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.path_fp = '/tmp/plugin_name-1.2.0.fp'
        self.path_rpm = '/tmp/plugin_name-1.2-1.2.0-1.noarch.rpm'
...
class TestPackageInstallManager(BasePackageManagerTestCase):
    ...
    def test_get_package_handler(self, meta_mock_fp, meta_mock_rpm):
        ...
        handler = self.manager._get_package_handler(self.path_fp)
        self.assertEqual(handler, PluginPackageFP)

        meta_mock_rpm.return_value = self.env.get_default_plugin_metadata(
            package_version='2.0.0')
        handler = self.manager._get_package_handler(self.path_rpm)
    ...
    @mock.patch.object(PluginPackageFP, 'get_metadata')
    def test_install_fp(self, meta_mock):
        meta_mock.return_value = self.env.get_default_plugin_metadata(
            package_version='1.0.0')
        self.assertEqual(self._process('install', self.path_fp), 'installed')

    @mock.patch.object(PluginPackageFP, 'get_metadata')
    def test_reinstall_fp(self, meta_mock):
        ...
        self.assertRaisesWithMessage(
            ...
            self._process, 'reinstall', self.path_fp
        )
        self.assertEqual(self._process('reinstall', self.path_fp, force=True),
                         'reinstalled')

    @mock.patch.object(PluginPackageRPM, 'get_metadata')
    def test_install_rpm(self, meta_mock):
        meta_mock.return_value = self.env.get_default_plugin_metadata(
            package_version='2.0.0')
        self.assertEqual(self._process('install', self.path_rpm), 'installed')

    @mock.patch.object(PluginPackageRPM, 'get_metadata')
    def test_reinstall_rpm(self, meta_mock):
        self._create_test_plugins()
        meta_mock.return_value = self.env.get_default_plugin_metadata(
        ...
        self.assertRaisesWithMessage(
            errors.AlreadyExists,
            "Plugin with the same version already exists",
            self._process, 'reinstall', self.path_rpm
        )
        self.assertEqual(self._process('reinstall', self.path_rpm, force=True),
                         'reinstalled')

    @mock.patch.object(PluginPackageFP, 'get_metadata')
    def test_upgrade_fp(self, meta_mock):
        self._create_test_plugins()
        meta_mock.return_value = self.env.get_default_plugin_metadata(
        ...
        self.assertRaisesWithMessage(
            ...
            "Upgrade action is not supported for plugins with package "
            "version '1.0.0'. You must use newer plugin format.",
            self._process, 'upgrade', self.path_fp, mock_action=False
        )

    @mock.patch.object(PluginPackageFP, 'get_metadata')
    def test_downgrade_fp(self, meta_mock):
        self._create_test_plugins()
        meta_mock.return_value = self.env.get_default_plugin_metadata(
        ...
        self.assertRaisesWithMessage(
            ...
            "Downgrade action is not supported for plugins with package "
            "version '1.0.0'. You must use newer plugin format.",
            self._process, 'downgrade', self.path_fp, mock_action=False
        )

    @mock.patch.object(PluginPackageRPM, 'get_metadata')
    def test_upgrade_rpm(self, meta_mock):
        self._create_test_plugins()
        meta_mock.return_value = self.env.get_default_plugin_metadata(
            name='single_plugin', version='0.1.7', package_version='2.0.0')
        self.assertEqual(self._process('upgrade', self.path_rpm), 'upgraded')

    @mock.patch.object(PluginPackageRPM, 'get_metadata')
    def test_downgrade_rpm(self, meta_mock):
        self._create_test_plugins()
        meta_mock.return_value = self.env.get_default_plugin_metadata(
            name='single_plugin', version='0.1.3', package_version='2.0.0')
        self.assertEqual(self._process('downgrade', self.path_rpm),
                         'downgraded')
...
class TestPackageRemoveManager(BasePackageManagerTestCase):
    ...
    @mock.patch('nailgun.plugins.package_manager.package_fp.shutil.rmtree')
    def test_remove_fp(self, rmtree_mock):
        self._create_test_plugins()
        pm = self.manager('multiversion_plugin', '2.0.0')
        pm.remove()
        rmtree_mock.assert_called_once_with('{}/{}-{}'.format(
