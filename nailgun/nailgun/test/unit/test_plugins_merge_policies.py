...
class TestNetworkRoleMergePolicy(BaseTestCase):
    ...
    def test_apply_for_conflicted_text_value(self):
        policy = NetworkRoleMergePolicy()
        policy.mergeable = frozenset(("test",))
        target = self._make_plugin_network_role(test=six.text_type('test'))
        patch = self._make_plugin_network_role(test='test1')
        ...
        self.assertEqual(target['properties']['test'], 'test1')
