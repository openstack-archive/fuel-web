...
class TestRBACMiddleware(BaseUnitTest):
    ...
    def test_non_admin_permitted_request(self):
        """Non-admin user should have permissions

        to send request with user settings.
        """
        middleware = RBACMiddleware(self.app)
        middleware.permitted_uris =\
            ['/some/permitted/uri'] + middleware.permitted_uris

        fake_req = webob.Request.blank('/some/permitted/uri', method='POST')
        fake_req.environ['is_public_api'] = False
        fake_req.environ['HTTP_X_ROLES'] = 'reader'
