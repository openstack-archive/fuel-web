...
class TestHandlers(BaseIntegrationTest):
    ...
    def test_release_legacy_tags(self):
        roles_meta = {
            'test': {
                'has_primary': True
            }
        }
        resp = self.env.create_release(api=True, roles_metadata=roles_meta)
        self.assertEqual(
            resp['roles_metadata']['test']['tags'], ['test'])
        self.assertEqual(
            resp['tags_metadata']['test']['has_primary'],
            roles_meta['test']['has_primary'])
