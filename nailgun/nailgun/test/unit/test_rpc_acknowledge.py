...
class TestRpcAcknowledge(base.BaseTestCase):
    ...
    def test_message_acked_if_no_task_found(self):
        def raise_no_task_found(*args, **kwargs):
            raise errors.NoTaskFound()
        self.receiver.test.side_effect = raise_no_task_found
        self.consumer.consume_msg(self.body, self.msg)
        self.assertEqual(self.receiver.test.call_count, 1)
        self.assertEqual(self.msg.ack.call_count, 1)
    ...
    def test_message_acked_if_exception(self):
        def raise_exception(*args, **kwargs):
            raise Exception
        self.receiver.test.side_effect = raise_exception
        self.consumer.consume_msg(self.body, self.msg)
        self.assertEqual(self.receiver.test.call_count, 1)
        self.assertEqual(self.msg.ack.call_count, 1)

    def test_message_not_acked_in_other_cases(self):
        def raise_base_exception(*args, **kwargs):
            raise BaseException
        self.receiver.test.side_effect = raise_base_exception
        self.assertRaises(
            BaseException,
            self.consumer.consume_msg, (self.body, self.msg))
        self.assertEqual(self.msg.ack.call_count, 0)
