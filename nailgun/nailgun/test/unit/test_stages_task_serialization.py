...
class TestHooksSerializers(BaseTaskSerializationTest):
    ...
    def test_copy_keys(self):
        task_config = {
            ...
            'parameters': {
                'files': [{
                    'src': '/var/www/nailgun/keys/{CLUSTER_ID}/{KEY_NAME}.key',
                    'dst': '/var/lib/astute/{KEY_NAME}.key]'}],
                'permissions': '0600',
        ...
        for key_name in consts.KEYS_TYPES:
            files.append({
                'src': '/var/www/nailgun/keys/{CLUSTER_ID}/{KEY_NAME}.key'.
                format(CLUSTER_ID=self.cluster.id, KEY_NAME=key_name),
                'dst': '/var/lib/astute/{KEY_NAME}.key]'.format(
                    KEY_NAME=key_name)})
