...
class TestHooksSerializers(BaseTaskSerializationTest):
    ...
    def test_create_repo_centos(self):
        ...
        self.assertEqual(serialized[1]['type'], 'upload_file')
        self.assertEqual(serialized[2]['type'], 'upload_file')
        self.assertEqual(serialized[3]['type'], 'shell')
        self.assertEqual(serialized[3]['parameters']['cmd'], 'yum clean all')
        self.assertItemsEqual(serialized[3]['uids'], self.all_uids)
    ...
    def test_upload_vms_info(self, m_open):
        vm = objects.VirtualMachinesRequestsCollection.create(
            {'node_id': self.cluster.nodes[0].id,
             'cluster_id': self.cluster.id,
             'params': {'cluster_id': self.cluster.id}})
        ...
        self.assertEqual(
            ...
            [{'data': '<tag>{cluster}</tag><tag>{vm}_vm</tag><tag></tag>'.
              format(cluster=self.cluster.id, vm=vm.id),
             'dst': '/var/lib/vms/{vm}_vm.xml'.format(vm=vm.id)}])
