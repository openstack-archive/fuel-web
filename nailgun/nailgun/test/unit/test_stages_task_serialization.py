...
from nailgun.orchestrator import stages_configuration
...
class TestPreTaskSerialization(base.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.tasks = yaml.load(stages_configuration.PRE_DEPLOYMENT_TASKS)
        self.all_uids = set([n.uid for n in self.nodes])

    def test_tasks_serialized_correctly(self):
        self.cluster.release.operating_system = consts.RELEASE_OS.ubuntu
        tasks = tasks_serializer.pre_tasks_serialize(
            self.tasks, self.cluster, self.nodes)
