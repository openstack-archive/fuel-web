...
class TestOSWLObject(BaseTestCase):
    ...
    def test_save_empty_data(self):
        ...
        self.assertEqual(
            OpenStackWorkloadStats.get_last_by_cluster_id_resource_type(
    ...
    def test_save_stats_data(self):
        ...
        last = OpenStackWorkloadStats.get_last_by_cluster_id_resource_type(
            2, consts.OSWL_RESOURCE_TYPES.vm)
        time_update4 = last.update_time
        ...
        self.assertEqual(last.creation_date, date_cur)
        for rec in saved:
            if rec.creation_date == date_cur:
                self.assertEqual(rec, last)
                for k, v in data_last_rec.iteritems():
                    self.assertEqual(v, getattr(rec, k))
            elif rec.creation_date == date_1st_rec:
                for k, v in data.iteritems():
                    self.assertEqual(v, getattr(rec, k))
