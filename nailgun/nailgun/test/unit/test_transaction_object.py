...
class TestTransactionUpdate(BaseTestCase):
    def setUp(self):
        super(TestTransactionUpdate, self).setUp()
        self.main = objects.Transaction.create({
            'name': consts.TASK_NAMES.deployment,
            'status': consts.TASK_STATUSES.pending,
            'cluster_id': None
        })
        self.child1 = self.main.create_subtask(
            name=consts.TASK_NAMES.deployment,
            status=consts.TASK_STATUSES.pending
        )
        self.child2 = self.main.create_subtask(
            name=consts.TASK_NAMES.deployment,
            status=consts.TASK_STATUSES.pending
        )

    def test_update_parent_to_error_status(self):
        self.child1.status = consts.TASK_STATUSES.ready

        objects.Transaction.update(
            self.main, {'status': consts.TASK_STATUSES.error}, True
        )
        self.assertEqual(consts.TASK_STATUSES.error, self.main.status)
        self.assertEqual(consts.TASK_STATUSES.error, self.child2.status)
        self.assertEqual(consts.TASK_STATUSES.ready, self.child1.status)

    def test_update_child_with_escalation_to_error_status(self):
        objects.Transaction.update(
            self.child1, {'status': consts.TASK_STATUSES.error}, True
        )
        self.assertEqual(consts.TASK_STATUSES.error, self.child1.status)
        self.assertEqual(consts.TASK_STATUSES.error, self.main.status)
        self.assertEqual(consts.TASK_STATUSES.error, self.child2.status)

    def test_update_child_without_escalation_to_error_status(self):
        objects.Transaction.update(
            self.child1, {'status': consts.TASK_STATUSES.error}, False
        )
        self.assertEqual(consts.TASK_STATUSES.error, self.child1.status)
        self.assertEqual(consts.TASK_STATUSES.pending, self.main.status)
        self.assertEqual(consts.TASK_STATUSES.pending, self.child2.status)

    def test_update_child_progress_with_escalation(self):
        objects.Transaction.update(self.child1, {'progress': 50}, True)
        objects.Transaction.update(self.child2, {'progress': 40}, True)
        self.assertEqual(50, self.child1.progress)
        self.assertEqual(40, self.child2.progress)
        self.assertEqual(45, self.main.progress)

    def test_update_child_progress_without_escalation(self):
        objects.Transaction.update(self.main, {'progress': 0}, False)
        objects.Transaction.update(self.child1, {'progress': 50}, False)
        self.assertEqual(50, self.child1.progress)
        self.assertEqual(0, self.main.progress)
