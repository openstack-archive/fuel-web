...
class TestTransactionsManager(BaseUnitTest):
    ...
    def test_execute_if_success(self, mule_mock, objects_mock, db_mock):
        nodes = [
            mock.MagicMock(id=1, uid='1'),
            mock.MagicMock(id=2, uid='2'),
        ]
        graph_types = ['graph1', 'graph2']
        objects_mock.TaskCollection.count.return_value = 0

        result = self.manager.execute(nodes, graph_types, ['task'])
    ...
    def test_execute_if_error(self, mule_mock, objects_mock, db_mock):
        ...
        result = self.manager.execute(None, [], dry_run=True, tasks=['task1'])
    ...
    @mock.patch('nailgun.transactions.manager.objects')
    def test_execute_async_if_success(self, objects_mock, db_mock):
        manager_mock = mock.MagicMock(spec=self.manager)
        manager.TransactionsManager._execute_async(
            manager_mock, 1, graph=['test']
        )
        objects_mock.Transaction.get_by_uid.assert_called_once_with(1)
        db_mock().commit.assert_called_once_with()
        manager_mock._execute.assert_called_once_with(
            objects_mock.Transaction.get_by_uid(), graph=['test']
        )

    @mock.patch('nailgun.transactions.manager.objects')
    def test_execute_async_if_error(self, objects_mock, _):
        manager_mock = mock.MagicMock(spec=self.manager)
        manager_mock._execute.side_effect = ValueError("error")
    ...
    @mock.patch('nailgun.transactions.manager.make_astute_message')
    @mock.patch('nailgun.transactions.manager.objects')
    @mock.patch('nailgun.transactions.manager.rpc')
    @mock.patch('nailgun.transactions.manager.lcm')
    def test_execute_with_force(self, lcm_m, rpc_mo, obj_m, mam_m, db_m):
        manager_mock = mock.MagicMock(spec=self.manager)
        ...
        mam_m.side_effect = [message]

        manager.TransactionsManager._execute(
            manager_mock, transaction, node_ids, graph_types, None, False, True
        )
        ...
        transaction.create_subtask.assert_called_once_with(
            ...
            tasks_snapshot=manager_mock._get_tasks.return_value
        )
        mam_m.assert_called_with(
    ...
    @mock.patch('nailgun.transactions.manager.rpc')
    @mock.patch('nailgun.transactions.manager.lcm')
    def test_execute_with_nodes(self, lcm_m, rpc_mo, obj_m, mam_m, db_m):
        manager_mock = mock.MagicMock(spec=self.manager)
        cluster = mock.MagicMock()
        transaction = mock.MagicMock(cluster=cluster)
        nodes = [
        ...
        obj_m.Cluster.get_by_uid.return_value = cluster
        obj_m.NodeCollection.lock_for_update().all.return_value = nodes
        mam_m.side_effect = messages
        tasks = [[{'id': 't1'}], [{'id': 't2'}]]
        manager_mock._get_tasks.side_effect = tasks
        manager.TransactionsManager._execute(
            manager_mock, transaction, node_ids, graph_types, None, True, False
        )
        ...
        transaction.create_subtask.assert_has_calls([
            ...
            for graph_type, task in zip(graph_types, tasks)
        ], any_order=True)

        mam_m.assert_has_calls([
            mock.call(
                transaction.create_subtask.return_value,
                lcm_m.TransactionContext.return_value,
                x,
