...
class TransactionsManager(object):
    ...
    def execute(self, nodes, graph_type, dry_run, task_names, **kwargs):
        logger.info(
            "Trying to start %s transaction at cluster '%s'",
            'virtual' if dry_run else '', self.cluster_id
        )
        ...
        elif isinstance(graph_type, six.string_types):
            graph_type = graph_type.split(",")

        logger.info("graphs to execute: %s", ', '.join(graph_type))
        if task_names:
            logger.info("selected tasks: %s", ','.join(task_names))
    ...
    def _get_current_state(cls, cluster, nodes, tasks):
        ...
        for tx, data in itertools.groupby(txs, lambda x: x[0]):
            ...
            dict_update(
                deferred_state,
                transaction_obj.get_deployment_info(tx, node_uids=node_ids),
                level=2
            )
        return state
