...
def get_iter_next(iter_obj):
    return iter_obj.__next__ if six.PY3 else iter_obj.next
...
class FakeNodesGenerator:
    ...
    def __init__(self):
        ...
        self.net1_next_ip = get_iter_next(cycle(self.net1.iter_hosts()))
        self.net2 = IPNetwork(NETWORK_2)
        self.net2_next_ip = get_iter_next(cycle(self.net2.iter_hosts()))
    ...
    def _get_network_data(self, net_name):
        if net_name == 'net1':
            return str(self.net1_next_ip()), str(self.net1.netmask)
        if net_name == 'net2':
            return str(self.net2_next_ip()), str(self.net2.netmask)
        return None, None

    def _generate_mac(self):
        # MAC's starts from FF:FF:FF:FF:FF:FE counting down
        mac = str(EUI(281474976710654 - self.mac_counter, dialect=mac_unix))
        self.mac_counter += 1
        return mac
    ...
    def _generate_cpu_meta(self, kind):
        ...
        return {
            'real': real_proc,
            'total': total_proc,
            'spec': [copy.deepcopy(proc) for _ in range(total_proc)]
        }
    ...
    def generate_fake_nodes(self, total_nodes_count, error_nodes_count=None,
        ...
        offloading_ifaces_nodes_indexes = random.sample(
