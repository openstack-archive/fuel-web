...
class FakeNodesGenerator(object):
    ...
    @staticmethod
    def _generate_numa_topology(memory, amaunt=2):
        dist = []
        for i in range(amaunt):
            row = []
            for j in range(amaunt):
                row.append('1.0' if i == j else '2.1')

            dist.append(row)

        cpus_count = random.choice([6, 12, 32]) * amaunt
        ...
        def gen_numa(id_):
            if id_ + 1 == cpus_count:
                ...
                cpus = remain[:cpus_count / amaunt]
                all_cpus.difference_update(cpus)

            return {
                'id': id_,
                'cpus': sorted(cpus),
                'memory': memory // amaunt
            }

        return {
            'distances': dist,
            'numa_nodes': [gen_numa(i) for i in range(amaunt)],
