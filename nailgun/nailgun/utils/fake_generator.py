...
class FakeNodesGenerator:
    ...
    def _get_disk_suffix(self, number):
        base = len(string.ascii_lowercase)
        suffix = ''
        while number >= base:
            suffix += string.ascii_lowercase[number % base]
            number = int(number / base)
        suffix += string.ascii_lowercase[number]
        return suffix[::-1]
    ...
    def _generate_disks_meta(self, amount):
        ...
        for i in six.moves.range(amount):
            suffix = self._get_disk_suffix(i)
            ...
            if i == amount - 1 and total_size == 0:
                while new_disk['size'] == 0:
                    new_disk = copy.deepcopy(random.choice(DISK_SAMPLES))
            new_disk.update({
                'name': 'sd{0}'.format(suffix),
                'disk': 'sd{0}'.format(suffix)
            })
    ...
    def _generate_memory_meta(self, amount):
        ...
        return {
            ...
            'fields': {
                ...
                'meta': {
                    ...
                    'memory': self._generate_memory_meta(random.randint(1, 9))
                }
            }
        }
