...
class GroupPatterns(object):
    ...
    *, !/mongo/   -> compute
    ...
    def all_matches(self, groups_name_patterns, groups):
        ...
        group_name_pattern = re.compile('^{0}$'.format(consts.GROUP_NAME_PATTERN))
        new_non_pattern_names = [g for g in groups_name_patterns if
                                 group_name_pattern.search(g) and g not in groups]
        matches.extend(new_non_pattern_names)
