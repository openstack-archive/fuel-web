...
class TailLPS(LogParsingStrategy):
    ...
    By means of this strategy log entries are being collected until
    `self.max_entries` is not exceeded.
    """
...
class RecentlyAddedLPS(LogParsingStrategy):
    ...
    Actually the base idea of this strategy is very simple. A client
    sends `to` (to_byte) to the API. If `to` is equal (or greater) to
    total log files size then the client receives empty entries
    list. Otherwise, a client receives log entries located between `to`
    byte (including offset) and the end of a log file(s). This strategy
    allows the client to be received only most recent, most recently
    added log entries.
    """
    ...
    def before_log_parsing(self, offset, parser):
        ...
        if not self.log_was_rotated:
            if self.parse_from_byte < 0 or self.parse_to_byte < 0:
                # `parse_from_byte` or `parse_to_byte` may become less
                # than zero only when a log file is being log rotated in
                # the same time when this log parsing is being
                # performed. Actually in such case additional cycle is
                # performed (because of additional file) thus we get
                # negative `parse_from_byte` or/and `parse_to_byte`.
...
class FetchOlderLPS(LogParsingStrategy):
    ...
    By means of this strategy log entries are being collected until
    `self.max_entries` is not exceeded. The log parsing is performed
    backward from `self.from_byte` (including offset).
    """
