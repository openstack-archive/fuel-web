...
class ComponentsRestrictions(object):
    ...
    def _check_component_incompatibles(cls, component, components_names,
        ...
        :raises: errors.InvalidData
        """
        component_incompatibles = component.get('incompatible', [])
        incompatibles_without_predicates = [
            r.get('name') for r in component_incompatibles]
        if all(incompatibles_without_predicates):
            for incompatible in component_incompatibles:
        ...
        elif any(incompatibles_without_predicates):
            raise errors.InvalidData(
                u"Component '{0}' has mixed format of incompatibilities."
                .format(component['name']))
        else:
            check_result = cls._check_predicates(component_incompatibles,
                                                 components_names,
                                                 available_components_names,
                                                 negative_logic=True)
            if check_result:
                raise errors.InvalidData(
                    u"Incompatible requirement was satisfied for component "
                    u"'{0}': {1}({2})".format(
                        component['name'],
        ...
        if all(requires_without_predicates):
            for c_type, group in groupby(sorted(component_requires,
                ...
                for require in group_components:
                    ...
                    raise errors.InvalidData(
                        u"Component '{0}' requires any of components from {1} "
                        u"set.".format(
                            component['name'],
            ...
            if check_result:
                raise errors.InvalidData(
                    u"Requirements was not satisfied for component '{0}': "
                    u"{1}({2})".format(
                        component['name'],
                        check_result['failed_predicate'],
                        sorted(item for item in check_result['items'])),
    ...
    def _check_required_component_types(components_types_set,
        ...
        if missed_types_set:
            raise errors.InvalidData(
                u"Components with {0} types are required but wasn't found "
                u"in data.".format(sorted(missed_types_set)),
                log_message=True
            )
