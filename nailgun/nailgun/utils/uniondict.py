...
class UnionDict(collections.Mapping):
    """Object, which acts like read-only union of two dicts.
    ...
    def __init__(self, a, b):
        if not isinstance(a, dict) or not isinstance(b, dict):
            raise ValueError("UnionDict can only be apllied to 2 dicts.")
    ...
    def __getitem__(self, key):
        ...
        if key not in self.a:
            return self.b[key]
        if isinstance(self.b[key], dict) and isinstance(self.a[key], dict):
