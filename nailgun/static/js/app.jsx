...
function($, _, i18n, Backbone, React, Router, utils, layoutComponents, Coccyx, coccyxMixins, models, KeystoneClient, RootComponent, LoginPage, WelcomePage, ClusterPage, ClustersPage, ReleasesPage, NotificationsPage, SupportPage, CapacityPage) {
    ...
    function App() {
        ...
        this.keystoneClient = new KeystoneClient('/keystone', {cacheTokenFor: 10 * 60 * 1000, tenant: 'admin'});
    ...
    _.extend(App.prototype, {
        ...
        startRouting: function() {
            var Route = Router.Route, Redirect = Router.Redirect;
            var routes = (
                <Route name='home' path='/' handler={RootComponent}>
                    <Route name='login' path='/login' handler={LoginPage} />
                    <Route name='welcome' path='/welcome' handler={WelcomePage} />
                    <Route name='clusters' path='/clusters' handler={ClustersPage} />
                    <Route name='releases' path='/releases' handler={ReleasesPage} />
                    <Route name='notifications' path='/notifications' handler={NotificationsPage} />
                    <Route name='support' path='/support' handler={SupportPage} />
                    <Route name='capacity' path='/capacity' handler={CapacityPage} />
                    <Redirect from='*' to='clusters' />
                </Route>
            );
            //'cluster/:id(/:tab)(/:opt1)(/:opt2)': 'showCluster',
            var container = $('#main-container')[0];
            var globalModels = _.pick(this, 'version', 'user', 'statistics', 'notifications');
            Router.run(routes, _.bind(function(Handler, state) {
                var Page = state.routes[1].handler;
                (Page.fetchData ? Page.fetchData.apply(Page) : $.Deferred().resolve()).done(_.bind(function(pageOptions) {
                    var handler = <Handler {...globalModels} Page={Page} pageOptions={pageOptions} />;
                    this.rootComponent = React.render(handler, container);
                }, this));
            }, this));
        },
