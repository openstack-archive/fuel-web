...
define([
    ...
    models.MirantisCredentials = Backbone.DeepModel.extend(superMixin).extend(cacheMixin).extend({
        ...
        url: 'http://dev_ware.mirantis.com/wp-content/themes/mirantis_responsive_v_1_0/scripts/fuel_forms_api/registration',
        validate: function(attrs) {
            var errors = {};
            _.each(attrs, function(group, groupName) {
                _.each(group, function(setting, settingName) {
        ...
        getValueAttribute: function(name) {
            return name == 'metadata' ? 'enabled' : 'value';
        }
    });
