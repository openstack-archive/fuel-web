            ...
            attrs.networks.each(function(network) {
                if (network.get('meta').configurable) {
                    ...
                    if (network.get('meta').notation == 'ip_ranges') {
                        var ipRangesErrors = this.validateIpRanges(network.get('ip_ranges'), network.get('cidr'));
                    ...
                    _.merge(networkErrors, this.validateVlan(network.get('vlan_start'), forbiddenVlans, 'vlan_start'));
                    ...
                    if (floatingRangeAttr) {
                        var floatingRangesErrors = this.validateIpRanges(attrs.networking_parameters.get(floatingRangeAttr), network.get('cidr'));
                ...
                if (!utils.isNaturalNumber(idRange[0]) || idRange[0] < 2 || idRange[0] > maxId) {
                    ...
                    _.each(_.compact(attrs.networks.pluck('vlan_start')), function(vlan) {
                        if (utils.validateVlanRange(idRange[0], idRange[1], vlan)) {
                            idStartError = $.t('cluster_page.network_tab.validation.vlan_intersection');
                        }
                        return idStartError;
                    });
                }
