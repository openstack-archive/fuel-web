        ...
        checkRestrictions: function(models, action, path) {
            ...
            var result = false,
                warnings = _.map(restrictions, function(restriction) {
                    result = result || Expression.prototype.compile(restriction.condition).evaluate(models);
        ...
        validate: function(attrs, options) {
            ...
            _.each(options.config, function(attributeConfig, attribute) {
                ...
                var hasNoSatisfiedRestrictions =  _.every(_.reject(attributeConfig.restrictions, {action: 'none'}), function(restriction) {
                    ...
                    return !Expression.prototype.compile(restriction.condition).evaluate({default: this});
                }, this);
