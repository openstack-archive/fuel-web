    ...
    models.WizardModel = Backbone.DeepModel.extend({
        ...
        validate: function(attrs, options) {
            ...
            _.each(options.config, function(attributeConfig, attribute) {
                ...
                var shouldValidate =  _.every(_.where(attributeConfig.restrictions, {action: 'hide'}), function(restriction) {
                    // this probably will be changed when other controls need validation
                    return (!utils.evaluateExpression(restriction.condition, {default: this}).value && (restriction.action == 'hide'));
                }, this);
                if (shouldValidate) {
