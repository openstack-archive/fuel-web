...
define(['utils', 'expression', 'deepModel'], function(utils, Expression) {
    ...
    var restrictionMixin = {
        ...
        checkRestrictions: function(models, action, path) {
            ...
            var result = false,
                messages = _.map(restrictions, function(restriction) {
                    if (new Expression(restriction.condition, models).evaluate()) {
                        result = true;
                        return restriction.message;
                    }
                });
            return {result: result, message: _.compact(messages).join(' ')};
        }
    };
