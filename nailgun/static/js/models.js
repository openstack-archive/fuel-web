    ...
    models.Task = Backbone.Model.extend({
        ...
        match: function(filters) {
            ...
            if (filters.group|| filters.name) {
                var names = _.union(utils.composeList(filters.name), _.flatten(_.map(utils.composeList(filters.group), _.bind(function(group) {return this.groups[group];}, this))));
                if (_.contains(names, this.get('name'))) {
                    result = true;
                    if (filters.status) {
