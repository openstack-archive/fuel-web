...
define(['utils', 'deepModel'], function(utils) {
    ...
    models.Cluster = Backbone.Model.extend({
        ...
        canAddNodes: function(role) {
            ...
            if (this.tasks({group: 'network', status: 'running'}).length || this.tasks({group: 'deployment', status: 'running'}).length) {
                return false;
            }
    ...
    models.Task = Backbone.Model.extend({
        ...
        match: function(filters) {
            ...
            if (filters.group || filters.name) {
                if (_.contains(this.groups[filters.group], this.get('name')) || _.contains(utils.composeList(filters.name), this.get('name'))) {
                    result = true;
                    if (filters.status) {
