...
define(['utils', 'deepModel'], function(utils) {
    ...
    models.Task = Backbone.Model.extend({
        ...
        match: function(filters) {
            ...
            if (filters.group|| filters.name) {
                var names = _.reduce(utils.composeList(filters.group), _.bind(function(names, group) {return _.union(names, this.groups[group]);}, this), []);
                if (_.contains(names, this.get('name')) || _.contains(utils.composeList(filters.name), this.get('name'))) {
                    result = true;
                    if (filters.status) {
    ...
    models.Tasks = Backbone.Collection.extend({
        ...
        filterTasks: function(filters) {
            return this.filter(function(task) {return task.match(filters);});
        },
