...
define(['utils', 'deepModel'], function(utils) {
    ...
    models.NetworkConfiguration = Backbone.Model.extend({
        ...
        validateVlan: function(vlan, forbiddenVlans, errorLabel) {
            var errors = {};
            if (!_.isNull(vlan)) {
                if (!utils.isNaturalNumber(vlan) || vlan < 1 || vlan > 4094) {
                    errors[errorLabel] = $.t('cluster_page.network_tab.validation.invalid_vlan');
                } else if (_.contains(forbiddenVlans, vlan)) {
                    errors[errorLabel] = $.t('cluster_page.network_tab.validation.forbidden_vlan');
                }
            }
            return errors;
        },
        ...
        validate: function(attrs) {
            ...
            attrs.networks.each(function(network) {
                ...
                if (network.get('meta').notation == 'ip_ranges') {
                    var ipRangesErrors = this.validateIpRanges(network.get('ip_ranges'), network.getCidr());
                ...
                _.merge(networksErrors, this.validateVlan(network.get('vlan_start'), forbiddenVlans, 'vlan_start'));
                if (!_.isEmpty(networkErrors)) {
                    networksErrors[network.id] = networkErrors;
                }
                var floatingRangeAttr = network.get('meta').floating_range_var;
                if (floatingRangeAttr) {
                    var floatingRangesErrors = this.validateIpRanges(attrs.networking_parameters.get(floatingRangeAttr), network.getCidr());
                    if (floatingRangesErrors.length) {
                        networkingParametersErrors[floatingRangeAttr] = floatingRangesErrors;
                    }
                }
            }, this);
            ...
            if (attrs.networking_parameters.get('net_manager')) {
                ...
                var vlanErrors = this.validateVlan(fixedVlan, attrs.networks.pluck('vlan_start'), 'fixed_networks_vlan_start');
                _.merge(networkingParametersErrors, vlanErrors);
                if (!utils.isNaturalNumber(fixedAmount)) {
                ...
                } else if (_.isEmpty(vlanErrors)) {
                    if (fixedAmount > 4095 - fixedVlan) {
                    ...
                    if (vlanIntersection) {
                        networkingParametersErrors.fixed_networks_vlan_start = $.t('cluster_page.network_tab.validation.forbidden_vlan');
                    }
                }
            } else {
                var idStartError = '', idEndError = '';
                ...
                if (!utils.isNaturalNumber(idRange[0]) || idRange[0] < 2 || idRange[0] > maxId) {
                    idStartError = $.t('cluster_page.network_tab.validation.invalid_id_start');
                } else if (!utils.isNaturalNumber(idRange[1]) || idRange[1] < 2 || idRange[1] > maxId) {
                    idEndError = $.t('cluster_page.network_tab.validation.invalid_id_end');
                } else if (idRange[0] > idRange[1]) {
                    idStartError = $.t('cluster_page.network_tab.validation.invalid_id_range');
                    ...
                    _.each(_.compact(attrs.networks.pluck('vlan_start')), function(vlan) {
                        if (utils.validateVlanRange(idRange[0], idRange[1], vlan)) {
                            idStartError = $.t('cluster_page.network_tab.validation.id_intersection');
                        }
                        return idStartError;
                    });
                }
                var idRangeErrors = [idStartError, idEndError];
