            ...
            _.each(this.attributes, function(group) {
                ...
                _.each(group, function(setting) {
                    ...
                    _.each(setting.values, function(value) {
                        var valueRestrictions = _.map(value.restrictions, utils.expandRestriction);
                        value.disabled = handleRestrictions(_.where(valueRestrictions, {action: 'disable'}));
                        value.visible = !handleRestrictions(_.where(valueRestrictions, {action: 'hide'}));
                    });
                });
            });
        },
