...
define(['utils', 'deepModel'], function(utils) {
    ...
    models.NetworkConfiguration = Backbone.Model.extend({
        ...
        validate: function(attrs) {
            var errors = {};
            var publicCidr = utils.composeCidr(attrs.networks.findWhere({name: 'public'}).get('ip_ranges'), attrs.networks.findWhere({name: 'public'}).get('netmask'));
            var fixedNetwork = attrs.networks.findWhere({name: 'fixed'});

            // validate networks
            var networksErrors = {};
            var netProvider = attrs.dns_nameservers ? 'nova_network' : 'neutron';
                    ...
                    if (attr == 'ip_ranges') {
                        ...
                        if (_.filter(network.get('ip_ranges'), function(range) {return _.compact(range).length;}).length) {
                            _.each(network.get('ip_ranges'), function(range, i) {
                                if (range[0] || range[1]) {
                                    var error = {};
                                    if (utils.validateIP(range[0]) || (network.get('name') == 'public' && !utils.validateIpCorrespondsToCIDR(publicCidr, range[0]))) {
                                        error.start = $.t('cluster_page.network_tab.validation.invalid_ip_start');
                                    } else if (utils.validateIP(range[1]) || (network.get('name') == 'public' && !utils.validateIpCorrespondsToCIDR(publicCidr, range[1]))) {
                                        error.end = $.t('cluster_page.network_tab.validation.invalid_ip_end');
                                    } else if (!utils.validateIPrange(range[0], range[1])) {
                                        error.range = $.t('cluster_page.network_tab.validation.invalid_ip_range');
                                    }
                                    if (!_.isEmpty(error)) {
                                        error.index = i;
                            ...
                            ipRangesErrors.push({index: 0, range: $.t('cluster_page.network_tab.validation.empty_ip_range')});
                        }
                        if (!_.isEmpty(ipRangesErrors)) {
                        ...
                        var forbiddenVlans = _.compact(_.pluck(attrs.networks.reject({id: network.id}), 'vlan_start'));
                        ...
                        } else if (attrs.net_manager == 'VlanManager' && network.get('name') != 'fixed' &&utils.validateVlanRange(fixedNetwork.get('vlan_start'), fixedNetwork.get('vlan_start') + fixedNetwork.get('amount') - 1, vlan)) {
                        ...
                        } else if (network.get('gateway') == 'public' && !utils.validateIpCorrespondsToCIDR(publicCidr, network.get('gateway'))) {
                        ...
                        if (!utils.isNaturalNumber(network.get('amount'))) {
                            ...
                            networkErrors.amount = $.t('cluster_page.network_tab.validation.needs_more_vlan');
                        }
                    }
                });
            ...
            if (!_.isEmpty(attrs.dns_nameservers.attributes)) {
                _.each(attrs.dns_nameservers.nameservers, function(nameserver, i) {
            ...
            var neutronErrors = {};
            if (!_.isEmpty(attrs.neutron_parameters.attributes)) {
                var segmentation = attrs.neutron_parameters.segmentation_type;
                ...
                if (!utils.isNaturalNumber(idRange[0]) || idRange[0] < 2 || idRange[0] > maxId) {
                    ...
                    neutronErrors.id_range = $.t('cluster_page.network_tab.validation.invalid_id_range');
                    ...
                    _.each(_.compact(attrs.networks.pluck('vlan_start')), function(vlan) {
                        if (utils.validateVlanRange(idRange[0], idRange[1], vlan)) {
                            neutronErrors.id_range = $.t('cluster_page.network_tab.validation.id_intersection');
                        }
                        return neutronErrors.id_range;
                    });
                }
                ...
                if (utils.utils.validateIP(floatingIpRange[0])) {
                ...
                } else if (utils.utils.validateIP(floatingIpRange[1])) {
                    ...
                    neutronErrors.floating = $.t('cluster_page.network_tab.validation.invalid_ip_range');
                }
    ...
    models.RedHatAccount = Backbone.Model.extend({
        ...
        validate: function(attrs) {
            ...
            _.each(fields, function(attr) {
                if (!utils.regexes[attr].test(attrs[attr])) {
