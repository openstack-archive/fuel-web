        ...
        validate: function(networkingParameters) {
            ...
            var untaggedNetworks = networks.filter(function(network) {
                if (!network.get('meta').neutron_vlan_range) {
                    var extNetworkData = network.get('meta').ext_net_data;
                    var vlan = extNetworkData ? networkingParameters.get(extNetworkData[0]) : network.get('vlan_start');
                    return _.isNull(vlan);
                }
                return false;
            });
