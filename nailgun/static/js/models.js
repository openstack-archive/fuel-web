    ...
    models.NeutronConfiguration = Backbone.Model.extend({
        ...
        validate: function(attrs) {
            ...
            _.each(_.keys(attrs), function(attr) {
                if (attr == 'L2') {
                    // ID range validation
                    var id_range = attrs[attr].tunnel_id_ranges || attrs[attr].phys_nets.physnet2.vlan_range;
                    ...
                    var mac = attrs[attr].base_mac;
                    if (mac == '' || !(_.isString(mac) && mac.match(macRegexp))) {
                        errors.base_mac = 'Invalid MAC address';
                    }
                } else if (attr == 'predefined_networks') {
                    // CIDR validation
                    errors = _.extend(errors, utils.validateCidr(attrs[attr].net04.L3.cidr, 'cidr-int'));
                    // floating IP range validation
                    var floatingIpRange = attrs[attr].net04_ext.L3.floating;
                    ...
                    var nameserverRegexp = /^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])$/;
                    _.each(attrs[attr].net04.L3.nameservers, function(nameserver, index) {
                        if (nameserver == '' || !(_.isString(nameserver) && nameserver.match(nameserverRegexp))) {
    ...
    models.NetworkConfiguration = Backbone.Model.extend({
        ...
        toJSON: function() {
            return {
                ...
                neutron_parameters: this.get('neutron_parameters')
            };
        },
        isNew: function() {
            return false;
