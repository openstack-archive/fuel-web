    ...
    models.Interface = Backbone.Model.extend({
        ...
        validate: function() {
            ...
            if (networks.where({name: 'public'}).length && networks.where({name: 'floating'}).length) {
                maxUntaggedNetworks += 1;
            }
            // networks with flag "allow_2nd_untagged" allow to have one more untagged network
            maxUntaggedNetworks += networks.filter(function(network) {
                return network.get('meta').allow_2nd_untagged;
            }).length;
            if (untaggedNetworks.length > maxUntaggedNetworks) {
