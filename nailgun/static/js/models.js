...
define(['utils', 'deepModel'], function(utils) {
    ...
    models.NetworkConfiguration = Backbone.Model.extend({
        ...
        validate: function(attrs) {
            ...
            // floating_range flag will be added to an appropriate network. There will be no dependency on the network name
            var publicNetwork = attrs.networks.findWhere({name: 'public'});
            var floatingRangesErrors = utils.validateIpRanges(attrs.networking_parameters.get('floating_ranges'), publicNetwork.get('cidr'));
