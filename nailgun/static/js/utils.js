...
define(['require'], function(require) {
    ...
    var utils = {
        regexes: {
            ...
            netmask: '^[1-2][2,4,5,9][0,2,4,5,8]\\.(0|([1-2][0,2,4,5,9][0,2,4,5,8]))\\.(0|([1-2][0,2,4,5,9][0,2,4,5,8]))\\.[0-9]{1,3}$'
        },
        ...
        urlify: function (text) {
            return utils.linebreaks(text).replace(new RegExp(utils.regexes.url, 'g'), utils.composeLink);
        },
        ...
        validateCidr: function(cidr, field) {
            ...
            var match;
            if (_.isString(cidr)) {
                match = cidr.match(new RegExp(utils.regexes.cidr));
                if (match) {
        ...
        validateIP: function(ip) {
            return _.isString(ip) && !ip.match(new RegExp(utils.regexes.ip));
        },
        ...
        validateNetmask: function(netmask) {
            return _.isString(netmask) && !netmask.match(new RegExp(utils.regexes.netmask));
        },
