...
function(_, i18n, React, models, componentMixins, controls) {
    ...
    var CapacityPage = React.createClass({
        ...
        title: function() {
            return i18n.t('capacity_page.title');
        },
        statics: {
            fetchData: function() {
        ...
        render: function() {
            ...
            return (
                <div>
                    <h3 className='page-title'>{i18n.t('capacity_page.title')}</h3>
    ...
    var LicenseUsage = React.createClass({
        render: function() {
            ...
            return (
                <div>
                    <h4>{i18n.t('capacity_page.license_usage')}</h4>
                    <controls.Table
                        head={[{label: i18n.t('capacity_page.fuel_version'), className: headClassName},
                                {label: i18n.t('capacity_page.fuel_uuid')}]}
                        body={[[capacityReport.fuel_data.release, capacityReport.fuel_data.uuid]]}
                        tableClassName={tableClassName}
                    />
                    <controls.Table
                        head={[{label: i18n.t('capacity_page.env_name'),  className: headClassName},
                            {label: i18n.t('capacity_page.node_count')}]}
                        body={_.map(capacityReport.environment_stats, _.values)}
                        tableClassName={tableClassName} />
                    <controls.Table
                        head={[{label: i18n.t('capacity_page.total_number_alloc_nodes'), className: headClassName},
                                {label: i18n.t('capacity_page.total_number_unalloc_nodes')}]}
                        body={[[capacityReport.allocation_stats.allocated,
                                capacityReport.allocation_stats.unallocated]] }
                        tableClassName={tableClassName} />
                    <controls.Table
                        head={[{label: i18n.t('capacity_page.node_role'),  className: headClassName},
                                {label: i18n.t('capacity_page.nodes_with_config')}]}
                    ...
                    <a href='/api/capacity/csv'  target='_blank' className='btn btn-info'>
                        ...
                        <span>{i18n.t('capacity_page.download_report')}</span>
                    </a>
                </div>
            );
        }
