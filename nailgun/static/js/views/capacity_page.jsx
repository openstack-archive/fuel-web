...
function(React, models, componentMixins) {
    ...
    var CapacityPage = React.createClass({
        ...
        shouldDataBeFetched: function() {
            var condition =  this.props.capacityLog.isNew();
            if (!condition) {
                this.forceUpdate();
            }
            return condition;
        ...
        fetchData: function() {
            if (_.isUndefined(this.props.capacityLog.get('progress')) || this.props.capacityLog.get('progress') < 100 ) {
    ...
    var TableWrapper = React.createClass({
        render: function() {
            var tableClass = "table table-bordered table-striped " + this.props.class;
            return (
                      ...
                      {_.map(this.props.head, function(row) {
                        return <tr> {
                            _.map(row, function(column) {
                                return <th className="name">{column}</th>;
                            })
                       }
                       </tr>;
                    })}
