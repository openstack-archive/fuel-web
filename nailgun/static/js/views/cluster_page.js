...
function(utils, models, commonViews, dialogViews, NodesTab, NetworkTab, SettingsTab, LogsTab, ActionsTab, HealthCheckTab, clusterPageTemplate, clusterCustomizationMessageTemplate, deploymentResultTemplate, deploymentControlTemplate) {
    ...
    ClusterPage = commonViews.Page.extend({
        ...
        getDeploymentTask: function(status) {
            return this.model.task('stop_deployment', status) || this.model.task({group: 'deployment', status: status});
        },
        ...
        dismissTaskResult: function() {
            var task = this.getDeploymentTask() || this.getReleaseSetupTask('error');
            if (task) {
                task.destroy();
            }
        },
        ...
        scheduleUpdate: function() {
            var runningTasks = !!this.model.tasks({group: 'network', status: 'running'}).length || !!this.model.tasks({group: 'deployment', status: 'running'}).length || this.getReleaseSetupTask();
    ...
    DeploymentResult = Backbone.View.extend({
        ...
        bindTaskEvents: function(task) {
            return task.match({group: 'deployment'}) || task.match({name: 'redhat_setup', release: this.model.get('release').id}) ? task.on('change:status', this.render, this) : null;
        },
        ...
        render: function() {
            this.$el.html(this.template(_.extend({
                cluster: this.model,
                task: this.page.getDeploymentTask() || this.page.getReleaseSetupTask('error')
    ...
    DeploymentControl = Backbone.View.extend({
        ...
        bindTaskEvents: function(task) {
            if (task.match({group: 'deployment'}) || task.match({name: 'redhat_setup', release: this.model.get('release').id})) {
        ...
        updateProgress: function() {
            var task = this.page.getDeploymentTask('running') || this.page.getReleaseSetupTask();
            if (task) {
        ...
        render: function() {
            this.$el.html(this.template({
                cluster: this.model,
                task: this.page.getDeploymentTask('running') || this.page.getReleaseSetupTask()
            })).i18n();
            this.updateProgress();
            return this;
        }
