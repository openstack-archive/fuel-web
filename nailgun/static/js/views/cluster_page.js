...
function(utils, models, commonViews, dialogViews, NodesTab, NetworkTab, SettingsTab, LogsTab, ActionsTab, HealthCheckTab, clusterPageTemplate, clusterCustomizationMessageTemplate, deploymentResultTemplate, deploymentControlTemplate) {
    ...
    ClusterPage = commonViews.Page.extend({
        ...
        tabs: [
            {url: 'nodes', title: $.t('cluster_page.nodes')},
            {url: 'network', title: $.t('cluster_page.networks')},
            {url: 'settings', title: $.t('cluster_page.settings')},
            {url: 'logs', title: $.t('cluster_page.logs')},
            {url: 'healthcheck', title: $.t('cluster_page.health_check')},
            {url: 'actions', title: $.t('cluster_page.actions')}
        ],
        updateInterval: 5000,
