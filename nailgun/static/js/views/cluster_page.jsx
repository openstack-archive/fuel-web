...
function($, _, i18n, Backbone, React, utils, models, componentMixins, BackboneViewWrapper, dialogs, NodesTab, NetworkTab, SettingsTab, LogsTab, ActionsTab, HealthCheckTab) {
    ...
    ClusterPage = React.createClass({
        mixins: [
            ...
            componentMixins.backboneMixin('model', 'change:is_customized'),
        ...
        breadcrumbsPath: function() {
            return [['home', '#'], ['environments', '#clusters'], [this.props.model.get('name'), null, true]];
        },
        title: function() {
        ...
        removeFinishedNetworkTasks: function(removeSilently) {
            return this.removeFinishedTasks(this.props.model.tasks({group: 'network'}), removeSilently);
        },
        removeFinishedDeploymentTasks: function(removeSilently) {
            return this.removeFinishedTasks(this.props.model.tasks({group: 'deployment'}), removeSilently);
        },
        removeFinishedTasks: function(tasks, removeSilently) {
            var requests = [];
            _.each(tasks, function(task) {
                if (!task.match({status: 'running'})) {
                    if (!removeSilently) this.props.model.get('tasks').remove(task);
        ...
        onTabLeave: function(e) {
            ...
            if (Backbone.history.getHash() != href.substr(1) && _.result(this.refs.tab, 'hasChanges')) {
                ...
                utils.showDialog(dialogs.DiscardSettingsChangesDialog, {
                    verification: this.props.model.tasks({group: 'network', status: 'running'}).length,
                    cb: function() {
            ...
            if (task) {
                return task.fetch()
                    ...
                    .then(_.bind(function() {
                        return this.props.model.fetchRelated('nodes');
                    }, this));
            } else {
                task = this.props.model.task('verify_networks', 'running');
                return task ? task.fetch() : $.Deferred().resolve();
            }
        },
        deploymentTaskStarted: function() {
            $.when(this.props.model.fetch(), this.props.model.fetchRelated('nodes'), this.props.model.fetchRelated('tasks')).always(_.bind(function() {
                // FIXME: hack to prevent 'Deploy' button flashing after deployment is finished
                this.props.model.set({changes: []}, {silent: true});
                this.startPolling();
            }, this));
        },
        deploymentTaskFinished: function() {
            $.when(this.props.model.fetch(), this.props.model.fetchRelated('nodes'), this.props.model.fetchRelated('tasks')).always(app.navbar.refresh);
        },
        ...
        componentWillMount: function() {
            ...
            if (!_.find(availableTabs, {url: this.props.activeTab})) {
                app.navigate('cluster/' + this.props.model.id + '/' + availableTabs[0].url, {replace: true});
                return;
            }
            this.props.model.on('change:name', app.updateTitle, app);
            this.props.model.on('change:release_id', function() {
                var release = new models.Release({id: this.props.model.get('release_id')});
                release.fetch().done(_.bind(function() {
                    this.props.model.set({release: release});
                }, this));
            }, this);
        ...
        render: function() {
            var cluster = this.props.model,
                ...
                Tab = _.find(availableTabs, {url: this.props.activeTab}).tab,
                tab = <Tab ref='tab' model={cluster} tabOptions={this.props.tabOptions} />;
            return (
                <div>
                    <ClusterInfo cluster={cluster} />
                    <DeploymentResult cluster={cluster} />
                    {cluster.get('is_customized') && <div className='alert alert-block globalalert'>
        ...
        render: function() {
            ...
            var error = task.match({status: 'error'}),
                ...
                classes = {
                    ...
                    'alert-success': !error,
                    'global-success': deploymentOrUpdate,
                    globalalert: !deploymentOrUpdate
                };
            return (
                <div className="deployment-result">
                    {task && <div className={cs(classes)}>
                        ...
                        <div className='pull-right deployment-control-box'>
                            <div className={itemClass}>
                                <button
                                    ...
                                    disabled={(!cluster.hasChanges() && !cluster.needsRedeployment()) || nodes.every({status: 'ready', pending_deletion: false})}
