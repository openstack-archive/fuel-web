...
function($, _, i18n, Backbone, React, utils, models, componentMixins, BackboneViewWrapper, dialogs, NodesTab, NetworkTab, SettingsTab, LogsTab, ActionsTab, HealthCheckTab) {
    ...
    ClusterPage = React.createClass({
        ...
        statics: {
            ...
            fetchData: function(id, activeTab) {
                ...
                if (currentClusterId == id) {
                    ...
                    promise = $.when(cluster.fetch(), cluster.get('settings').fetch(), cluster.fetchRelated('nodes'), cluster.fetchRelated('tasks'), app.tasks.fetch())
                        .then(function() {
        ...
        getTask: function(status) {
            ...
            return this.props.cluster.task({group: 'deployment', status: status});
        },
        render: function() {
