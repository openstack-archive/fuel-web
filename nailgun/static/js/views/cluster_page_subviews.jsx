                    ...
                    <div className='cluster-name-box'>
                        <div className='cluster-name-placeholder'>
                            <div className='name-box'>
                                ...
                                <div className='node-list-name-count'>({i18n.t('common.node', {count: cluster.get('nodes').length})})</div>
                                <div className='clearfix'/>
                            </div>
                        </div>
                        <div className='cluster-summary-placeholder'>
                            <div>
                                <strong>{i18n.t('cluster_page.openstack_release')}: </strong>
                                {cluster.get('release').get('name')} ({cluster.get('release').get('version')})
                            </div>
                            <div>
                                <strong>{i18n.t('cluster_page.deployment_mode')}: </strong>
                                {i18n.t('cluster.mode.' + cluster.get('mode'))}
                            </div>
                            <div className={_.contains(['error', 'update_error'], cluster.get('status')) ? 'error' : ''}>
                                <strong>{i18n.t('cluster_page.environment_status')}: </strong>
                                {i18n.t('cluster.status.' + cluster.get('status'))}
                            </div>
                        </div>
                    </div>
                </div>
        ...
        render: function() {
            ...
            return !task ? (<div/>) : (
                <div className={'alert alert-block ' + (task.match({status: 'error'}) ? 'alert-error global-error' : 'alert-success ' + (task.match({name: ['deploy', 'update']}) ? 'global-success' : 'globalalert'))}>
                    ...
                    <h4>{i18n.t('common.' + (task.match({status: 'error'}) ? 'error' : 'success'))}</h4>
        ...
        render: function() {
            ...
            return task ? (
                    <div className={'pull-right deployment-progress-box ' + taskName}>
                                ...
                                {taskName != 'update' &&
                                    <div className={itemClass}>
                                        <button
                                            ...
                                            title={i18n.t('cluster_page.stop_deployment_button')}
                        ...
                        <div className='progress-bar-description'>{i18n.t('cluster_page.' + taskName, {defaultValue: ''})}</div>
                    </div>
                ) : (
                    <div className='pull-right deployment-control-box'>
                        <div className={itemClass}>
                            ...
                            >
                                ...
                                {i18n.t('cluster_page.deploy_changes')}
                            </button>
                        </div>
                        {nodes.hasChanges() &&
                            <div className={itemClass}>
                                <button
                                    ...
                                    title={i18n.t('cluster_page.discard_changes')}
        ...
        render: function() {
            return !this.props.model.get('is_customized') ? (<div/>) : (
                <div className='alert alert-block globalalert'>
                    <p className='enable-selection'>{i18n.t('cluster_page.cluster_was_modified_from_cli')}</p>
                </div>
            );
        }
    });
