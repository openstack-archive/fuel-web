...
function(models, commonViews, dialogViews, actionsTabTemplate, renameEnvironmentTemplate, resetEnvironmentTemplate, deleteEnvironmentTemplate, updateEnvironmentTemplate, rollbackEnvironmentTemplate) {
    ...
    ActionsTab = commonViews.Tab.extend({
        ...
        renderAction: function(actionData) {
            var options = _.extend({model: this.model, tab: this}, actionData.options);
    ...
    ResetEnvironmentAction = Backbone.View.extend({
        ...
        getDescriptionKey: function() {
            ...
            if (this.tab.isLocked()) { return 'reset_disabled_for_deploying_cluster'; }
        ...
        initialize: function(options) {
            ...
            this.isResetDisabled = this.model.get('status') == 'new' || this.tab.isLocked();
        },
        render: function() {
    ...
    UpdateEnvironmentAction = Backbone.View.extend({
        ...
        bindings: {
            'select[name=update_release]': {
                ...
                attributes: [{
                    name: 'disabled',
                    onGet: function() { return this.releases.length == 0 || this.tab.isLocked() || this.model.get('status') == 'new';}
                }],
        ...
        getDescriptionKey: function() {
            ...
            if (this.tab.isLocked()) { return 'update_disabled_for_deploying_cluster'; }
            if (this.model.get('status') == 'new') { return 'update_disabled_for_new_cluster'; }
        ...
        render: function() {
            this.$el.html(this.template({cluster: this.model, releases: this.releases, isUpdateDisabled: this.isUpdateDisabled, descriptionKey: this.getDescriptionKey()})).i18n();
            this.stickit();
            return this;
        }
    });
