...
function(React, utils, models, dialogViews) {
            ...
            return (
                <Action title={$.t('cluster_page.actions_tab.reset_environment')}>
                    <div className='action-body'>
                        ...
                        {!this.isLocked() && <div className='important action-item-description'>{$.t('cluster_page.actions_tab.reset_environment_warning')}</div>}
                    </div>
                    <button
    ...
    var UpdateEnvironmentAction = React.createClass({
        ...
        applyAction: function() {
            var cluster = this.props.cluster;
            var isDowngrade = _.contains(cluster.get('release').get('can_update_from_versions'), this.props.releases.findWhere({id: this.state.pendingReleaseId || cluster.get('release_id')}).get('version'));
            ...
            return (
                <Action className='span12 action-update' title={$.t('cluster_page.actions_tab.update_environment')}>
                    <div className='action-body'>
                        ...
                        {(this.getAction() == 'update' && !this.isLocked() && this.state.pendingReleaseId) &&
                    ...
                    <button
                        ...
                        onClick={this.applyAction}
                        disabled={this.getAction() == 'update' && _.isNull(this.state.pendingReleaseId) || this.isLocked()}>
                        {$.t('common.' + this.getAction() + '_button')}
                    </button>
                </Action>
            );
        }
