...
function(React, componentMixins, utils, models, commonViews, dialogViews) {
    ...
    var UpdateEnvironmentAction = React.createClass({
        ...
        applyAction: function() {
            ...
            (new dialogViews.UpdateEnvironmentDialog({model: this.props.cluster, action: this.state.action, isDowngrade: isDowngrade, pendingReleaseId: this.state.pendingReleaseId})).render();
        },
        ...
        getDescriptionKey: function() {
            ...
            if (action == 'update' && this.props.cluster.get('status') == 'operational' && releasesForUpdate.length == 0) {
        ...
        render: function() {
            var options = this.state.releasesFiltered.map(function(release){
                return <option value={release.id}>{release.get('name') + ' (' + release.get('version') + ')'}</option>;
            }, this);
            return (
                <Action className='span12 action-update' title={$.t('cluster_page.actions_tab.update_environment')}>
                    <div className="action-body">
                        ...
                        <select className={this.state.action != 'update' || this.state.disabled ? 'hide' : ''} onChange={this.handleChange}>
                            {options}
                        </select>
                    </div>
                    <button className="btn btn-danger action-btn update-environment-btn" onClick={this.applyAction} disabled={this.state.disabled}><span>{$.t('common.update_button')}</span></button>
                </Action>
            );
        }
    });
