...
function(React, componentMixins, utils, models, dialogViews) {
        ...
        render: function() {
            ...
            return (
                <Action title={$.t('cluster_page.actions_tab.rename_environment')}>
                    ...
                    <button className="btn btn-success rename-environment-btn" 
    ...
    var UpdateEnvironmentAction = React.createClass({
        ...
        componentDidMount: function() {
            ...
            if (!releases.length) {
                releases.fetch().done(_.bind(function() {
                    var filteredReleases = releases.filter(this.isAvailableForUpdate, this);
                    if (filteredReleases.length) {
                        this.setState({pendingReleaseId: _.first(filteredReleases).id});
                    }
                }, this));
            }
        },
        ...
        render: function() {
            ...
            var valueLink = {
                value: this.state.pendingReleaseId,
                requestChange: this.handleChange
            };
            ...
            return (
                <Action className='span12 action-update' title={$.t('cluster_page.actions_tab.update_environment')}>
                    <div className="action-body">
                        ...
                        {(this.state.action == 'update' && !this.state.disabled && releases.length > 0) &&
