...
function(_, i18n, React, utils, models, dialogs) {
        ...
        render: function() {
            ...
            return (
                <div className='wrapper'>
                    <h3>{i18n.t('cluster_page.actions_tab.title')}</h3>
    ...
    var RenameEnvironmentAction = React.createClass({
                    ...
                    deferred
                        .fail(_.bind(function(response) {
                            if (response.status == 409) {
                                ...
                                utils.showErrorDialog({title: i18n.t('cluster_page.actions_tab.rename_error.title')});
                            }
                        }, this))
                        .done(function() {
                            app.updateTitle();
        ...
        render: function() {
            ...
            return (
                <Action title={i18n.t('cluster_page.actions_tab.rename_environment')}>
    ...
    var ResetEnvironmentAction = React.createClass({
        ...
        render: function() {
            ...
            return (
                <Action title={i18n.t('cluster_page.actions_tab.reset_environment')}>
                    <div className='action-body'>
                        <div className='action-item-description'>
                            {i18n.t('cluster_page.actions_tab.' + this.getDescriptionKey())}
                        </div>
                        {!isLocked && <div className='important action-item-description'>{i18n.t('cluster_page.actions_tab.reset_environment_warning')}</div>}
                    </div>
                    <button
                        className='btn btn-danger reset-environment-btn'
                        onClick={this.applyAction}
                        disabled={isLocked}>
                        {i18n.t('common.reset_button')}
                    </button>
                </Action>
            );
        }
    ...
    var DeleteEnvironmentAction = React.createClass({
        ...
        render: function() {
            return (
                <Action title={i18n.t('cluster_page.actions_tab.delete_environment')}>
                    <div className='action-body'>
                        <div className='action-item-description important'>
                            {i18n.t('cluster_page.actions_tab.alert_delete')}
                        </div>
                    </div>
                    <button
    ...
    var UpdateEnvironmentAction = React.createClass({
        ...
        render: function() {
            ...
            return (
                <Action className='span12 action-update' title={i18n.t('cluster_page.actions_tab.update_environment')}>
                    <div className='action-body'>
                        {(action == 'rollback' || releases) &&
                            <div className='action-item-description'>
                                {i18n.t('cluster_page.actions_tab.' + this.getDescriptionKey(), {release: pendingRelease ? pendingRelease.get('name') + ' (' + pendingRelease.get('version') + ')' : ''})}
                            </div>
                        }
                        {action == 'rollback' &&
                            <div className='action-item-description'>
                                {i18n.t('cluster_page.actions_tab.rollback_message')}
                            </div>
                        }
                    </div>
                    {action == 'update' &&
