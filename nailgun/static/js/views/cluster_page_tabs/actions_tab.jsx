...
function(React, utils, models, dialogViews) {
    ...
    var UpdateEnvironmentAction = React.createClass({
        ...
        isAvailableForUpdate: function(release) {
            ...
            return (_.contains(currentRelease.get('can_update_from_versions'), release.get('version')) || _.contains(release.get('can_update_from_versions'), currentRelease.get('version')))  && release.get('operating_system') == currentRelease.get('operating_system') && release.get('id') != cluster.get('release_id');
        },
        getDescriptionKey: function() {
            var cluster = this.props.cluster,
                action = this.getAction(),
                    ...
                    {action == 'update' &&
                        <div>
                            ...
                            <button
                                ...
                                disabled={action == 'update' && _.isNull(this.state.pendingReleaseId) || isLocked}>
