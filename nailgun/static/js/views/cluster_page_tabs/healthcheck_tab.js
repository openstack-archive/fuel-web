...
function(utils, models, commonViews, dialogViews, healthcheckTabTemplate, healthcheckTestSetTemplate, healthcheckTestsTemplate) {
    ...
    HealthCheckTab = commonViews.Tab.extend({
        ...
        isLocked: function() {
            return this.model.get('status') == 'new' || this.hasRunningTests() || !!this.model.task('deploy', 'running') || (this.model.get('status') != 'stopped' && this.model.get('nodes').where({status: 'ready'}).length) ;
        },
