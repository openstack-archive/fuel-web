...
function(utils, models, commonViews, dialogViews, healthcheckTabTemplate, healthcheckTestSetTemplate, healthcheckTestsTemplate) {
    ...
    HealthCheckTab = commonViews.Tab.extend({
        ...
        runTests: function() {
            ...
            if (testruns.models) {
                defferedArray.push( Backbone.sync('create', testruns));
            }
            if (oldTestruns.models) {
                defferedArray.push( Backbone.sync('update', oldTestruns));
            }
            $.when.apply($, defferedArray).done(_.bind(this.update, this));
        },
        stopTests: function() {
