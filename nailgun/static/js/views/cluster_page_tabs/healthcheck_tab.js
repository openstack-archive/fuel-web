...
function(utils, models, viewMixins, commonViews, dialogViews, healthcheckTabTemplate, credentialsTemplate, healthcheckTestSetTemplate, healthcheckTestsTemplate) {
    ...
    HealthCheckTab = commonViews.Tab.extend({
        ...
        toggleCredentialsForm: _.debounce(function(e) {
            ...
            var credentialsWrapperBindings = {
                '.toggle-credentials' : {
                    ...
                    update: function($el, value) {
                        if (!this.isLocked()) {
                            $el.html('<i class="icon-' + (value ? 'minus' : 'plus') + '-circle"></i><div>' + $.t('cluster_page.healthcheck_tab.' + (value ? 'hide' : 'show') + '_credentials') + '</div>');
                        }
                    }
                },
                '.credentials': {
        ...
        render: function() {
            ...
            this.$el.html(this.template({cluster: this.model, showCredentials: this.credentialsWrapper.get('visible')})).i18n();
