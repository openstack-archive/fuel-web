...
function(utils, models, commonViews, dialogViews, healthcheckTabTemplate, healthcheckTestSetTemplate, healthcheckTestsTemplate) {
    ...
    HealthCheckTab = commonViews.Tab.extend({
        ...
        disableControls: function(disable) {
            ...
            this.stopTestsButton.set({disabled: disabledState || !this.hasRunningTests()});
        ...
        render: function() {
            ...
            if (_.filter(this.subViews).length > 0){
                    this.checkIfAllTestsChecked();
                }
            return this;
        },
        checkIfAllTestsChecked: function() {
            if (this.getNumberOfCheckedTests() == this.tests.length) {
                this.selectAllCheckbox.set({checked: true});
                _.each(this.subViews, function(testSetView) {
                    testSetView.selectAllCheckbox.set({checked: true});
                }, this);
            }
        }
    });
