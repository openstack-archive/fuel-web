        ...
        render: function() {
            ...
            return (
                <div className='wrapper'>
                    <h3 className='span6 healthcheck-title'>{i18n.t('cluster_page.healthcheck_tab.title')}</h3>
                    {this.state.loadingFailure ?
                        <div className='cleared'>
                            <div className='alert error-message alert-error'>
                                {i18n.t('cluster_page.healthcheck_tab.not_available_alert')}
                            </div>
                        </div>
                    : !this.state.loaded ?
                    ...
                    <div className='row-fluid page-sub-title'>
                        <div className='span2 ostf-controls'>
                            {!disabledState &&
                                <div className='toggle-credentials pull-right' onClick={this.toggleCredentials}>
                                    ...
                                    <div>{i18n.t('cluster_page.healthcheck_tab.provide_credentials')}</div>
                                </div>
                            }
                        </div>
                        <controls.Input
                        ...
                        <div className='span2 ostf-controls'>
                            {hasRunningTests ?
                                ...
                                >
                                    {i18n.t('cluster_page.healthcheck_tab.stop_tests_button')}
                                </button>)
                            :
                                ...
                                >
                                    {i18n.t('cluster_page.healthcheck_tab.run_tests_button')}
                                </button>)
                            }
                        </div>
                    </div>
                    {(this.props.cluster.get('status') == 'new') &&
                        <div className='row-fluid'>
                            <div className='span12'>
                                <div className='alert'>{i18n.t('cluster_page.healthcheck_tab.deploy_alert')}</div>
                            </div>
                        </div>
                    }
                    ...
                    <div className='fieldset-group wrapper'>
                        <div className='healthcheck-group' >
                            <div className='clearfix note'>
                                {i18n.t('cluster_page.healthcheck_tab.credentials_description')}
                            </div>
                            {_.map(inputFields, function(name) {
                                return (<controls.Input
                                    ...
                                    name={name}
                                    label={i18n.t('cluster_page.healthcheck_tab.' + name + '_label')}
                                    value={this.props.credentials[name]}
                                    onChange={this.props.onInputChange}
                                    toggleable={name == 'password'}
                                    description={i18n.t('cluster_page.healthcheck_tab.' + name + '_description')}
                    ...
                    <table className='table table-bordered healthcheck-table enable-selection'>
                        <thead>
                            <tr>
                                ...
                                <th className='healthcheck-col-duration'>{i18n.t('cluster_page.healthcheck_tab.expected_duration')}</th>
                                <th className='healthcheck-col-duration'>{i18n.t('cluster_page.healthcheck_tab.actual_duration')}</th>
                                <th className='healthcheck-col-status'>{i18n.t('cluster_page.healthcheck_tab.status')}</th>
                            </tr>
                        </thead>
                        <tbody>
