...
function(React, models, utils, componentMixins, controls) {
    ...
    var HealthcheckTabContent = React.createClass({
        ...
        componentDidMount: function() {
            this.credentials = this.credentials || _.cloneDeep(this.props.cluster.get('settings').get('access'));
        },
        ...
        stopTests: function() {
            ...
            if (testruns.length) {
                ...
                Backbone.sync('update', testruns)
                    .done(_.bind(this.startPolling, this, true));
            }
        },
        render: function() {
                    ...
                    <div className='fieldset-group wrapper'>
                        <div className='healthcheck-group' >
                            ...
                            {_.map(inputFields, function(name, index) {
                                var inputKind = name == 'password' ? controls.PasswordField : controls.TextField;
                                return (<inputKind
                                    key={name}
