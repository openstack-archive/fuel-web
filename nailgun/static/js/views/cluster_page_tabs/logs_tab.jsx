...
function(React, utils, models, common, componentMixins) {
    ...
    var LogsTab = React.createClass({
        ...
        fetchData: function() {
            var request;
            if (this.to && this.logsEntries) {
                request = this.fetchLogs({to: this.to})
                    .done(_.bind(function(data) {
                        this.logsEntries = _.union(data.entries, this.logsEntries);
                        this.setState({logsLength: this.logsEntries.length});
                        this.to = data.to;
                    }, this));
                }
            return $.when(request);
        ...
        fetchSources: function(type, nodeId) {
            var cluster = this.props.model,
                chosenNode = nodeId ? nodeId : this.getFirtsNode().id;
            ...
            this.fetchLogs(params)
                .done(_.bind(function(data) {
                    ...
                    if (data.entries.length) {
                        ...
                        logsLength = data.entries.length;
                    }
                    this.logsEntries = data.entries;
                    this.setState({
                        logsFetchFail: undefined,
                        logEntriesHasMore: has_more,
        ...
        onShowMoreClick: function(value) {
            if (value == 'all') {
                ...
                var count = parseInt(value, 10) + this.state.logsLength;
        ...
        render: function() {
            ...
            return (
                <div className='wrapper'>
                    ...
                    {this.logsEntries &&
                        <LogsTable
                            logEntries={this.logsEntries}
                            logEntriesHasMore={this.state.logEntriesHasMore}
    ...
    var LogsTable = React.createClass({
        ...
        render: function() {
            ...
            if (logEntries && logEntries.length) {
                tabRows = logEntries.map(function(entry, i){
                    // TODO: fix keys generation method
                    return  <tr key={btoa(entry[2] + entry[0]).substring(50,200)} className={'enable-selection ' + entry[1].toLowerCase()}>
