    ...
    var LogFilterBar = React.createClass({
        ...
        fetchSources: function(type, nodeId) {
            var cluster = this.props.cluster,
                ...
                chosenNodeId = nodeId ? nodeId : nodes.length ? nodes.first().id : null;
            ...
            this.sources.deferred.done(_.bind(function() {
                var filteredSources = this.sources.filter(function(source) {return source.get('remote') == (type == 'local' ? false : true);}),
                    chosenSource = (this.state.chosenSourceId && _.findWhere(filteredSources, {id: this.state.chosenSourceId})) ? _.findWhere(filteredSources, {id: this.state.chosenSourceId}) : _.first(filteredSources),
                    chosenLevelId = chosenSource ? _.first(chosenSource.get('levels')) : null;
                if (this.state.chosenLevelId && _.contains(chosenSource.get('levels'), this.state.chosenLevelId)) chosenLevelId = this.state.chosenLevelId;
                if (this.isMounted())
                    this.setState({
                        chosenType: type,
