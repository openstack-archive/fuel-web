        ...
        render: function() {
            return (
                <div className='wrapper'>
                    <h3>{i18n.t('cluster_page.logs_tab.title')}</h3>
                    <LogFilterBar
                        cluster={this.props.model}
                        tabOptions={this.props.tabOptions}
                        onShowButtonClick={this.onShowButtonClick} />
                    {this.state.loading == 'fail' &&
                        <div className='logs-fetch-error alert alert-error'>{i18n.t('cluster_page.logs_tab.log_alert')}</div>
                    }
    ...
    var LogFilterBar = React.createClass({
                    ...
                    <div className='filter-bar'>
                        ...
                        <div className='filter-bar-item'>
                            <button
                                ...
                                {i18n.t('cluster_page.logs_tab.show')}
                            </button>
                        </div>
                    </div>
                    {this.state.sourcesLoadingState == 'fail' &&
                        <div className='node-sources-error alert alert-error'>{i18n.t('cluster_page.logs_tab.source_alert')}</div>
                    }
                </div>
            );
        },
        renderTypeSelect: function() {
            ...
            return <controls.Input
                ...
                label={i18n.t('cluster_page.logs_tab.logs')}
        ...
        renderNodeSelect: function() {
            ...
            return (<controls.Input
                type='select'
                labelClassName='filter-bar-label'
                label={i18n.t('cluster_page.logs_tab.node')}
        ...
        renderSourceSelect: function() {
            ...
            return (<controls.Input
                type='select'
                labelClassName='filter-bar-label'
                label={i18n.t('cluster_page.logs_tab.source')}
        ...
        renderLevelSelect: function() {
            ...
            return (<controls.Input
                type='select'
                labelClassName='filter-bar-label'
                label={i18n.t('cluster_page.logs_tab.min_level')}
        ...
        render: function() {
            ...
            return (
                <table className='table enable-selection table-bordered table-condensed table-logs'>
                    <thead>
                        <tr>
                            <th>{i18n.t('cluster_page.logs_tab.date')}</th>
                            <th>{i18n.t('cluster_page.logs_tab.level')}</th>
                            <th>{i18n.t('cluster_page.logs_tab.message')}</th>
                        </tr>
                    </thead>
                    ...
                    {this.props.showMoreLogsLink &&
                        <tfoot className='entries-skipped-msg'>
                            <tr>
                                <td colSpan='3'>
                                    <span>{i18n.t('cluster_page.logs_tab.bottom_text')}
                    ...
                    {!logsEntries.length &&
                        <tfoot className='no-logs-msg'>
                            <tr>
                                <td colSpan='3'>{i18n.t('cluster_page.logs_tab.no_log_text')}</td>
                            </tr>
                        </tfoot>
                    }
                </table>
