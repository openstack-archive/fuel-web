...
function(utils, models, commonViews, dialogViews, networkTabTemplate, networkTemplate, rangeTemplate, networkingParametersTemplate, networkTabVerificationControlTemplate) {
    ...
    NetworkTab = commonViews.Tab.extend({
        ...
        applyChanges: function() {
            ...
            if (!this.networkConfiguration.validationError) {
                this.disableControls();
                this.removeEmptyRanges();
                this.networkConfiguration.get('networks').invoke('unset', 'meta');
            ...
            this.networkConfiguration.on('invalid', function(model, errors) {
                    ...
                    if (!_.contains(field, 'floating')) {
                        ...
                        if (_.isArray(error)) {
                            _.each(error, _.bind(function(msg, index) {
                                if (msg) {
                                    this.$('.' + field + '-row input[name=range' + index + ']').addClass('error').parents('.network-attribute').find('.error .help-inline').text(msg);
                                }
                            }, this));
                        } else {
                            $el = this.$('.networking-parameters input[name=' + field + ']');
                        }
                ...
                _.each(errors.networks, _.bind(function(networkErrors, networkId) {
                    _.each(networkErrors, _.bind(function(error, field) {
                        if (field != 'ip_ranges') {
                            this.$('div[data-network-id="' + networkId + '"] input[name=' + field + ']').addClass('error').parents('.network-attribute').find('.error .help-inline').text(error);
                        } else {
    ...
    Network = Backbone.View.extend({
        ...
        stickitNetwork: function () {
            ...
            this.stickit(this.network, _.merge(bindings, this.tab.composeVlanBindings.call(this)));
        },
        initialize: function(options) {
            _.defaults(this, options);
        ...
        renderIpRanges: function(config) {
            config = config || {domClass: 'ip', observe: 'ip_ranges', model: this.network};
            ...
            _.each(ranges, function(range, rangeIndex) {
                ...
                _.each(range, function(ip, index) {
                    rangeBindings[rangesBlockSelector + ' input[name=range' + index + '][data-range=' + rangeIndex + ']'] = {
                        ...
                        onGet: function(value) { return value[rangeIndex][index]; },
                        getVal: function($el) {
        ...
        render: function() {
            ...
            if (this.floatingRange) {
                this.renderIpRanges({domClass: 'floating', observe: this.floatingRange, model: this.tab.networkingParameters});
            }
            this.stickitNetwork();
            return this;
        }
        ...
        composeRangeFieldBindings: function(observe, index) {
            ...
            bindings['.' + observe + '-row input[name=range' + index + ']'] = {
                ...
                getVal: _.bind(function($el) {
                    ...
                    range[this.$('.' + observe + '-row .range').index($el)] = Number($el.val()) || '';
                    return range;
                }, this)
            };
            return bindings;
