...
function(utils, models, commonViews, dialogViews, networkTabTemplate, networkTemplate, rangeTemplate, novaNetworkConfigurationTemplate, neutronParametersTemplate, networkTabVerificationControlTemplate) {
    ...
    NetworkTab = commonViews.Tab.extend({
        ...
        isLocked: function() {
            return !!this.model.tasks({group: 'network', status: 'running'}).length || !!this.model.tasks({group: 'deployment', status: 'running'}).length || !this.model.isAvailableForSettingsChanges();
        },
        ...
        bindTaskEvents: function(task) {
            return task.match({group: 'deployment'}) || task.match({group: 'network'}) ? task.on('change:status', this.render, this) : null;
        },
        ...
        render: function() {
            this.$el.html(this.template({
                ...
                verificationLocked: !!this.model.tasks({group: 'network', status: 'running'}).length || !!this.model.tasks({group: 'deployment', status: 'running'}).length,
