...
function(utils, models, commonViews, dialogViews, networkTabTemplate, networkTemplate, rangeTemplate, networkingParametersTemplate, networkTabVerificationControlTemplate) {
    ...
    NetworkTab = commonViews.Tab.extend({
        ...
        stickitRanges: function(config) {
            var rangesSelector = '.' + config.domSelector + '-ranges-rows';
            this.$(rangesSelector).empty();
            var ranges = config.model.get(config.attribute);
            config.bindings[rangesSelector] = {
                observe: config.attribute,
                update: _.bind(function($el, value, model, options) {
                    $el.html('');
                    ...
                    this.stickitRangeFields(config);
                    this.stickit(config.model, config.bindings);
                }, this)
            };
            this.stickitRangeFields(config);
        },
        stickitRangeFields: function(config) {
    ...
    NetworkTabSubview = Backbone.View.extend({
        events: {
            'click .ip-ranges-control btn:not([disabled])': 'changeIPRanges'
        },
        changeIPRanges: function(e) {
            var addRange = this.$(e.currentTarget).hasClass('ip-ranges-add');
            var config;
            if (this.network) {
                config = {model: this.network, attribute: 'ip_ranges', domSelector: 'div[data-network-id=' + this.network.id + ']'};
            } else {
                config = {model: this.networkingParameters, attribute: 'floating_range', domSelector: '.floating'};
            }
            var index = this.$(config.domSelector).find('.range-row').index($(e.currentTarget).parents('.range-row'));
            var ipRanges = _.cloneDeep(config.model.get(config.attribute));
            if (addRange) {
    ...
    Network = NetworkTabSubview.extend({
        ...
        render: function() {
            ...
            if (this.network.get('meta').notation == 'ip_ranges') {
                this.tab.stickitRanges({model: this.network, attribute: 'ip_ranges', domSelector: 'ip', bindings: this.bindings});
            }
    ...
    NetworkingParameters = NetworkTabSubview.extend({
        ...
        composeBindings: function() {
           ...
           _.merge(this.bindings, this.tab.composeVlanBindings.call(this, 'fixed_networks_vlan_start'));
           this.tab.stickitRanges({domClass: 'floating', observe: 'floating_ranges', model: this.parameters, bindings: this.bindings});
        },
        render: function() {
            this.$el.html(this.template({
