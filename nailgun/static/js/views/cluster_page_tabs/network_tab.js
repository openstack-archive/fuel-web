...
function(utils, models, commonViews, dialogViews, networkTabTemplate, networkTemplate, novaNameserversTemplate, neutronParametersTemplate, networkTabVerificationControlTemplate) {
    'use strict';
    var NetworkTab, Network, NeutronConfiguration, NovaNameservers, NetworkTabVerificationControl;
    ...
    NetworkTab = commonViews.Tab.extend({
        ...
        checkForChanges: function() {
            ...
            var novaNameserversErrors = !!(this.networkConfiguration.get('dns_nameservers') && this.networkConfiguration.get('dns_nameservers').validationError);
            this.defaultButtonsState(_.some(this.networkConfiguration.get('networks').models, 'validationError') || neutronConfigurationErrors || novaNameserversErrors);
        },
        ...
        verifyNetworks: function() {
            var neutronConfigurationErrors = !!(this.networkConfiguration.get('neutron_parameters') && this.networkConfiguration.get('neutron_parameters').validationError);
            var novaNameserversErrors = !!(this.networkConfiguration.get('dns_nameservers') && this.networkConfiguration.get('dns_nameservers').validationError);
            if (!_.some(this.networkConfiguration.get('networks').models, 'validationError') && !neutronConfigurationErrors && !novaNameserversErrors) {
        ...
        renderNovaNameservers: function() {
            if (this.model.get('net_provider') == 'nova_network' && this.networkConfiguration.get('dns_nameservers')) {
                var novaNameserversView = new NovaNameservers({
                    novaNameservers: this.networkConfiguration.get('dns_nameservers'),
                    tab: this
                });
                this.registerSubView(novaNameserversView);
                this.$('.nova-nameservers').html(novaNameserversView.render().el);
            }
        },
    ...
    NovaNameservers = Backbone.View.extend({
        template: _.template(novaNameserversTemplate),
        events: {
        ...
        onChange: function(e) {
            this.$('input[type=text]').removeClass('error');
            this.$('.help-inline').text('');
            this.novaNameservers.set({nameservers: [$.trim(this.$('input[name=nameserver-0]').val()), $.trim(this.$('input[name=nameserver-1]').val())]}, {validate: true});
            this.tab.checkForChanges();
            this.tab.page.removeFinishedTasks();
        },
        initialize: function(options) {
            _.defaults(this, options);
            this.novaNameservers.on('invalid', function(model, errors) {
                this.$('input.error').removeClass('error').find('.help-inline').text('');
        ...
        render: function() {
            this.$el.html(this.template({
                nameservers: this.novaNameservers.get('nameservers'),
                locked: this.tab.isLocked()
            }));
            return this;
        }
