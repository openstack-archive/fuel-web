...
function(utils, models, commonViews, dialogViews, networkTabTemplate, networkTemplate, neutronParametersTemplate, networkTabVerificationControlTemplate) {
    ...
    NetworkTab = commonViews.Tab.extend({
        ...
        checkForChanges: function() {
            ...
            this.defaultButtonsState(_.some(this.networkConfiguration.get('networks').models, 'validationError') || !!(this.neutronParameters && this.neutronParameters.validationError));
        },
        ...
        verifyNetworks: function() {
            if (!_.some(this.networkConfiguration.get('networks').models, 'validationError') && (!this.neutronParameters || !this.neutronParameters.validationError)) {
        ...
        applyChanges: function() {
            ...
            if (!_.some(this.networkConfiguration.get('networks').models, 'validationError') && (!this.neutronParameters || !this.neutronParameters.validationError)) {
                ...
                this.filterEmptyIpRanges();
                if (this.model.get('net_provider') == 'neutron') {
                    this.networkConfiguration.set({neutron_parameters: this.neutronParameters});
                }
                    ...
                    .done(_.bind(function(task) {
                        if (task && task.status == 'error') {
                            ...
                            this.model.get('networkConfiguration').set({
                                ...
                                neutron_parameters: this.neutronParameters
                            });
                        }
                    }, this))
                    .fail(_.bind(function() {
        ...
        setInitialData: function() {
            ...
            if (this.model.get('net_provider') == 'neutron') {
                this.networkConfiguration.set({neutron_parameters: new models.NeutronConfiguration(this.model.get('networkConfiguration').get('neutron_parameters'))});
            }
        ...
        renderNeutronConfiguration: function() {
            if (this.model.get('net_provider') == 'neutron' && this.networkConfiguration.get('neutron_parameters')) {
                this.neutronParameters = new models.NeutronConfiguration(this.networkConfiguration.get('neutron_parameters').attributes);
