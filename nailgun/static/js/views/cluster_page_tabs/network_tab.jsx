...
function($, _, i18n, Backbone, React, models, utils, componentMixins, controls) {
    ...
    var NetworkModelManipulationMixin = {
        setValue: function(attribute, value, options) {
            if (options && options.isInteger && !_.isNull(value)) {
                // for ranges values
                if (_.isArray(value)) {
                    var convertedToNumberValues = _.map(value, function(singleValue) {
                        return this.convertToStringIfNaN(parseInt(singleValue));
                    }, this);
                    value = convertedToNumberValues;
                } else {
                    value = this.convertToStringIfNaN(parseInt(value));
                }
            }
        ...
        getModel: function() {
            if (this.props.network) {
                return this.props.networkConfiguration.get('networks').get(this.props.network.id);
            }
            return this.props.networkConfiguration.get('networking_parameters');
        },
        convertToStringIfNaN: function(value) {
    ...
    var NetworkInputsMixin = {
        composeProps: function(attribute, isRange, isInteger) {
            ...
            return {
                key: attribute,
                onChange: _.partial(this.setValue, {isInteger: isInteger}),
    ...
    var NetworkTab = React.createClass({
        mixins: [
            ...
            componentMixins.backboneMixin({
                modelOrCollection: function(props) {
                    return props.cluster.get('networkConfiguration');
                },
                renderOn: 'change'
            }),
            componentMixins.backboneMixin({
        ...
        render: function() {
            ...
            if (!this.state.loading) {
                error = this.networkConfiguration.validationError;
            ...
            return (
                <div id='network-form'>
                    ...
                    <div className='networks-table'>
                        {this.props.networkConfiguration.get('networks').map(this.renderNetwork, this)}
                    </div>
    ...
    var NetworkControls = React.createClass({
        mixins: [
            componentMixins.backboneMixin('networkConfiguration', 'change'),
            componentMixins.backboneMixin({
                modelOrCollection: function(props) {
                ...
                <div className='row'>
                    <div className='page-control-box'>
                        <div className='page-control-button-placeholder'>
                            <button key='verify_networks' className='btn verify-networks-btn' onClick={this.props.verifyNetworks}
                                disabled={this.props.error || this.isLocked()}>
                            ...
                            <button key='apply_changes' className='btn btn-success apply-btn' onClick={this.props.applyChanges}
                                disabled={this.props.error || this.isLocked() || !this.hasChanges()}>
