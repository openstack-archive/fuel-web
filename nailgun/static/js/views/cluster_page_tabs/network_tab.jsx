    ...
    var NetworkMixin = {
        getProps: function(attribute, isRange) {
            var network = this.props.network,
                ns = network ? 'cluster_page.network_tab.network.' : 'cluster_page.network_tab.networking_parameters.';
            return {
                key: attribute,
        ...
        renderInput: function(config) {
            return (
                <controls.Input {...config}
                    ...
                    error={(config.error || _.contains(this.props.verificationErrorField, config.name) || null) && ''}
                />
            );
        },
        getError: function(attribute) {
            var network = this.props.network,
                validationErrors = this.props.networkConfiguration.validationError,
                errors;
            if (validationErrors) {
                if (network) {
                    ...
                    { (this.props.type == 'normal') ?
                        <div className={this.props.rowsClassName}>
                            {_.map(ranges, function(range, index) {
                                var correspondingError = _.findWhere(error, {index: index}) || {};
                                return (
                                    <div className='range-row autocomplete clearfix' key={index}>
                                        <controls.Input
                                            {...baseRangeProps}
                                            error={(correspondingError.start || verificationError) && ''}
                                            value={range[0]}
                                        ...
                                        <controls.Input
                                            ...
                                            error={correspondingError.end && ''}
                                            value={range[1]}
                                            onChange={this.onRangeChange.bind(this, true, index, attributeName)}
                                            onFocus={this.autoCompleteIPRange.bind(this, correspondingError && correspondingError.start, range[0])}
                                        ...
                                        <div className='error validation-error'>
                                            <span className='help-inline'>
                                                {correspondingError.start || correspondingError.end}
                                            </span>
                                        </div>
                                    </div>
                                );
            ...
            return (
                <div>
                    ...
                    <div className={network.get('name')}>
                        ...
                        {this.renderInput(this.getProps('cidr'))}
                        <VlanTagInput
                            {...this.getProps('vlan_start', false, network, networkId)}
                                label={i18n(ns + 'use_vlan_tagging')}
                                value={vlanTagging}
                                onInputChange={this.onInputChange}
                        />
                        {networkConfig.use_gateway &&
                            this.renderInput(this.getProps('gateway'))
                        }
                    </div>
                </div>
            );
                ...
                <div>
                    {manager ?
                        <div>
                            ...
                            <div>
                                {this.renderInput(this.getProps('fixed_networks_cidr'))}
                                {(manager == 'VlanManager') ?
                                    <div>
                                        ...
                                        {this.renderInput(this.getProps('fixed_networks_amount'))}
                                        <Range
                    ...
                    :
                        <div>
                            ...
                            <Range
                                ...
                                type='mini'
                            />
                            {this.renderInput(this.getProps('base_mac'))}
                            <div>
                                <legend className='networks'>{i18n(ns + 'l3_configuration')}</legend>
                            </div>
                            <div>
                                {this.renderInput(this.getProps('internal_cidr'))}
                                {this.renderInput(this.getProps('internal_gateway'))}
                            </div>
                        </div>
                    }
                    <Range
