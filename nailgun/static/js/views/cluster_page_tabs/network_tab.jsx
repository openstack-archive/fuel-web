...
function($, _, i18n, Backbone, React, models, utils, componentMixins, controls) {
            ...
            if (options && options.isInteger && !_.isNull(value)) {
                ...
                if (_.isArray(value)) {
                    var convertedToNumberValues = _.map(value, function(singleValue) {
                        return convertToStringIfNaN(parseInt(singleValue));
                    }, this);
                    value = convertedToNumberValues;
                } else {
                    value = convertToStringIfNaN(parseInt(value));
                }
            }
    ...
    var NetworkInputsMixin = {
        composeProps: function(attribute, isRange, isInteger) {
            ...
            return {
                key: attribute,
                onChange: this.setValue.bind(this, {isInteger: isInteger}),
    ...
    var NetworkTabContent = React.createClass({
        ...
        loadInitialConfiguration: function() {
            this.props.networkConfiguration.get('networks').reset(_.cloneDeep(this.props.initialConfiguration.networks));
            this.props.networkConfiguration.get('networking_parameters').set(_.cloneDeep(this.props.initialConfiguration.networking_parameters));
        },
        updateInitialConfiguration: function() {
            this.props.initialConfiguration = _.cloneDeep(this.props.networkConfiguration.toJSON());
        },
