...
function($, _, i18n, Backbone, React, models, utils, componentMixins, controls) {
    ...
    var NetworkMixin = {
        getProps: function(attribute, isRange, network, networkId) {
            var ns, error, value;
            if (networkId) {
            ...
            return (
                <div className='network-attribute vlan-tagging'>
                    ...
                    {!_.isNull(this.props.value) &&
                        <controls.Input {...this.props}
                            autoFocus={this.state.hasUpdated}
                            label={false}
    ...
    var NetworkTab = React.createClass({
        ...
        renderNetwork: function(network) {
            return <Network
                ...
                verificationErrorField={_.pluck(_.where(this.getVerificationErrors(), {network: network.id}), 'field')}
                ...
                <div name={networkId}>
                    ...
                    <div className={network.get('name')}>
                        {(networkConfig.notation == ipRangesLabel) &&
                            <Range
                                {...this.getProps(ipRangesLabel, true, network, networkId)}
                                    rowsClassName='ip-ranges-rows'
                                    verificationError={_.contains(this.props.verificationErrorField, 'ip_ranges')}
                                    page={this.props.page}
                            />
                        }
                        {this.renderNetworkInput(this.getProps('cidr', false, network, networkId))}
                        <VlanTagInput
                        ...
                        {networkConfig.use_gateway &&
                            this.renderNetworkInput(this.getProps('gateway', false, network, networkId))
                        }
                    </div>
                </div>
            );
