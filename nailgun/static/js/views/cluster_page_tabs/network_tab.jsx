...
define(
    ...
    function(React, models, utils, componentMixins, controls) {
        ...
        var NetworkRangeMixin = {
            onRangeChange: function(attribute, hasManyRanges, rowIndex, event) {
                var name = event.target.name,
                    newNetworkModel = this.props.network,
                    valuesToSet = this.props.network.get(attribute),
                    valuesToModify = valuesToSet,
                    target = $(event.target);

                if (hasManyRanges) {
                    valuesToModify = valuesToSet[rowIndex];
                }
                if (name == 'range0') {
                    valuesToModify[0] = event.currentTarget.value;
                } else if (name == 'range1') {
                    valuesToModify[1] = event.currentTarget.value;
                }
        ...
        var NetworkMixin = {
            ...
            renderNetworkInput: function(config) {
                return (
                    <div className='network-attribute' key={config.title}>
                        <controls.Input
                            title={config.title}
                            type='text'
                            name={config.name}
                    ...
                    <div className={cx(wrapperClasses)}>
                        ...
                        :
                            <div className='range-row'>
                                <div className='parameter-control'>
                                    {this.transferPropsTo(
                                        <input type='text' className={cx(_.extend(inputClassName, {error: !!startErrorMessage}))}
                                ...
                                <div className='parameter-control'>
                                    {this.transferPropsTo(
                                        <input type='text' className={cx(_.extend(inputClassName, {error: !!endErrorMessage}))}
                                            name='range1' value={this.props.networkAttribute[1]}
                                            disabled={this.props.disableEnd || this.props.disabled}
                                            onChange={_.partial(this.props.onChange, 0)}
                                        />
                                    )}
                                </div>
    ...
    var CheckboxAndInput = React.createClass({
        render: function() {
            return (
    ...
    var NetworkTab = React.createClass({
        ...
        loadInitialConfiguration: function() {
            this.setState({networkConfiguration:
    ...
    var Network = React.createClass({
        ...
        render: function() {
            ...
            this.props.errors = (_.has(this.props.errors, this.props.network.id)) ? this.props.errors : false;
            return (
                <div>
