    ...
    var Range = React.createClass({
        ...
        componentDidUpdate: function() {
            ...
            if (this.props.extendable) {
                if ((this.getModel().get(this.props.name).length > 1) && this.state.elementToFocus) {
        ...
        removeRange: function(attribute, rowIndex) {
            var newValue = _.clone(this.getModel().get(attribute)),
                rowToFocus = (rowIndex == 0) ? rowIndex + 1 :
                    (rowIndex == _.values(this.refs).length - 1) ? rowIndex - 1 : rowIndex;

            newValue.splice(rowIndex, 1);
            this.setValue(attribute, newValue);
            this.setState({
                elementToFocus: 'start' + rowToFocus
            });
        },
