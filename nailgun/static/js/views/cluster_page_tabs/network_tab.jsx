...
function($, _, i18n, Backbone, React, models, utils, componentMixins, controls) {
    ...
    var NetworkMixin = {
        getProps: function(attribute, isRange) {
            ...
            var props = {
                ...
                validationError: this.getError(attribute),
                network: network,
                networkConfiguration: this.props.networkConfiguration,
                wrapperClassName: isRange ? 'network-attribute ' + attribute : false,
                page: this.props.page
            };
            if (this.props.cancelledChanges) {
                props = _.extend(props, {value: this.getModel().get(attribute)});
            } else {
                props = _.extend(props, {defaultValue: this.getModel().get(attribute)});
            }
            return props;
        },
        onInputChange: function(name, value) {
            if (_.contains(name, 'vlan') || (name == 'fixed_networks_amount')) value = _.parseInt(value);
            if (_.isNaN(value)) value = '';
            this.setValue(name, value);
        },
        renderInput: function(attribute) {
            var props = this.getProps(attribute),
                error = props.validationError || null;
            // in case of verification error we need to pass an empty string,
            // but not overwriting validation error
            if (!error && _.contains(this.props.verificationErrorField, props.name)) {
                error = '';
            }
            return (
        ...
        getError: function(attribute) {
            ...
            if (validationErrors) {
                if (network) {
                    errors = _.has(validationErrors.networks, network.id) ? validationErrors.networks : false;
                    return (_.find(errors, attribute) || {})[attribute] || null;
                }
                errors = validationErrors.networking_parameters;
                // specific format needed for vlan_start errors
                if (errors && (attribute == 'fixed_networks_vlan_start')) {
                    return errors[attribute] ? [errors[attribute]] : null;
                }
                return (errors || {})[attribute] || null;
            }
            return null;
        }
    };
    ...
    var Range = React.createClass({
        ...
        onRangeChange: function(hasManyRanges, rowIndex, attribute, name, newValue) {
            ...
            this.setValue(attribute, valuesToSet);
        },
        ...
        removeRange: function(attribute, rowIndex, event) {
            ...
            this.setValue(attribute, newValue);
            this.props.page.setState({pendingRangeRemoval: true});
        },
        getRangeProps: function() {
            var error = this.props.validationError || null,
                attributeName = this.props.name;
            return {
                type: 'text',
                placeholder: error ? '' : '127.0.0.1',
    ...
    var VlanTagInput = React.createClass({
        ...
        componentDidUpdate: function() {
            ...
            // isString check is need to prevent NaN value in input
            if (!_.isNull(value) && _.isString(value) && this.state.pendingFocus) {
        ...
        render: function() {
            if (this.props.name == 'fixed_networks_vlan_start') {
            ...
            return (
                <div className='network-attribute vlan-tagging'>
                    <controls.Input {...this.props}
                        ...
                        type='checkbox'
                        checked={!_.isNull(this.props.defaultValue)}
                        error={null}
                    />
                    {!_.isNull(this.props.defaultValue) &&
                        <controls.Input {...this.props}
                            ref={this.props.name}
                            label={false}
    ...
    var NetworkTab = React.createClass({
        ...
        getInitialState: function() {
            return {
                ...
                pendingRangeRemoval: false
            };
        },
        componentDidUpdate: function() {
            if (this.state.cancelledChanges) this.setState({cancelledChanges: false});
            if (this.state.pendingRangeRemoval) this.setState({pendingRangeRemoval: false});
        },
        ...
        revertChanges: function(event) {
            event.preventDefault();
            this.setState({cancelledChanges: true});
        ...
        renderNetwork: function(network) {
            return <Network
                ...
                cancelledChanges={this.state.cancelledChanges}
                page={this}
            />;
        },
                            ...
                            <form id='network-form'>
                                ...
                                <div className='networking-parameters'>
                                    <NetworkingParameters
                                        networkConfiguration={this.networkConfiguration}
                                        error={(this.networkConfiguration.validationError || {}).networking_parameters}
                                        disabled={this.isLocked()}
                                        cancelledChanges={this.state.cancelledChanges}
                                        page={this}
                                    />
                ...
                <div>
                    ...
                    <div className={network.get('name')}>
                        {(networkConfig.notation == ipRangesLabel) &&
                            // Date.now() is needed to force update for uncontrolled inputs
                            <Range
                                {...this.getProps(ipRangesLabel, true)}
                                rowsClassName='ip-ranges-rows'
                                verificationError={_.contains(this.props.verificationErrorField, 'ip_ranges')}
                                key={this.props.page.state.pendingRangeRemoval ?  Date.now() : ipRangesLabel}
                            />
                        }
                        {this.renderInput('cidr')}
                        <VlanTagInput
                            {...this.getProps('vlan_start')}
                ...
                <div>
                            ...
                            <div>
                                ...
                                {(manager == 'VlanManager') ?
                                    <div>
                                        <div className='network-attribute'>
                                            <controls.Input
                                                {...this.getProps('fixed_network_size')}
                                                type='select'
                                        ...
                                        {this.renderInput('fixed_networks_amount')}
                                        <Range
                                            {...this.getProps('fixed_networks_vlan_start', true)}
                                ...
                                :
                                    <div className='clearfix'>
                                        <VlanTagInput
                                            {...this.getProps('fixed_networks_vlan_start')}
                    ...
                    :
                        <div>
                            ...
                            <Range
                                {...this.getProps(idRangePrefix + '_range', true)}
                                wrapperClassName='network-attribute clearfix'
                                type='mini'
                            />
                    ...
                    // Date.now() is needed to force update for uncontrolled inputs
                    <Range
                        {...this.getProps('floating_ranges', true)}
                        rowsClassName='floating-ranges-rows'
                        key={this.props.page.state.pendingRangeRemoval ?  Date.now() : 'floating_ranges'}
                    />
                    <Range
                        {...this.getProps('dns_nameservers', true)}
                        type='mini'
