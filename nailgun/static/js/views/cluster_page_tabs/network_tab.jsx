    ...
    function(React, models, utils, componentMixins, controls) {
        ...
        var NetworkMixin = {
            onInputChange: function(name, value) {
                var newNetworkModel = this.state.network;
                ...
                return (
                    <div className='network-attribute'>
                        <controls.Input
                            ...
                            type='text'
                            name={config.name}
                            value={this.state.network.get(config.name)}
        ...
        var NetworkTab = React.createClass({
            mixins: [
                ...
                React.BackboneMixin({modelOrCollection: function(props) {
                    return props.model.task({group: 'verify_networks', status: 'running'});
                }}),
                        ...
                        <div className='row'>
                            <div className='page-control-box'>
                                <div className='page-control-button-placeholder'>
                                    <button className='btn verify-networks-btn' onClick={this.verifyNetworks} disabled={this.state.errors ||
                                            !!cluster.task({group: ['deployment', 'network'], status: 'running'})}>
                                        {$.t('cluster_page.network_tab.verify_networks_button')}
                                    </button>
                                    <button className='btn btn-revert-changes' disabled={this.isLocked() || !this.hasChanges()}>
                                        {$.t('common.cancel_changes_button')}
                                    </button>
                                    <button className='btn btn-success apply-btn' disabled={this.isLocked()}>
    ...
    var Network = React.createClass({
        ...
        render: function() {
            var disabled = this.props.tab.isLocked() ? 'disabled' : '',
                ...
                vlanTagging = this.state.network.get('vlan_start');
            this.props.errors = (_.keys(this.props.errors)[0] == this.state.network.id) ? this.props.errors : false;
            return (
                <div>
    ...
    var NetworkParameter = React.createClass({
        ...
        getInitialState: function() {
            return {
                ...
                fixedAmount: this.props.parameters.get('fixed_networks_amount') || 1
            };
        },
        componentDidMount: function() {
            this.state.network.on('change:net_manager', function(parameters, manager) {
                parameters.set({fixed_networks_amount: manager == 'FlatDHCPManager' ? 1 : this.state.fixedAmount}, {silent: true});
                this.forceUpdate();
            }, this);
            this.state.network.on('change:fixed_networks_amount', this.updateFixedAmount, this);
        },
        updateFixedAmount: function() {
            this.state.fixedAmount = this.state.network.get('fixed_networks_amount') || 1;
        },
        render: function() {
