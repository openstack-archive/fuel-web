...
function($, _, i18n, Backbone, React, models, utils, componentMixins, controls) {
    ...
    var NetworkMixin = {
        getProps: function(attribute, network, networkId) {
            ...
            if (networkId) {
                ...
                error = this.getParameterError(attribute);
                value = this.props.networkConfiguration.get('networking_parameters').get(attribute);
            }
            return {
                key: attribute,
        ...
        getRangeProps: function(attribute) {
            return {
                attributeName: attribute,
                wrapperClassName: 'network-attribute ' + attribute,
        ...
        getParameterError: function(attribute) {
            var errors = (this.props.networkConfiguration.validationError || {}).networking_parameters;
    ...
    var Range = React.createClass({
        ...
        onRangeChange: function(hasManyRanges, rowIndex, attribute, name, newValue) {
            ...
            // if any other (fixed_network_vlan_start in particular)
            } else {
    ...
    var VlanTagInput = React.createClass({
        ...
        componentDidUpdate: function() {
            ...
            // TODO(morale): this will work wrong in case of multiple vlan tag imputs with errors - need to think more about this
            if (!_.isNull(value) && _.isString(value)) {
                $(this.refs[this.props.name].getDOMNode()).find('input').focus();
            }
        },
        onTaggingChange: function(attribute, value) {
            this.setToProperNetworkModel(attribute, value ? '' : null);
        },
        render: function() {
            return (
                    ...
                    <div className={network.get('name')}>
                        {(networkConfig.notation == ipRangesLabel) &&
                            <Range
                                ...
                                {...this.getRangeProps(ipRangesLabel)}
                                rowsClassName='ip-ranges-rows'
                                verificationError={_.contains(this.props.verificationErrorField, ipRangesLabel)}
                            />
                        }
                ...
                <div>
                    {manager ?
                        <div>
                            ...
                            <div>
                                {this.renderNetworkInput(this.getProps('fixed_networks_cidr'))}
                                {(manager == 'VlanManager') ?
                                    <div>
                                        ...
                                        {this.renderNetworkInput(this.getProps('fixed_networks_amount'))}
                                        <Range
                                            {...this.getProps('fixed_networks_vlan_start')}
                                            {...this.getRangeProps('fixed_networks_vlan_start')}
                    ...
                    :
                        <div>
                            ...
                            <Range
                                {...this.getProps(idRangePrefix + '_range')}
                                {...this.getRangeProps(idRangePrefix + '_range')}
                                wrapperClassName='network-attribute clearfix'
                                type='mini'
                            />
                            {this.renderNetworkInput(this.getProps('base_mac'))}
                            <div>
                                <legend className='networks'>{i18n(ns + 'l3_configuration')}</legend>
                            </div>
                            <div>
                                {this.renderNetworkInput(this.getProps('internal_cidr'))}
                                {this.renderNetworkInput(this.getProps('internal_gateway'))}
                            </div>
                        </div>
                    }
                    <Range
                        {...this.getProps('floating_ranges')}
                        {...this.getRangeProps('floating_ranges')}
                        rowsClassName='floating-ranges-rows'
                    />
                    <Range
                        {...this.getProps('dns_nameservers')}
                        {...this.getRangeProps('dns_nameservers')}
                        type='mini'
