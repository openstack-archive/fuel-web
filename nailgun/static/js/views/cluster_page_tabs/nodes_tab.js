...
function(utils, models, commonViews, dialogViews, nodesManagementPanelTemplate, assignRolesPanelTemplate, nodeListTemplate, nodeGroupTemplate, nodeTemplate, nodeRoleTemplate, editNodeDisksScreenTemplate, nodeDisksTemplate, volumeStylesTemplate, editNodeInterfacesScreenTemplate, nodeInterfaceTemplate) {
    ...
    AssignRolesPanel = Backbone.View.extend({
                ...
                if (role.get('cant_be_added_to_deployed_env')) {
                    ...
                    if (roleMaximumAmount) {
                        ...
                        if (availableNodesAmount == 0) {
                            unavailable = true;
                            unavailabityReasons.push($.t('cluster_page.nodes_tab.role_max_amount_restriction', {count: roleMaximumAmount, role: role.get('label')}));
                        }
                    }
                }
                if (unavailable) {
        ...
        initialize: function(options) {
            ...
            this.checkRolesAvailability(); // FIXME: do we really need this call?
            (this.loading = this.cluster.get('settings').fetch({cache: true})).done(_.bind(function() {
