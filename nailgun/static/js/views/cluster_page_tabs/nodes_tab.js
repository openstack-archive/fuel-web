...
function(utils, models, commonViews, dialogViews, nodesManagementPanelTemplate, assignRolesPanelTemplate, nodeListTemplate, nodeGroupTemplate, nodeTemplate, nodeRoleTemplate, editNodeDisksScreenTemplate, nodeDisksTemplate, volumeStylesTemplate, editNodeInterfacesScreenTemplate, nodeInterfaceTemplate) {
    ...
    AssignRolesPanel = Backbone.View.extend({
        ...
        assignRoles: function() {
            _.each(this.roles.where({indeterminate: false}), function(role) {
                ...
                // assign role to nodes which don't have this role deployed
                _.each(this.nodes.filter(function(node) { return node.get('checked') && !node.hasDeployedRole(roleName); }), function(node) {
        ...
        checkRolesAvailability: function(e) {
            this.roles.each(function(role) {
                var conflict = '';
                var disabled = !this.nodes.length || this.loading.state() == 'pending';
                ...
                if (!disabled && !role.isSelected()) {
                    var roleMaximumAmount = role.getMaximumAmount();
                    if (!_.isNull(roleMaximumAmount)) {
                        if (!roleMaximumAmount) {
                            disabled = true;
                            conflict = $.t('cluster_page.nodes_tab.role_cant_be_added');
                        } else {
                            var availableNodesAmount = roleMaximumAmount - this.cluster.getNodesByRole(role.get('name')).length;
                            if (availableNodesAmount <= 0 || availableNodesAmount < this.nodes.where({checked: true}).length) {
                                disabled = true;
                                var deploymentMode = $.t('cluster.mode.' + this.cluster.get('mode'), {defaultValue: this.cluster.get('mode')});
                                conflict = $.t('cluster_page.nodes_tab.role_max_amount_restriction', {count: roleMaximumAmount, role: role.get('label'), deploymentMode: deploymentMode});
                            }
                        }
                    }
                }
                ...
                if (!disabled) {
                    var selectedRoles = this.roles.filter(function(role) { return role.isSelected(); });
                    if (_.contains(_.pluck(selectedRoles, 'conflicts'), role.get('name'))) {
                        disabled = true;
            ...
            // disable nodes to prevent node redundant role assignment
            if (this.screen.nodeList) {
                var isNodesLocked = false;
                _.each(this.roles.where({checked: true}), function(role) {
                    var roleMaximumAmount = role.getMaximumAmount();
                    isNodesLocked = !_.isNull(roleMaximumAmount) && roleMaximumAmount <= this.cluster.getNodesByRole(role.get('name')).length + this.nodes.where({checked: true}).length;
                    return !isNodesLocked;
                }, this);
                _.each(this.nodes.where({checked: false}), function(node) {
                    var isNodeDisabled = isNodesLocked || !node.isSelectable() || this.screen instanceof EditNodesScreen || this.screen.isLocked();
                    node.set('disabled', isNodeDisabled);
                    var filteredNode = this.screen.nodeList.filteredNodes.get(node.id);
                    if (filteredNode) {
                        filteredNode.set('disabled', isNodeDisabled);
                    }
                }, this);
        ...
        checkRolesDependencies: function() {
            this.roles.each(function(role) {
                var unavailable = false, unavailabityReason;
                var dependencies = role.get('depends');
                if (dependencies) {
                    ...
                    _.each(dependencies, function(dependency) {
                        ...
                        if (utils.parseModelPath(path, configModels).get() != value) {
                            unavailable = true;
                            unavailabityReason = dependency.warning;
                        }
                    });
                }
                if (unavailable) {
                    role.set({unavailable: true, unavailabityReason: unavailabityReason});
                }
            }, this);
        },
        initialize: function(options) {
            _.defaults(this, options);
            this.cluster = this.screen.tab.model;
            this.roles = this.cluster.get('roleCollection');
            this.roles.each(function(role) {
            ...
            (this.loading = this.settings.fetch({cache: true})).done(_.bind(this.checkRolesDependencies, this));
        },
    ...
    NodeList = Backbone.View.extend({
        ...
        calculateSelectAllDisabledState: function() {
            ...
            if (!disabled && !(this.screen instanceof ClusterNodesScreen)) {
                _.each(this.screen.rolePanel.roles.where({checked: true}), function(role) {
                    var roleMaximumAmount = role.getMaximumAmount();
                    var availableNodesAmount = roleMaximumAmount - this.cluster.getNodesByRole(role.get('name')).length - this.filteredNodes.where({checked: true}).length;
                    disabled = !_.isNull(roleMaximumAmount) && (!availableNodesAmount ||  availableNodesAmount < availableNodes.length);
                    return !disabled;
                }, this);
            }
        ...
        groupNodes: function(grouping) {
            ...
            if (grouping != 'hardware') {
                var preferredOrder = this.cluster.get('release').get('roles');
    ...
    NodeGroup = Backbone.View.extend({
        ...
        calculateSelectAllDisabledState: function() {
            ...
            var disabled = !availableNodes.length;
            if (!disabled && !(this.screen instanceof ClusterNodesScreen)) {
                _.each(this.screen.rolePanel.roles.where({checked: true}), function(role) {
                    var roleMaximumAmount = role.getMaximumAmount();
                    var availableNodesAmount = roleMaximumAmount - this.screen.model.getNodesByRole(role.get('name')) - this.nodes.where({checked: true}).length;
                    disabled = !_.isNull(roleMaximumAmount) && (!availableNodesAmount || availableNodesAmount < availableNodes.length);
                    return !disabled;
                }, this);
            }
