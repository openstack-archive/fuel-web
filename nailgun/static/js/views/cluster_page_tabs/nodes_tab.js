    ...
    Node = Backbone.View.extend({
        ...
        startNodeRenaming: function() {
            ...
            $('html').on(this.eventNamespace, _.after(2, _.bind(function(e) {
                if (!$(e.target).closest(this.$('input[type=checkbox]')).length && !$(e.target).closest(this.$('.name input')).length) {
                    this.endNodeRenaming();
                } else {
                    this.$('input[type=checkbox]').unbind();
                    this.$('input[type=checkbox]').on('click', function(e) { e.preventDefault(); });
