    ...
    AssignRolesPanel = Backbone.View.extend({
        ...
        checkForConflicts: function(e) {
            this.collection.each(function(role) {
                ...
                var isDisabled = !this.screen.nodes.length || !this.isControllerSelectable(role) || roleConflict || !role.get('requiredSettingState');
        ...
        initialize: function(options) {
            ...
            this.collection = new Backbone.Collection(_.map(this.cluster.get('release').get('roles'), function(role) {
                ...
                var requiredSettingState = true;
                var requiredDescription;
                try {
                    requiredSettingState = this.clusterSettings.get('editable.'+roleData.requires.settings);
                    requiredDescription = roleData.requires.description;
                } catch (ignore) {}
                return {
                    name: role,
                    label: roleData.name,
                    ...
                    disabled: false,
                    requiredSettingState: requiredSettingState,
            ...
            bindings['.role-conflict.' + role.get('name')] = {
                ...
                onGet: _.bind(function(value, options) {
                    ...
                    if (value && this.screen.nodes.length) {
                        if (!inactiveRole.get('requiredSettingState')){
                            return inactiveRole.get('requiredDescription');
                        }
