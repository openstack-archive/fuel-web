...
function(utils, models, commonViews, dialogViews, nodesManagementPanelTemplate, assignRolesPanelTemplate, nodeListTemplate, nodeGroupTemplate, nodeTemplate, editNodeDisksScreenTemplate, nodeDisksTemplate, volumeStylesTemplate, editNodeInterfacesScreenTemplate, nodeInterfaceTemplate) {
    ...
    Node = Backbone.View.extend({
        ...
        bindings: {
            ...
            '.progress': {
	            ...
	            name: 'class',
                    observe: 'progress',
	            onGet: function(value, options) {
                        var progressBarClass = this.node.get('status') == 'deploying' ? 'progress-success' : this.node.get('status') == 'provisioning' ? '' : 'hide';
        ...
        formatNodePanelClass: function(value, options) {
            return 'node-box ' + value ? '' : 'node-offline';
        },
        ...
        formatStatusLabel: function(value) {
            ...
            var labels = {
                ...
                ready: 'Ready',
                provisioning: 'Installing ' + operatingSystem,
                provisioned: operatingSystem + ' is installed',
        ...
        initialize: function(options) {
            ...
            // FIXME: Select All group tumbler state should be calculated
            //this.node.on('change:pending_deletion change:online', this.group.render, this);
