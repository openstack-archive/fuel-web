    ...
    AssignRolesPanel = Backbone.View.extend({
        ...
        initialize: function(options) {
            ...
            this.collection = new Backbone.Collection(_.map(this.cluster.get('release').get('roles'), function(role) {
                ...
                try {
                    // We are supporting only one dependent setting
                    _.each(_.keys(roleData.depends.condition), _.bind(function(condition) {
                        dependentSettingState = this.cluster.get('settings').get(condition) == roleData.depends.condition[condition];
                    }, this));
                ...
                onGet: _.bind(function(value, options) {
                    if (value && this.screen.nodes.length) {
                        ...
                        if (!inactiveRole.get('dependentSettingState')){
                            return inactiveRole.get('dependent');
                        }
