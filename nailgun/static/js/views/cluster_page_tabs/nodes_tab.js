    ...
    AssignRolesPanel = Backbone.View.extend({
        ...
        checkRolesDependencies: function() {
            this.roles.each(function(role) {
                ...
                _.each(role.get('restrictions'), function(restriction) {
                    if (utils.evaluateExpression(restriction.condition, configModels).value) {
                        unavailable = true;
                        unavailabityReasons.push(restriction.warning);
                    }
                });
                if (unavailable) {
