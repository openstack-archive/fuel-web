...
function(utils, models, commonViews, dialogViews, nodesManagementPanelTemplate, assignRolesPanelTemplate, nodeListTemplate, nodeGroupTemplate, nodeTemplate, nodeRoleTemplate, editNodeDisksScreenTemplate, nodeDisksTemplate, volumeStylesTemplate, editNodeInterfacesScreenTemplate, nodeInterfaceTemplate) {
    ...
    NodeListScreen = Screen.extend({
        ...
        duplicateRolesToFilteredNodeCollection: function(node, roles, options) {
    ...
    NodesManagementPanel = Backbone.View.extend({
        ...
        goToEditNodesRolesScreen: function() {
            app.navigate('#cluster/' + this.cluster.id + '/nodes/edit/' + utils.serializeTabOptions({nodes: _.pluck(this.screen.nodes.where({checked: true}), 'id')}), {trigger: true});
        },
    ...
    AssignRolesPanel = Backbone.View.extend({
        ...
        initialize: function(options) {
            ...
            this.nodes.on('change:pending_roles', function(node, roles, options) {
                if (options.assign) {
                    this.screen.duplicateRolesToFilteredNodeCollection(node,roles, options);
                }
            }, this);
        },
    ...
    Node = Backbone.View.extend({
        ...
        duplicateCheckedToScreenNodeCollection: function(node, checked, options) {
        ...
        initialize: function(options) {
            ...
            this.node.on('change:checked', this.duplicateCheckedToScreenNodeCollection, this);
            this.node.set('checked', this.screen instanceof EditNodesScreen || this.screen.nodes.get(this.node.id).get('checked'));
