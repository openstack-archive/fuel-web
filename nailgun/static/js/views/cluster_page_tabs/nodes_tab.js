...
function(utils, models, commonViews, dialogViews, nodesManagementPanelTemplate, assignRolesPanelTemplate, nodeListTemplate, nodeGroupTemplate, nodeTemplate, nodeRoleTemplate, editNodeDisksScreenTemplate, nodeDisksTemplate, volumeStylesTemplate, editNodeInterfacesScreenTemplate, nodeInterfaceTemplate) {
    ...
    AssignRolesPanel = Backbone.View.extend({
        ...
        checkRoleRestrictions: function(e) {
            this.roles.each(function(role) {
                ...
                if (!disabled && !_.isNull(roleMaximumAmount)) {
                    var cantAddRole = roleMaximumAmount == 0;
                    if (cantAddRole && !role.isSelected()) {
                        disabled = true;
                        conflict = $.t('cluster_page.nodes_tab.' + roleName + '_cant_be_added');
                    }
                    if (!cantAddRole) {
                        var availableNodesAmount = roleMaximumAmount - this.cluster.getNodesByRole(roleName).length;
                        if (availableNodesAmount == 0 && !role.isSelected()) {
                            disabled = true;
                            conflict = $.t('cluster_page.nodes_tab.role_max_amount_reached');
                        }
                        if (availableNodesAmount == this.nodes.where({checked: true}).length && role.isSelected()) {
                            populated = true;
                            conflict = $.t('cluster_page.nodes_tab.role_max_amount_reached');
                        }
                        if (availableNodesAmount < this.nodes.where({checked: true}).length && !role.isSelected()) {
                ...
                if (!disabled) {
                    ...
                    if (_.contains(_.flatten(forbiddenRoles), roleName)) {
                        disabled = true;
        ...
        initialize: function(options) {
            _.defaults(this, options);
            this.cluster = this.screen.tab.model;
            this.roles = this.cluster.get('roles');
            this.roles.each(function(role) {
            ...
            this.checkRolesDependencies();
        },
        handleChanges: function() {
            this.assignRoles();
            this.checkRoleRestrictions();
        },
            ...
            if (!disabled && !(this.screen instanceof ClusterNodesScreen)) {
                _.each(this.screen.rolePanel.roles.where({checked: true}), function(role) {
                    ...
                    if (!_.isNull(roleMaximumAmount)) {
                        ...
                        disabled = !availableNodesAmount ||  availableNodesAmount < availableNodes.length;
                    }
                    return !disabled;
                }, this);
            }
