...
function(utils, models, dialogViews, EditNodeScreen, EditNodeControllersScreenTemplate) {
    'use strict';
    ...
    EditNodeControllersScreen = EditNodeScreen.extend({
        ...
        resetConfiguration: function() {
            ...
            if (lengthVD > 0) {
                while (lengthVD) {
                    ...
                    lengthVD = this.controller.virtual_drives.length;
                }
            }
        ...
        applyChanges: function(e) {
            ...
            $.ajax({
                ...
                statusCode: {
                    404: function(errMsg) {
                        utils.showErrorDialog({title: "Page not found", message: errMsg});
                    }
                },
                failure: function(errMsg) {
                    utils.showErrorDialog({title: "Failure", message: errMsg});
                }
            });
        },
        startApply: function() {
        ...
        scheduleUpdate: function() {
            ...
            var raidTask = this.model.get('tasks').filter(function(task) {
                return task.get('node') == IDnode && task.get('name') == 'raid';
            })[0];
        ...
        updateInfo: function(e) {
            ...
            if (this.controller.tasks.length) {
                ...
                if (task.action == 'add_hotspare') {
                    ...
                    this.controller.virtual_drives.push(nwd);
                } else {
                    _.each(this.controller.physical_drives, function(pd) {
                        if (pd.enclosure == task.eid) {
                            _.each(task.phys_devices, function(slot) {
        ...
        checkOnTypeDisk: function(e) {
            ...
            for(i; i<allCheckBox.length; i += 1) {
        ...
        checkOffTypeDisk: function(e) {
            ...
            var d = 0;
            var i = 0;
            for (d; d<this.Raid.drive.length; d += 1) {
