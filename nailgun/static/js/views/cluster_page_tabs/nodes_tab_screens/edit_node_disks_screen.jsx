...
function($, _, i18n, Backbone, React, utils, models, dialogs) {
    'use strict';
    ...
    var NodeDisk = React.createClass({
        ...
        handleVolumeSize: function(event) {
            var volume = {};
        ...
        renderVisualGraph: function() {
            ...
            if (!disk.get('volumes').some('validationError') && disk.isValid()) {
                ...
                disk.get('volumes').each(function(volume) {
                    var volumeSize = this.state[volume.get('name')],
                        width = disk.get('size') ? utils.floor(volumeSize / disk.get('size') * 100, 2) : 0;
                    unallocatedWidth -= width;
                    this.renderVolume(volume.get('name'), width, volumeSize);
                }, this);
        ...
        componentWillMount: function() {
            this.setState(this.getVolumesInfo());
        },
                                        ...
                                        <div className="volume-group-box volume-group" data-volume={volume.get('name')} >
                                            ...
                                            <div className="pull-right">
                                                ...
                                                <div className="volume-group-box-input">
                                                    <input id={disk.id + '-' + volume.get('name')} className="input-medium" type="text" name={volume.get('name')} value={this.state[volume.get('name')]} onChange={this.handleVolumeSize} />
                                                </div>
