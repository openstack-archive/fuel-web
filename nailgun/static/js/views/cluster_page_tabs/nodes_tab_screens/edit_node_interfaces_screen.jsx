...
function($, _, Backbone, React, i18n, utils, models, dispatcher, dialogs, controls, ComponentMixins) {
    ...
    EditNodeInterfacesScreen = React.createClass({
        ...
        render: function() {
            ...
            // TODO: rewrite this with lodash functions
            var extractedSpeeds = [];
            _.each(nodes.map(function(node) {return node.interfaces;}), function(ifcsOnNode) {
                extractedSpeeds.push(_.map(_.times(numberOfInterfaces), function(indexOfIfc) {
                    return ifcsOnNode.at(indexOfIfc).get('current_speed');
                }, this));
            }, this);

            var resultingSpeeds = [];
            // this loop to be avoided
            for (var i = 0; i < numberOfInterfaces; i++) {
                resultingSpeeds.push(_.map(extractedSpeeds, function(speedsOnNode) {
                    return speedsOnNode[i];
                }, this));
            }
                                ...
                                interfaces.map(_.bind(function(ifc, index) {
                                    if (!_.contains(slaveInterfaceNames, ifc.get('name'))) {
                                        return <NodeInterface {...this.props}
                                            ...
                                            interfaceSpeeds={resultingSpeeds[index]}
                                        />;
                                    }
                                }, this))
                            }
                                    ...
                                    <div className='network-connections-info-description'>
                                        ...
                                        <div>
                                            {i18n(configureInterfacesTransNS + 'speed')}: {isMultipleNodesConfiguration ?
                                                _.unique(_.map(this.props.interfaceSpeeds, utils.showBandwidth)).join(', ') :
