...
function($, _, Backbone, React, i18n, utils, models, dialogs, controls, ComponentMixins) {
    ...
    EditNodeInterfacesScreen = React.createClass({
        ...
        setInitialData: function() {
            this.initialInterfaces = this.interfacesToJSON(this.props.interfaces);
        },
        ...
        interfacesOmitFromJSON: function(json) {
            // Omit certain interface fields that don't have influence on hasChanges.
            // As for state: sometimes 'state' is sent from the API and sometimes not
            // It's better to just unify all inputs to the one without state.
            return _.omit(json, 'id', 'current_speed', 'mac', 'max_speed', 'name', 'state');
        },
        interfacesToJSON: function(interfaces, omitMore) {
            var omitter = omitMore ? this.interfacesOmitFromJSON : function(json) { return _.omit(json, 'state'); };

            return interfaces.map(function(ifc) {
                return omitter(ifc.toJSON());
            });
        },
        hasChangesInRemainingNodes: function() {
            var initialInterfacesOmitted = _.map(this.initialInterfaces, this.interfacesOmitFromJSON);
        ...
        hasChanges: function() {
            return !_.isEqual(this.initialInterfaces, this.interfacesToJSON(this.props.interfaces)) ||
