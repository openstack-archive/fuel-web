...
function($, _, React, utils, models, dialogs, controls) {
    ...
    var NodeInterface = React.createClass({
        ...
        isLocked: function() {
            ...
            return !nodesAvailableForChanges.length;  // TODO: || this.constructor.__super__.isLocked.apply(this);
        },
        bondingAvailable: function() {
            var iserDisabled =  this.props.model.get('settings').get('storage.iser.value') != true;
            var mellanoxSriovDisabled = this.props.model.get('settings').get('neutron_mellanox.plugin.value') != "ethernet";
            return !this.isLocked() && this.props.model.get('net_provider') == 'neutron' && iserDisabled && mellanoxSriovDisabled;
        },
        render: function() {
            ...
            var bondable = bondingAvailable && !cluster.get('assigned_networks').find(function(interfaceNetwork) {return getFullNetwork(interfaceNetwork).get('meta').unmovable;});
                                ...
                                return (
                                    <div className="network-connections-info-block-item">
                                        ...
                                        <div className="network-connections-info-description">
                                            ...
                                            { (bondingAvailable && slaveInterfaces.length >= 3) ?
                            ...
                            { _.map(assignedNetworksGrouped, function(networkGroup) {
                                ...
                                return (
                                    <div className={cx(classes)}>
                                        ...
                                        <div class="network-help-message hide">{$.t("cluster_page.nodes_tab.configure_interfaces.drag_and_drop_description")}</div>
                                    </div>
                                )
                            })
                            }
            ...
            $.when.apply($, this.nodes.map(function(node) {
                ...
                .done(_.bind(function() {
                    if(this.nodes.length) {
                        ...
                        console.log('interfaces', this.interfaces.toJSON());
