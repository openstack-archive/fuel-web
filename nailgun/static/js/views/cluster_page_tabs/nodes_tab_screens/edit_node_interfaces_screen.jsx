...
function($, _, Backbone, React, i18n, utils, models, dispatcher, dialogs, controls, ComponentMixins) {
    ...
    EditNodeInterfacesScreen = React.createClass({
                ...
                node.interfaces.each(function(ifc, index) {
                    ...
                    if (ifc.isBond()) {
                        ...
                        ifc.set({bond_properties: _.extend(bondProperties, {type__:
                            _.contains(this.getBondPropertyValues(), 'linux') ? 'linux' : 'ovs'})});
                    }
                }, this);
        ...
        bondingAvailable: function() {
            var availableBondTypes = this.getBondPropertyValues();
            return !this.isLocked() && availableBondTypes && !!availableBondTypes.length;
        },
        getBondPropertyValues: function() {
        ...
        bondInterfaces: function() {
            ...
            if (!bonds) {
                ...
                var bondMode = _.flatten(_.pluck(bondingProperties[this.getBondPropertyValues()[0]].mode, 'values'))[0];
                bonds = new models.Interface({
                    type: 'bond',
                    name: this.props.interfaces.generateBondName(_.contains(this.getBondPropertyValues(), 'linux') ? 'bond' : 'ovs-bond'),
                    mode: bondMode,
                                ...
                                interfaces.map(_.bind(function(ifc) {
                                    if (!_.contains(slaveInterfaceNames, ifc.get('name'))) {
                                        return <NodeInterface {...this.props}
                                            ...
                                            bondingProperties={this.props.bondingConfig.properties}
                                            bondType={this.getBondPropertyValues()[0]}
                                        />;
                                    }
                                }, this))
                            }
    ...
    NodeInterface = React.createClass({
        ...
        onInterfacePropertiesChange: function(name, value) {
            ...
            interfaceProperties = _.extend(interfaceProperties, interfaceProperties[name] = value);
