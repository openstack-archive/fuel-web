    ...
    EditNodeInterfacesScreen = React.createClass({
        ...
        bondingAvailable: function() {
            var cluster = this.props.cluster,
                // bonding won't be experimental with linux bonds
                isExperimental = app.version.get('release') == LINUX_BONDING_RELEASE || _.contains(app.version.get('feature_groups'), 'experimental'),
                ...
                // bonding will be available in nova also in 6.1
                isNeutron = app.version.get('release') == LINUX_BONDING_RELEASE || cluster.get('net_provider') == 'neutron',
                // minimum 3 interfaces is required - 1 for admin which cannot be bonded and 2 other to actually create a bond
                hasEnoughInterfaces = this.props.interfaces.length >= 3;

            return !this.isLocked() && isExperimental && isNeutron && iserDisabled && mellanoxSriovDisabled && hasEnoughInterfaces;
        },
