...
function($, _, Backbone, React, i18n, utils, models, controls) {
    ...
    var NodeInterfaces = React.createClass({
        mixins: [
            ...
            React.BackboneMixin('interface', 'change mode'),
            React.BackboneMixin({
        ...
        componentDidMount: function() {
            $(this.refs.logicalNetworkBox.getDOMNode()).sortable({
                ...
                containment: $('.node-networks', this.getDOMNode()),
        ...
        render: function() {
            var configureInterfacesTransNS = 'cluster_page.nodes_tab.configure_interfaces',
                ...
                selectOptions = _.map(models.Interface.prototype.bondingModes, function(mode) {
                    return {value: mode, label: i18n(configureInterfacesTransNS + '.bonding_modes.' + mode)};
                }),
                                ...
                                <div className='network-bond-mode pull-right'>
                                    ...
                                    <span>
                                        <controls.Input
                                            ...
                                            value={ifc.get('mode')}
                                            children={_.map(selectOptions, function(option) {
                                                    return (
    ...
    var EditNodeInterfacesScreen = React.createClass({
        ...
        componentWillMount: function() {
            ...
            $.when.apply($, this.nodes.map(_.bind(function(node) {
            ...
            return (
                <div className='edit-node-networks-screen' style={{display: 'block'}} ref='nodeNetworksScreen'>
                    <div className={cx({'edit-node-interfaces': true, 'changes-locked': locked})}>
                        <h3>
                            {i18n(configureInterfacesTransNS + '.title', {count: 1, name: node && node.get('name')})}
                        </h3>
                    </div>
