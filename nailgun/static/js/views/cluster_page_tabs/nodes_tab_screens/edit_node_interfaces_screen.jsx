...
function($, _, Backbone, React, i18n, utils, models, controls) {
    'use strict';
    ...
    var NodeInterfaces = React.createClass({
        mixins: [
            ...
            React.BackboneMixin('interface', 'change:checked change:mode'),
            React.BackboneMixin({
                        ...
                        {ifc.isBond() &&
                            <div className='network-box-name'>
                                ...
                                <div className='network-bond-mode pull-right'>
                                    <b>{i18n(configureInterfacesTransNS + 'bonding_mode')}:</b>
                                    <span>
                                        <controls.Input
                                            type='select'
    ...
    var EditNodeInterfacesScreen = React.createClass({
        ...
        interfaceToJSON: function(ifc) {
            return _.omit(ifc.toJSON(), 'checked');
        },
        setInterfacesEvents: function() {
            this.interfaces.on('change:checked change:slaves sync', function() { this.forceUpdate(); }, this);
        },
        setInitialData: function() {
            this.initialInterfaces = this.interfaces.map(this.interfaceToJSON);
        },
        hasChanges: function() {
            return !_.isEqual(this.initialInterfaces, this.interfaces.map(this.interfaceToJSON));
        },
