...
function($, _, Backbone, React, i18n, utils, models, controls, ComponentMixins) {
    ...
    var cx = React.addons.classSet,
        ScreenMixin, EditNodeInterfacesScreen, NodeInterfaces;
    ...
    ScreenMixin = {
        ...
        isLockedScreen: function() {
            return this.props.model && !!this.props.model.tasks({group: 'deployment', status: 'running'}).length;
        },
    ...
    EditNodeInterfacesScreen = React.createClass({
        mixins: [
            ScreenMixin,
            ComponentMixins.backboneMixin('model', 'change:status change:networkConfiguration change:nodes')
        ],
        ...
        componentWillMount: function() {
            ...
            if (this.nodes.length != nodeIds.length) {
                utils.showErrorDialog({
                    title: i18n('cluster_page.nodes_tab.configure_interfaces.node_loading_error.title'),
                    message: i18n('cluster_page.nodes_tab.configure_interfaces.node_loading_error.load_error')
                });
                this.goToNodeList();
                return;
            }
            ...
            $.when.apply($, this.nodes.map(function(node) {
                ...
                .done(_.bind(function() {
                    this.interfaces = this.nodes.at(0).interfaces;
                    this.setInterfacesEvents();
                    this.setInitialData();
        ...
        setInterfacesEvents: function() {
            this.interfaces.on('change:checked change:slaves sync', function() { this.forceUpdate(); }, this);
        },
        interfacesToJSON: function() {
            ...
            return this.interfaces.map(function(ifc) { return _.omit(ifc.toJSON(), 'state'); });
        },
        ...
        loadDefaults: function() {
            ...
            $.when(this.interfaces.fetch({
                url: _.result(this.nodes.at(0), 'url') + '/interfaces/default_assignment', reset: true
        ...
        revertChanges: function() {
            ...
            this.interfaces.reset(_.cloneDeep(this.initialInterfaces), {parse: true});
        ...
        render: function() {
            ...
            var configureInterfacesTransNS = 'cluster_page.nodes_tab.configure_interfaces.',
                ...
                returnEnabled = !this.state.loading && !this.state.actionInProgress,
                loadDefaultsEnabled = !this.state.loading && !this.state.actionInProgress,
                revertChangesEnabled = !this.state.loading && !this.state.actionInProgress && hasChanges,
                applyEnabled = !hasErrors && !this.state.loading && !this.state.actionInProgress && hasChanges;
    ...
    NodeInterfaces = React.createClass({
        mixins: [
