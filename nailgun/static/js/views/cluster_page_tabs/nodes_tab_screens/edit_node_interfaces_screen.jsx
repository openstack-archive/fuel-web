...
function($, _, Backbone, React, utils, models, controls) {
    'use strict';
        ...
        render: function() {
            var configureInterfacesTransNS = 'cluster_page.nodes_tab.configure_interfaces',
                ...
                selectOptions = _.map(models.Interface.prototype.bondingModes, function(mode) {
                    return {value: mode, label: $.t(configureInterfacesTransNS + '.bonding_modes.' + mode)};
                }),
                ...
                <div className={cx({'physical-network-box': true, nodrag: this.props.errors})} data-name={ifc.get('name')}>
                    <div className='network-box-item'>
                        { ifc.isBond() ?
                            <div className='network-box-name'>
                                ...
                                <div className='network-bond-mode pull-right'>
                                    <b>{$.t(configureInterfacesTransNS + '.bonding_mode')}:</b>
                                    <span>
                        ...
                        <div className='physical-network-checkbox'>
                            { !ifc.isBond() && bondable && (<controls.Input type='checkbox' onChange={this.bondingChanged} />) }
                        </div>
                                ...
                                return (
                                    <div className='network-connections-info-block-item'>
                                        ...
                                        <div className='network-connections-info-description'>
                                            ...
                                            <div>{$.t(configureInterfacesTransNS + '.speed')}:
                                            ...
                                            { (this.props.bondingAvailable && slaveInterfaces.length >= 3) ?
                                                <button className='btn btn-link btn-remove-interface'
                                                        type='button' data-interface-name={slaveInterface.get('name')}
                                                        onClick={this.bondingRemoveInterface.bind(this, slaveInterface.get('name'))}>Remove</button>
                                                : null
                                            }
                                        </div>
                                    </div>
                        ...
                        <div className='logical-network-box' ref='logicalNetworkBox'>
                                        ...
                                        { _.map(networkGroup, function(interfaceNetwork) {
                                            return (
                                                <div className='logical-network-item' data-name={interfaceNetwork.get('name')}>
                                                    ...
                                                    { vlanRange &&
                                                        (<div className='id'>
                                                            {$.t(configureInterfacesTransNS + '.vlan_id', {count: _.uniq(vlanRange).length})}:
                            ...
                            : <div className='network-help-message'>{$.t(configureInterfacesTransNS + '.drag_and_drop_description')}</div>
                            }
                        </div>
                    </div>
    ...
    var EditNodeInterfacesScreen = React.createClass({
        ...
        applyChanges: function() {
            ...
            return $.when.apply($, nodes.map(function(n, idx) {
                ...
                oldNodeBonds = n.interfaces.filter(function(ifc) {return ifc.isBond();});
                ...
                _.each(nodeBonds, function(bond, bondIndex) {
                    var slaveIndexes = bondingMap[bondIndex],
                        slaveInterfaces = _.map(slaveIndexes, n.interfaces.at, n.interfaces);
                ...
                .done(_.bind(function() {
                    ...
                    $(this.refs.nodeNetworksScreen.getDOMNode()).find('input[type=checkbox]').prop('checked', false);
                }, this))
                .fail(function() {
                    utils.showErrorDialog({
                        title: $.t('cluster_page.nodes_tab.configure_interfaces.configuration_error.title'),
        ...
        isLocked: function() {
            var hasLockedNodes = this.nodes.any(function(node) {
        ...
        bondingAvailable: function() {
            var cluster = this.props.model,
                ...
                iserDisabled =  cluster.get('settings').get('storage.iser.value') != true,
            ...
            return (
                <div className='edit-node-networks-screen' style={{display: 'block'}} ref='nodeNetworksScreen'>
                    <div className={cx({'edit-node-interfaces': true, 'changes-locked': locked})}>
                        <h3>
                            {$.t(configureInterfacesTransNS + '.title', {count: 1, name: node && node.get('name')})}
                        </h3>
                    </div>
                    ...
                    <div className='row'>
                        <div className='page-control-box'>
                            <div className='page-control-button-placeholder'>
                                <button className='btn btn-bond' disabled={!bondingAvailable || !bondingPossible} onClick={this.bondInterfaces}>{$.t(configureInterfacesTransNS + '.bond_button')}</button>
                                <button className='btn btn-unbond' disabled={!bondingAvailable || !unbondingPossible} onClick={this.unbondInterfaces}>{$.t(configureInterfacesTransNS + '.unbond_button')}</button>
                            </div>
                        </div>
                        {bondingAvailable &&
                            (<div className='bond-speed-warning alert hide'>{$.t(configureInterfacesTransNS + '.bond_speed_warning')}</div>)
                        }
                        ...
                        <div className='page-control-box'>
                            <div className='back-button pull-left'>
                                <button className='btn btn-return' onClick={this.returnToNodeList} disabled={!returnEnabled}>{$.t('cluster_page.nodes_tab.back_to_nodes_button')}</button>
                            </div>
                            <div className='page-control-button-placeholder'>
