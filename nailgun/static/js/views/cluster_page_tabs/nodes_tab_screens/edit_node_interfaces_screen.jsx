...
function($, _, Backbone, React, i18n, utils, models, dispatcher, dialogs, controls, ComponentMixins) {
    ...
    EditNodeInterfacesScreen = React.createClass({
        ...
        bondInterfaces: function() {
            ...
            if (!bonds) {
                ...
                var bondMode = _.flatten(_.pluck(bondingProperties[this.getAvailableBondTypes()[0]].mode, 'values'))[0];
                                ...
                                interfaces.map(_.bind(function(ifc) {
                                    if (!_.contains(slaveInterfaceNames, ifc.get('name'))) {
                                        return <NodeInterface {...this.props}
                                            ...
                                            bondingProperties={this.props.bondingConfig.properties}
                                            bondType={this.getAvailableBondTypes()[0]}
    ...
    NodeInterface = React.createClass({
        ...
        isLacpRateAvailable: function() {
            return _.contains(this.getAllPossibleBondPropertyValues('lacp_rate', 'for_modes'), this.getBondMode());
        },
        isHashPolicyNeeded: function() {
            return _.contains(this.getAllPossibleBondPropertyValues('xmit_hash_policy', 'for_modes'), this.getBondMode());
        },
        getBondMode: function() {
            var ifc = this.props.interface;
            return ifc.get('mode') || (ifc.get('bond_properties') || {}).mode;
        },
        getAllPossibleBondPropertyValues: function(propertyName, value) {
        ...
        bondingModeChanged: function(name, value) {
            ...
            if (this.isHashPolicyNeeded()) {
                this.updateBondProperties({xmit_hash_policy: this.getAllPossibleBondPropertyValues('xmit_hash_policy', 'values')[0]});
            }
            if (this.isLacpRateAvailable()) {
                this.updateBondProperties({lacp_rate: this.getAllPossibleBondPropertyValues('lacp_rate', 'values')[0]});
            }
        },
                            ...
                            <div className='network-box-name'>
                                ...
                                {this.isLacpRateAvailable() &&
                                    <div className='network-lacp-rate pull-right'>
                                        <controls.Input
                                            ...
                                            children={this.getBondingOptions(this.getAllPossibleBondPropertyValues('lacp_rate', 'values'), 'lacp_rates')}
                                        />
                                    </div>
                                }
                                {this.isHashPolicyNeeded() &&
                                    <div className='network-bond-policy pull-right'>
                                        <controls.Input
                                            ...
                                            children={this.getBondingOptions(this.getAllPossibleBondPropertyValues('xmit_hash_policy', 'values'), 'hash_policy')}
                                        />
                                    </div>
                                }
                                <div className='network-bond-mode pull-right'>
                                    <controls.Input
                                        ...
                                        children={this.getBondingOptions(this.getAllPossibleBondPropertyValues('mode', 'values'), 'bonding_modes')}
                                    />
                                </div>
