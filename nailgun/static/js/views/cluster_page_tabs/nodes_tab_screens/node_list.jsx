    ...
    var SelectAllMixin = {
        ...
        renderSelectAllCheckbox: function() {
            var selectableNodes = this.getSelectableNodes(),
                roles;
            // FIXME: definitely need to avoid this try-catch
            try {
                roles = app.page.tab.screen.roles;
            } catch(e) {}
            // FIXME: one more role limits management hack
            var roleLimitation = roles && ((roles.isRoleSelected('controller') && this.props.cluster.get('mode') == 'multi-node') || roles.isRoleSelected('zabbix-server')) && selectableNodes.length > 1;
            return (
                <controls.Input
                    type='checkbox'
