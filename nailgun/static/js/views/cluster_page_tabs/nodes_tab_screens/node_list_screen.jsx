...
function($, _, i18n, React, utils, models, controls, dialogs, componentMixins) {
    ...
    RolePanel = React.createClass({
        ...
        checkLimits: function(role, nodes) {
            return role.checkLimits(this.configModels, false, ['max'], nodes);
        },
            ...
            return (
                <div className='role-panel'>
                    ...
                    {this.props.cluster.get('release').get('role_models').map(function(role) {
                        if (!role.checkRestrictions(this.configModels, 'hide').result) {
                            var name = role.get('name'),
                                processedRestrictions = this.props.nodes.length ? this.processRestrictions(role, this.configModels, nodesForLimitsCheck) : {};
                            return (
                                <controls.Input
                                    key={name}
                                    ref={name}
