...
function($, _, i18n, React, utils, models, controls, dialogs, componentMixins) {
            ...
            return (
                <div>
                    {this.props.mode == 'edit' &&
                        <div className='alert'>{i18n.t('cluster_page.nodes_tab.disk_configuration_reset_warning')}</div>
                    }
                    <ManagementPanel
    ...
    ManagementPanel = React.createClass({
        ...
        goToConfigurationScreen: function(action, conflict) {
            if (conflict) {
                ...
                utils.showErrorDialog({title: i18n.t(ns + 'title'), message: i18n.t(ns + action + '_configuration_warning')});
                return;
            }
        ...
        applyChanges: function() {
            ...
            this.props.nodes.sync('update', this.props.nodes)
                ...
                .fail(_.bind(function() {
                    ...
                    utils.showErrorDialog({message: i18n.t('cluster_page.nodes_tab.node_management_panel.node_management_error.saving_warning')});
                }, this));
        },
                ...
                <div className='node-management-panel'>
                    ...
                    <div className='node-filter'>
                        <controls.Input
                            ...
                            label={i18n.t(ns + 'filter_by')}
                            placeholder={i18n.t(ns + 'filter_placeholder')}
                    ...
                    <div className='buttons'>
                        {this.props.mode != 'list' ? [
                            ...
                            >
                                {i18n.t('common.apply_changes_button')}
                            </button>
                        ] : [
                            <button
                                key='disks'
                            ...
                            >
                                ...
                                <span>{i18n.t('dialog.show_node.disk_configuration_button')}</span>
                            </button>,
                            !this.props.nodes.any(function(node) {return node.get('status') == 'error';}) &&
                                ...
                                >
                                    ...
                                    <span>{i18n.t('dialog.show_node.network_configuration_button')}</span>
                                </button>,
                            !!this.props.nodes.length && this.props.nodes.any(function(node) {return !node.get('pending_deletion');}) &&
                                ...
                                >
                                    ...
                                    <span>{i18n.t('common.delete_button')}</span>
                                </button>,
                            !!this.props.nodes.length && !this.props.nodes.any(function(node) {return !node.get('pending_addition');}) &&
                                ...
                                >
                                    ...
                                    <span>{i18n.t(ns + 'edit_roles_button')}</span>
                                </button>,
                            !this.props.nodes.length &&
                                ...
                                >
                                    ...
                                    <span>{i18n.t(ns + 'add_nodes_button')}</span>
                                </button>
                        ]}
                    </div>
                </div>
    ...
    RolePanel = React.createClass({
        ...
        processRestrictions: function(role, models) {
            ...
            if (_.contains(conflicts, name)) messages.push(i18n.t('cluster_page.nodes_tab.role_conflict'));
            if (!isAvailable) messages.push(i18n.t('cluster_page.nodes_tab.' + name + '_restriction'));
            return {
        ...
        render: function() {
            ...
            return (
                <div className='role-panel'>
                    <h4>{i18n.t('cluster_page.nodes_tab.assign_roles')}</h4>
    ...
    NodeList = React.createClass({
        ...
        getEmptyListWarning: function() {
            ...
            if (this.props.mode == 'add') return i18n.t(ns + 'no_nodes_in_fuel');
            if (this.props.cluster.get('nodes').length) return i18n.t(ns + 'no_filtered_nodes_warning');
            return i18n.t(ns + 'no_nodes_in_environment');
        },
        groupNodes: function() {
    ...
    Node = React.createClass({
        ...
        discardNodeChanges: function() {
            ...
            this.props.node.save(data, {patch: true, wait: true, silent: true})
                ...
                .fail(function() {utils.showErrorDialog({title: i18n.t('dialog.discard_changes.cant_discard')});});
        },
                        ...
                        <div className='node-content'>
                            ...
                            <div className='node-name-roles'>
                                <div className='name enable-selection'>
                                    {this.state.renaming ?
                                        ...
                                        />
                                    :
                                        <p title={i18n.t(ns + 'edit_name')} onClick={!disabled && this.startNodeRenaming}>
                                            {node.get('name') || node.get('mac')}
                                        </p>
                                    }
                                </div>
                                <div className='role-list'>
                                    {_.compact(roles).length ? roles : i18n.t(ns + 'unallocated')}
                                </div>
                            </div>
                            <div className='node-button'>
                                {!!node.get('cluster') && (
                                    (this.props.locked || !node.hasChanges()) ?
                                        <a className='btn btn-link' title={i18n.t(ns + 'view_logs')} href={this.getNodeLogsLink()}>
                                            <i className='icon-logs' />
                                        </a>
                                    :
                                        <button
                                            className='btn btn-link'
                                            title={i18n.t(ns + (node.get('pending_addition') ? 'discard_addition' : 'discard_deletion'))}
                            ...
                            <div className={cx(statusClasses)}>
                                <div className='node-status-container'>
                                    ...
                                    <span>
                                        {i18n.t(ns + 'status.' + status, {os: this.props.cluster.get('release').get('operating_system') || 'OS'})}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className='node-hardware'>
                                <span>
                                    {i18n.t('node_details.cpu')}: {node.resource('cores') || '0'} ({node.resource('ht_cores') || '?'})
                                </span>
                                <span>
                                    {i18n.t('node_details.hdd')}: {node.resource('hdd') ? utils.showDiskSize(node.resource('hdd')) : '?' + i18n.t('common.size.gb')}
                                </span>
                                <span>
                                    {i18n.t('node_details.ram')}: {node.resource('ram') ? utils.showMemorySize(node.resource('ram')) : '?' + i18n.t('common.size.gb')}
                                </span>
                            </div>
                        </div>
                    </label>
