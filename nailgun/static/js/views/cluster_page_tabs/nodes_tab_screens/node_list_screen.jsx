...
function($, _, i18n, Backbone, React, utils, models, dispatcher, controls, dialogs, componentMixins) {
    ...
    NodeListScreen = React.createClass({
        ...
        processRoleLimits: function() {
            var cluster = this.props.cluster,
                ...
                maxNumberOfNodes = [this.props.nodes.length],
                roleNamesWithMaxLimitReached = [],
                processedRoleLimits = [];

            cluster.get('release').get('role_models').map(function(role) {
                var checkLimitsResults = role.checkLimits({
                    cluster: cluster,
                    settings: cluster.get('settings'),
                    version: app.version,
                    default: cluster.get('settings')
                }, false, ['max'], nodesForLimitCheck);

                _.each(processedRoleLimits, function(roleLimit) {
                    if (_.intersection(_.keys(roleLimit), this.state.selectedRoles).length) {
                        maxNumberOfNodes.push(_.pluck(roleLimit, 'limits')[0].max);
                    }
                }, this);

                var formattedRoleLimits = {};
                formattedRoleLimits[role.get('name')] = checkLimitsResults;
                processedRoleLimits.push(formattedRoleLimits);
                if (!checkLimitsResults.valid) {
                    roleNamesWithMaxLimitReached.push(role.get('name'));
                }
            }, this);
            ...
            return {
                ...
                // needed for NodeList to disable nodes if role with limit selected
                roleNamesWithMaxLimitReached: roleNamesWithMaxLimitReached,
                // needed for NodeList to disable select all on demand
                maxNumberOfNodes: _.min(maxNumberOfNodes)
            };
        },
        ...
        render: function() {
            var cluster = this.props.cluster,
                ...
                processesRolesData = this.processRoleLimits();
                    ...
                    {this.state.loading ? <controls.ProgressBar /> :
                        <div>
                            {this.props.mode != 'list' &&
                                <RolePanel
                                    {...this.props}
                                    {... _.pick(processesRolesData, 'processedRoleLimits')}
                            ...
                            <NodeList {...this.props}
                                ...
                                {... _.pick(processesRolesData, 'roleNamesWithMaxLimitReached', 'maxNumberOfNodes')}
                                locked={locked}
    ...
    RolePanel = React.createClass({
        ...
        processRestrictions: function(role, models) {
            var name = role.get('name'),
                ...
                roleLimitsCheckResults = _.find(this.props.processedRoleLimits, role.get('name'))[role.get('name')],
            ...
            if (!roleLimitsCheckResults.valid && roleLimitsCheckResults.message) messages.push(roleLimitsCheckResults.message);
            if (_.contains(conflicts, name)) messages.push(i18n('cluster_page.nodes_tab.role_conflict'));

            return {
                result: restrictionsCheck.result || _.contains(conflicts, name) || (!roleLimitsCheckResults.valid && !_.contains(this.props.selectedRoles, name)),
            ...
            return (
                <controls.Input
                    ...
                    disabled={this.props.mode == 'edit' || this.props.locked || !availableNodesIds.length ||
                        this.props.maxNumberOfNodes < (availableNodesIds.length + _.filter(this.props.selectedNodeIds).length)}
    ...
    NodeGroup = React.createClass({
        mixins: [SelectAllMixin],
        render: function() {
            var availableNodes = this.props.nodes.filter(function(node) {if (node.isSelectable()) return node;}),
                nodesWithRestrictionsIds = _.compact(_.map(availableNodes, function(node) {
                if (_.any(this.props.roleNamesWithMaxLimitReached, function(role) {
                        return !_.contains(node.get('pending_roles'), role) && _.contains(this.props.selectedRoles, role);
                    }, this)
                ) {
                    return node.id;
                }
