    ...
    NodeListScreen = React.createClass({
        ...
        processRoleLimits: function() {
            ...
            cluster.get('release').get('role_models').map(function(role) {
                var isRoleAlreadyAssigned,
                    roleName = role.get('name'),
                    roleLimits = role.get('limits');

                if (roleLimits && roleLimits.max) {
                    isRoleAlreadyAssigned = nodesForLimitCheck.any(function(node) {
                        return node.hasRole(roleName);
                    }, this);

                    var checkLimitsResults = role.checkLimits(this.getConfigModels(),
                        !isRoleAlreadyAssigned, ['max'], nodesForLimitCheck);

                    processedRoleLimits[roleName] = checkLimitsResults;
                }
            }, this);
    ...
    NodeList = React.createClass({
        ...
        render: function() {
            ...
            var groups = this.groupNodes(),
                rolesWithLimitReached = _.keys(_.omit(this.props.processedRoleLimits, function(role) {return role.valid;}));
            return (
        ...
        render: function() {
            var availableNodes = this.props.nodes.filter(function(node) {return node.isSelectable();}),
                nodesWithRestrictionsIds = _.pluck(_.filter(availableNodes, function(node) {
                    return _.any(this.props.rolesWithLimitReached, function(role) {
                        debugger;
                            return !node.hasRole(role) && _.contains(this.props.selectedRoles, role);
                        }, this);
                }, this), 'id');
            return (
