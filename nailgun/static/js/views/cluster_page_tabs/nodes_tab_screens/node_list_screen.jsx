    ...
    NodeListScreen = React.createClass({
        ...
        processRoleLimits: function() {
            ...
            cluster.get('release').get('role_models').map(function(role) {
                var checkLimitsResults = role.checkLimits({
                    ...
                    version: app.version,
                    default: settings
                }, false, ['max'], nodesForLimitCheck, _.filter(this.state.selectedNodeIds).length);

                if (_.intersection(_.keys(processedRoleLimits), this.state.selectedRoles).length) {
                    maxNumberOfNodes.push(role.get('limits') && role.get('limits').max);
                }

                processedRoleLimits[role.get('name')] = checkLimitsResults;

                if (!checkLimitsResults.valid || checkLimitsResults.limits.max < _.filter(this.state.selectedNodeIds).length) {
                    rolesWithMaxLimitReached.push(role.get('name'));
                }
            }, this);
            return {
                ...
                rolesWithMaxLimitReached: rolesWithMaxLimitReached,
                // needed for NodeList to disable select all on demand
                maxNumberOfNodes: maxNumberOfNodes.length ? _.min(maxNumberOfNodes) - _.filter(this.state.selectedNodeIds).length : this.state.selectedNodeIds.length
            };
        },
        ...
        render: function() {
            var cluster = this.props.cluster,
                ...
                processesRoleData = this.processRoleLimits();
                    ...
                    {this.state.loading ? <controls.ProgressBar /> :
                        <div>
                            {this.props.mode != 'list' &&
                                <RolePanel
                                    {...this.props}
                                    {... _.pick(processesRoleData, 'processedRoleLimits', 'rolesWithMaxLimitReached')}
                            ...
                            <NodeList {...this.props}
                                ...
                                {... _.pick(processesRoleData, 'rolesWithMaxLimitReached', 'maxNumberOfNodes')}
                                locked={locked}
    ...
    RolePanel = React.createClass({
        ...
        processRestrictions: function(role, models) {
            var name = role.get('name'),
                ...
                roleLimitsCheckResults = this.props.processedRoleLimits[role.get('name')],
            ...
            if (roleLimitsCheckResults && !roleLimitsCheckResults.valid && roleLimitsCheckResults.message ||
                _.contains(this.props.rolesWithMaxLimitReached, role.get('name'))) messages.push(roleLimitsCheckResults.message);
                ...
                <div className='role-panel'>
                    ...
                    {this.props.cluster.get('release').get('role_models').map(function(role) {
                        if (!role.checkRestrictions(this.state.configModels, 'hide').result) {
                            var name = role.get('name'),
                                processedRestrictions = this.props.nodes.length ? this.processRestrictions(role, this.state.configModels) : {};
                            return (
                                <controls.Input
                                    key={name}
                                    ref={name}
    ...
    SelectAllMixin = {
        ...
        renderSelectAllCheckbox: function() {
            var availableNodesIds = _.compact(this.props.nodes.map(function(node) {if (node.isSelectable()) return node.id;})),
                checked = this.props.mode == 'edit' || (availableNodesIds.length &&
                    !_.any(availableNodesIds, function(id) {return !this.props.selectedNodeIds[id];}, this));
            return (
                <controls.Input
                    ref='select-all'
                    type='checkbox'
                    ...
                    disabled={this.props.mode == 'edit' || this.props.locked || !availableNodesIds.length ||
                        !checked && (availableNodesIds.length < this.props.maxNumberOfNodes)}
        ...
        render: function() {
            var availableNodes = this.props.nodes.filter(function(node) {return node.isSelectable();}),
                nodesWithRestrictionsIds = _.pluck(_.filter(availableNodes, function(node) {
                    return _.any(this.props.rolesWithMaxLimitReached, function(role) {
