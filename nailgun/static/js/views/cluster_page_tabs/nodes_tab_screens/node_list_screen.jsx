...
function(React, utils, models, controls, dialogs, componentMixins) {
    ...
    NodeListScreen = React.createClass({
        ...
        getInitialState: function() {
            return {
                ...
                selectedNodeIds: this.props.mode == 'edit' && this.props.screenOptions[0] ? utils.deserializeTabOptions(this.props.screenOptions[0]).nodes.split(',').map(function(id) {return parseInt(id, 10);}) : [],
                actionInProgress: false
            };
        },
        selectNodes: function(ids, name, checked) {
            this.setState({
                selectedNodeIds: checked ? _.union(this.state.selectedNodeIds, ids) : _.difference(this.state.selectedNodeIds, ids)
            });
        },
        ...
        componentWillMount: function() {
            var clusterId = this.props.mode == 'add' ? '' : this.props.model.id;
        ...
        hasChanges: function() {
            return !_.isEqual(this.props.nodes.pluck('pending_roles'), this.initialRoles);
        },
        changeFilter: _.debounce(function(name, value) {
            this.setState({filter: value});
        }, 200),
        clearFilter: function() {
            this.refs.filter.refs.input.getDOMNode().value = '';
            this.setState({filter: ''});
        },
        changeGrouping: function(name, value) {
            this.setState({grouping: value});
            this.props.model.save({grouping: value}, {patch: true, wait: true});
        },
        ...
        showDeleteNodesDialog: function(nodes) {
            utils.showDialog(dialogs.DeleteNodesDialog, {
        ...
        applyChanges: function(nodes) {
            ...
            nodes.sync('update', nodes)
                ...
                .fail(_.bind(function() {
                    ...
                    this.showErrorDialog('saving_warning');
                }, this));
        },
        showErrorDialog: function(warning) {
            var ns = 'cluster_page.nodes_tab.node_management_panel.node_management_error.';
            utils.showErrorDialog({title: $.t(ns + 'title'), message: $.t(ns + warning)});
        },
        goToConfigurationScreen: function(action, conflict) {
            if (conflict) {
                this.showErrorDialog(action + '_configuration_warning');
                return;
            }
        ...
        renderManagementButtons: function() {
            var checkedNodes = this.state.selectedNodeIds.map(this.props.nodes.get, this.props.nodes);
            if (this.props.mode != 'list') {
                return [
                    <button key='cancel' className='btn' disabled={this.state.actionInProgress} onClick={_.bind(this.changeScreen, this, '', false)}>
                        {$.t('common.cancel_button')}
                    </button>,
                    <button key='apply' className='btn btn-success btn-apply' disabled={this.state.actionInProgress || !this.isMounted() || !this.hasChanges()} onClick={_.bind(this.applyChanges, this, checkedNodes)}>
                        {$.t('common.apply_changes_button')}
                    </button>
                ];
            }
            if (this.props.model.tasks({group: 'deployment', status: 'running'}).length) return null;
            var sampleNode = checkedNodes[0],
                ...
                buttons = [
                    <button key='disks' className={cx(classes)} disabled={!checkedNodes.length} onClick={_.bind(this.goToConfigurationScreen, this, 'disks', disksConflict)}>
            ...
            if (!_.any(checkedNodes, function(node) {return node.get('status') == 'error';})) {
                ...
                buttons.push(
                    <button key='interfaces' className={cx(classes)} disabled={!checkedNodes.length} onClick={_.bind(this.goToConfigurationScreen, this, 'interfaces', interfaceConflict)}>
                    ...
                    {this.state.loading ? <controls.ProgressBar /> :
                        <div>
                            ...
                            <NodeList {...this.props}
                                ...
                                roleLimitation={(this.props.model.get('mode') == 'multi-node' && _.contains(assignedRoles, 'controller')) || _.contains(assignedRoles, 'zabbix-server')}
                            />
                        </div>
                    }
                </div>
    ...
    RolePanel = React.createClass({
        ...
        isRoleAvailable: function(role) {
            ...
            if ((role == 'controller' && this.props.model.get('mode') == 'multinode') || role == 'zabbix-server') {
                return this.props.selectedNodeIds.length <= 1 && !this.props.model.get('nodes').any(function(node) {
        ...
        render: function() {
            ...
            return (
                <div className='role-panel'>
                    ...
                    {this.props.model.get('release').get('role_models').map(function(role) {
    ...
    NodeList = React.createClass({
        ...
        groupNodes: function() {
            var nodes = this.props.nodes.filter(function(node) {
                    var matchesFilter = _.contains(node.get('name').concat(' ', node.get('mac')).toLowerCase(), this.props.filter);
                    return matchesFilter && (this.props.mode == 'edit' && _.contains(this.props.selectedNodeIds, node.id) || this.props.mode != 'edit');
                }, this),
    ...
    Node = React.createClass({
        ...
        showNodeDetails: function(e) {
            ...
            utils.showDialog(dialogs.ShowNodeInfoDialog, {node: node, title: node.get('name') || node.get('mac')});
        },
