...
function(React, Expression, utils, models, dialogs, controls) {
    ...
    panels.NodeManagementPanel = React.createClass({
        ...
        groupNodes: function(name, grouping) {
            ...
            app.page.tab.screen.nodeList.groupNodes(grouping);
        },
        ...
        goToNodeList: function() {
            app.page.tab.screen.revertChanges();
        ...
        renderButtons: function() {
            if (!this.props.clusterNodesScreen && app.page) return [
                ...
                <button key='apply' className='btn btn-success' disabled={this.state.actionInProgress || !app.page.tab.screen.hasChanges()} onClick={this.applyChanges}>
            ...
            if (!this.props.cluster.tasks({group: 'deployment', status: 'running'}).length) {
                ...
                if (checkedNodes.length) {
                    ...
                    buttons.push(
                        <button key='disks' className={cx({btn: true, conflict: disksConflict})} onClick={_.bind(this.goToConfigurationScreen, this, 'disks', disksConflict)}>
                    ...
                    if (!_.any(checkedNodes, function(node) {return node.get('status') == 'error';})) {
                        ...
                        buttons.push(
                            <button key='interfaces' className={cx({btn: true, conflict: interfaceConflict})} onClick={_.bind(this.goToConfigurationScreen, this, 'interfaces', interfaceConflict)}>
                    ...
                    if (_.any(checkedNodes, function(node) {return !node.get('pending_deletion');})) buttons.push(
                        <button key='delete' className='btn btn-danger delete-nodes' onClick={this.showDeleteNodesDialog}>
                    ...
                    if (!_.any(checkedNodes, function(node) {return !node.get('pending_addition');})) buttons.push(
                        <button key='roles' className='btn btn-success edit-roles' onClick={this.goToEditRolesScreen}>
                ...
                return (
                    <button key='add' className='btn btn-success add-nodes' onClick={this.goToAddNodesScreen}>
