...
function(React, utils, models, controls, dialogs, componentMixins) {
    ...
    var cx = React.addons.classSet,
        Screen, ManagementPanel, RolePanel, SelectAllMixin, NodeList, NodeGroup, Node;
    ...
    Screen = React.createClass({
        ...
        getInitialState: function() {
            var ids = this.props.nodeIds || this.props.nodes.pluck('id');
            return {
                loading: this.props.mode == 'add',
                filter: '',
                grouping: this.props.mode == 'add' ? 'hardware' : this.props.cluster.get('grouping'),
                selectedNodeIds: _.zipObject(ids, _.times(ids.length, _.constant(this.props.mode == 'edit')))
            };
        },
    ...
    ManagementPanel = React.createClass({
        ...
        render: function() {
            if (this.props.mode != 'list') return (
            ...
            var sampleNode = this.props.nodes.at(0),
            ...
            return (
                <div className='buttons'>
                    ...
                    {!!this.props.nodes.length ?
                        [
                            this.props.nodes.any(function(node) {return !node.get('pending_deletion');}) && (
                                <button
                                    key='delete'
                            ...
                            !this.props.nodes.any(function(node) {return !node.get('pending_addition');}) && (
                                <button
                                    key='roles'
                                ...
                                >
                                    ...
                                    <span>{$.t('cluster_page.nodes_tab.node_management_panel.edit_roles_button')}</span>
                                </button>
                            )
                        ]
                    :
                        ...
                        >
                            ...
                            <span>{$.t('cluster_page.nodes_tab.node_management_panel.add_nodes_button')}</span>
                        </button>
                    }
                </div>
            );
    ...
    NodeList = React.createClass({
        ...
        groupNodes: function() {
            var nodes = this.props.nodes.filter(function(node) {
                    ...
                    return matchesFilter && (this.props.mode == 'edit' && this.props.selectedNodeIds[node.id] || this.props.mode != 'edit');
                }, this),
                    ...
                    <label className={cx(nodeBoxClasses)}>
                        ...
                        <div className='node-content'>
                            ...
                            <div className='node-button'>
                                {!!node.get('cluster') &&
                                    <button className='btn btn-link' title={$.t(ns + buttonOptions.title)} onClick={!this.state.actionInProgress && buttonOptions.onClick}>
                                        <i className={buttonOptions.className} />
                                    </button>
                                }
                            </div>
