...
function(utils, models, commonViews, dialogViews, settingsTabTemplate, settingsGroupTemplate) {
    ...
    SettingsTab = commonViews.Tab.extend({
        ...
        handleCondition: function(groupName, settingName, conditionType, condition, isDisabled) {
            var path = _.keys(condition)[0];
            var check = utils.parseModelPath(path, this.configModels).get() != condition[path];
            isDisabled = isDisabled || conditionType == 'depends' ? check : !check;
            if (this.composeListeners) {
                utils.parseModelPath(path, this.configModels).change(_.bind(this.calculateSettingDisabledState, this, groupName, settingName));
            }
            return isDisabled;
        },
        calculateSettingDisabledState: function(groupName, settingName) {
            ...
            var handleCondition = _.bind(this.handleCondition, this, groupName, settingName);
            _.each(this.settings.get(settingPath + '.depends'), function(dependency) {
                isSettingDisabled = handleCondition('depends', dependency, isSettingDisabled);
            }, this);
            _.each(this.settings.get(settingPath + '.conflicts'), function(conflict) {
                isSettingDisabled = handleCondition('conflicts', conflict, isSettingDisabled);
            }, this);
            ...
            _.each(this.settings.get(settingPath + '.values'), function(value, index) {
                ...
                _.each(value.depends, function(dependency) {
                    isOptionDisabled = handleCondition('depends', dependency, isOptionDisabled);
                }, this);
                _.each(value.conflicts, function(conflict) {
                    isOptionDisabled = handleCondition('conflicts', conflict, isOptionDisabled);
                }, this);
