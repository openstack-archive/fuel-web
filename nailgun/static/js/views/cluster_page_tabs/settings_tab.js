...
function(utils, models, commonViews, dialogViews, settingsTabTemplate, settingsGroupTemplate) {
    ...
    SettingsTab = commonViews.Tab.extend({
                ...
                _.each(group, function(setting, settingName) {
                    ...
                    bindings['input[name="' + settingPath + '"]'] = {
                        ...
                        visibleFn: function($el, isVisible) {
                            $el.parents('.parameter-box:first').toggle(isVisible);
                        },
                        attributes: [{
                            name: 'disabled',
        ...
        calculateSettingState: function(settingPath) {
        ...
        composeListeners: function(settingPath) {
            var callback = _.bind(this.calculateSettingState, this, settingPath);
            ...
            this.settings.on('change:' + settingPath + '.visible', function(settings, value) {
                settings.trigger('change:' + settingPath + '.value', settings);
            });
            ...
            (this.loading = $.when(this.settings.fetch({cache: true}), this.model.get('networkConfiguration').fetch({cache: true}))).done(_.bind(function() {
                ...
                _.each(this.settings.attributes, function(group, groupName) {
                    ...
                    _.each(group, function(setting, settingName) {
                        ...
                        this.calculateSettingState(settingPath);
                    }, this);
                }, this);
