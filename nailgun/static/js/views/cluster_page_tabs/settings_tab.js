...
function(utils, models, viewMixins, commonViews, dialogViews, settingsTabTemplate, settingsGroupTemplate) {
    ...
    SettingsTab = commonViews.Tab.extend({
        ...
        checkDependentSettings: function(groupName, settingName) {
            var settingPath = groupName + '.' + settingName;
            var notToggleableGroup = settingName == 'metadata' && !this.settings.get(groupName).toggleable;
            ...
            var isDependent = function(restriction) {
                return restriction.action == 'disable' && _.contains(restriction.condition, 'settings:' + settingPath + '.' + valueAttribute);
            };
            var checkedSettings = [];
            _.each(this.settings.attributes, function(group, groupName) {
                if (!group.metadata.visible) { return; }
                _.each(group, function(setting, settingName) {
                    var isActive = setting[this.getValueAttribute(settingName)] === true; // right now only checkboxes are supported
                    if (isActive && _.any(setting.restrictions, isDependent)) {
                        checkedSettings.push(setting);
                    }
                }, this);
            }, this);
            if (checkedSettings.length) {
                var restrictions = _.filter(_.flatten(_.pluck(checkedSettings, 'restrictions')), isDependent);
                var processedValues = this.settings.get(settingPath).values || [true, false];
                var configModels = this.configModels;
                configModels.settings = new models.Settings(this.settings.toJSON().editable);
                return _.every(restrictions, function(restriction) {
                    ...
                    return suitableValues.length <= 1;
                });
            }
            return false;
        },
