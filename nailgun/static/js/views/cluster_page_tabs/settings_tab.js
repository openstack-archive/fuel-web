...
function(utils, models, commonViews, dialogViews, settingsTabTemplate, settingsGroupTemplate) {
    ...
    SettingsTab = commonViews.Tab.extend({
            ...
            _.each(this.settings.attributes, function(group, groupName) {
                _.each(group, function(setting, settingName) {
                    ...
                    if (isDependent) {
                        ...
                        if (composeListeners) {
                            utils.parseModelPath(groupName + '.' + settingName + '.value', this.configModels).change(callback);
                        }
                    }
                }, this);
            }, this);
        ...
        calculateSettingDisabledState: function(groupName, settingName, composeListeners) {
            ...
            _.each(utils.parseModelPath(settingPath + '.depends', this.configModels).get(), function(dependency) {
            ...
            _.each(utils.parseModelPath(settingPath + '.conflicts', this.configModels).get(), function(conflict) {
            ...
            if (utils.parseModelPath(groupName + '.metadata.toggleable', this.configModels).get()) {
                disable = disable || !utils.parseModelPath(groupName + '.metadata.enabled', this.configModels).get();
                if (composeListeners) {
                    utils.parseModelPath(groupName + '.metadata.enabled', this.configModels).change(callback);
                }
            }
            utils.parseModelPath(settingPath + '.disabled', this.configModels).set(disable);
        },
        render: function() {
