...
function(utils, models, commonViews, dialogViews, settingsTabTemplate, settingsGroupTemplate) {
    ...
    SettingsTab = commonViews.Tab.extend({
        ...
        defineGroupVisibility: function(groupName) {
            var visible = true;
            _.each(this.settings.get(groupName + '.metadata.restrictions'), function(restriction) {
                visible = !utils.evaluateExpression(restriction, this.configModels).value;
                return visible;
            }, this);
            this.settings.set(groupName + '.metadata.visible', visible);
        },
        initialize: function(options) {
            ...
            (this.loading = $.when(this.settings.fetch({cache: true}), this.model.get('networkConfiguration').fetch({cache: true}))).done(_.bind(function() {
                ...
                _.each(this.settings.attributes, function(group, groupName) {
                    _.each(group, function(setting, settingName) {
                        this.defineGroupVisibility(groupName);
