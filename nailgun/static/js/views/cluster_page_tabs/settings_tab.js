    ...
    SettingsTab = commonViews.Tab.extend({
                ...
                _.each(settingsGroup, function(setting, settingTitle) {
                    ...
                    if (settingsGroup.metadata.toggleable) {
                        this.bindings['input[name=' + settingTitle + ']'] = {
                            observe: [attr + '.metadata.enabled', attr + '.' + settingTitle + '.value'],
        ...
        render: function() {
            ...
            if (this.model.get('settings').deferred.state() != 'pending') {
                this.$('.settings').html('');
                var sortedSettings = _.sortBy(_.keys(this.settings.attributes),
                                              _.bind(function(setting) {
                                                    return this.settings.attributes[setting].metadata.weight
                                              }, this));
