...
function($, _, i18n, React, utils, models, Expression, componentMixins, controls, customControls) {
    ...
    var SettingsTab = React.createClass({
        ...
        applyChanges: function() {
            ...
            var settings = this.props.cluster.get('settings'),
                data = this.props.cluster.isAvailableForSettingsChanges() ? null : {
                    editable: _.pick(settings.attributes, function(group) {
                        return (group.metadata || {}).always_editable;
                    })
                };

            var deferred = settings.save(data, {patch: true, wait: true, validate: false});
            if (deferred) {
        ...
        render: function() {
            ...
            return (
                <div key={this.state.key} className='openstack-settings wrapper'>
