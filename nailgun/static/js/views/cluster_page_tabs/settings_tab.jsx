...
function(React, utils, models, controls) {
    ...
    var dependenciesMixin = {
        ...
        checkRestrictions: function(path, action) {
            return this.settings.checkRestrictions(this.configModels, path, action);
        },
        checkDependentRoles: function(path) {
            ...
            return _.filter(allocatedRoles, function(roleName) {
                return _.any(roles.findWhere({name: roleName}).expandedRestrictions, function(restriction) {
                    return _.contains(restriction.condition, 'settings:' + path) && utils.compileExpression(restriction.condition).evaluate(this.configModels);
                });
            });
        },
                ...
                return _.compact(_.map(dependentSettings, function(conditions, label) {
                    return _.any(conditions, function(condition) {
                        return _.all(values, function(value) {
                            ...
                            return utils.compileExpression(condition).evaluate(settingsForTests);
                        });
                    }) ? label : null;
                }));
            }
    ...
    var SettingsTab = React.createClass({
        ...
        onChange: function(groupName, settingName, value) {
            ...
            if (!_.contains(['text', 'password'], this.settings.get(path).type)) {
                this.settings.set(path, value, {validate: true, models: this.configModels});
            } else {
                            ...
                            {_.map(sortedSettingGroups, function(groupName) {
                                ...
                                if (!this.checkRestrictions(path, 'hide').result) {
                                    ...
                                    if (!disabled && this.settings.get(path).toggleable) {
                                        var restrictionsCheck = this.checkRestrictions(path);
                                ...
                                if (setting.values) {
                                    var values = _.compact(_.map(_.cloneDeep(setting.values), function(value) {
                                        if (!this.checkRestrictions(path + '.' + value.data, 'hide').result) {
                                            var restrictionsCheck = this.checkRestrictions(path + '.' + value.data);
