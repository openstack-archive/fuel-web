...
function($, _, i18n, React, utils, models, Expression, componentMixins, controls) {
    ...
    var SettingsTab = React.createClass({
        ...
        componentDidUpdate: function() {
            if (this.state.forceValues) this.setState({forceValues: false});
        },
        ...
        revertChanges: function() {
            this.setState({forceValues: true});
            this.loadInitialSettings();
        },
        ...
        render: function() {
            var group = this.settings.get(this.props.groupName),
                ...
                sortedSettings = _.chain(_.keys(group))
                    ...
                    .value(),
                valueProp = this.props.forceValues ? {checked: metadata.enabled} : {};
            return (
                        ...
                        {_.map(sortedSettings, function(settingName) {
                            ...
                            if (!this.checkRestrictions('hide', path).result) {
                                ...
                                var checked = _.isBoolean(setting.value) ? setting.value : false,
                                    valueProps = this.props.forceValues ? {
                                        value: setting.value,
                                        checked: checked
                                    } : {
