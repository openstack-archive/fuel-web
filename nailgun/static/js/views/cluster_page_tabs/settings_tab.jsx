...
function($, _, i18n, React, utils, models, Expression, componentMixins, controls) {
    ...
    var SettingsTab = React.createClass({
        ...
        getInitialState: function() {
            ...
            return {
                ...
                settingsForTests: {
                    settings: new models.Settings(_.cloneDeep(settings.attributes))
                },
                loading: true,
        ...
        onChange: function(groupName, settingName, value) {
            ...
            this.state.settingsForTests.settings.set(name, value);
                        ...
                        <div>
                            {_.map(sortedSettingGroups, function(groupName) {
                                return <SettingGroup
                                    ...
                                    settings={settings}
                                    settingsForTests={this.state.settingsForTests}
    ...
    var SettingGroup = React.createClass({
        ...
        checkValues: function(values, path, condition) {
            return _.all(values, function(value) {
                this.props.settingsForTests.settings.set(path, value);
                return new Expression(condition, this.props.settingsForTests).evaluate();
            }, this);
        },
                ...
                if (_.any(role.expandedRestrictions.restrictions, function(restriction) {
                    ...
                    if (_.contains(condition, 'settings:' + path) && !(new Expression(condition, this.props.configModels).evaluate())) {
                        var valueAttribute = this.props.getValueAttribute(settingName),
                            ...
                            result = this.checkValues(this.getValuesToCheck(setting, valueAttribute), pathToCheck, condition);
                        this.props.settingsForTests.settings.set(pathToCheck, setting[valueAttribute]);
                        return result;
                    }
                    return false;
        ...
        checkDependentSettings: function(groupName, settingName) {
            ...
            if (!_.isEmpty(dependentSettings)) {
                var valueAttribute = this.props.getValueAttribute(currentSetting),
                    pathToCheck = this.props.makePath(path, valueAttribute),
                    values = this.getValuesToCheck(currentSetting, valueAttribute),
                    result = _.compact(_.map(dependentSettings, function(conditions, label) {
                        return _.any(conditions, _.bind(this.checkValues, this, values, pathToCheck)) ? label : null;
                    }, this));
                this.props.settingsForTests.settings.set(pathToCheck, currentSetting[valueAttribute]);
                return result;
            }
            return [];
        },
