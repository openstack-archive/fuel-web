...
function($, _, i18n, React, utils, models, Expression, componentMixins, controls) {
    ...
    var SettingsTab = React.createClass({
        ...
        hasChanges: function() {
            return this.state.loading ? false : this.props.cluster.get('settings').hasChanges(this.state.initialAttributes, this.configModels);
        },
        applyChanges: function() {
            var deferred = this.props.cluster.get('settings').save(null, {patch: true, wait: true, models: this.configModels});
            if (deferred) {
        ...
        onChange: function(groupName, settingName, value) {
            ...
            settings.isValid({models: this.configModels});
        },
        render: function() {
            var cluster = this.props.cluster,
    ...
    var SettingGroup = React.createClass({
        mixins: [configModelsMixin],
        checkRestrictions: function(action, path) {
            return this.props.settings.checkRestrictions(this.configModels, action, path);
        },
        ...
        checkDependentSettings: function(path) {
            ...
            if (!this.areCalulationsPossible(currentSetting)) return [];
            ...
            _.each(this.props.settings.attributes, function(group, groupName) {
                ...
                _.each(group, function(setting, settingName) {
                    ...
                    if (!this.areCalulationsPossible(setting) || pathToCheck == path || this.checkRestrictions('hide', pathToCheck).result) return;
