...
function(React, utils, models, controls) {
    ...
    var SettingMixin = {
        checkRestrictions: function(setting, action) {
            return this.props.model.get('settings').checkRestrictions(this.props.configModels || this.configModels, setting, action);
        },
        ...
        checkDependentSettings: function(settingPath, valueAttr) {
            ...
            _.each(settings.attributes, function(group, groupName) {
                if (this.checkRestrictions(group.metadata, 'hide').result) { return; }
                _.each(group, function(setting, settingName) {
                    if (_.contains(['text', 'password', 'hidden'], setting.type) || groupName + '.' + settingName == settingPath || this.checkRestrictions(setting, 'hide').result) { return; }
                    ...
                    if (setting[value] == true) { // for checkboxes and toggleable setting groups
                        ...
                        if (dependentRestriction) {
                            dependentSettings[setting.label] = dependentSettings[setting.label] || [];
                            dependentSettings[setting.label].push(utils.expandRestriction(dependentRestriction).condition);
                        }
                    } else {
                        var activeOption = _.find(setting.values, {data: setting.value}); // for dropdowns and radio groups
                        if (activeOption) {
                            var dependentRestriction = _.find(activeOption.restrictions, _.bind(this.isDependent, this, settingPath));
                            if (dependentRestriction) {
                                dependentSettings[setting.label] = dependentSettings[setting.label] || [];
                                dependentSettings[setting.label].push(utils.expandRestriction(dependentRestriction).condition);
                            }
                ...
                return _.compact(_.map(dependentSettings, function(conditions, label) {
                    return _.any(conditions, function(condition) {
                        return _.all(values, function(value) {
                            ...
                            return utils.evaluateExpression(condition, configModels).value;
                        });
                    }) ? label : null;
                }));
            }
    ...
    var SettingsTab = React.createClass({
        ...
        loadDefaults: function() {
            var settings = this.props.model.get('settings');
            // FIXME: this .fetch() is not trigger 'change' event
        ...
        loadInitialSettings: function() {
            // FIXME: this .set() is not trigger 'change' event
                            ...
                            {_.map(sortedSettingGroups, function(groupName) {
                                ...
                                if (!this.checkRestrictions(metadata, 'hide').result) {
                                    ...
                                    if (!disabled && metadata.toggleable) {
                                        var restrictionsCheck = this.checkRestrictions(metadata, 'disable');
    ...
    var SettingGroup = React.createClass({
        ...
        handleChange: function(valueAttr, groupName, settingName, value) {
            ...
            // FIXME: validationError property is not updated automatically
            settings.validationError = settings.validate(settings.attributes, {configModels: this.props.configModels});
        },
                ...
                <div className='fieldset-group wrapper'>
                    ...
                    <div className='settings-group table-wrapper'>
                        {_.map(sortedSettings, function(settingName) {
                            ...
                            if (setting.type != 'hidden' && !this.checkRestrictions(setting, 'hide').result) {
                                var path = this.props.groupName + '.' + settingName;
                                var disabled = this.props.disabled,
                                    warnings = [];
                                if (!disabled) {
                                    var restrictionsCheck = this.checkRestrictions(setting, 'disable');
                                ...
                                if (setting.values) {
                                    _.each(setting.values, function(value) {
                                        if (this.checkRestrictions(value, 'hide').result) {
                                            hiddenValues.push(value.data);
                                        } else {
                                            var restrictionsCheck = this.checkRestrictions(value, 'disable')
