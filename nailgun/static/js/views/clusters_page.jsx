...
function($, _, i18n, React, models, utils, componentMixins, wizard) {
    ...
    ClustersPage = React.createClass({
        ...
        title: function() {
            return i18n.t('clusters_page.title');
        },
        statics: {
            fetchData: function() {
        ...
        render: function() {
            return (
                <div>
                    <h3 className="page-title">{i18n.t('clusters_page.title')}</h3>
                ...
                <div className="cluster-list">
                    <div className="roles-block-row">
                        ...
                        <div key="add" className="span3 clusterbox create-cluster" onClick={this.createCluster}>
                            ...
                            <div className="create-cluster-text">{i18n.t('clusters_page.create_cluster_text')}</div>
                        </div>
                    </div>
                </div>
            );
            ...
            return (
                <a className={'span3 clusterbox ' + (deletionTask ? 'disabled-cluster' : '')} href={!deletionTask ? '#cluster/' + cluster.id + '/nodes' : 'javascript:void 0'}>
                    ...
                    <div className="cluster-hardware">
                        {(!nodes.deferred || nodes.deferred.state() == 'resolved') &&
                            <div className="row-fluid">
                                <div key="nodes-title" className="span6">{i18n.t('clusters_page.cluster_hardware_nodes')}</div>
                                <div key="nodes-value" className="span4">{nodes.length}</div>
                                {!!nodes.length && [
                                    <div key="cpu-title" className="span6">{i18n.t('clusters_page.cluster_hardware_cpu')}</div>,
                                    <div key="cpu-value" className="span4">{nodes.resources('cores')} ({nodes.resources('ht_cores')})</div>,
                                    <div key="hdd-title" className="span6">{i18n.t('clusters_page.cluster_hardware_hdd')}</div>,
                                    <div key="hdd-value" className="span4">{nodes.resources('hdd') ? utils.showDiskSize(nodes.resources('hdd')) : '?GB'}</div>,
                                    <div key="ram-title" className="span6">{i18n.t('clusters_page.cluster_hardware_ram')}</div>,
                    ...
                    <div className="cluster-status">
                        ...
                        :
                            i18n.t('cluster.status.' + cluster.get('status'), {defaultValue: cluster.get('status')})
                        }
                    </div>
                </a>
            );
