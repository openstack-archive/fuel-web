...
define([
    ...
    var repoRegexp = /^(deb|deb-src)\s+((?:[A-Za-z0-9]+:\/\/[\w-.\/]+)(?::[0-9]+)?(?:[\w-.\/]+))\s+([\w-.\/]+)\s+([\w-.\/\s]+)$/i,
        repoAttributes = ['type', 'uri', 'suite', 'section'],
        repoToString = function(repo) {
            var repoData = repoAttributes.map(function(attribute) {return repo[attribute];});
            if (!_.any(repoData)) return ''; // in case of new repo
