...
function(require, React, utils, models, viewMixins, componentMixins, baseDialogTemplate, resetEnvironmentDialogTemplate, updateEnvironmentDialogTemplate, showNodeInfoTemplate, discardSettingsChangesTemplate, deleteNodesTemplate) {
    ...
    views.ChangePasswordDialog = React.createClass({
        ...
        renderFooter: function() {
            return [
                <button key="cancel" className="btn" onClick={this.close} disabled={this.state.actionInProgress}>{$.t('common.cancel_button')}</button>,
                <button key="apply" className="btn btn-success" onClick={this.changePassword} disabled={this.state.actionInProgress || !this.isPasswordChangeAvailable()}>{$.t('common.apply_button')}</button>
            ];
        },
