...
function(require, React, utils, models, componentMixins, controls) {
    ...
    views.DeployChangesDialog = React.createClass({
        ...
        deployCluster: function() {
            ...
            task.save({}, {url: _.result(this.props.cluster, 'url') + '/changes', type: 'PUT'})
                ...
                .always(_.bind(function() {this.close();}, this));
        },
        renderChangedNodeAmount: function(nodes, dictKey) {
            return nodes.length ? <div key={dictKey} className='deploy-task-name'>
                {$.t('dialog.display_changes.' + dictKey, {count: nodes.length})}
    ...
    views.UpdateEnvironmentDialog = React.createClass({
        ...
        renderFooter: function() {
            ...
            return ([
                <button key='cancel' className='btn' onClick={this.close}>{$.t('common.cancel_button')}</button>,
                <button key='reset' className={React.addons.classSet({'btn update-environment-btn': true, 'btn-success': action == 'update', 'btn-danger': action != 'update'})} onClick={this.updateEnvironment} disabled={this.state.actionInProgress}>{$.t('common.' + action + '_button')}</button>
            ]);
        }
    });
    ...
    views.DiscardSettingsChangesDialog = React.createClass({
        mixins: [componentMixins.dialogMixin],
        getDefaultProps: function() {return {title: $.t('dialog.dismiss_settings.title')};},
        proceed: function() {
            this.close();
            app.page.removeFinishedNetworkTasks().always(_.bind(this.props.cb, this.props));
        },
        renderBody: function() {
            var message = this.props.verification ? $.t('dialog.dismiss_settings.verify_message') : $.t('dialog.dismiss_settings.default_message');
            return (
    ...
    views.DeleteNodesDialog = React.createClass({
        ...
        renderBody: function() {
            return (<div className='deploy-task-notice'>{this.renderImportant()} {$.t('dialog.delete_nodes.message')}</div>);
        },
        renderFooter: function() {
            return [
                <button key='cancel' className='btn' onClick={this.close}>{$.t('common.cancel_button')}</button>,
