...
function($, _, i18n, Backbone, React, utils, models, controls) {
    ...
    var dialogMixin = {
        ...
        renderImportantLabel: function() {
            return <span className='label label-important'>{i18n.t('common.important')}</span>;
        },
        render: function() {
            ...
            return (
                <div className={cx(classes)} tabIndex="-1">
                    <div className='modal-header'>
                        ...
                        <h3>{this.props.title || (this.props.error ? i18n.t('dialog.error_dialog.title') : '')}</h3>
                    </div>
                    <div className='modal-body'>
                        {this.props.error ?
                            <div className='text-error'>
                                {this.props.message || i18n.t('dialog.error_dialog.warning')}
                            </div>
                        : this.renderBody()}
                    </div>
                    <div className='modal-footer'>
                        {this.renderFooter && !this.props.error ? this.renderFooter() : <button className='btn' onClick={this.close}>{i18n.t('common.close_button')}</button>}
                    </div>
                </div>
            );
        }
    ...
    dialogs.DiscardNodeChangesDialog = React.createClass({
        mixins: [dialogMixin],
        getDefaultProps: function() {return {title: i18n.t('dialog.discard_changes.title')};},
        ...
        renderChangedNodeAmount: function(nodes, dictKey) {
            return nodes.length ? <div key={dictKey} className='deploy-task-name'>
                {i18n.t('dialog.display_changes.' + dictKey, {count: nodes.length})}
            </div> : null;
        },
        renderBody: function() {
            ...
            return (
                <div>
                    <div className='msg-error'>
                        ...
                        {i18n.t('dialog.discard_changes.alert_text')}
                    </div>
                    <br/>
        ...
        renderFooter: function() {
            return ([
                <button key='cancel' className='btn' disabled={this.state.actionInProgress} onClick={this.close}>{i18n.t('common.cancel_button')}</button>,
                <button key='discard' className='btn btn-danger' disabled={this.state.actionInProgress} onClick={this.discardNodeChanges}>{i18n.t('dialog.discard_changes.discard_button')}</button>
            ]);
        }
    });
    ...
    dialogs.DeployChangesDialog = React.createClass({
        mixins: [dialogMixin],
        getDefaultProps: function() {return {title: i18n.t('dialog.display_changes.title')};},
        getInitialState: function() {
        ...
        renderChange: function(change, nodeIds) {
            ...
            return (
                <div key={change}>
                    <div className='deploy-task-name'>{i18n.t('dialog.display_changes.settings_changes.' + change)}</div>
                    <ul>
                ...
                <div className='display-changes-dialog'>
                    {(isNewOrNeedsRedeployment || !this.state.areSettingsValid) &&
                        <div>
                            <div className={warningMessageClasses}>
                                ...
                                <span>{i18n.t(ns + (!this.state.areSettingsValid ? 'warnings.settings_invalid' :
                    ...
                    <div className='amount-restrictions'>
                        {this.state.amountRestrictions.controller &&
                            <div className='alert alert-error'>{i18n.t(ns + 'warnings.controller', {count: requiredNodeAmount})}</div>
                        }
                        {this.state.amountRestrictions.compute &&
                            <div className='alert alert-error'>{i18n.t(ns + 'warnings.compute')}</div>
                        }
                        {this.state.amountRestrictions.mongo &&
                            <div className='alert alert-error'>{i18n.t(ns + 'warnings.mongo', {count: requiredNodeAmount})}</div>
                        }
                    </div>
                </div>
            );
        ...
        renderFooter: function() {
            return ([
                ...
                >{i18n.t('dialog.display_changes.deploy')}</button>
            ]);
        }
    });
    ...
    dialogs.StopDeploymentDialog = React.createClass({
        mixins: [dialogMixin],
        getDefaultProps: function() {return {title: i18n.t('dialog.stop_deployment.title')};},
        stopDeployment: function() {
            ...
            task.save({}, {url: _.result(this.props.cluster, 'url') + '/stop_deployment', type: 'PUT'})
                ...
                .fail(_.bind(function(response) {
                    this.showError(utils.getResponseText(response) || i18n.t('dialog.stop_deployment.stop_deployment_error.stop_deployment_warning'));
                }, this));
        },
        renderBody: function() {
            return (
                <div className='msg-error'>
                    {this.renderImportantLabel()}
                    {i18n.t('dialog.stop_deployment.' + (this.props.cluster.get('nodes').where({status: 'provisioning'}).length ? 'provisioning_warning' : 'text'))}
                </div>
            );
        },
        renderFooter: function() {
            return ([
                <button key='cancel' className='btn' disabled={this.state.actionInProgress} onClick={this.close}>{i18n.t('common.cancel_button')}</button>,
                <button key='deploy' className='btn stop-deployment-btn btn-danger' disabled={this.state.actionInProgress} onClick={this.stopDeployment}>{i18n.t('common.stop_button')}</button>
            ]);
        }
    });
    ...
    dialogs.RemoveClusterDialog = React.createClass({
        mixins: [dialogMixin],
        getDefaultProps: function() {return {title: i18n.t('dialog.remove_cluster.title')};},
        ...
        renderBody: function() {
            return (
                <div className='msg-error'>
                    {this.renderImportantLabel()}
                    {i18n.t('dialog.remove_cluster.' + (this.props.cluster.tasks({status: 'running'}).length ? 'incomplete_actions_text' : 'node_returned_text'))}
                </div>
            );
        },
        renderFooter: function() {
            return ([
                <button key='cancel' className='btn' disabled={this.state.actionInProgress} onClick={this.close}>{i18n.t('common.cancel_button')}</button>,
                <button key='deploy' className='btn remove-cluster-btn btn-danger' disabled={this.state.actionInProgress} onClick={this.removeCluster}>{i18n.t('common.delete_button')}</button>
            ]);
        }
    });
    ...
    dialogs.ResetEnvironmentDialog = React.createClass({
        mixins: [dialogMixin],
        getDefaultProps: function() {return {title: i18n.t('dialog.reset_environment.title')};},
        ...
        renderBody: function() {
            return (
                <div className='msg-error'>
                    {this.renderImportantLabel()}
                    {i18n.t('dialog.reset_environment.text')}
                </div>
            );
        },
        renderFooter: function() {
            return ([
                <button key='cancel' className='btn' onClick={this.close}>{i18n.t('common.cancel_button')}</button>,
                <button key='reset' className='btn btn-danger reset-environment-btn' onClick={this.resetEnvironment} disabled={this.state.actionInProgress}>{i18n.t('common.reset_button')}</button>
            ]);
        }
    });
    ...
    dialogs.UpdateEnvironmentDialog = React.createClass({
        mixins: [dialogMixin],
        getDefaultProps: function() {return {title: i18n.t('dialog.update_environment.title')};},
            ...
            return (
                <div>
                    {action == 'update' && this.props.isDowngrade ?
                        <div className='msg-error'>
                            {this.renderImportantLabel()}
                            {i18n.t('dialog.' + action + '_environment.downgrade_warning')}
                        </div>
                    :
                        <div>{i18n.t('dialog.' + action + '_environment.text')}</div>
                    }
                </div>
            );
        },
        renderFooter: function() {
            ...
            return ([
                <button key='cancel' className='btn' onClick={this.close}>{i18n.t('common.cancel_button')}</button>,
                <button key='reset' className={classes} onClick={this.updateEnvironment} disabled={this.state.actionInProgress}>{i18n.t('common.' + action + '_button')}</button>
            ]);
        }
    });
    ...
    dialogs.ShowNodeInfoDialog = React.createClass({
                ...
                switch (group) {
                    ...
                    case 'memory':
                        if (_.isArray(meta.memory.devices) && meta.memory.devices.length) {
                            ...
                            summary += ', ' + utils.showMemorySize(meta.memory.total) + ' ' + i18n.t('dialog.show_node.total');
                        } else summary = utils.showMemorySize(meta.memory.total) + ' ' + i18n.t('dialog.show_node.total');
                        break;
                    case 'disks':
                        summary = meta.disks.length + ' ';
                        summary += i18n.t('dialog.show_node.drive', {count: meta.disks.length});
                        summary += ', ' + utils.showDiskSize(_.reduce(_.pluck(meta.disks, 'size'), function(sum, n) {return sum + n;}, 0)) + ' ' + i18n.t('dialog.show_node.total');
                        break;
                    case 'cpu':
                    ...
                    {(node.deferred && node.deferred.state() == 'pending') ?
                        ...
                        <div>
                            <div className='row-fluid'>
                                ...
                                <div className='span7'>
                                    <div><strong>{i18n.t('dialog.show_node.manufacturer_label')}: </strong>{node.get('manufacturer') || i18n.t('common.not_available')}</div>
                                    <div><strong>{i18n.t('dialog.show_node.mac_address_label')}: </strong>{node.get('mac') || i18n.t('common.not_available')}</div>
                                    <div><strong>{i18n.t('dialog.show_node.fqdn_label')}: </strong>{(node.get('meta').system || {}).fqdn || node.get('fqdn') || i18n.t('common.not_available')}</div>
                                </div>
                            </div>
                                        ...
                                        <div className='accordion-group' key={group}>
                                            <div className='accordion-heading' onClick={this.toggle.bind(this, groupIndex)}>
                                                <div className='accordion-toggle' data-group={group}>
                                                    <b>{i18n.t('node_details.' + group, {defaultValue: group})}</b>
        ...
        renderFooter: function() {
            ...
            return (
                <div>
                    {node.get('cluster') &&
                        <span>
                            <button className='btn btn-edit-networks' onClick={this.goToConfigurationScreen.bind(this, 'interfaces')}>{i18n.t('dialog.show_node.network_configuration_button')}</button>
                            <button className='btn btn-edit-disks' onClick={this.goToConfigurationScreen.bind(this, 'disks')}>{i18n.t('dialog.show_node.disk_configuration_button')}</button>
                        </span>
                    }
                    <button className='btn' onClick={this.close}>{i18n.t('common.cancel_button')}</button>
                </div>
            );
        },
        renderNodeInfo: function(name, value) {
            return (
                <div key={name + value}>
                    <label>{i18n.t('dialog.show_node.' + name, {defaultValue: this.showPropertyName(name)})}</label>
                    <span>{value}</span>
                </div>
            );
        }
    ...
    dialogs.DiscardSettingsChangesDialog = React.createClass({
        mixins: [dialogMixin],
        getDefaultProps: function() {return {title: i18n.t('dialog.dismiss_settings.title'), defaultMessage: i18n.t('dialog.dismiss_settings.default_message')};},
        proceed: function() {
            this.close();
            app.page.removeFinishedNetworkTasks().always(_.bind(this.props.cb, this.props));
        },
        renderBody: function() {
            var message = this.props.verification ? i18n.t('dialog.dismiss_settings.verify_message') : this.props.defaultMessage;
            return (
        ...
        renderFooter: function() {
            var verification = !!this.props.verification,
                buttons = [<button key='stay' className='btn btn-return' onClick={this.close}>{i18n.t('dialog.dismiss_settings.stay_button')}</button>];
            if (!verification) buttons.push(<button key='leave' className='btn btn-danger proceed-btn' onClick={this.proceed}>{i18n.t('dialog.dismiss_settings.leave_button')}</button>);
            return buttons;
        }
    });
    ...
    dialogs.DeleteNodesDialog = React.createClass({
        mixins: [dialogMixin],
        getDefaultProps: function() {return {title: i18n.t('dialog.delete_nodes.title')};},
        renderBody: function() {
            return (<div className='deploy-task-notice'>{this.renderImportantLabel()} {i18n.t('dialog.delete_nodes.message')}</div>);
        },
        renderFooter: function() {
            return [
                <button key='cancel' className='btn' onClick={this.close}>{i18n.t('common.cancel_button')}</button>,
                <button key='delete' className='btn btn-danger btn-delete' onClick={this.deleteNodes} disabled={this.state.actionInProgress}>{i18n.t('common.delete_button')}</button>
            ];
        },
        deleteNodes: function() {
            ...
            this.props.nodes.sync('update', this.props.nodes)
                ...
                .fail(_.bind(function() {
                    this.showError(i18n.t('cluster_page.nodes_tab.node_deletion_error.node_deletion_warning'));
                }, this));
        }
    });
    ...
    dialogs.ChangePasswordDialog = React.createClass({
        ...
        getDefaultProps: function() {
            return {
                title: i18n.t('dialog.change_password.title'),
        ...
        getError: function(name) {
            ...
            if (name == 'currentPassword' && this.state.validationError) return i18n.t(ns + 'wrong_current_password');
            if (this.state.newPassword != this.state.confirmationPassword) {
                if (name == 'confirmationPassword') return i18n.t(ns + 'new_password_mismatch');
                ...
                <form className='change-password-form'>
                    {_.map(fields, function(name, index) {
                        return (<controls.Input
                            ...
                            ref={name}
                            type='password'
                            label={i18n.t(ns + translationKeys[index])}
                            maxLength='50'
