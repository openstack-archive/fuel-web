    ...
    dialogs.DeployChangesDialog = React.createClass({
        ...
        getInitialState: function() {
            var cluster = this.props.cluster,
                ...
                limitValidations = _.zipObject(roleModels.map(function(role) {
                    return [role.get('name'), role.checkLimits(configModels)];
                })),
                limitRecommendations = _.zipObject(roleModels.map(function(role) {
                    return [role.get('name'), role.checkLimits(configModels, true, ['recommended'])];
                })),
                roleRestrictions = _.zipObject(roleModels.map(function(role) {
                    return [role.get('name'), role.checkRestrictions(configModels)];
                }));
            return {
                ...
                <div className='display-changes-dialog'>
                    ...
                    <div className='amount-restrictions'>
                        {roleModels.map(_.bind(function(role) {
                            ...
                            if (!limits.valid && !this.state.roleRestrictions[name].result) {
                        ...
                        {roleModels.map(_.bind(function(role) {
                            ...
                            if (!recommendation.valid && !this.state.roleRestrictions[name].result) {
