    ...
    dialogs.UploadISODialog = React.createClass({
        ...
        uploadISO: function() {
            ...
            var request = $.ajax({
                ...
                error: _.bind(function(response, textStatus) {
                    if (textStatus != 'abort') {
        ...
        renderFooter: function() {
            if (this.state.request) {
                return <button key='stop' className='btn btn-danger' onClick={this.stopUploading}>{i18n('dialog.upload_iso.stop_uploading_button')}</button>;
            }
            return ([
                <button key='cancel' className='btn' onClick={this.close}>{i18n('common.cancel_button')}</button>,
                <button key='upload' className='btn btn-success' disabled={!this.state.file} onClick={this.uploadISO}>{i18n('dialog.upload_iso.upload_button')}</button>
            ]);
        }
    });
