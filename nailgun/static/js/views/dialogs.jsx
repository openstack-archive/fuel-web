...
function(require, React, utils, models, componentMixins, baseDialogTemplate, controls) {
    'use strict';
    ...
    views.ResetEnvironmentDialog = React.createClass({
        ...
        renderBody: function() {
            return (
                <div className='msg-error'>
                    <span className='label label-important'>{$.t('common.important')}</span>
                    <span>{$.t('dialog.reset_environment.text')}</span>
                </div>
            );
        },
        renderFooter: function() {
    ...
    views.ShowNodeInfoDialog = React.createClass({
        ...
        goToDisksConfiguration: function() {
            app.navigate('#cluster/' + this.props.node.get('cluster') + '/nodes/disks/' + utils.serializeTabOptions({nodes: this.props.node.id}), {trigger: true});
        },
        goToInterfacesConfiguration: function() {
            app.navigate('#cluster/' + this.props.node.get('cluster') + '/nodes/interfaces/' + utils.serializeTabOptions({nodes: this.props.node.id}), {trigger: true});
        },
                        ...
                        <div>
                            ...
                            <div className='accordion' id='nodeDetailsAccordion'>
                                {_.map(groups, function(group, groupIndex) {
                                    ...
                                    if (_.isObject(groupEntries)) subEntries = _.find(_.values(groupEntries), _.isArray);
                                    return (
                                        <div className='accordion-group' key={group}>
                                            ...
                                            <div className='accordion-body collapse' ref={'togglable_' + groupIndex}>
                                                <div className='accordion-inner'>
                                                                ...
                                                                return (
                                                                    <div className='nested-object' key={'entry_' + groupIndex + entryIndex}>
                                                                        {_.map(utils.sortEntryProperties(entry, sortOrder[group]), function(propertyName, propertyArrIndex) {
                                                                            return <NodeInfoBlock label={$.t('dialog.show_node.' + propertyName, {defaultValue: this.showPropertyName(propertyName)})} span={this.showPropertyValue(group, propertyName, entry[propertyName])} key={'entry_' + groupIndex + entryIndex + propertyArrIndex} />;
                                                                        }, this)}
                                                                    </div>
                                                                );
                                                            }, this)}
                                                    ...
                                                    {_.isObject(groupEntries) &&
                                                        <div>
                                                            {_.map(groupEntries, function(propertyValue, propertyName) {
                                                                if (!_.isArray(propertyValue) && !_.isNumber(propertyName))
                                                                    return <NodeInfoBlock label={$.t('dialog.show_node.' + propertyName, {defaultValue: this.showPropertyName(propertyName)})} span={this.showPropertyValue(group, propertyName, propertyValue)} key={'entry_' + groupIndex + propertyName} />;
                                                            }, this)}
                                                            {_.isArray(subEntries) &&
                                                                <div>
                                                                        ...
                                                                        return (
                                                                            <div className='nested-object' key={'subentries_' + groupIndex + subentrysIndex}>
                                                                                {_.map(utils.sortEntryProperties(subentry), function(propertyName, subentryIndex) {
                                                                                    return <NodeInfoBlock label={$.t('dialog.show_node.' + propertyName, {defaultValue: this.showPropertyName(propertyName)})} span={this.showPropertyValue(group, propertyName, subentry[propertyName])} key={'subentry_' + groupIndex + subentryIndex} />;
                                                                                }, this)}
                                                                            </div>
                                                                        );
                                                                    }, this)}
                                                    ...
                                                    {(!_.isObject(groupEntries) && !_.isObject(groupEntries)) &&
        ...
        renderFooter: function() {
            ...
            return (
                <div>
                    {node.get('cluster') &&
                        <span>
                            <button className='btn btn-edit-networks' data-dismiss='modal' onClick={this.goToInterfacesConfiguration}>{$.t('dialog.show_node.network_configuration_button')}</button>
                            <button className='btn btn-edit-disks' data-dismiss='modal' onClick={this.goToDisksConfiguration}>{$.t('dialog.show_node.disk_configuration_button')}</button>
                        </span>
                    }
    ...
    var NodeInfoBlock = React.createClass({
        render: function() {
            return (
