...
function($, _, i18n, Backbone, React, utils, models, dispatcher, controls, statisticsMixin, componentMixins) {
    ...
    dialogs.DeployChangesDialog = React.createClass({
        ...
        validate: function(cluster) {
            var _this = this;
            return this.verifications.reduce(
                function(p, v) {
                    return _.merge(p, v.call(_this, cluster), function(a, b) {
        ...
        showVerificationMessages: function() {
            ...
            return (
                <div>
                {
                    ['danger', 'warning'].map(function(v) {
                        if (_.isEmpty(result[v])) return null;
                        return (
                            <ul key={v} className={'alert alert-' + v}>
                                {result[v].map(function(line, i) {
                                    return (<li key={v + i}>{line}</li>);
                                })}
                            </ul>
                        );
                    }, [])
