...
function($, _, i18n, Backbone, React, utils, models, dispatcher, controls, statisticsMixin, componentMixins) {
    ...
    dialogs.DeployChangesDialog = React.createClass({
        ...
        validate: function(cluster) {
            return _.reduce(
                this.verifications,
                function(accumulator, verificator) {
                    return _.merge(accumulator, verificator.call(this, cluster), function(a, b) {
                    ...
                    ['danger', 'warning'].map(function(severity) {
                        ...
                        return (
                            <ul key={severity} className={'alert alert-' + severity}>
                                {result[severity].map(function(line) {
                                    return (<li key={lineNumber}>
                                        {lineNumber++ < blockers && (<i className='icon-minus-circle' />)}
                                        {line}
                                    </li>);
                                })}
                            </ul>
