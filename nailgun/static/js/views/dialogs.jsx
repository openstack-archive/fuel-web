    ...
    views.ChangePasswordDialog = React.createClass({
        ...
        renderBody: function() {
            ...
            var ns = 'dialog.change_password.',
                ...
                inputClassesArray = ['current-password',  'change-password',  'change-password'];
            return (
                <form className='change-password-form'>
                    {_.map(fields, function(name, index) {

                        var inputClass = {};
                        inputClass[inputClassesArray[index]] = true;
                        return (<controls.PasswordField
                            ...
                            maxLength='50'
                            onChange={this.handleChange.bind(this, (index == 0))}
                            onKeyDown={this.handleKeyDown}
                            disabled={this.state.locked}
                            validate={_.bind(this.validate, this)}
                            hiddenPassword={index > 0}
                            cs={{
                                common: '',
        ...
        isPasswordChangeAvailable: function() {
            return !!(!this.state.validationError && (this.state.newPassword == this.state.confirmationPassword));
        },
        ...
        validate: function(name) {
            var ns = 'dialog.change_password.',
                hasConfirmationError = this.state.newPassword != this.state.confirmationPassword;
            if (name == 'currentPassword' && this.state.validationError) return $.t(ns + 'wrong_current_password');
            if ((name == 'confirmationPassword') && hasConfirmationError) return $.t(ns + 'new_password_mismatch');
            if ((name == 'newPassword') && hasConfirmationError) return ' ';
            return null;
        },
            ...
            if (this.isPasswordChangeAvailable()) {
                ...
                app.keystoneClient.changePassword(this.state.currentPassword, this.state.newPassword)
                    ...
                    .fail(_.bind(function() {
                        ...
                        $(this.refs.currentPassword.getDOMNode()).find('input').focus();
                    }, this));
            }
        }
    });
