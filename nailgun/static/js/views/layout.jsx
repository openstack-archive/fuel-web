...
function(React, utils, models, dialogViews) {
    ...
    components.Navbar = React.Component.extend({
        ...
        showChangePasswordDialog: function(e) {
            e.preventDefault();
            (new dialogViews.ChangePasswordDialog()).render();
        },
                    ...
                    <div className="navigation-bar">
                        <div className="navigation-bar-box">
                            <ul className="navigation-bar-ul">
                                ...
                                <Notifications ref="notifications" notifications={this.props.notifications} />
    ...
    var Notifications = React.Component.extend({
        togglePopover: function() {
            this.getOwner().togglePopover();
        },
        render: function() {
    ...
    var NotificationsPopover = React.Component.extend({
        ...
        toggle: function(visible) {
            this.getOwner().togglePopover(visible);
        },
        handleBodyClick: function(e) {
            if (_.all([this.el, this.getOwner().refs.notifications.el], function(el) {
                    ...
                    <ul className="message-list-popover">
                        {this.props.notifications.length ? (
                            _.map(notifications, function(notification, index, collection) {
                                return [
                                    <li className={cx({'enable-selection': true, 'new': notification.get('status') == 'unread'}) + ' ' + notification.get('topic')} key={'notification' + index} onClick={notification.get('node_id') && _.bind(this.showNodeInfo, this, notification.get('node_id'))}>
                                    ...
                                    (showMore || index < (collection.length - 1)) && <li key={'divider' + index} className="divider"></li>
                                ];
                            }, this)
