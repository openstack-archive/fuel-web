                    ...
                    <div className='user-info-box'>
                        {this.props.version.get('auth_required') && this.props.user.get('authenticated') &&
                            <div>
                                ...
                                <a className='change-password' onClick={this.showChangePasswordDialog}>{i18n.t('common.change_password')}</a>
                            ...
                            <ul className='navigation-bar-ul'>
                                ...
                                {_.map(this.props.elements, function(element) {
                                    return <li key={element.label}>
                                        <a className={cx({active: this.state.activeElement == element.url.slice(1)})} href={element.url}>{i18n.t('navbar.' + element.label, {defaultValue: element.label})}</a>
                                    </li>;
                                }, this)}
                    ...
                    <div className='statistic'>
                        {_.map(['total', 'unallocated'], function(prop) {
                            ...
                            return _.isUndefined(value) ? '' : [
                                ...
                                <div className='stat-title' dangerouslySetInnerHTML={{__html: utils.linebreaks(_.escape(i18n.t('navbar.stats.' + prop, {count: value})))}}></div>
                            ];
                        }, this)}
                    </div>
                </li>
            ...
            return (
                <div className='message-list-placeholder'>
                    <ul className='message-list-popover'>
                        ...
                        ) : <li key='no_notifications'>{i18n.t('notifications_popover.no_notifications_text')}</li>}
                    </ul>
                    {showMore && <div className='show-more-notifications'><a href='#notifications'>{i18n.t('notifications_popover.view_all_button')}</a></div>}
                </div>
            );
        }
    });
    ...
    components.Footer = React.createClass({
                ...
                <div className='footer-box'>
                    {_.contains(this.props.version.get('feature_groups'), 'mirantis') &&
                        <div>
                            ...
                            <div className='footer-copyright pull-left'>{i18n.t('common.copyright')}</div>
                        </div>
                    }
        ...
        getAvailableLocales: function() {
            return _.map(_.keys($.i18n.options.resStore).sort(), function(locale) {
                return {locale: locale, name: i18n.t('language', {lng: locale})};
            }, this);
        },
            ...
            return <ul className='breadcrumb'>
                {_.map(this.props.path, function(breadcrumb, index) {
                    if (_.isArray(breadcrumb)) {
                        ...
                        return <li key={index}><a href={breadcrumb[1]}>{i18n.t('breadcrumbs.' + breadcrumb[0], {defaultValue: breadcrumb[0]})}</a><span className='divider'>/</span></li>;
                    }
                    return <li key={index} className='active'>{i18n.t('breadcrumbs.' + breadcrumb, {defaultValue: breadcrumb})}</li>;
                })}
            </ul>;
        }
    });
