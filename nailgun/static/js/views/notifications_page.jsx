...
function(React, utils, models, dialogViews) {
            ...
            return (
                <div>
                    ...
                    <div className='notification-list page-wrapper'>
                        <ul>
                            {_.map(notifications.last(notifications.length).reverse(), function(notification) {
    ...
    Notification = React.createClass({
        ...
        onNotificationClick: function() {
            ...
            if (notification.get('node_id')) {
                this.showNodeInfo(notification.get('node_id'));
            }
        },
        render: function() {
            var notification = this.props.notification;
            var iconClass = {'error': 'icon-attention', 'discover': 'icon-bell'}[notification.get('topic')] || 'icon-info-circled';
            return (
                <li className={notification.get('topic') + ' ' + notification.get('status') + ' ' + (notification.get('node_id') ? 'clickable' : '')} onClick={this.onNotificationClick}>
                    <div className='icon'><i className={iconClass}></i></div>
                    <div className='datetime enable-selection'>{notification.get('date')} {notification.get('time')}</div>
                    <div className='message enable-selection' dangerouslySetInnerHTML={{__html: utils.urlify(notification.escape('message'))}}></div>
                </li>
            );
        }
    });
