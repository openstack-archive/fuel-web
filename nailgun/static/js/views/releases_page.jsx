...
function($, _, i18n, React, models, utils, dialogs, controls, componentMixins) {
    ...
    var ReleasesPage = React.createClass({
        ...
        fetchData: function() {
            return $.when(_.union(_.invoke(this.props.releases.where({state: 'processing'}), 'fetch'), this.props.tasks.fetch()));
        },
            ...
            return _.map(this.props.columns, function(attr) {
                if (attr == 'state') {
                    ...
                    if (release.get(attr) == 'unavailable') {
                        ...
                        var task = this.props.tasks.findWhere({name: 'prepare_release', release_id: release.id, status: 'error'});
                        if (task) {
