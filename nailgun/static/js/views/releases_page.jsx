...
function(_, i18n, React, models, controls) {
    ...
    var ReleasesPage = React.createClass({
        ...
        title: function() {
            return i18n.t('release_page.title');
        },
        ...
        getReleaseData: function(release) {
            return _.map(this.props.columns, function(attr) {
                if (attr == 'state') return i18n.t('release_page.release.' + release.get(attr));
                return release.get(attr) || i18n.t('common.not_available');
            });
        },
        render: function() {
            return (
                <div>
                    <h3 className='page-title'>{i18n.t('release_page.title')}</h3>
                    <div className='table-releases-box'>
                        {this.props.releases.length ?
                            <controls.Table
                                head={_.map(this.props.columns, function(column) {
                                    return {label: i18n.t('release_page.' + column), className: column};
                                })}
                                body={this.props.releases.map(this.getReleaseData)}
                                tableClassName='releases-table'
                            />
                        :
                            <div className='alert'>{i18n.t('release_page.no_releases_message')}</div>
                        }
                    </div>
                </div>
            );
