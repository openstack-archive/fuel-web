...
define([
    ...
    return {
        ...
        onSettingChange: function(name, value) {
            this.setState({actionInProgress: false});
            this.props.settings.set(this.makePath('statistics', name, 'value'), value);
            this.props.settings.isValid({models: this.configModels});
        },
        checkRestrictions: function(name, action) {
            action = action || 'disable';
            return this.props.settings.checkRestrictions(this.configModels, action, this.makePath('statistics', name));
        },
        makePath: function() {
            return this.props.settings.makePath.apply(this.props.settings, arguments);
        },
        get: function(settingName) {
            return this.props.settings.get(this.makePath('statistics', settingName));
        },
        ...
        renderInput: function(setting, settingName, labelClassName, wrapperClassName, hideErrors) {
            if (this.checkRestrictions('metadata', 'hide') || this.checkRestrictions(settingName, 'hide')) return null;
            var errors = (this.props.settings.validationError || {}).statistics,
            ...
            return <controls.Input
                ...
                error={(errors || {})[settingName] ? hideErrors ? '' : $.t(errors[settingName]) : null}
            />;
        },
