...
function($, _, i18n, Backbone, React, dialogs, componentMixins, models, statisticsMixin) {
    ...
    var StatisticsSettings = React.createClass({
        ...
        componentWillUnmount: function() {
            // FIXME: this ugly hack cause the registration form uses some statistics data
            // like email, name, company info
            var initialData = this.initialAttributes.statistics;
            this.props.settings.set('statistics.send_anonymous_statistic.value', initialData.send_anonymous_statistic.value);
            this.props.settings.set('statistics.send_user_info.value', initialData.send_user_info.value);
        },
        render: function() {
            var statistics = this.props.settings.get('statistics'),
                sortedSettings = _.chain(_.keys(statistics))
                    ...
                    .value(),
                hasChanges = !_.isEqual(this.initialAttributes.statistics, statistics);
            return (
                <SupportPageElement
