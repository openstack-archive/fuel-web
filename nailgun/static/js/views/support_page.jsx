            ...
            return (
                <div>
                    ...
                    <div className='support-page page-wrapper'>
                        ...
                        <SupportContacts />
                        <hr/>
                        <DiagnosticSnapshot tasks={this.props.tasks} />
                        <hr/>
                        <CapacityAudit />
                    </div>
                </div>
    ...
    DiagnosticSnapshot = React.createClass({
        mixins: [
            React.BackboneMixin({modelOrCollection: function(props) {
                return props.tasks.findTask({name: 'dump', status: 'running'});
            }}),
            componentMixins.pollingMixin(2)
        ],
        shouldDataBeFetched: function() {
            return this.props.tasks.findTask({name: 'dump', status: 'running'});
        },
        fetchData: function() {
            var task = this.props.tasks.findTask({name: 'dump', status: 'running'});
            if (task) {
                return task.fetch();
            }
        },
        ...
        render: function() {
            var task = this.props.tasks.findTask({name: 'dump'});
            return (
