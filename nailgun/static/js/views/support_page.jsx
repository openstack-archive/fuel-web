...
function(React, componentMixins, models) {
    ...
    var DiagnosticSnapshot = React.createClass({
        ...
        getInitialState: function() {
            return {disabled: this.isDumpTaskRunning()};
        },
        ...
        fetchData: function() {
            return this.props.task.fetch().done(_.bind(function() {
                if (!this.isDumpTaskRunning()) this.setState({disabled: false});
            }, this));
        },
        isDumpTaskRunning: function(task) {
            task = task || this.props.task;
            return task && task.match({status: 'running'});
        },
            ...
            return (
                ...
                >
                    <p className='snapshot'>
                        <button className='btn' disabled={this.state.disabled} onClick={this.downloadLogs}>
                            {this.isDumpTaskRunning() ? $.t('support_page.gen_logs_snapshot_text') : $.t('support_page.gen_diagnostic_snapshot_text')}
                        </button>
                        {task &&
