...
function(_, i18n, React, componentMixins, models, statisticsMixin) {
    ...
    var SupportPage = React.createClass({
        ...
        title: function() {
            return i18n.t('support_page.title');
        },
        statics: {
            fetchData: function() {
        ...
        render: function() {
            ...
            return (
                <div>
                    <h3 className='page-title'>{i18n.t('support_page.title')}</h3>
        ...
        render: function() {
            ...
            return (
                <SupportPageElement
                    ...
                    title={i18n.t('support_page.register_fuel_title')}
                    text={i18n.t('support_page.register_fuel_content')}
                >
                    <p><a className='btn registration-link' href={_.isUndefined(key) ? '/' : 'http://fuel.mirantis.com/create-subscriber/?key=' + key} target='_blank'>
                        {i18n.t('support_page.register_fuel_title')}
                    </a></p>
                </SupportPageElement>
            );
        }
        ...
        render: function() {
            return (
                <SupportPageElement
                    ...
                    title={i18n.t('support_page.contact_support')}
                    text={i18n.t('support_page.contact_text')}
                >
                    <p>{i18n.t('support_page.irc_text')} <strong>#fuel</strong> on <a href='http://freenode.net' target='_blank'>freenode.net</a>.</p>
                    <p><a className='btn' href='https://mirantis.zendesk.com/requests/new' target='_blank'>{i18n.t('support_page.contact_support')}</a></p>
                </SupportPageElement>
            );
        }
    });
        ...
        render: function() {
            ...
            return (
                <SupportPageElement
                    ...
                    title={i18n.t('support_page.download_diagnostic_snapshot_text')}
                    text={i18n.t('support_page.log_text')}
                >
                    <p className='snapshot'>
                        <button className='btn' disabled={generating} onClick={this.downloadLogs}>
                            {generating ? i18n.t('support_page.gen_logs_snapshot_text') : i18n.t('support_page.gen_diagnostic_snapshot_text')}
                        </button>
                        {!generating && task &&
                            <span className={task.get('status')}>
                                {task.match({status: 'ready'}) &&
                                    <a href={task.get('message')} target='_blank'>
                                        ...
                                        <span>{i18n.t('support_page.diagnostic_snapshot')}</span>
                                    </a>
                                }
        ...
        render: function() {
            return (
                <SupportPageElement
                    ...
                    title={i18n.t('support_page.capacity_audit')}
                    text={i18n.t('support_page.capacity_audit_text')}
                >
                    <p><a className='btn' href='#capacity'>{i18n.t('support_page.view_capacity_audit')}</a></p>
                </SupportPageElement>
            );
        }
    });
        ...
        render: function() {
            ...
            var settings = this.props.settings.get('statistics'),
                sortedSettings = _.chain(_.keys(settings))
                    ...
                    .value();
            return (
                <SupportPageElement title={i18n.t('support_page.send_statistics_title')}>
                    {this.renderIntro()}
                    ...
                    <p>
                        <a className='btn' disabled={this.state.actionInProgress || !this.hasChanges()} onClick={this.saveSettings}>
                            {i18n.t('support_page.save_changes')}
                        </a>
                    </p>
                </SupportPageElement>
            );
