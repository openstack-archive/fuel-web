...
function(require, utils, models, dialogs, createClusterWizardTemplate, clusterNameAndReleasePaneTemplate, commonWizardTemplate, modePaneTemplate, storagePaneTemplate, clusterReadyPaneTemplate, controlTemplate, warningTemplate, wizardInfo) {
    ...
    views.CreateClusterWizard = dialogs.Dialog.extend({
        ...
        calculateDependencies: function() {
            ...
            function processRestrictions(restriction) {
                this.trackedAttributes.push(_.filter(restriction.condition.split(' '), function(elem) {return _.contains(elem, '.');})[0]);
            }
    ...
    views.WizardPane = Backbone.View.extend({
        ...
        composeDisabledBindings: function(restrictions, attributeName, elementName) {
                this.createDisabledBindings(_.pluck(restrictions, 'condition'), attributeName, elementName);
        },
            ...
            this.bindings['[' + attributeName + '=' + elementName +']'] = {
                ...
                attributes: [{
                    ...
                    onGet: function() {
                        var results = [];
                        _.each(restrictedAttributes, function(attribute) {
                            results.push(!utils.evaluateExpression(attribute, {default: this.wizard.model}, {strict:false}).value);
                        }, this);
        ...
        handleWarnings: function(restrictionObject) {
            ...
            _.each(restrictionObject, function(restriction) {
                var evaluatedExpression = utils.evaluateExpression(restriction.condition, {default: this.wizard.model}, {strict: false});
                if (!evaluatedExpression.value) {
                    this.showWarning($.t(restriction.warning, translationParams));
                }
            }, this);
        },
