...
function(require, utils, models, dialogs, createClusterWizardTemplate, clusterNameAndReleasePaneTemplate, commonWizardTemplate, modePaneTemplate, storagePaneTemplate, clusterReadyPaneTemplate, controlTemplate, warningTemplate, wizardInfo) {
    ...
    views.CreateClusterWizard = dialogs.Dialog.extend({
        ...
        initialize: function(options) {
            ...
            this.attachModelListeners(this.restrictionsDictionary);
            this.attachModelListeners(this.warningsDictionary);
        },
        attachModelListeners: function(dictionary) {
            _.each(dictionary, function(paneConfig) {
                _.each(paneConfig, function(paneRestrictions) {
                    _.each(paneRestrictions, function(restrictionObject) {
                        _.each(restrictionObject, function(warning, restriction) {
                            var evaluatedExpression = utils.evaluateExpression(restriction, {default: this.model}, {strict: false});
        ...
        handleRestrictions: function (config, paneKey, paneName) {
            this.restrictionsDictionary[paneName][paneKey] = [];
            this.warningsDictionary[paneName][paneKey] = [];
            _.each(config.restrictions, function(warning, restriction) {
                this.processRestrictions(warning, restriction, this.restrictionsDictionary[paneName][paneKey]);
            }, this);
            _.each(config.warnings, function(warning, condition) {
                this.processRestrictions(warning, condition, this.warningsDictionary[paneName][paneKey]);
            }, this);
        },
        processRestrictions: function (warning, condition, listToUpdate) {
            var restrictionJSON = {};
            restrictionJSON[condition] = warning;
            listToUpdate.push(restrictionJSON);
        },
        calculateDependencies: function() {
            this.restrictionsDictionary = {};
            this.warningsDictionary = {};
            _.each(this.config, function(paneConfig, paneName) {
                this.restrictionsDictionary[paneName] = {};
                this.warningsDictionary[paneName] = {};
                _.each(paneConfig, function(attributeConfig, attribute) {
                    if (attributeConfig.type == 'checkbox') {
                        this.handleRestrictions(attributeConfig, attribute, paneName);
                    }
                    if (attributeConfig.type == 'radio') {
                        _.each(attributeConfig.values, function(attributeValueConfig) {
                            this.handleRestrictions(attributeValueConfig, attribute, paneName);
                        }, this);
                    }
                }, this);
            }, this);
        },
//        handleWarnings: function(model) {
//            if (_.contains(this.panesWithWarnings,  this.activePane.constructorName)) {
//                _.each(this.warningContainingAttributes, function (attributeWithWarning) {
//                     _.each(attributeWithWarning.warnings, function(warning, condition) {
//                        var evaluatedExpression = utils.evaluateExpression(condition, {default: this.model}, {strict: false});
//                        if (evaluatedExpression.value) {
//                            this.activePane.showWarning($.t(warning), true);
//                        }
//                    }, this);
//                }, this);
//            }
//        },
        ...
        renderPane: function(Pane) {
            ...
            if (_.contains(this.panesWithWarnings, Pane.prototype.constructorName)) {
                _.each(_.keys(this.activePane.config), function(key) {
                    this.model.on('change:' + Pane.prototype.constructorName + '.' + key, this.handleWarnings, this);
                }, this);
            }
            this.handleWarnings();
        },
    ...
    views.WizardPane = Backbone.View.extend({
        ...
        composePaneBindings: function() {
            ...
            if (_.pluck(this.config, 'type')[0] == 'radio') {
                if (_.compact(_.pluck(_.pluck(this.config, 'values')[0], 'restrictions')).length) {
            ...
            _.each(this.config, function(paneAttribute, paneAttributeName) {
                if (type == 'checkbox') {
                    if (!_.isUndefined(paneAttribute.restrictions)) {
                        this.createDisabledBindings(_.keys(paneAttribute.restrictions), 'name', paneAttributeName);
                        this.handleWarningMessages(paneAttribute.restrictions);
                    }
                    else {
                ...
                if (type == 'radio') {
                    _.each(paneAttribute.values, function(value) {
                        if (!_.isUndefined(value.restrictions)) {
                            this.createDisabledBindings(_.keys(value.restrictions), 'name', paneAttributeName, {'value': value.data});
                            this.handleWarningMessages(value.restrictions);
                        }
                        else {
                            this.bindings['[name=' + paneAttributeName + ']' + '[value=' + value.data + ']'] = {observe: this.constructorName + '.' + paneAttributeName};
                        }
                    }, this);
                }
            }, this);
        },
        createDisabledBindings: function(restriction, domAttribute, modelAttributeToObserve, additionalOptions) {
            var disabledAttributesToObserve = [];
            _.each(restriction, function(attribute) {
                var evaluatedExpression = utils.evaluateExpression(attribute, {default: this.wizard.model});
                disabledAttributesToObserve.push(_.keys(evaluatedExpression.modelPaths));
            }, this);
            var additionalOptionString = '';
            _.each(additionalOptions, function(option, key) {
                additionalOptionString += '[' + key + '=' + option + ']';
            }, this);
            var bindingKey = '[' + domAttribute + '=' + modelAttributeToObserve +']' + additionalOptionString;
            this.bindings[bindingKey] = {
                observe: this.constructorName + '.' + modelAttributeToObserve,
                attributes: [{
                    name: 'disabled',
                    observe: _.uniq(_.flatten(disabledAttributesToObserve)),
                    onGet: function() {
                        return _.any(restriction, function(attribute) {
                            return !utils.evaluateExpression(attribute, {default: this.wizard.model}).value;
                        }, this);
                    }
                }]
            };
        ...
        handleWarningMessages: function(restrictionObject) {
            ...
            _.each(restrictionObject, function(warning, restriction) {
                var evaluatedExpression = utils.evaluateExpression(restriction, {default: this.wizard.model});
                if (!evaluatedExpression.value) {
                    this.showWarning($.t(warning, translationParams));
                }
            }, this);
        },
        showWarning: function(warningText, leavePreviousWarnigs) {
            if (!leavePreviousWarnigs) {
                this.$('.alert').remove();
            }
...
//            this.attributesWithWarnings = [];
//            this.warningContainingAttributes = [];
//            this.panesWithWarnings = [];
//            var wizardModel = this.wizard.model;
//            function processRestrictions(restriction, listToUpdate) {
//                var evaluatedRestriction = utils.evaluateExpression(restriction, {default: model}, {strict:false});
//                listToUpdate.push(_.keys(evaluatedRestriction.modelPaths));
//            }
//            _.each(this.config, function(paneConfig, paneName) {
//                _.each(paneConfig, function(attributeConfig, attribute) {
//                    _.each(_.keys(attributeConfig.restrictions), function(restriction) {
//                        processRestrictions (restriction, this.model, this.trackedAttributes);
//                    }, this);
//                    _.each(_.keys(attributeConfig.warnings), function(restriction) {
//                        processRestrictions(restriction, this.model, this.attributesWithWarnings);
//                        this.warningContainingAttributes.push(attributeConfig);
//                        this.panesWithWarnings.push(paneName);
//                    }, this);
//                    if (attributeConfig.type == 'radio') {
//                        _.each(attributeConfig.values, function(attributeValueConfig) {
//                            _.each(_.keys(attributeValueConfig.restrictions), function(restriction) {
//                                processRestrictions(restriction, this.model, this.trackedAttributes);
//                            }, this);
//                            _.each(_.keys(attributeValueConfig.warnings), function(restriction) {
//                                processRestrictions(restriction, this.model, this.attributesWithWarnings);
//                                this.warningContainingAttributes.push(attributeValueConfig);
//                                this.panesWithWarnings.push(paneName);
//                            }, this);
//                        }, this);
//                    }
//                }, this);
//            }, this);
//            this.trackedAttributes = _.uniq(_.flatten(this.trackedAttributes));
//            this.trackedPanes = _.uniq(_.map(this.trackedAttributes, function(attribute) {
//                return attribute.split('.')[0];
//            }));
//            this.warningContainingAttributes = _.uniq(this.warningContainingAttributes);
//            this.attributesWithWarnings = _.uniq(_.flatten(this.attributesWithWarnings));
//            this.panesWithWarnings = _.uniq(this.panesWithWarnings);
//        },
        render: function() {
