...
function(require, utils, models, dialogs, createClusterWizardTemplate, clusterNameAndReleasePaneTemplate, commonWizardTemplate, modePaneTemplate, storagePaneTemplate, clusterReadyPaneTemplate, controlTemplate, warningTemplate, wizardInfo) {
    ...
    views.CreateClusterWizard = dialogs.Dialog.extend({
        ...
        initialize: function(options) {
            ...
            this.masterPanes = _.uniq(_.flatten(_.values(this.dependentPanes)));
                ...
                _.each(pane, function(paneAttribute) {
                    if (paneAttribute.type == 'checkbox') {
                        if (!_.isUndefined(paneAttribute.restrictions)) {
                            dependencies = this.getMasterPaneAttributes(paneAttribute.restrictions);
                            dependentPanes.push(dependencies[0]);
                            dependingConditions.push(dependencies[1]);
                        }
                    }
                    else {
                        var restrictions = _.compact(_.pluck(paneAttribute.values, 'restrictions'));
                        if (restrictions.length) {
                            dependencies = this.getMasterPaneAttributes(restrictions[0]);
                            dependentPanes.push(dependencies[0]);
                            dependingConditions.push(dependencies[1]);
                        }
                    }
                }, this);
                this.dependencies[paneName] = _.compact(_.uniq(dependingConditions));
                this.dependentPanes[paneName] = _.compact(_.uniq(dependentPanes));
            }, this);
        },
        getMasterPaneAttributes: function(restrictions) {
            var paneConditionArray = _.zip(this.getConditionsFromRestrictions(_.compact(_.union(restrictions.conflicts, restrictions.depends))));
            return [paneConditionArray[0], paneConditionArray[1]];
        },
        getConditionsFromRestrictions: function(restrictions) {
            var panes = [],
                conditions = [];
            _.each(restrictions, function(restriction) {
                _.each(_.keys(restriction.condition), function(condition) {
                    panes.push(condition.split('.')[0]);
                    conditions.push(condition);
                }, this);
            }, this);
            return [_.compact(_.uniq(panes)), _.compact(_.uniq(conditions))];
        },
    ...
    views.WizardPane = Backbone.View.extend({
        ...
        composeDisabledBindings: function(restrictions, attributeName, elementName) {
            ...
            var conflicts = _.map(restrictions.conflicts, function(elem){return elem.condition});
            var conflictValues = _.map(conflicts, function(conflict) {return _.values(conflict)[0]});
            var dependencies = _.map(restrictions.depends, function(elem){return elem.condition});
            if (conflicts.length) {
                attributesToObserve = _.map(_.pluck(restrictions.conflicts, 'condition'), function(elem) {return _.keys(elem)[0];})
                this.bindings['[' + attributeName + '=' + elementName +']'] = {
                    ...
                    attributes: [{
                        ...
                        onGet: function(values) {
                            return !!_.intersection(conflictValues, values).length;
                        }
                    }]
                };
            }
            else if (dependencies.length) {
                attributesToObserve = _.map(_.pluck(restrictions.depends, 'condition'), function(elem, value) {return _.keys(elem)[0];})
                _.each(restrictions.depends, function(conflict) {
                    this.bindings['[' + attributeName + '=' + elementName +']'] = {
                        ...
                        attributes: [{
                            ...
                            observe: attributesToObserve,
                            onGet: function(values) {
                                return !_.contains(values[0], _.values(conflict.condition)[0]);
                            }
                        }]
                    };
                }, this);
        ...
        handleWarnings: function(restrictions) {
            function processKeys(json) {
                function processPiece(base, piece) {
                    return _.map(piece, function(value, key) {
                        var localBase = base ? base + '.' + key : key;
                        return _.isObject(value) ? processPiece(localBase, value) : localBase;
                    });
                }
                return _.uniq(_.flatten(processPiece(null, json))).sort();
            }
            var translationParams = {};
                var keys = processKeys(this.wizard.model.toJSON());
