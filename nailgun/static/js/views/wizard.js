...
function(require, utils, models, dialogs, createClusterWizardTemplate, clusterNameAndReleasePaneTemplate, commonWizardTemplate, modePaneTemplate, storagePaneTemplate, clusterReadyPaneTemplate, controlTemplate, warningTemplate, wizardInfo) {
    ...
    views.CreateClusterWizard = dialogs.Dialog.extend({
        ...
        handleRestrictions: function (config, paneKey, paneName) {
            function processRestrictions (warning, condition, listToUpdate) {
                var restrictionJSON = {};
                restrictionJSON[condition] = warning;
                listToUpdate.push(restrictionJSON);
            }
            this.restrictions[paneName][paneKey] = [];
            this.warnings[paneName][paneKey] = [];
            _.each(config.restrictions, function(message, condition) {
                processRestrictions(message, condition, this.restrictions[paneName][paneKey]);
            }, this);
            _.each(config.warnings, function(message, condition) {
                processRestrictions(message, condition, this.warnings[paneName][paneKey]);
            }, this);
        },
        calculateDependencies: function() {
            ...
            _.each(this.config, function(paneConfig, paneName) {
                this.restrictions[paneName] = {};
                this.warnings[paneName] = {};
    ...
    views.WizardPane = Backbone.View.extend({
        ...
        attachWarningListeners: function() {
            ...
            _.each(this.wizard.warnings[this.constructorName], function(paneRestrictions, paneAttribute) {
        ...
        composePaneBindings: function() {
            ...
            _.each(this.config, function(attributeConfig) {
                if (attributeConfig.type == 'radio') {
                    if (_.compact(_.pluck(attributeConfig.values, 'restrictions')).length) {
        ...
        showWarning: function(warningText, leavePreviousWarnings) {
            if (!leavePreviousWarnings) {
                this.$('.alert').remove();
            }
            this.$('.form-horizontal').before(_.template(warningTemplate, {warningMessage: warningText}));
        },
        handleWarningsAndBindings: function() {
            this.handleWarnings();
    ...
    clusterWizardPanes.Mode = views.WizardPane.extend({
        ...
        render: function() {
            ...
            this.handleWarningsAndBindings();
            return this;
        }
    });
