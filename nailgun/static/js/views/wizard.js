...
function(require, utils, models, dialogs, createClusterWizardTemplate, clusterNameAndReleasePaneTemplate, commonWizardTemplate, modePaneTemplate, storagePaneTemplate, clusterReadyPaneTemplate, controlTemplate, warningTemplate, wizardInfo) {
    ...
    views.CreateClusterWizard = dialogs.Dialog.extend({
        ...
        initialize: function(options) {
            ...
            this.model.on('change', this.modelAttributeChange, this);
        },
        modelAttributeChange:function() {
            var maxIndex = this.panesModel.get('maxAvailablePaneIndex');
            var currentIndex = this.panesModel.get('activePaneIndex');
            if (maxIndex > currentIndex) {
                this.panesModel.set('maxAvailablePaneIndex', this.panesModel.get('activePaneIndex'));
                this.model.restoreDefaultValues(JSON.parse(wizardInfo), this.panesModel.toJSON());
            }
        },
    ...
    views.WizardPane = Backbone.View.extend({
        ...
        composePaneBindings: function() {
            ...
            if (_.pluck(this.config, 'type')[0] == 'custom') {
                this.createSimpleBindings();
            }
            this.stickit(this.wizard.model);
        },
        createSimpleBindings: function() {
                ...
                if (type == 'radio') {
                    _.each(paneAttribute.values, function(value) {
                        if (!_.isUndefined(value.restrictions)) {
                            this.createDisabledBindings(_.keys(value.restrictions), 'value', value.data);
                            this.handleWarnings(value.restrictions);
                        }
                        else {
        ...
        createDisabledBindings: function(restriction, attributeName, elementName) {
            ...
            _.each(_.keys(this.config), function(key) {
                attributesToObserve.push(this.constructorName + '.' + key);
            }, this);
        ...
        handleWarnings: function(restrictionObject) {
