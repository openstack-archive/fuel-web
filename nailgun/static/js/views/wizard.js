...
function(require, utils, models, dialogs, createClusterWizardTemplate, clusterNameAndReleasePaneTemplate, commonWizardTemplate, modePaneTemplate, storagePaneTemplate, clusterReadyPaneTemplate, controlTemplate, warningTemplate, wizardInfo) {
    ...
    views.WizardPane = Backbone.View.extend({
        ...
        attachWarningListeners: function() {
            ...
            _.each(this.wizard.warningsObject[this.constructorName], function(paneRestrictions, paneAttribute) {
                _.each(paneRestrictions, function(restrictionObject) {
                    _.each(restrictionObject, function(message, condition) {
        ...
        handleWarnings: function() {
            var warningsShown = [];
            _.each(this.wizard.warningsObject[this.constructorName], function (paneConfig) {
                _.each(paneConfig, function(paneRestrictions) {
                    _.each(paneRestrictions, function(warning, condition) {
                        var evaluatedExpression = utils.evaluateExpression(condition, {default: this.wizard.model});
                        if (evaluatedExpression.value) {
                            this.showWarning($.t(warning), true);
                            warningsShown.push(warning);
                        }
                    }, this);
                }, this);
            }, this);
            if (!warningsShown.length) {
                 this.$('.alert').remove();
            }
        },
