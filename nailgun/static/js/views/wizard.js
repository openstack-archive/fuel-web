...
function(require, utils, models, dialogs, createClusterWizardTemplate, clusterNameAndReleasePaneTemplate, commonWizardTemplate, modePaneTemplate, storagePaneTemplate, clusterReadyPaneTemplate, controlTemplate, warningTemplate, wizardInfo) {
    ...
    views.CreateClusterWizard = dialogs.Dialog.extend({
        ...
        initialize: function(options) {
            ...
            this.panesModel.on('change:maxAvailablePaneIndex', this.updatePanesStatuses, this);
            this.model = new models.WizardModel();
            this.model.processConfig(this.config);
            this.calculateDependencies();
            this.masterPanes = _.uniq(_.flatten(_.values(this.dependantPanes)));
            this.masterConditions = _.uniq(_.flatten(_.values(this.dependencies)));
            _.each(this.masterConditions, function(master) {
                this.model.on('change:' + master, this.handleMasterChange, this);
            }, this);
        },
        handleMasterChange: function(model) {
            var maxIndex = this.panesModel.get('maxAvailablePaneIndex');
            var currentIndex = this.panesModel.get('activePaneIndex');
            if (!_.isUndefined(maxIndex)) {
                if ((maxIndex > currentIndex) && _.contains(this.masterPanes, this.activePane.constructorName)) {
        ...
        calculateDependencies: function() {
            ...
            _.each(this.config, function(pane, paneName) {
                dependentPanes = '';
                dependingConditions = '';
                _.each(pane, function(paneAttribute) {
                    if (paneAttribute.type == 'checkbox') {
                        if (!_.isUndefined(paneAttribute.restrictions)) {
                            dependentPanes += ',' + this.getMasterPaneAttributes(paneAttribute.restrictions)[0];
                            dependingConditions += ',' + this.getMasterPaneAttributes(paneAttribute.restrictions)[1];
                        }
                    }
                    else {
                        var restrictions = _.compact(_.pluck(paneAttribute.values, 'restrictions'));
                        if (restrictions.length) {
                            dependentPanes += ',' + this.getMasterPaneAttributes(restrictions[0])[0];
                            dependingConditions += ',' + this.getMasterPaneAttributes(restrictions[0])[1];
                        }
                    }
                }, this);
                this.dependencies[paneName] = _.compact(_.uniq(dependingConditions.split(',')));
                this.dependantPanes[paneName] = _.compact(_.uniq(dependentPanes.split(',')));
            }, this);
        },
        getMasterPaneAttributes: function(restrictions) {
            var panes = [],
                conditions = [];
                _.each(restrictions.conflicts, function(conflict) {
                    _.each(_.keys(conflict.condition), function(condition) {
                        panes.push(condition.split('.')[0]);
                        conditions.push(condition);
                    }, this);
                }, this);
                _.each(restrictions.depends, function(dep) {
                    _.each(_.keys(dep.condition), function(condition) {
                        panes.push(condition.split('.')[0]);
                        conditions.push(condition);
                    }, this);
                }, this);
            return [_.uniq(panes).join(','), _.uniq(conditions).join(',')];
        },
    ...
    views.WizardPane = Backbone.View.extend({
            ...
            _.each(this.config, function(attributeConfig, attribute) {
                if (attributeConfig.type == 'checkbox') {
                    controlsHtml += (controlTpl(_.extend(attributeConfig, {
                        ...
                        hasDescription: _.isUndefined(config.hasDescription) ? false : config.hasDescription ,
                        description: attributeConfig.description || ''
                    })));
                }
                else {
                    _.each(attributeConfig.values, function(value, valueIndex) {
                        var shouldBeAdded = _.isUndefined(config.additionalAttribute) ? true : attribute == config.additionalAttribute;
                        if (shouldBeAdded) {
                            controlsHtml += (controlTpl(_.extend(attributeConfig, {
                                value: value.data,
                                pane: attribute,
        ...
        composePaneBindings: function() {
            ...
            this.prepareDisabledBindings();
            this.stickit(this.wizard.model);
        },
        prepareDisabledBindings: function() {
            _.each(this.config, function(paneAttribute, paneName) {
                if (paneAttribute.type == 'checkbox') {
                    if (!_.isUndefined(paneAttribute.restrictions)) {
                        this.composeDisablesBindings(paneAttribute.restrictions, 'name', paneName);
                    }
                }
                var restrictions = _.compact(_.pluck(paneAttribute.values, 'restrictions'));
                if (restrictions.length) {
                    _.each(paneAttribute.values, function(value) {
                        if (!_.isUndefined(value.restrictions)) {
                            this.composeDisablesBindings(value.restrictions, 'value', value.data);
                        }
                    }, this);
                }
            }, this);
        },
        composeDisablesBindings: function(restrictions, attributeName, elementName) {
            _.each(restrictions.conflicts, function(conflict) {
                this.bindings['[' + attributeName + '=' + elementName +']'] = {
                    attributes: [{
                        name: 'disabled',
                        observe: _.keys(conflict.condition)[0],
                        onGet: function(value, options) {
                            return value == _.values(conflict.condition)[0];
                        }
                    }]
                };
            }, this);
            _.each(restrictions.depends, function(conflict) {
                this.bindings['[' + attributeName + '=' + elementName +']'] = {
                    attributes: [{
                        ...
                        observe: _.keys(conflict.condition)[0],
                        onGet: function(value, options) {
                            return !_.contains(value,_.values(conflict.condition)[0]);
                        }
                    }]
                };
            }, this);
        ...
        checkForRestrictions: function() {
            _.each(this.wizard.config, function(paneAttribute, paneName) {
                if (paneName == this.constructorName) {
                    _.each(paneAttribute, function(bindingAttribute, bindingKey) {
                        var restrictions = _.compact(_.pluck(bindingAttribute.values, 'restrictions'));
                        if (bindingAttribute.type == 'checkbox') {
                            if (!_.isUndefined(bindingAttribute.restrictions)) {
                                this.handleWarnings(bindingAttribute.restrictions);
                            }
                        }
                        if (restrictions.length) {
                            _.each(restrictions, function(restriction) {
                                this.handleWarnings(restriction);
                            }, this);
                        }
                    }, this);
                }
        ...
        handleWarnings: function(restrictions) {
            _.each(restrictions.conflicts, function(conflict) {
                if (this.wizard.model.get(_.keys(conflict.condition)[0]) == _.values(conflict.condition)[0]) {
                    this.showWarning($.t(conflict.warning, {releaseName: this.wizard.model.get('NameAndRelease.release').get('name')}));
                }
            }, this);
            _.each(restrictions.depends, function(conflict) {
    ...
    clusterWizardPanes.Compute = views.WizardPane.extend({
        ...
        render: function() {
            ...
            this.$('.control-group').append(this.renderControls({labelClasses: '', descriptionClasses: '', hasDescription: true})).i18n();
            this.composePaneBindings();
            return this;
        }
    });
