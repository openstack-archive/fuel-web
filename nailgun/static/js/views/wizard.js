    ...
    views.CreateClusterWizard = dialogs.Dialog.extend({
        ...
        calculateDependencies: function() {
            ...
            function processRestrictions(restriction) {
                var evaluatedRestriction = utils.evaluateExpression(_.keys(restriction)[0], {default: this.model}, {strict:false});
                this.trackedAttributes.push(_.keys(evaluatedRestriction.modelPaths)[0]);
            }
            _.each(this.config, function(paneConfig, paneName) {
                _.each(paneConfig, function(attributeConfig, attribute) {
                    _.each(attributeConfig.restrictions, processRestrictions, this);
                    if (attributeConfig.type == 'radio') {
                        _.each(attributeConfig.values, function(attributeValueConfig) {
                            _.each(attributeValueConfig.restrictions, processRestrictions, this);
                        }, this);
                    }
                }, this);
            }, this);
            this.trackedAttributes = _.uniq(this.trackedAttributes);
    ...
    views.WizardPane = Backbone.View.extend({
        ...
        composeDisabledBindings: function(restrictions, attributeName, elementName) {
            ...
            _.each(restrictions, function(restriction) {
                conditions.push(_.keys(restriction)[0]);
            });
            this.createDisabledBindings(conditions, attributeName, elementName);
        },
        createDisabledBindings: function(restrictedAttributes, attributeName, elementName) {
            var attributesToObserve = [];
            _.each(restrictedAttributes, function(attribute) {
            ...
            this.bindings['[' + attributeName + '=' + elementName +']'] = {
                ...
                attributes: [{
                    ...
                    onGet: function() {
                        var results = [];
                        _.each(restrictedAttributes, function(attribute) {
                            results.push(!utils.evaluateExpression(attribute, {default: this.wizard.model}).value);
                        }, this);
        ...
        handleWarnings: function(restrictionObject) {
            ...
            _.each(restrictionObject, function(restriction) {
                var evaluatedExpression = utils.evaluateExpression(_.keys(restriction)[0], {default: this.wizard.model});
                if (!evaluatedExpression.value) {
                    this.showWarning($.t(_.values(restriction)[0], translationParams));
                }
            }, this);
        },
