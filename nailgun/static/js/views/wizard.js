    ...
    views.CreateClusterWizard = dialogs.Dialog.extend({
        ...
        handleTrackedAttributeChange: function() {
            ...
            if (maxIndex == currentIndex) {
                // FIXME: very brude hack - needs change
                this.$('.alert').remove();
                var values = _.compact(_.pluck(this.activePane.config, 'values'));
                if (values.length) {
                    var elementsWithWarnings = _.where(values[0], 'warnings');
                    if (elementsWithWarnings.length) {
                        this.activePane.handleWarnings(elementsWithWarnings);
                    }
                }
                else {
                    var warnings = _.where(this.activePane.config, 'warnings');
                    if (warnings.length) {
                        this.activePane.handleWarnings(warnings);
                    }
                }
            }
        },
        calculateDependencies: function() {
            ...
            function processRestrictions(restriction) {
                var evaluatedRestriction = utils.evaluateExpression(restriction, {default: this.model}, {strict:false});
                this.trackedAttributes.push(_.keys(evaluatedRestriction.modelPaths));
            }
            _.each(this.config, function(paneConfig, paneName) {
                _.each(paneConfig, function(attributeConfig, attribute) {
                    _.each(_.keys(attributeConfig.restrictions), processRestrictions, this);
                    _.each(_.keys(attributeConfig.warnings), processRestrictions, this);
                    if (attributeConfig.type == 'radio') {
                        _.each(attributeConfig.values, function(attributeValueConfig) {
                            _.each(_.keys(attributeValueConfig.restrictions), processRestrictions, this);
                            _.each(_.keys(attributeValueConfig.warnings), processRestrictions, this);
                        }, this);
                    }
                }, this);
            }, this);
    ...
    views.WizardPane = Backbone.View.extend({
        ...
        composePaneBindings: function() {
            ...
            if (_.pluck(this.config, 'type')[0] == 'radio') {
                ...
                var elementsWithWarnings = _.where(_.pluck(this.config, 'values')[0], 'warnings');
                if (elementsWithWarnings.length) {
                    this.handleWarnings(elementsWithWarnings);
                }
            }
            if (_.pluck(this.config, 'type')[0] == 'checkbox') {
                ...
                var warnings = _.compact(_.pluck(this.config, 'restrictions'));
                if (warnings.length) {
                    this.handleWarnings(warnings[0]);
                }
            }
        ...
        handleWarnings: function(warningsElementsArray) {
            _.each(warningsElementsArray, function (elementWithWarning) {
                 _.each(elementWithWarning.warnings, function(warning, condition) {
