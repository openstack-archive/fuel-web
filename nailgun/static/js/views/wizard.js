...
function(require, utils, models, dialogs, createClusterWizardTemplate, clusterNameAndReleasePaneTemplate, commonWizardTemplate, modePaneTemplate, storagePaneTemplate, clusterReadyPaneTemplate, controlTemplate, warningTemplate, wizardInfo) {
    ...
    views.CreateClusterWizard = dialogs.Dialog.extend({
        ...
        getMasterPaneAttributes: function(restrictions) {
            var panes = [],
                conditions = [];
            panes.push(this.getConditionsFromRestrictions(restrictions.conflicts)[0]);
            panes.push(this.getConditionsFromRestrictions(restrictions.depends)[0]);
            conditions.push(this.getConditionsFromRestrictions(restrictions.conflicts)[1]);
            conditions.push(this.getConditionsFromRestrictions(restrictions.depends)[1]);
            return [_.compact(_.uniq(panes))[0], _.compact(_.uniq(conditions))[0]];
        },
            ...
            _.each(restrictions.conflicts, function(conflict) {
                this.bindings['[' + attributeName + '=' + elementName +']'] = {
                    ...
                    onSet: function(value) {
                        return (attributeName == 'name') ? !_.isNull(value) : value;
                    },
                    attributes: [{
                        name: 'disabled',
        ...
        handleWarnings: function(restrictions) {
            _.each(restrictions.conflicts, function(conflict) {
                if (this.wizard.model.get(_.keys(conflict.condition)[0]) == _.values(conflict.condition)[0]) {
                    this.showWarning($.t(conflict.warning, {
                        releaseName: this.wizard.model.get('NameAndRelease.release').get('name'),
                        hypervisor: this.wizard.model.get('Compute.hypervisor')
                    }));
                }
            }, this);
            _.each(restrictions.depends, function(conflict) {
