...
function(require, utils, models, dialogs, warningTemplate, controlTemplate, createClusterWizardTemplate, clusterNameAndReleasePaneTemplate, clusterModePaneTemplate, commonWizardTemplate, clusterStoragePaneTemplate, clusterReadyPaneTemplate, rhelCredentialsDialogTemplate, wizardInfo) {
    ...
    views.WizardPane = Backbone.View.extend({
        configKey: null,
        configurableOptions: {},
        model: new models.WizardPaneModel(),
        ...
        renderConfigurablePane: function(paneName, valueKey, selector, isAppending) {
             var orderedMeaningfulKeys = this.currentRestrictions.toJSON().order,
                renderingObject = {},
                sortedConfig = _.sortBy(this.currentRestrictions.toJSON(), 'weight');
            _.each(orderedMeaningfulKeys, _.bind(function(name, index) {
                renderingObject[name] = sortedConfig[index];
            }, this));
            _.each(renderingObject, _.bind(function(plugin, key) {
                var resultingLayout = this.renderControl(
                     {
                         pane: paneName || key,
                         value: valueKey || key,
                         label: (!_.isUndefined(plugin.label)) ? $.t(plugin.label) : '',
                         description: (!_.isUndefined(plugin.description)) ? $.t(plugin.description) : false,
                         type: plugin.type,
                         label_classes: '',
                         description_classes: ''
                     }
                ...
                if (isAppending) {
                     this.$(selector).append(resultingLayout);
                }
                else {
                     this.$(selector).after(resultingLayout);
                }
            }, this));
        }
    });
    ...
    clusterWizardPanes.ClusterComputePane = views.WizardPane.extend({
        ...
        render: function() {
            ...
            this.renderConfigurablePane('hypervisor', null, '.control-group', true);
    ...
    clusterWizardPanes.ClusterNetworkPane = views.WizardPane.extend({
        ...
        render: function() {
            ...
            this.renderConfigurablePane('manager', null, '.control-group', true);
            if (disabled && release) {
    ...
    clusterWizardPanes.ClusterStoragePane = views.WizardPane.extend({
        ...
        render: function() {
            ...
            if (!_.isEmpty(this.model.toJSON())) {
                _.each(_.uniq(this.currentRestrictions.attributesToTrack), _.bind(function(attribute) {
                    _.each(_.uniq(this.currentRestrictions.valuesToTrack), _.bind(function(value) {
                        if (!_.contains(this.model.get(attribute), value)) {
                            disabled = true;
                        }
                    }, this));
                }, this));
            ...
            _.each(['cinder', 'glance'], _.bind(function(image) {
                this.renderConfigurablePane(image, null, 'h5[data-i18n="dialog.create_cluster_wizard.storage.' + image + '"]', false);
            }, this));
            if (disabled && release) {
                var currentWarnings = this.getWarnings(this.configurableOptions);
                _.each(_.filter(_.uniq(currentWarnings)), _.bind(function(message) {
    ...
    clusterWizardPanes.ClusterAdditionalServicesPane = views.WizardPane.extend({
        ...
        render: function() {
            ...
            this.renderConfigurablePane(null, null, '.control-group', true);
            var currentWarnings = this.getWarnings(this.configurableOptions);
