...
function(require, utils, models, dialogs, createClusterWizardTemplate, clusterNameAndReleasePaneTemplate, clusterModePaneTemplate, clusterComputePaneTemplate, clusterNetworkPaneTemplate, clusterStoragePaneTemplate, clusterAdditionalServicesPaneTemplate, clusterReadyPaneTemplate, rhelCredentialsDialogTemplate, wizardInfo) {
    ...
    views.WizardPane = Backbone.View.extend({
        ...
        isDisabledDueToRelease: function(paneAttr) {
            var pluginCollection = new Backbone.Collection(paneAttr.plugins);
            var disabledDueToRelease = [];
            pluginCollection.each(_.bind(function(element) {
                var pluginConflicts = element.get('restrictions').conflicts;
                var wizardPanel = this.wizard.findPane(clusterWizardPanes[pluginConflicts.panelName]);
                if (_.isUndefined(wizardPanel[pluginConflicts.attributeName])) return;
                disabledDueToRelease.push(wizardPanel[pluginConflicts.attributeName].get(pluginConflicts.attributeType) == pluginConflicts.value);
            }, this));
            return _.filter(disabledDueToRelease).length;
        },
        isDisabledDueToNetworkMode: function(paneAttr) {
            var pluginCollection = new Backbone.Collection(paneAttr.plugins);
            var disabledDueToNetworkMode = [];
            pluginCollection.each(_.bind(function(element) {
                var pluginConflicts = element.get('restrictions').conflicts;
                var wizardPanel = this.wizard.findPane(clusterWizardPanes[pluginConflicts.panelName]);
                if (_.isUndefined(wizardPanel[pluginConflicts.attributeName])) return;
                disabledDueToNetworkMode.push(wizardPanel[pluginConflicts.attributeName] == pluginConflicts.value);  // no Murano for Nova Network
            }, this));
            return _.filter(disabledDueToNetworkMode).length;
        }
    });
    ...
    clusterWizardPanes.ClusterNetworkPane = views.WizardPane.extend({
        ...
        render: function() {
            ...
            var disabledDueToRelease = this.isDisabledDueToRelease(paneAttributes);
    ...
    clusterWizardPanes.ClusterAdditionalServicesPane = views.WizardPane.extend({
        ...
        render: function() {
            ...
            var release = this.wizard.findPane(clusterWizardPanes.ClusterNameAndReleasePane).release;
            var disabledDueToRelease = this.isDisabledDueToRelease(paneAttributes);
            var disabledDueToNetworkMode = this.isDisabledDueToNetworkMode(paneAttributes);
            var services = [
                {
                    name: 'savanna',
            ...
            this.$el.html(this.template({
                ...
                release: release,
                services: services,
                warning: $.t(pluginCollection.first().get('restrictions').conflicts.warning)
            })).i18n();
            if (disabledDueToRelease) {
