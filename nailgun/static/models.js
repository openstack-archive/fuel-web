    ...
    models.Task = BaseModel.extend({
        ...
        match: function(filters) {
            ...
            if (!_.isEmpty(filters)) {
                ...
                if (filters.status && !_.contains(utils.composeList(filters.status), this.get('status'))) {
                    return false;
                }
                if (_.isBoolean(filters.active)) {
                    var isTaskActive = _.contains(['running', 'pending'], this.get('status'));
                    if (filters.active ? !isTaskActive : isTaskActive) return false;
                }
            }
            return true;
        },
        ...
        isStoppableTask: function() {
            return this.match({name: 'deploy', active: true});
        }
    });
