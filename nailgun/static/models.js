...
define([
    ...
    models.NetworkConfiguration = BaseModel.extend(cacheMixin).extend({
        ...
        validate: function(model, nodeNetworkGroups) {
            var errors = {},
                networkingParametersErrors = {},
                ns = 'cluster_page.network_tab.validation.',
                networks = model.get('networks'),
                networkParameters = model.get('networking_parameters'),
                networkGroupsErrors = {};

            if (nodeNetworkGroups) {
                nodeNetworkGroups.map(function(nodeNetworkGroup) {
                    ...
                    if (!_.isEmpty(nodeNetworkGroupErrors)) {
                        networkGroupsErrors[nodeNetworkGroup.id] = nodeNetworkGroupErrors;
                    }
                }, this);

                if (!_.isEmpty(networkGroupsErrors)) {
                    errors.networks = networkGroupsErrors;
                }
            }
            ...
            var networkWithFloatingRange = model.get('networks').filter(function(network) {return network.get('meta').floating_range_var;})[0];
            if (networkWithFloatingRange && !_.has(errors.networks &&
                    errors.networks[networkWithFloatingRange.get('group_id')] &&
                    errors.networks[networkWithFloatingRange.get('group_id')][networkWithFloatingRange.id], 'cidr')) {
            ...
            var baremetalNetwork = networks.findWhere({name: 'baremetal'});
