...
define([
    ...
    models.Nodes = BaseCollection.extend({
        ...
        resources: function(resourceName) {
            let resources = this.map(function(node) {return node.resource(resourceName);});
    ...
    models.Disk = BaseModel.extend({
        ...
        getUnallocatedSpace: function(options) {
            ...
            let allocatedSpace = volumes.reduce(function(sum, volume) {return volume.get('name') == options.skip ? sum : sum + volume.get('size');}, 0);
    ...
    models.Interface = BaseModel.extend({
        ...
        validate: function(attrs) {
            ...
            let untaggedNetworks = networks.filter(function(network) { return _.isNull(network.getVlanRange(attrs.networkingParameters)); });
