...
models.NetworkConfiguration = BaseModel.extend(cacheMixin).extend({
  ...
  validateNeutronFloatingRange(floatingRanges, networks, networkErrors) {
    ...
    return utils.validateIPRanges(
      floatingRanges,
      networkToCheck ? networkToCheck.get('cidr') : null,
  ...
  validateNetwork(network, networksToCheck, novaNetworking = false) {
    var cidr = network.get('cidr');
    var errors = utils.validateCidr(cidr);
  ...
  validateNovaNetworkParameters(parameters, networks, manager) {
    var errors = utils.validateCidr(parameters.get('fixed_networks_cidr'), 'fixed_networks_cidr');
  ...
  validateBaremetalNetwork(cidr, networkingParameters) {
    var errors = utils.validateGateway(
