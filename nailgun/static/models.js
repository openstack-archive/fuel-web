...
define([
    ...
    models.Settings = Backbone.DeepModel.extend(superMixin).extend(cacheMixin).extend(restrictionMixin).extend({
        ...
        root: 'editable',
        cacheFor: 60 * 1000,
        predefinedGroupList: ['general', 'security', 'compute', 'network', 'storage', 'logging', 'openstack_services', 'other'],
        isNew: function() {
            return false;
        },
        parse: function(response) {
        ...
        getGroupList: function() {
            ...
            _.each(this.attributes, function(section) {
                if (section.metadata.group) {
                    ...
                    _.each(section, function(setting, name) {
                        if (name != 'metadata') groups.push(setting.group || 'other');
                    });
                }
            });
            return _.intersection(this.predefinedGroupList, _.uniq(groups));
        }
    });
