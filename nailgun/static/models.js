...
define([
    ...
    models.Node = BaseModel.extend({
        ...
        getRolesSummary: function(roles) {
            return _.map(this.sortedRoles(_.pluck(roles, 'name')), function(role) {
                return _.find(roles, function(roleData) { return roleData.get('name') == role;}).get('label');
            }).join(', ');
        },
