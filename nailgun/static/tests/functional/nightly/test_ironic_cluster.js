    ...
    return {
      ...
      'Baremetal Network "IP Ranges" adding and deletion additional fields': function() {
        return this.remote
          ...
          .assertElementEnabled(divIp + 'div.range-row[data-reactid$="$1"] input[name*="range-start"]',
            'Baremetal new "Start IP Range" textfield is enabled')
          .assertElementEnabled(divIp + 'div.range-row[data-reactid$="$1"] input[name*="range-end"]',
          ...
          .assertElementEnabled(divIp + 'div.range-row[data-reactid$="$1"] button.ip-ranges-delete',
            'Delete IP range button is enabled')
          .clickByCssSelector(divIp + 'div.range-row[data-reactid$="$1"] button.ip-ranges-delete')
      ...
      'Baremetal Network "CIDR" field validation': function() {
        ...
        return this.remote
          ...
          .assertElementContainsText('div[data-reactid$="$baremetal-net"] div.validation-error span[class^="help"]',
            'IP address does not match the network CIDR', 'True error message is displayed')
          .assertElementExists('a[class$="default"]', '"Default" link is existed')
          .clickByCssSelector('a[class$="default"]')
          .then(function() {
            return networksLib.checkIncorrectValueInput();
      ...
      'Baremetal Network "Use the whole CIDR" option works': function() {
        return this.remote
          ...
          .assertElementPropertyEquals(divIp + 'input[name*="range-start"]', 'value', '192.168.3.1',
            'Baremetal "Start IP Range" textfield  has true value')
          .assertElementPropertyEquals(divIp + 'input[name*="range-end"]', 'value', '192.168.3.254',
