    ...
    return {
      ...
      'User returns to the selected segment on "Networks" tab': function() {
        return this.remote
          ...
          .assertElementExists('div.network-tab div.network-tab-content', '"Networks" tab is not empty')
          .assertElementTextEquals('ul.node-network-groups-list li.group-title', 'Node Network Groups',
            '"Node Network Groups" segment opens by default')
          .assertElementExists('li.active a.subtab-link-default', '"Default" node network group opens by default')
      ...
      'Check "Node Network Groups" segment on "Networks" tab': function() {
        return this.remote
          .then(function() {
            return networksLib.checkNetrworkInitialState('Public');
          })
          .then(function() {
            return networksLib.checkNetrworkInitialState('Storage');
          })
          .then(function() {
            return networksLib.checkNetrworkInitialState('Management');
          });
      },
      ...
      'Success network verification exists only on "Network Verification" segment': function() {
        return this.remote
          ...
          .assertElementContainsText('div.alert-success', 'Verification succeeded', 'True message is observed')
          .assertElementContainsText('div.alert-success', 'Your network is configured correctly', 'True msg observed')
      ...
      'Failed network verification presents on each subtab on "Networks" tab': function() {
        return this.remote
          .clickByCssSelector('.subtab-link-default')
          .setInputValue('div.public input[name=gateway]', '172.16.0.2')
          .clickByCssSelector('.subtab-link-network_verification')
          .assertElementEnabled('.verify-networks-btn', '"Verify Networks" is enabled')
          .clickByCssSelector('.verify-networks-btn')
          ...
          .assertElementContainsText('div.alert-danger',
            'Address intersection between public gateway and IP range of public network', 'True message is observed')
          .clickByCssSelector('.subtab-link-default')
    ...
    return {
      ...
      'Check "Node Network Groups" segment on "Networks" tab': function() {
        return this.remote
          ...
          .then(function() {
            return networksLib.checkNetrworkInitialState('Private');
          });
      },
      'Check "Settings" segment on "Networks" tab': function() {
        return this.remote
