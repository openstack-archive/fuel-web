                ...
                return this.remote
                    ...
                    .findByCssSelector('.sticker button')
                        .then(function(button) {
                            ...
                            return showLogsButton.isEnabled().then(function(isEnabled) {
                                assert.isFalse(isEnabled, '"Show" button should be disabled until source shange');
                                return true;
                            });
                        })
                        .end()
                    ...
                    .findAllByCssSelector('.log-entries tr:first-child td:last-child')
                        .then(function(elements) {
                            return assert.ok(elements.length, 'Log tab entries are not presented');
                        })
                        .end()
                    ...
                    .findAllByCssSelector('.sticker select[name=node] > option')
                        .then(function(elements) {
                            return assert.ok(elements.length, '"Node" dropdown is not presented');
                        })
                        .end()
            }
        };
