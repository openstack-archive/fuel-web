...
define([
  ...
  registerSuite(function() {
    ...
    return {
      ...
      'Show all node network groups': function() {
        return this.remote
          .clickByCssSelector('.btn.show-all-networks')
          .waitForCssSelector('.node_network_groups li.active.show-all', 2000)
          .assertElementTextEquals(
            '.node_network_groups li.active',
            'Networks',
            'Active pill is called "Networks"'
          )
          .assertElementsExist(
            '.network-group-name',
            2,
          ...
          .assertElementTextEquals(
            ...
            'Button text is changed to "Show Network Groups"'
          )
          .then(function() {
          ...
          .clickByCssSelector('.temp.network-group-name .glyphicon-remove')
          .then(function() {
            return modal.waitToOpen();
          })
          .then(function() {
          ...
          .waitForElementDeletion('.temp.network-group-name', 2000)
          .clickByCssSelector('.btn.show-all-networks')
          .assertElementsExist(
          ...
          .getCurrentUrl()
            .then(function(url) {
              assert.include(
                url,
                'network/group/2',
                'Subtab url is changed after clicking  "Show all node groups"'
              );
            });
      },
