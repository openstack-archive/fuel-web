        ...
        return {
            ...
            'Testing cluster networks: change network manager': function() {
                return this.remote
                    ...
                    .findAllByCssSelector('input[name=fixed_networks_amount]')
                        .then(function(elements) {
                            assert.equal(elements.length, 0, 'Amount field was hidden after revert to FlatDHCP');
                        })
                        .end()
                    .findAllByCssSelector('select[name=fixed_network_size]')
                        .then(function(elements) {
                            assert.equal(elements.length, 0, 'Size field was hidden after revert to FlatDHCP');
                        })
                        .end()
                    .then(function() {
                        return networksPage.isApplyButtonEnabled()
            ...
            'Testing cluster networks: save changes': function() {
                return this.remote
                    ...
                    .findByCssSelector('input[name=net_provider]:not(:checked)')
                        ...
                        .end()
                    .then(function() {
                        networksPage.getApplyButton()
                            .click()
                            .end()
                    })
                    .findAllByCssSelector('input:not(:disabled)')
                        .then(function(elements) {
                            assert.ok(elements.length > 0, 'Input is not disabled');
                        })
                        .end()
                    .findAllByCssSelector('.alert-error')
                        .then(function(elements) {
                            assert.equal(elements.length, 0, 'Correct settings were saved successfully');
                        })
                        .end();
            },
            ...
            'Testing cluster networks: verification task deletion': function() {
                return this.remote
                    ...
                    .findAllByCssSelector('.page-control-box .alert')
                        .then(function(elements) {
                            assert.equal(elements.length, 0, 'Verification task was removed after settings has been changed');
                        })
                        .end();
            }

            //@TODO: the tests below should be uncommented and changed after modal manipulation method merged
