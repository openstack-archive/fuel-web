    ...
    registerSuite(function() {
        ...
        return {
            ...
            'Test steps manipulations': function() {
                return this.remote
                    .then(function() {
                        return common.elementExists('.wizard-step.active', 'There is only one active and available step at the beginning');
                    })
                    // Compute
                    .pressKeys('\uE007')
                    // Network
                    ...
                    .findAllByCssSelector('.wizard-step.available')
                        ...
                        .end()
                    .clickLinkByText('.wizard-step a', 'Compute')
                    .clickOnElement('input[value=kvm]')
                    .findAllByCssSelector('.wizard-step.available')
                        .then(function(elements) {
                            return assert.isTrue(elements.length == 5, 'All steps are available if no changes to restrictions are made');
                        })
                        .end()
                    .clickOnElement('input[type=checkbox]')
                    .findAllByCssSelector('.wizard-step.available')
                        .then(function(elements) {
