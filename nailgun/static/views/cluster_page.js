  ...
  statics: {
    ...
    breadcrumbsPath(pageOptions) {
      ...
      var breadcrumbs = [
        ...
        [
          i18n('cluster_page.tabs.' + activeTab),
          '#cluster/' + cluster.id + '/' + activeTab,
          {active: !subtab}
        ]
      ];
      if (subtab) {
  ...
  checkTabSubroute(props = {}) {
    ...
    if (tab.checkSubroute) {
      this.setState(
        tab.checkSubroute(
          props.tabOptions,
          _.pick(props, 'cluster', 'nodeNetworkGroups', 'defaultLogLevel')
        )
      );
    }
  },
          ...
          <div className='tabs'>
            {tabUrls.map((tabUrl) => {
              var url = '#cluster/' + cluster.id + '/' + tabUrl +
                (activeSectionNames[tabUrl] ? '/' + activeSectionNames[tabUrl] : '');
              return (
                <a
                  key={tabUrl}
                  className={tabUrl + ' ' + utils.classNames({
                    'cluster-tab': true,
