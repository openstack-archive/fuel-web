  ...
  statics: {
    ...
    breadcrumbsPath(pageOptions) {
      ...
      var breadcrumbs = [
        ...
        [i18n('cluster_page.tabs.' + pageOptions.activeTab), '#cluster/' + cluster.get('id') +
          '/' + pageOptions.activeTab, {active: !addScreenBreadcrumb}]
      ];
      ...
      if (currentClusterId == id) {
        ...
        promise = $.when(
          ...
          .then(() => {
            ...
            return $.when(cluster.get('networkConfiguration').fetch(),
    ...
    return (
      <div className='cluster-page' key={cluster.id}>
        <div className='page-title'>
          <h1 className='title'>
            {cluster.get('name')}
            <div className='title-node-count'>({i18n('common.node',
              {count: cluster.get('nodes').length})})
            </div>
          </h1>
        </div>
          ...
          <div className='tabs'>
            {tabUrls.map((url) => {
              return (
                <a
                  key={url}
                  className={url + ' ' + utils.classNames({'cluster-tab': true,
