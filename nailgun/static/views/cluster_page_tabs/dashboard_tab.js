...
var DashboardTab = React.createClass({
  ...
  render() {
    ...
    var runningDeploymentTask = cluster.task('deploy') ?
        cluster.task({name: 'deploy', active: true})
      :
    ...
    if (name !== cluster.get('name')) {
      ...
      if (deferred) {
        ...
        deferred
          ...
          .done(() => dispatcher.trigger('updatePageLayout'))
          .always(() => {
...
var ResetEnvironmentAction = React.createClass({
  ...
  getDescriptionKey() {
    ...
    if (cluster.get('status') === 'new') return 'no_changes_to_reset';
    return 'reset_environment_description';
  },
  render() {
    var {cluster, task} = this.props;
    var isLocked = cluster.get('status') === 'new' || !!task;
    return (
