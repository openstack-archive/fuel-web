...
import {DiscardNodeChangesDialog, DeployChangesDialog, ProvisionVMsDialog,
  RemoveClusterDialog, ResetEnvironmentDialog, StopDeploymentDialog} from 'views/dialogs';
    ...
    return (
      <div className={utils.classNames(classes)}>
        ...
        <div className={utils.classNames({'task-result-details': true, hidden: !details})}>
          <pre className='collapse result-details'
    ...
    return (
      <div className='row content-elements'>
        ...
        <div className='documentation col-xs-12'>
          {isMirantisIso ?
            [
              this.renderDocumentationLinks('https://www.mirantis.com/openstack-documentation/',
                'mos_documentation'),
              this.renderDocumentationLinks(utils.composeDocumentationLink('plugin-dev' +
                '.html#plugin-dev'), 'plugin_documentation'),
              this.renderDocumentationLinks('https://software.mirantis.com/' +
                'mirantis-openstack-technical-bulletins/', 'technical_bulletins')
            ]
          :
            [
              this.renderDocumentationLinks('http://docs.openstack.org/',
                'openstack_documentation'),
              this.renderDocumentationLinks('https://wiki.openstack.org/wiki/Fuel/Plugins',
...
var DeployReadinessBlock = React.createClass({
  ...
  validations: [
    ...
    function(cluster) {
      ...
      var limitValidations = _.zipObject(validRoleModels.map((role) =>
        [role.get('name'), role.checkLimits(configModels, cluster.get('nodes'))]));
      var limitRecommendations = _.zipObject(validRoleModels.map((role) =>
        [role.get('name'), role.checkLimits(configModels, cluster.get('nodes'), true,
          ['recommended'])]));
      return {
  ...
  render() {
    ...
    var isVMsProvisioningAvailable = nodes.any((node) => node.get('pending_addition') &&
      node.hasRole('virt'));
          ...
          <div className='col-xs-3 changes-list'>
            {nodes.hasChanges() &&
              <div>
                ...
                <ul>
                  {this.renderChangedNodesAmount(nodes.where({pending_addition: true}),
                    'added_node')}
                  {this.renderChangedNodesAmount(nodes.where({status: 'provisioned'}),
                    'provisioned_node')}
                  {this.renderChangedNodesAmount(nodes.where({pending_deletion: true}),
                    'deleted_node')}
                </ul>
              </div>
            }
            ...
            :
              <button
                className={utils.classNames({
                  ...
                  'btn-warning': _.isEmpty(alerts.blocker) && (!_.isEmpty(alerts.error) ||
...
var ClusterInfo = React.createClass({
  ...
  renderStatistics() {
    ...
    var fieldStatuses = ['offline', 'error', 'pending_addition', 'pending_deletion', 'ready',
