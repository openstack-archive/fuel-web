...
var ClusterActionsPanel = React.createClass({
            ...
            return cluster.needsRedeployment() && {
              error: [
                this.renderInstruction(
                  'unsuccessful_deploy',
                  false,
                  'operations.html#troubleshooting',
                  'user_guide'
                )
              ]
            };
        ...
        return [
          ...
          function(cluster) {
            if (cluster.get('nodes').any(
              (node) => node.isAvailableForProvisioning() && !node.get('online')
            )) {
              return {blocker: [i18n(ns + 'offline_nodes')]};
            }
          }
  ...
  renderInstruction(description, isAlert = false, link, linkTitle, explanation) {
    return (
  ...
  renderActionControls() {
    ...
    var blockerDescriptions = {
      provision: this.renderInstruction('provisioning_cannot_be_started', true),
      spawn_vms: this.renderInstruction('provisioning_cannot_be_started', true),
      deploy: this.renderInstruction(
        'deployment_cannot_be_started',
        true,
        'user-guide.html#add-nodes-ug',
        'user_guide',
        'for_more_information_roles'
      )
    };
    var alertList = (
    ...
    switch (action) {
      ...
      case 'provision':
        var nodesToProvision = nodes.filter((node) => node.isAvailableForProvisioning());
        return [
      ...
      <ul className='nav navbar-nav navbar-right'>
        ...
        <li className='dropdown'>
          <button className='btn btn-link dropdown-toggle' data-toggle='dropdown'>
            {i18n(ns + 'actions.' + this.state.currentAction + '.title')}
