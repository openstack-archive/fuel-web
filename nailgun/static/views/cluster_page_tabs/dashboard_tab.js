        ...
        validate(cluster) {
            return _.reduce(
                ...
                function(accumulator, validator) {
                    return _.merge(accumulator, validator.call(this, cluster), (a, b) =>
        ...
        validations: [
            ...
            function(cluster) {
                var configModels = this.getConfigModels(),
                    ...
                    validRoleModels = roleModels.filter((role) =>
                        !role.checkRestrictions(configModels).result
                    ),
                    limitValidations = _.zipObject(validRoleModels.map((role) =>
                        [role.get('name'), role.checkLimits(configModels, cluster.get('nodes'))]
                    )),
                    limitRecommendations = _.zipObject(validRoleModels.map((role) =>
                        [role.get('name'), role.checkLimits(configModels, cluster.get('nodes'), true, ['recommended'])]
                    ));
                return {
