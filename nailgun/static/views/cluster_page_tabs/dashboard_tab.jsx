...
function(_, i18n, $, React, utils, models, dispatcher, dialogs, componentMixins, controls) {
    ...
    var DashboardTab = React.createClass({
        ...
        renderAlertBlock: function(text) {
            return (
        ...
        render: function() {
            var cluster = this.props.cluster,
                ...
                title = this.getTitle(),
                finishedDeploymentTask = cluster.task({group: 'deployment', status: ['ready', 'error']}),
                ...
                <div className='row'>
                    ...
                    <div className='col-xs-12'>
                        {release.get('state') == 'unavailable' &&
                            this.renderAlertBlock(i18n('cluster_page.unavailable_release', {name: release.get('name')}))
                        }
                        {cluster.get('is_customized') &&
                            this.renderAlertBlock(i18n('cluster_page.cluster_was_modified_from_cli'))
                        }
                        <DeploymentResult task={finishedDeploymentTask} />
                                ...
                                !hasNodes &&
                                    <div className='introduction-block'>
                                        ...
                                        <InstructionElement
                                            ...
                                            />
                                        <AddNodesButton
                                            disabled={!!runningDeploymentTask}
                    ...
                    <div className='validation-result'>
                        {
                            _.map(['danger', 'warning'], function(severity) {
                                if (_.isEmpty(result[severity]) || (this.props.hasNoNodes && severity == 'warning')) return null;
                                return (
        ...
        render: function() {
            ...
            return (
                <div className='deploy-readiness'>
                    ...
                    <DeploymentControlsBlock
                        isVMsProvisioningAvailable={isVMsProvisioningAvailable}
                    ...
                    <ErrorsBlock
                        deploymentImpossible={!isDeploymentPossible}
                        hasDeploymentError={!!this.props.deploymentErrorTask}
                        needsRedeployment={cluster.needsRedeployment()}
                        alerts={alerts}
                        hasNoNodes={!nodes.length}
                    />
                </div>
            );
        }
    ...
    var ErrorsBlock = React.createClass({
        render: function() {
            var alerts = this.props.alerts,
                hasNoNodes = this.props.hasNoNodes;
            return (
                <div className='recommendations-warnings-block'>
                    {this.props.deploymentImpossible &&
                        <div className='informational-block'>
                            {this.props.hasDeploymentError &&
                                <InstructionElement
                    ...
                    {!_.isEmpty(alerts.warning) &&
                        [
                            <p className='recommendations-title'>{i18n(namespace + 'note_recommendations')}</p>,
                            <WarningsBlock
                                key='warning'
                                hasNoNodes={hasNoNodes}
    ...
    var DeploymentControlsBlock = React.createClass({
        render: function() {
            var buttonOptions = {},
                buttonText;
            if (this.props.isVMsProvisioningAvailable) {
                buttonOptions.key = 'provision-vms';
                buttonOptions.onClick = _.partial(this.props.showDialog, dialogs.ProvisionVMsDialog);
                buttonText = i18n('cluster_page.provision_vms');
            } else if (this.props.isDeploymentPossible) {
                buttonOptions.key = 'deploy-changes';
                buttonOptions.onClick = _.partial(this.props.showDialog, dialogs.DeployChangesDialog);
                buttonText = i18n('cluster_page.deploy_changes');
            } else {
                return null;
            }
                        ...
                        <div className='col-xs-6'>
                            <div className='row'>
                                ...
                                <div className='col-xs-12 dashboard-actions-wrapper'>
                                    <CommonEnvironmentAction
                                        cluster={cluster}
                                        disabled={runningDeploymentTask}
                                        Dialog={dialogs.RemoveClusterDialog}
                                        keyName='delete'
                                    />
                                    {!this.props.isNew &&
                                        <CommonEnvironmentAction
                                            ...
                                            task={task}
                                            disabled={runningDeploymentTask}
                                            Dialog={dialogs.ResetEnvironmentDialog}
                                            keyName='reset'
                                        />
                                    }
                                </div>
        ...
        render: function() {
            ...
            return (
                    <a
                        ...
                        href={'#cluster/' + this.props.clusterId + '/nodes/add'}
                        disabled={disabled}
                    >
    ...
    var CommonEnvironmentAction = React.createClass({
        mixins: [
            componentMixins.backboneMixin('cluster'),
            componentMixins.backboneMixin('task')
        ],
        getResetDescriptionKey: function() {
            var task = this.props.task;
            if (task) {
        ...
        applyAction: function(e) {
            e.preventDefault();
            this.props.Dialog.show({cluster: this.props.cluster});
        },
        render: function() {
            var classes,
                buttonText,
                tooltipText,
                buttonClasses = {'btn btn-default': true};
            switch (this.props.keyName) {
                case 'reset':
                    classes = 'reset-environment pull-right';
                    buttonClasses = _.extend(buttonClasses, {'reset-environment-btn': true});
                    buttonText = i18n(namespace + 'reset_environment');
                    tooltipText = !this.props.disabled ?
                        i18n(namespace + 'reset_environment_warning') : i18n(namespace + this.getResetDescriptionKey());
                break;
                case 'delete':
                    classes = 'delete-environment pull-left';
                    buttonClasses = _.extend(buttonClasses, {'delete-environment-btn': true});
                    buttonText = i18n(namespace + 'delete_environment');
            ...
            return (
                <div className={classes}>
                    <button
                        className={utils.classNames(buttonClasses)}
                    ...
                    <controls.Tooltip
                        key={this.props.keyName + '_tooltip'}
                        placement='right'
