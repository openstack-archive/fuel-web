    ...
    var DashboardTab = React.createClass({
        ...
        render: function() {
            var cluster = this.props.cluster,
                ...
                hasOfflineNodes = !!nodes.where({online: false}).length;

            return (
                <div>
                    {isOperational &&
                        <div className='row'>
                            <div className='col-xs-12 plugin-entry'>
                                <div className='title'>{i18n(namespace + 'horizon')}</div>
                                <div className='description'>{i18n(namespace + 'horizon_description')}</div>
                                <a className='link' target='_blank' href={'http://' + cluster.get('networkConfiguration').get('public_vip')}>
                                    {cluster.get('networkConfiguration').get('public_vip')}
                                </a>
                            </div>
                        </div>
                    }
                    ...
                    {((failedDeploymentTask || !isNew || stopDeploymentTask) && hasOfflineNodes) &&
                    ...
                    {(!failedDeploymentTask && isNew || hasChanges) &&
                        <div className='row'>
                            ...
                            {hasNodes && runningDeploymentTask ?
                                <DeploymentInProgressControl
                                    cluster={cluster}
                                    task={runningDeploymentTask}
                                />
                            :
                                !!((isNew && !failedDeploymentTask) || stopDeploymentTask || hasChanges) &&
    ...
    var DeploymentInProgressControl = React.createClass({
        ...
        render: function() {
            var task = this.props.task,
                ...
                isInfiniteTask = task.isInfiniteTask(),
                    ...
                    <div className={'deploy-process ' + this.props.taskName}>
                        <div>
                            <span>
                                ...
                                {_.capitalize(taskName) + i18n(namespace + 'dots')}
                            </span>
                        </div>
            ...
            return (
                <div className='row content-elements'>
                    ...
                    <div className='documentation col-xs-12'>
                        {isMirantisIso && this.renderDocumentationLinks('https://www.mirantis.com/openstack-documentation/', 'mos_documentation')}
    ...
    var ClusterInfo = React.createClass({
                    ...
                    return numberOfNodes ?
                        <div key={field}>
                            <div className='col-xs-10'>
                                <div className='cluster-info-title'>
                                    {isRole && field != 'total' ? this.props.cluster.get('roles').find({name: field}).get('label') : i18n(namespace + 'cluster_info_fields.' + field)}
                                </div>
                            </div>
        ...
        renderStatistics: function() {
            var hasNodes = !!this.props.cluster.get('nodes').length,
                ...
                fieldStatuses = ['offline', 'error', 'pending_addition', 'pending_deletion', 'operational'];
            return (
