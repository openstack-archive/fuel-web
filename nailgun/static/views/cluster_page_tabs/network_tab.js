...
var NetworkTab = React.createClass({
  ...
  statics: {
    ...
    getSubtabs(options) {
      return _.map(options.nodeNetworkGroups.where({cluster_id: options.cluster.id}),
          (nodeNetworkGroup) => String(nodeNetworkGroup.id)
        )
        .concat(
    ...
    checkSubroute(tabOptions, subrouteOptions) {
      ...
      if (subtabs && (!subtab || !_.contains(subtabs, subtab))) {
        app.navigate(
          'cluster/' + subrouteOptions.cluster.id + '/network/' + _.first(subtabs),
    ...
    RemoveNodeNetworkGroupDialog
      ...
      .done(() => {
        return nodeNetworkGroup
          ...
          .then(() => {
            this.updateInitialConfiguration();
            var defaultSubtabId = _.first(
              this.constructor.getSubtabs(_.pick(this.props, 'cluster', 'nodeNetworkGroups'))
            );
            app.navigate(
              '#cluster/' + this.props.cluster.id + '/network/' + defaultSubtabId,
              {trigger: true, replace: true}
            );
          });
      });
      ...
      .done(() => {
        ...
        return this.nodeNetworkGroups.fetch()
          ...
          .then(() => {
            this.updateInitialConfiguration();
            app.navigate(
              '#cluster/' + this.props.cluster.id + '/network/' + newNodeNetworkGroup.id,
              {trigger: true, replace: true}
            );
          });
      });
...
var NetworkSubtabs = React.createClass({
  ...
  render() {
    ...
    var networkSettingsSectionNames = _.difference(
      subtabs,
      nodeNetworkGroups.pluck('id').map(String),
          ...
          {this.renderClickablePills([
            {
              ...
              isInvalid: showVerificationResult &&
                !!cluster.task({name: 'verify_networks', status: 'error'})
            }
          ])}
        </CSSTransitionGroup>
      </div>
