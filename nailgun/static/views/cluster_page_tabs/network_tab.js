...
var defaultNetworkSubtabs = ['neutron_l2', 'neutron_l3', 'network_settings',
...
var Range = React.createClass({
  ...
  componentDidUpdate() {
    ...
    if (this.props.extendable && this.state.elementToFocus && this.getModel()
        .get(this.props.name).length) {
              ...
              return (
                <div className='range-row clearfix' key={index}>
                  ...
                  <Input
                    ...
                    value={range[1]}
                    onChange={_.partialRight(this.onRangeChange, attributeName, index)}
                    onFocus={_.partial(this.autoCompleteIPRange, rangeError &&
                     rangeError.start, range[0])}
                    disabled={this.props.disabled || !!this.props.autoIncreaseWith}
                    placeholder={rangeError.end ? '' : this.props.placeholder}
                    extraContent={!this.props.hiddenControls &&
...
var NetworkTab = React.createClass({
  ...
  componentDidMount() {
    ...
    this.props.cluster.get('tasks').on('change:status change:unsaved',
      this.destroyUnsavedNetworkVerificationTask, this);
  },
  ...
  isNetworkConfigurationChanged() {
    return !_.isEqual(this.state.initialConfiguration,
      this.props.cluster.get('networkConfiguration').toJSON());
  },
  isNetworkSettingsChanged() {
    return this.props.cluster.get('settings').hasChanges(this.state.initialSettingsAttributes,
  ...
  loadInitialConfiguration() {
    var networkConfiguration = this.props.cluster.get('networkConfiguration');
    networkConfiguration.get('networks')
      .reset(_.cloneDeep(this.state.initialConfiguration.networks));
    networkConfiguration.get('networking_parameters')
      .set(_.cloneDeep(this.state.initialConfiguration.networking_parameters));
  },
  loadInitialSettings() {
    var settings = this.props.cluster.get('settings');
    settings.set(_.cloneDeep(this.state.initialSettingsAttributes),
  ...
  updateInitialConfiguration() {
    this.setState({initialConfiguration:
      _.cloneDeep(this.props.cluster.get('networkConfiguration').toJSON())});
  },
  isLocked() {
  ...
  prepareIpRanges() {
    ...
    if (floatingRanges) {
      networkConfiguration.get('networking_parameters').set({floating_ranges:
        removeEmptyRanges(floatingRanges)});
    }
  },
  onManagerChange(name, value) {
    var networkConfiguration = this.props.cluster.get('networkConfiguration');
    var networkingParameters = networkConfiguration.get('networking_parameters');
    var fixedAmount = networkConfiguration.get('networking_parameters')
      .get('fixed_networks_amount') || 1;
  ...
  removeNodeNetworkGroup() {
    ...
    RemoveNodeNetworkGroupDialog
      ...
      .done(() => {
        this.props.setActiveNetworkSectionName(this.nodeNetworkGroups.find({is_default:
          true}).get('name'));
  ...
  addNodeNetworkGroup(hasChanges) {
    if (hasChanges) {
      utils.showErrorDialog({
        ...
        message: <div><i className='glyphicon glyphicon-danger-sign' /> {i18n(networkTabNS +
            ...
            <div className='col-xs-5 node-network-groups-controls'>
              {!isNovaEnvironment &&
                <button
                  ...
                  disabled={!!cluster.task({group: ['deployment', 'network'], active: true}) ||
                   this.state.actionInProgress}
                >
...
var NodeNetworkGroup = React.createClass({
  render() {
    var {cluster, networks, nodeNetworkGroup, nodeNetworkGroups, verificationErrors,
        ...
        {networks.map((network) => {
          return (
            <Network
              ...
              verificationErrorField={_.pluck(_.where(verificationErrors, {network: network.id}),
                'field')}
              currentNodeNetworkGroup={nodeNetworkGroup}
            />
          );
  ...
  renderClickablePills(sections, isNetworkGroupPill) {
    ...
    return (sections.map((groupName) => {
      ...
      if (groupName == 'neutron_l2') {
        isInvalid = !!_.intersection(NetworkingL2Parameters.renderedParameters,
          _.keys(networkParametersErrors)).length;
        ...
        isInvalid = !!_.intersection(NovaParameters.renderedParameters,
          _.keys(networkParametersErrors)).length;
        ...
        {(manager == 'VlanManager') ?
          <div>
            ...
            <Range
              ...
              extendable={false}
              autoIncreaseWith={parseInt(networkingParameters.get('fixed_networks_amount'),
                10) || 0}
              integerValue
              placeholder=''
              mini
...
var NetworkSettings = React.createClass({
  ...
  checkRestrictions(action, setting) {
    return this.props.cluster.get('settings').checkRestrictions(this.props.configModels,
      action, setting);
  },
  render() {
    var cluster = this.props.cluster;
    var settings = cluster.get('settings');
    var locked = this.props.locked || !!cluster.task({group: ['deployment', 'network'],
        active: true});
          ...
          _.chain(settings.attributes)
            ...
            .filter(
              (sectionName) => {
                ...
                return (section.metadata.group == 'network' || _.any(section,
                  {group: 'network'})) &&
            ...
            .map(
              (sectionName) => {
                ...
                return <SettingSection
                  {... _.pick(this.props, 'cluster', 'initialAttributes', 'settingsForChecks',
                    'configModels')}
                  key={sectionName}
            ...
            <div className='verification-box row'>
              <div className='verification-network-placeholder col-xs-10 col-xs-offset-2'>
                ...
                <div className='animation-box'>
                  {_.times(3, (index) => {
                    ++index;
                    return <div key={index} className={this.getConnectionStatus(task, index == 1) +
                      ' connect-' + index}></div>;
                  })}
                </div>
            ...
            <Table
              ...
              body={
                _.map(task.get('result'), (node) => {
                  ...
                  return [node.name || 'N/A', node.mac ||
                    'N/A', node.interface, absentVlans.join(', ')];
                })
              }
            />
          </div>
