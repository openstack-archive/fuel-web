...
function($, _, i18n, Backbone, React, models, dispatcher, utils, dialogs, componentMixins, controls, SettingSection) {
            ...
            return (
                <div>
                    <NodeNetworkGroupTitle
                        ...
                        setActiveNetworkSectionName={this.props.setActiveNetworkSectionName}
                        isLocked={!!cluster.task({group: ['deployment', 'network'], active: true})}
                    />
    ...
    var NodeNetworkGroupTitle = React.createClass({
        ...
        render: function() {
            var {currentNodeNetworkGroup, isLocked} = this.props,
                classes = {
            ...
            return (
                <div className={utils.classNames(classes)} key={currentNodeNetworkGroup.id}>
                    {this.state.isRenaming ?
                        ...
                        />
                    :
                        <div className='name' onClick={!isLocked && this.startNodeNetworkGroupRenaming}>
                            <button className='btn-link'>{currentNodeNetworkGroup.get('name')}</button>
                            {!isLocked && <i className='glyphicon glyphicon-pencil' />}
                        </div>
                    }
                    {!isLocked && (
