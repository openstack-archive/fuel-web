...
var Range = React.createClass({
  ...
  renderRangesExtendable(options) {
    return _.map(options.ranges, (range, index) => {
      var rangeError = _.findWhere(options.error, {index: index}) || {};
      return (
        <div className='range-row clearfix' key={index}>
          <Input
            {...this.getRangeProps()}
            error={(rangeError.start || options.verificationError) ? '' : null}
            value={range[0]}
            onChange={_.partialRight(this.onRangeChange, options.attributeName, index)}
            ref={'start' + index}
          ...
          <Input
            ...
            extraContent={!this.props.hiddenControls && this.renderRangeControls(options.attributeName, index, options.ranges.length)}
          />
  ...
  renderRanges(options) {
    return (
      <div className='range-row clearfix'>
        <Input
          {...this.getRangeProps()}
          value={options.ranges[0]}
          error={options.startInputError ? '' : null}
          inputClassName='start'
        />
        <Input
          {...this.getRangeProps(true)}
          disabled={this.props.disabled || !!this.props.autoIncreaseWith}
          value={options.ranges[1]}
          error={options.endInputError ? '' : null}
        />
        {options.error && (options.startInputError || options.endInputError) &&
          <div className='validation-error text-danger pull-left'>
            <span className='help-inline'>{options.startInputError || options.endInputError}</span>
          </div>
        }
      </div>
    );
      ...
      <div className={utils.classNames(wrapperClasses)}>
        ...
        <div className='col-xs-12'>
          ...
          {this.props.extendable ?
            this.renderRangesExtendable({error, attributeName, attribute, ranges, wrapperClasses, verificationError, startInputError, endInputError})
          :
