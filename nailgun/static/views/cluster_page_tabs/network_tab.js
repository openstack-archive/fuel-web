...
function($, _, i18n, Backbone, React, models, dispatcher, utils, dialogs, componentMixins, controls) {
    ...
    var NetworkInputsMixin = {
        composeProps: function(attribute, isRange, isInteger) {
            var network = this.props.network,
                ns = network ? networkTabNamespace + 'network.' : parametersNS,
    ...
    var NetworkTab = React.createClass({
        mixins: [
            NetworkInputsMixin,
            NetworkModelManipulationMixin,
            componentMixins.backboneMixin('cluster', 'change:status change:networkConfiguration'),
        ...
        removeNodeNetworkGroup: function() {
            dialogs.RemoveNodeNetworkGroupDialog.show({})
                .done(_.bind(function() {
                    var currentNodeNetworkGroup = this.nodeNetworkGroups.findWhere({name: this.props.activeNetworkGroupName});
                    this.nodeNetworkGroups.remove(currentNodeNetworkGroup);
        ...
        render: function() {
            var isLocked = this.isLocked(),
                ...
                nodeNetworkGroups = this.nodeNetworkGroups = new models.NodeNetworkGroups(app.nodeNetworkGroups.where({cluster_id: cluster.id})),
                ...
                isNodeNetworkGroupSectionSelected = !_.contains(defaultNetworkSubtabs, activeNetworkGroupName);
            ...
            return (
                <div className={utils.classNames(classes)}>
                    ...
                    <div className='row network-tab-content'>
                        <NetworkSubtabs
                            cluster={this.props.cluster}
                            setActiveNetworkSectionName={this.props.setActiveNetworkSectionName}
                            nodeNetworkGroups={nodeNetworkGroups}
    ...
    var NetworkSubtabs = React.createClass({
        renderClickablePills: function(sections, isNetworkGroupPill) {
            ...
            invalidSections.network_verification = this.props.hasChanges &&
                cluster.task({group: 'network', status: 'error'});
            ...
            return (sections.map(function(groupName) {
                ...
                return (
                    <li
                        key={groupName}
                        role='presentation'
                        className={utils.classNames({active: isActive, warning: isMultiRack && groupName == 'network_verification'})}
            ...
            if (e.key == 'Enter') {
                ...
                if (nodeNetworkGroupNewName != currentNodeNetworkGroup.get('name')) {
                    if (_.contains(nodeNetworkGroups.pluck('name'), nodeNetworkGroupNewName)) {
                        validationError = i18n(networkTabNamespace + 'node_network_group_duplicate_error');
                        if (nodeNetworkGroupNewName == nodeNetworkGroups.min('id').get('name')) {
                            validationError = i18n(networkTabNamespace + 'node_network_group_default_name');
                        }
                    }
                    if (validationError) {
                        this.setState({
            ...
            return (
                <div className={utils.classNames(classes)} key={currentNodeNetworkGroup.id}>
                    ...
                    {isDefaultNodeNetworkGroup &&
                        <span className='explanation'>{i18n(networkTabNamespace + 'default_node_network_group_info')}</span>
                    }
    ...
    var NetworkVerificationResult = React.createClass({
        ...
        render: function() {
            ...
            return (
                <div className='forms-box'>
                    <h3>{i18n(networkTabNamespace + 'tabs.network_verification')}</h3>
