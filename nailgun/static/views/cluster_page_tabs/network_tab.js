...
function($, _, i18n, Backbone, React, models, dispatcher, utils, dialogs, componentMixins, controls) {
    ...
    var NetworkSubtabs = React.createClass({
        renderClickablePills: function(sections, isNetworkGroupPill) {
            ...
            //    }, this);
            //} else {
            //    _.each(errors && errors.networking_parameters, function(error, name) {
            //
            ...
            return (sections.map(function(groupName) {
                ...
                if (!isNetworkGroupPill) {
                    switch (groupName) {
                        case 'neutron_l2':
                            isInvalid = _.contains(NetworkingL2Parameters.renderedParameters, _.keys(networkParametersErrors));
                            break;
                        case 'neutron_l3':
                            isInvalid = _.contains(NetworkingL3Parameters.renderedParameters, _.keys(networkParametersErrors));
                            break;
                        case 'nova_configuration':
                            isInvalid = _.contains(NovaParameters.renderedParameters, _.keys(networkParametersErrors));
                            break;
                        case 'network_verification':
                            isInvalid = this.props.hasChanges && cluster.task({group: 'network', status: 'error'});
                            break;
                    }
                    tabLabel = i18n(networkTabNS + 'tabs.' + groupName);
                } else if (this.props.isMultiRack && !showAll) {
                    tabLabel = groupName;
                    isInvalid = networksErrors && !!errors.networks[activeNodeNetworkGroup.id]
