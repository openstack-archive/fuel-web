...
function($, _, i18n, Backbone, React, models, dispatcher, utils, componentMixins, controls) {
    ...
    var MultipleValuesInput = React.createClass({
        ...
        renderControls: function(attributeName, index, length) {
            var props = {className: 'btn btn-link', disabled: this.props.disabled};
            return (
                <div className='ip-ranges-control'>
                    <button {...props} onClick={_.partial(this.addValue, attributeName, index)}>
                        <i className='glyphicon glyphicon-plus-sign' />
                    </button>
                    {length > 1 &&
                        <button {...props} onClick={_.partial(this.removeValue, attributeName, index)}>
