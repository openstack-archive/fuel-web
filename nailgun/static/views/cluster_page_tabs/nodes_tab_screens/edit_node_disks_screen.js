            ...
            <div className='well clearfix'>
              ...
              {!locked && !!this.props.disks.length &&
                <div className='btn-group pull-right'>
                  ...
                  <button className='btn btn-default btn-revert-changes'
                    onClick={this.revertChanges} disabled={revertChangesDisabled}>
                    {i18n('common.cancel_changes_button')}
                  </button>
                  <button className='btn btn-success btn-apply'
                    onClick={this.applyChanges} disabled={!this.isSavingPossible()}>
...
var NodeDisk = React.createClass({
  ...
  updateDisk(name, value) {
    ...
    this.props.disk.get('volumes').findWhere({name: name}).set({size: size}).isValid({minimum:
      volumeInfo.min});
  ...
  render() {
    ...
    var requiredDiskSize = _.sum(disk.get('volumes').map((volume) => {
      return volume.getMinimalSize(this.props.volumes.findWhere({name:
        volume.get('name')}).get('min_size'));
    }));
              ...
              >
                ...
                {!this.props.disabled && volumesInfo[volumeName].min <= 0 && this.state.collapsed &&
                  <div className='close-btn'
                    onClick={_.partial(this.updateDisk, volumeName, 0)}>&times;</div>
                }
              </div>
            );
          })}
          <div className='volume-group pull-left' data-volume='unallocated'
                  ...
                  <div key={'edit_' + volumeName} data-volume={volumeName}>
                    <div className='form-group volume-group row'>
                      <label className='col-xs-4 volume-group-label'>
                        <span ref={'volume-group-flag ' + volumeName}
                          className={'volume-type-' + (index + 1)}> &nbsp; </span>
                        {volume.get('label')}
                      </label>
