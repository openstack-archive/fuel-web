...
var EditNodeInterfacesScreen = React.createClass({
  ...
  getIntersectedOffloadingModes(interfaces) {
    ...
    return offloadingModes.reduce((result, modes) => this.findOffloadingModesIntersection(result,
      modes));
  },
  bondInterfaces() {
    this.setState({actionInProgress: true});
    ...
    if (!bonds) {
      ...
      bonds = new models.Interface({
        type: 'bond',
        name: this.props.interfaces.generateBondName(this.getBondType() == 'linux' ? 'bond' :
          'ovs-bond'),
        mode: bondMode,
  ...
  removeInterfaceFromBond(bondName, slaveInterfaceName) {
    ...
    if (slaveInterfaceName) {
      ...
      var bondSlaveInterfaces = this.props.interfaces.filter((ifc) => _.contains(names,
        ifc.get('name')));
            ...
            <div className='page-buttons'>
              <div className='well clearfix'>
                <div className='btn-group pull-right'>
                  <button className='btn btn-default btn-bond' onClick={this.bondInterfaces}
                    disabled={!bondingPossible}>
                    {i18n(ns + 'bond_button')}
                  </button>
                  <button className='btn btn-default btn-unbond' onClick={this.unbondInterfaces}
        ...
        <div className='col-xs-12 page-buttons content-elements'>
          <div className='well clearfix'>
            <div className='btn-group'>
              <a className='btn btn-default' href={'#cluster/' + this.props.cluster.id + '/nodes'}
            ...
            {!locked &&
              <div className='btn-group pull-right'>
                <button className='btn btn-default btn-defaults' onClick={this.loadDefaults}
                ...
                <button className='btn btn-success btn-apply' onClick={this.applyChanges}
  ...
  statics: {
    target: {
      drop(props, monitor) {
        ...
        var network = sourceInterface.get('assigned_networks').findWhere({name:
          monitor.getItem().networkName});
  ...
  getAvailableBondingModes() {
    ...
    _.each(interfaces, (ifc) => {
      ...
      availableModes.push(_.reduce(modes, (result, modeSet) => {
        if (modeSet.condition && !utils.evaluateExpression(modeSet.condition,
            configModels).value) return result;
            ...
            <div className='col-xs-3'>
              ...
              <div className='pull-left'>
                {_.map(slaveInterfaces, (slaveInterface, index) => {
                  return (
                    <div key={'info-' + slaveInterface.get('name')}
    ...
    return this.props.connectDragSource(
      <div className={utils.classNames(classes)}>
        <div className='network-name'>
          {i18n('network.' + interfaceNetwork.get('name'), {defaultValue:
            interfaceNetwork.get('name')})}
        </div>
        {vlanRange &&
