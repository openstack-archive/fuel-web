...
($, _, Backbone, React, i18n, utils, models, dispatcher, dialogs, controls, ComponentMixins, DND, OffloadingModes) => {
    ...
    var EditNodeInterfacesScreen = React.createClass({
        ...
        applyChanges: function() {
            ...
            return $.when(...nodes.map(function(node) {
                ...
                nodeBonds = _.map(bonds, (bond) =>
                    new models.Interface(_.omit(bond.toJSON(), 'slaves'), {parse: true})
                );
        ...
        getIntersectedOffloadingModes: function(interfaces) {
            var offloadingModes = interfaces.map((ifc) =>
                ifc.get('offloading_modes') || []
            );
            if (!offloadingModes.length) return [];

            return offloadingModes.reduce((result, modes) =>
                this.findOffloadingModesIntersection(result, modes)
            );
        },
        bondInterfaces: function() {
            this.setState({actionInProgress: true});
            var interfaces = this.props.interfaces.filter((ifc) =>
                    ifc.get('checked') && !ifc.isBond()
                ),
                bonds = this.props.interfaces.find((ifc) =>
                    ifc.get('checked') && ifc.isBond()
                ),
        ...
        removeInterfaceFromBond: function(bondName, slaveInterfaceName) {
            var networks = this.props.cluster.get('networkConfiguration').get('networks'),
                ...
                bondHasUnmovableNetwork = bond.get('assigned_networks').any((interfaceNetwork) =>
                        interfaceNetwork.getFullNetwork(networks).get('meta').unmovable
                ),
            ...
            if (bondHasUnmovableNetwork) {
                var pxeInterface = this.props.interfaces.find((ifc) =>
                    ifc.get('pxe') && _.contains(slaveInterfaceNames, ifc.get('name'))
                );
            ...
            if (slaveInterfaceName) {
                var slavesUpdated = _.reject(slaves, {name: slaveInterfaceName}),
                    ...
                    bondSlaveInterfaces = this.props.interfaces.filter((ifc) =>
                        _.contains(names, ifc.get('name'))
                    );
