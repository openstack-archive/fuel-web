    ...
    var EditNodeInterfacesScreen = React.createClass({
        ...
        removeInterfaceFromBond: function(bondName, slaveInterfaceName) {
            ...
            if (bondHasUnmovableNetwork) {
                pxeInterface = this.props.interfaces.find(function(ifc) {
                    return ifc.get('pxe') && _.contains(slaveInterfaceNames, ifc.get('name'));
                });
                if (!slaveInterfaceName || pxeInterface && 
                    pxeInterface.get('name') == slaveInterfaceName) {
            ...
            if (slaveInterfaceName) {
                slavesUpdated = _.reject(slaves, {name: slaveInterfaceName});
                names = _.pluck(slavesUpdated, 'name');

                bond.set({
                    slaves: slavesUpdated,
                    offloading_modes: this.getIntersectedOffloadingModes(
                        this.props.interfaces.filter(function(ifc) {
