...
var Node = React.createClass({
  ...
  getNodeLogsLink() {
    ...
    } else if (status == 'provisioning' || status == 'provisioned' || (status == 'error' &&
      error == 'provision')) {
      options.source = 'install/fuel-agent';
    } else if (status == 'deploying' || status == 'ready' || (status == 'error' &&
      error == 'deploy')) {
  ...
  renderLogsLink(iconRepresentation) {
    return (
      <Tooltip key='logs'
        ...
        <a className={'btn-view-logs ' + (iconRepresentation ? 'icon icon-logs' : 'btn')}
