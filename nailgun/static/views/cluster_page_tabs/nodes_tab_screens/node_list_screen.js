        ...
        getFilterOptions: function(filter) {
            if (filter.isLabel) {
                ...
                return values.map((value) => {
                    return {
                        name: _.isUndefined(value) ? false : value,
                        label: _.isNull(value) ? i18n(ns + 'label_value_not_specified') : _.isUndefined(value) ? i18n(ns + 'label_not_assigned') : value
                    };
                });
            }
        ...
        render: function() {
            ...
            var filteredNodes = nodes.filter(function(node) {
                ...
                return _.all(this.state.activeFilters, function(filter) {
                    ...
                    if (filter.isLabel) {
                        var nodeLabelValue = node.getLabel(filter.name);
                        return _.any(filter.values,
                            (value) => value === false && _.isUndefined(nodeLabelValue) || value === nodeLabelValue
                        );
                    }
