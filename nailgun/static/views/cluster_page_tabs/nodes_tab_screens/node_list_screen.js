...
function($, _, i18n, Backbone, React, utils, models, dispatcher, controls, dialogs, componentMixins, Node) {
    ...
    NodeListScreen = React.createClass({
        ...
        getFilterOptions: function(filter) {
            ...
            var options,
                isClusterPage = !!this.props.cluster;
            switch (filter.name) {
                case 'status':
                    var os = isClusterPage && this.props.cluster.get('release').get('operating_system') || 'OS';
                ...
                case 'manufacturer':
                    ...
                    break;
                case 'roles':
                    options = isClusterPage ?
                            this.props.cluster.get('roles').invoke('pick', 'name', 'label')
                        :
                            _.chain(this.props.clusters.map(function(cluster) {
                                return cluster.get('roles').invoke('pick', 'name', 'label');
                            })).flatten().uniq().value();
                    break;
                case 'group_id':
                    options = _.uniq(this.props.nodes.pluck('group_id')).map(function(groupId) {
                        var nodeNetworkGroup = app.nodeNetworkGroups.get(groupId);
                        return {
                            name: groupId,
                            label: nodeNetworkGroup && nodeNetworkGroup.get('name') || i18n('common.not_specified')
                        };
                    });
                    break;
                case 'cluster':
                    options = _.map(_.uniq(this.props.nodes.pluck('cluster')), function(clusterId) {
                        return {
                            name: clusterId,
        ...
        render: function() {
            var cluster = this.props.cluster,
                isClusterPage = !!cluster,
                locked = isClusterPage && !!cluster.task({group: 'deployment', active: true}),
                nodes = this.props.nodes,
                processedRoleData = isClusterPage ? this.processRoleLimits() : {};
            ...
            return (
                <div>
                    ...
                    />
                    {isClusterPage && this.props.mode != 'list' &&
                        <RolePanel
                    ...
                    <NodeList
                        ...
                        {... _.pick(this.props, 'cluster', 'mode', 'statusesToFilter', 'selectedNodeIds', 'clusters', 'releases')}
                        ...
                        <div className='node-list-management-buttons col-xs-5'>
                            <div className='view-mode-switcher'>
                                <div className='btn-group' data-toggle='buttons'>
                                    {_.map(this.props.nodes.viewModes, function(mode) {
                                        return (
    ...
    NodeList = React.createClass({
        ...
        groupNodes: function() {
            var isClusterPage = !!this.props.cluster,
                roles = isClusterPage ?
                        this.props.cluster.get('roles').models
                    :
                        _.flatten(this.props.clusters.map(function(cluster) {
                            return cluster.get('roles').models;
                        }));
            ...
            var groupingMethod = _.bind(function(node) {
                return _.compact(_.map(this.props.activeSorters, function(sorter) {
                    ...
                    var result,
                        ...
                        nodeCluster = isClusterPage ? this.props.cluster : this.props.clusters.get(node.get('cluster'));
                    switch (sorter.name) {
                        case 'roles':
                            result = node.getRolesSummary(roles) || i18n(ns + 'no_roles');
                            break;
                        case 'status':
                            result = i18n(ns + 'status.' + node.getStatusSummary(), {
                                os: nodeCluster && nodeCluster.get('release').get('operating_system') || 'OS'
                            });
                            break;
                        case 'manufacturer':
                        ...
                        case 'group_id':
                            ...
                            result = nodeNetworkGroup && i18n(ns + 'node_network_group', {group: nodeNetworkGroup.get('name')})
                                || i18n(ns + 'no_node_network_group');
                            break;
                        case 'cluster':
                            result = nodeCluster ? i18n(ns + 'cluster', {
                                    cluster: nodeCluster.get('name')
                                }) : i18n(ns + 'unallocated');
                            break;
                        case 'hdd':
            ...
            var preferredRolesOrder = _.pluck(roles, 'name');
    ...
    NodeGroup = React.createClass({
        ...
        render: function() {
            var isClusterPage = !!this.props.cluster,
                availableNodes = this.props.nodes.filter(function(node) {return node.isSelectable();}),
            ...
            return (
                <div className='nodes-group'>
                    ...
                    <div className='row'>
                        {this.props.nodes.map(function(node) {
                            return <Node
                                {... _.pick(this.props, 'mode', 'viewMode')}
                                key={node.id}
                                node={node}
