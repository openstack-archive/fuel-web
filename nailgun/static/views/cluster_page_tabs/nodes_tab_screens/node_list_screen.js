...
var NodeListScreen, MultiSelectControl, NumberRangeControl, ManagementPanel, NodeLabelsPanel,
  RolePanel, SelectAllMixin, NodeList, NodeGroup;
...
class Filter {
  constructor(name, values, isLabel) {
    ...
    this.title = isLabel ? name : i18n('cluster_page.nodes_tab.filters.' + name,
      {defaultValue: name});
    this.isLabel = isLabel;
    this.isNumberRange = !isLabel && !_.contains(['roles', 'status', 'manufacturer',
      'group_id', 'cluster'], name);
    return this;
  }
    ...
    if (this.isNumberRange) {
      ...
      if (nodes.length) {
        ...
        if (this.name == 'hdd' || this.name == 'ram') {
          limits = [Math.floor(limits[0] / Math.pow(1024, 3)), Math.ceil(limits[1] /
            Math.pow(1024, 3))];
        }
      }
      this.limits = limits;
...
NodeListScreen = React.createClass({
  ...
  getInitialState() {
    ...
    var indeterminateRoles = nodes.length ?
      _.filter(roles, (role) => !_.contains(selectedRoles, role) &&
        nodes.any((node) => node.hasRole(role))) : [];
    ...
    if (!this.props.cluster || this.props.mode != 'add') {
      var normalizedFilters = _.map(this.state.activeFilters, (activeFilter) => {
        ...
        if (filter.values.length) {
          ...
          } else if (checkStandardNodeFilters && _.contains(['manufacturer', 'group_id', 'cluster'],
              filter.name)) {
  ...
  getFilterOptions(filter) {
    if (filter.isLabel) {
      ...
      return values.map((value) => {
        return {
          name: value,
          label: _.isNull(value) ? i18n(ns + 'label_value_not_specified') :
            value === false ? i18n(ns + 'label_not_assigned') : value
        };
      });
    }
    ...
    switch (filter.name) {
      case 'status':
        var os = this.props.cluster &&
          this.props.cluster.get('release').get('operating_system') || 'OS';
        ...
        options = _.uniq(this.props.nodes.pluck('group_id')).map((groupId) => {
          ...
          return {
            ...
            label: nodeNetworkGroup ?
                nodeNetworkGroup.get('name') + (this.props.cluster ? '' :
                  ' (' + this.props.clusters.get(nodeNetworkGroup.get('cluster_id'))
                    .get('name') + ')')
              :
  ...
  render() {
    ...
    var selectedNodeLabels = _.chain(selectedNodes.pluck('labels')).flatten()
      ...
      return _.all(this.state.activeFilters, (filter) => {
        ...
        switch (filter.name) {
          ...
          default:
            ...
            if (filter.name == 'hdd' || filter.name == 'ram') currentValue = currentValue /
              Math.pow(1024, 3);
            result = currentValue >= filter.values[0] && (_.isUndefined(filter.values[1]) ||
              currentValue <= filter.values[1]);
            break;
        }
        return result;
      });
      ...
      <div>
        ...
        <ManagementPanel
          {... _.pick(this.state, 'viewMode', 'search', 'activeSorters', 'activeFilters',
            'availableSorters', 'availableFilters', 'isLabelsPanelOpen')}
          {... _.pick(this.props, 'cluster', 'mode', 'defaultSorting', 'statusesToFilter',
            'defaultFilters')}
...
MultiSelectControl = React.createClass({
  ...
  render() {
    ...
    return (
      <div className={utils.classNames(classNames)} tabIndex='-1'
...
ManagementPanel = React.createClass({
  ...
  showDeleteNodesDialog() {
    DeleteNodesDialog.show({nodes: this.props.nodes, cluster: this.props.cluster})
      .done(_.partial(this.props.selectNodes, _.pluck(this.props.nodes.where({status: 'ready'}),
        'id'), null, true));
  },
  hasChanges() {
  ...
  applyChanges() {
    ...
    var nodes = new models.Nodes(this.props.nodes.map((node) => {
      ...
      if (node.get('pending_roles').length) {
        if (this.props.mode == 'add') return _.extend(data, {cluster_id: this.props.cluster.id,
  ...
  activateSearch() {
    ...
    $('html').on('click.search', (e) => {
      if (!this.props.search && this.refs.search && !$(e.target)
          .closest(ReactDOM.findDOMNode(this.refs.search)).length) {
  ...
  isFilterOpen(filter) {
    return !_.isNull(this.state.openFilter) && this.state.openFilter.name == filter.name &&
  ...
  renderDeleteFilterButton(filter) {
    ...
    return (
      <i className='btn btn-link glyphicon glyphicon-minus-sign btn-remove-filter'
  ...
  renderDeleteSorterButton(sorter) {
    return (
      <i className='btn btn-link glyphicon glyphicon-minus-sign btn-remove-sorting'
  ...
  render() {
    ...
    if (this.props.mode != 'edit') {
      var checkSorter = (sorter, isLabel) => !_.any(this.props.activeSorters, {name: sorter.name,
        isLabel: isLabel});
      inactiveSorters = _.union(_.filter(this.props.availableSorters,
        _.partial(checkSorter, _, false)), _.filter(this.props.labelSorters,
        _.partial(checkSorter, _, true)))
        .sort((sorter1, sorter2) => utils.natsort(sorter1.title, sorter2.title,
          {insensitive: true}));
      canResetSorters = _.any(this.props.activeSorters, {isLabel: true}) ||
        !_(this.props.activeSorters).where({isLabel: false}).map(Sorter.toObject)
          .isEqual(this.props.defaultSorting);

      var checkFilter = (filter, isLabel) => !_.any(this.props.activeFilters,
        {name: filter.name, isLabel: isLabel});
      inactiveFilters = _.union(_.filter(this.props.availableFilters,
        _.partial(checkFilter, _, false)), _.filter(this.props.labelFilters,
        _.partial(checkFilter, _, true)))
        .sort((filter1, filter2) => utils.natsort(filter1.title, filter2.title,
          {insensitive: true}));
              ...
              this.state.activeSearch && (
                <div className='search pull-left' key='search'>
                  ...
                  {this.state.isSearchButtonVisible &&
                    <button className='close btn-clear-search' onClick={this.clearSearchField}>
                      &times;
                    </button>
                  }
                </div>
              ...
              <div className='col-xs-12 sorters' key='sorters'>
                <div className='well clearfix' key={this.state.sortersKey}>
                  <div className='well-heading'>
                    ...
                    {canResetSorters &&
                      <button className='btn btn-link pull-right btn-reset-sorting'
                ...
                <div className='well clearfix' key={this.state.filtersKey}>
                  <div className='well-heading'>
                    ...
                    {!!appliedFilters.length &&
                      <button className='btn btn-link pull-right btn-reset-filters'
                  ...
                  {_.map(this.props.activeFilters, (filter) => {
                    var props = {
                      ...
                      prefix: i18n('cluster_page.nodes_tab.filters.prefixes.' + filter.name,
                        {defaultValue: ''}),
                    ...
                    if (filter.isNumberRange) {
                      return <NumberRangeControl {...props} min={filter.limits[0]}
                        max={filter.limits[1]}
                      />;
                    }
                ...
                <div className='active-sorters-filters'>
                  {!this.state.areFiltersVisible && !!appliedFilters.length &&
                    <div className='active-filters row' onClick={this.toggleFilters}>
                      ...
                      <button className='btn btn-link btn-reset-filters'
                        onClick={this.resetFilters}>
                  ...
                  {!this.state.areSortersVisible &&
                    <div className='active-sorters row' onClick={this.toggleSorters}>
                      ...
                      {canResetSorters &&
                        <button className='btn btn-link btn-reset-sorting'
                          onClick={this.resetSorters}>
                          <i className='glyphicon glyphicon-remove-sign' />
                        </button>
                      }
        ...
        <div className='well clearfix'>
          ...
          <div className='forms-box form-inline'>
            ...
            {_.map(this.state.labels, (labelData, index) => {
              ...
              return (
                <div className={utils.classNames({clearfix: true, 'has-label': showControlLabels})}
                  key={index}>
                  <Input
                    type='checkbox'
                    ref={labelData.key}
...
RolePanel = React.createClass({
  ...
  processRestrictions(role, models) {
    ...
    return {
      result: restrictionsCheck.result || _.contains(conflicts, name) || (roleLimitsCheckResults &&
        !roleLimitsCheckResults.valid && !_.contains(this.props.selectedRoles, name)),
...
SelectAllMixin = {
  ...
  renderSelectAllCheckbox() {
    var checked = this.props.mode == 'edit' ||
      (this.props.nodes.length &&
...
NodeList = React.createClass({
      ...
      return _.compact(_.map(this.props.activeSorters, (sorter) => {
        ...
        switch (sorter.name) {
          ...
          case 'cluster':
            ...
            break;
          case 'hdd':
            result = i18n('node_details.total_hdd', {total:
              utils.showDiskSize(node.resource('hdd'))});
            break;
          case 'disks':
            result = composeNodeDiskSizesLabel(node);
            break;
          case 'ram':
            result = i18n('node_details.total_ram', {total:
              utils.showMemorySize(node.resource('ram'))});
            break;
          case 'interfaces':
  ...
  render() {
    ...
    return (
      <div className={utils.classNames({'node-list row': true,
        compact: this.props.viewMode == 'compact'})}>
        {groups.length > 1 &&
