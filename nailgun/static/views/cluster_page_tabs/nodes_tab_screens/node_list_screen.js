...
function($, _, i18n, Backbone, React, utils, models, dispatcher, controls, dialogs, componentMixins, Node) {
    ...
    NodeListScreen = React.createClass({
        ...
        getFilterOptions: function(roles, filter) {
            if (filter.isLabel) {
            ...
            switch (filter.name) {
                ...
                case 'manufacturer':
                    ...
                    break;
                case 'roles':
                    options = roles.invoke('pick', 'name', 'label');
                    break;
                case 'group_id':
                    options = _.uniq(_.map(this.props.nodes.pluck('group_id'), function(groupId) {
                    ...
                    break;
                case 'cluster':
                    options = _.uniq(_.map(this.props.nodes.pluck('cluster'), function(clusterId) {
                        return {
                            name: clusterId,
        ...
        render: function() {
            ...
            if (isClusterPage) {
                ...
                this.props.clusters.each(function(cluster) {
                    roles.add(cluster.get('roles').filter(function(role) {
                        return !roles.findWhere({name: role.get('name')});
                    }));
                });
            }
            ...
            return (
                <div>
                    ...
                    <ManagementPanel
                        ...
                        getFilterOptions={_.partial(this.getFilterOptions, roles)}
                    ...
                    switch (sorter.name) {
                        ...
                        case 'status':
                            result = i18n(ns + 'status.' + node.getStatusSummary(), {
                                os: isClusterPage && nodeCluster.get('release').get('operating_system') || 'OS'
                            });
                            break;
                        case 'manufacturer':
                        ...
                        case 'group_id':
                            ...
                            break;
                        case 'cluster':
                            result = nodeCluster && i18n(ns + 'cluster', {cluster: nodeCluster.get('name')})
                                || i18n(ns + 'unallocated');
                            break;
                        case 'hdd':
