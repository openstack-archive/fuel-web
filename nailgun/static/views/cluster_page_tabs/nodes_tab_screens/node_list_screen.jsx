...
function($, _, i18n, Backbone, React, utils, models, dispatcher, controls, dialogs, componentMixins, Node) {
    ...
    NodeListScreen = React.createClass({
        ...
        getInitialState: function() {
            ...
            return {
                ...
                activeFilters: this.props.mode == 'add' ?
                    ...
                    :
                        _.union(
                            _.map(_.union(this.props.defaultFilters, _.keys(uiSettings.filter)), function(filterName) {
                                return Filter(uiSettings.filter[filterName] || [], filterName, false);
                            }),
        ...
        changeSortingOrder: function(sorter) {
            // FIXME(jkirnosova): need to get rid of direct mutating of state
        ...
        updateFilters: function(filters) {
            ...
            if (this.props.mode != 'add') {
                var groupedFilters = _.groupBy(filters, 'isLabel'),
                    serializeFilter = function(result, filter) {
                        result[filter.name] = filter.values;
                        return result;
                    };
                this.changeUISettings('filter', _.reduce(groupedFilters.false, serializeFilter, {}));
                this.changeUISettings('filter_by_labels', _.reduce(groupedFilters.true, serializeFilter, {}));
            }
        },
        ...
        changeFilter: function(filter, values) {
            ...
            var activeFilters = _.clone(this.state.activeFilters),
                filterToChange = _.find(activeFilters, {name: filter.name, isLabel: filter.isLabel});
            if (filterToChange) {
                filterToChange.values = values;
            } else {
        ...
        resetFilters: function() {
            this.updateFilters(_.map(this.props.defaultFilters, _.bind(Filter, null, [], _, false)));
        },
    ...
    ManagementPanel = React.createClass({
        ...
        renderDeleteFilterButton: function(filter) {
            if (!filter.isLabel && _.contains(this.props.defaultFilters, filter.name)) return null;
            return (
        ...
        render: function() {
            ...
            if (this.props.mode != 'edit') {
                ...
                var createFilter = _.bind(function(filterName, isLabel) {
                    if (!_.any(this.props.activeFilters, {name: filterName, isLabel: isLabel})) return Filter([], filterName, isLabel);
                }, this);
