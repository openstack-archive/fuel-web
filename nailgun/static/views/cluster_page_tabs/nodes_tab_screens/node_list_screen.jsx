...
function($, _, i18n, Backbone, React, utils, models, dispatcher, controls, dialogs, componentMixins, Node) {
    ...
    function Sorter(sortObject, isLabel) {
        ...
        this.label = isLabel ? this.name : i18n('cluster_page.nodes_tab.sorters.' + this.name, {defaultValue: this.name});
    ...
    NodeListScreen = React.createClass({
        ...
        getInitialState: function() {
            ...
            var createSorter = function(isLabel, sortObject) {return new Sorter(sortObject, isLabel);};

            return {
                search: this.props.mode == 'add' ? '' : uiSettings.search,
                activeSorters: this.props.mode == 'add' ?
                        _.map(this.props.defaultSorting, _.partial(createSorter, false))
                    :
                        _.union(_.map(uiSettings.sort, _.partial(createSorter, false)), _.map(uiSettings.sort_by_labels, _.partial(createSorter, true))),
        ...
        removeSorting: function(sorter) {
            var sorters = _.difference(this.state.activeSorters, [sorter]);
            this.updateSorting(sorters);
        },
        resetSorters: function() {
            this.updateSorting(_.map(this.props.defaultSorting, function(sorter) {
                return new Sorter(sorter);
            }));
        },
        changeSortingOrder: function(sorter) {
            var sorters = this.state.activeSorters,
                changedSorter = _.find(sorters, function(sortObject) {return sortObject.name == sorter.name;});
            changedSorter.order = changedSorter.order == 'asc' ? 'desc' : 'asc';
            this.updateSorting(sorters);
        },
            ...
            if (this.props.mode != 'add') {
                ...
                _.each(sorters, function(sorter) {
                    if (sorter.isLabel) {
                        ...
                        attributes.push(sorter);
                    }
                });
                this.changeUISettings('sort', _.invoke(attributes, 'toObject'));
                this.changeUISettings('sort_by_labels', _.invoke(labels, 'toObject'));
            }
        },
        ...
        removeDeletedLabelsFromActiveSortersAndFilters: function() {
            var labelSorters = _.filter(this.state.activeLabelSorters, function(sorter) {
                return !_.all(this.props.nodes.getLabelValues(_.keys(sorter)[0]), _.isUndefined);
            }, this);
            if (!_.isEqual(this.state.activeLabelSorters, labelSorters)) {
                this.updateSorting(labelSorters, true);
            }
                ...
                <div className={utils.classNames(classNames)} tabIndex='-1' onKeyDown={this.closeOnEscapeKey}>
                    ...
                    {this.props.isOpen &&
                        <controls.Popover toggle={this.props.toggle}>
                            {!this.props.dynamicValues && [
                                    <div key='all'>
                                        <controls.Input
                                            ...
                                            name='all'
                                            checked={valuesAmount == attributes.length}
                            ...
                            {_.map(attributes, function(attribute, index) {
                                return (
                                    <controls.Input
                                        key={index}
                                        type='checkbox'
                                        name={attribute.name}
                                        label={attribute.label || i18n('common.not_specified')}
                                        checked={_.contains(this.props.values, attribute.name)}
                                        onChange={this.props.dynamicValues ? _.partial(this.onChange, attribute) : this.onChange}
                                    />
                                );
                            }, this)}
                            {this.props.dynamicValues && !!labels.length &&
                                <div>
                                    {attributes.length && <div key='divider' className='divider' />}
                                    {_.map(labels, function(label, index) {
                                        return (
                                            <controls.Input
                                                key={'label-' + index}
                                                type='checkbox'
                                                name={label.name}
                                                label={label.name}
                                                onChange={_.partial(this.onChange, label)}
                                            />
                                        );
                                    }, this)}
                                </div>
            ...
            if (this.props.mode != 'edit') {
                var createSorter = _.bind(function(isLabel, sorterName) {
                    if (!_.any(this.props.activeSorters, function(sorter) {
                        return sorter.name == sorterName && sorter.isLabel == isLabel;
                    })) {
                        var data = {};
                        data[sorterName] = 'asc';
                        return new Sorter(data, isLabel);
                    }
                }, this);
                                ...
                                <div className='col-xs-12 sorters' key='sorters'>
                                    <div className='well clearfix' key={this.state.sortersKey}>
                                        <div className='well-heading'>
                                            ...
                                            {!_.isEqual(_.invoke(this.props.activeSorters, 'toObject'), this.props.defaultSorting) &&
                                                    ...
                                                    <button className='btn btn-default' onClick={_.partial(this.props.changeSortingOrder, sorter)}>
                                                        ...
                                                        <i
                                                            className={utils.classNames({
                                                                glyphicon: true,
                                                                'glyphicon-arrow-down': sorter.order == 'asc',
                                                                'glyphicon-arrow-up': sorter.order == 'desc'
                                                            })}
                                                        />
                                                    </button>
                                                    ...
                                                    {this.props.activeSorters.map(function(sorter, index) {
                                                        return (
                                                            <span key={sorter.name + (sorter.isLabel && '-label')}>
                                                                ...
                                                                />
                                                                {this.props.activeSorters.length != index + 1 && ' + '}
                                                            </span>
                                                        );
                                                    }, this)}
                                                </div>
