            ...
            switch (filter.name) {
                ...
                case 'group_id':
                    options = _.uniq(this.props.nodes.pluck('group_id')).map(function(groupId) {
                        return {
                            name: groupId,
                            label: this.props.cluster.get('nodeNetworkGroups').get(groupId).get('name')
                        };
                    }, this);
                    break;
            }
                    ...
                    switch (sorter.name) {
                        ...
                        case 'group_id':
                            result = i18n('cluster_page.nodes_tab.node.node_network_group', {
                                group: this.props.cluster.get('nodeNetworkGroups').get(node.get('group_id')).get('name')
                            });
                            break;
                        case 'hdd':
        ...
        render: function() {
            ...
            return (
                <div className='nodes-group'>
                    ...
                    <div className='row'>
                        {this.props.nodes.map(function(node) {
                            var nodeNetworkGroupId = node.get('group_id');
                            return <Node
                                ...
                                nodeNetworkGroupName={nodeNetworkGroupId && this.props.cluster.get('nodeNetworkGroups').get(nodeNetworkGroupId).get('name')}
                            />;
                        }, this)}
                    </div>
                </div>
