...
function($, _, i18n, Backbone, React, utils, models, dispatcher, controls, dialogs, componentMixins, Node) {
    ...
    NodeListScreen = React.createClass({
        ...
        render: function() {
            var cluster = this.props.cluster,
                locked = cluster && !!cluster.tasks({group: 'deployment', status: 'running'}).length,
            ...
            return (
                <div>
                    ...
                    <NodeList
                        ...
                        {... _.pick(this.props, 'cluster', 'mode', 'statusesToFilter', 'selectedNodeIds', 'networkGroups', 'clusters')}
                ...
                return _.compact(_.map(this.props.activeSorters, function(sorter) {
                    ...
                    switch (sorter.name) {
                        ...
                        case 'manufacturer':
                            ...
                            break;
                        case 'group_id':
                            var networkGroup = node.get('group_id');
                            result = networkGroup ? i18n('cluster_page.nodes_tab.node.network_group', {
                                    group: this.props.networkGroups.get(networkGroup).get('name')
                                }) : i18n('cluster_page.nodes_tab.node.no_network_group');
                            break;
                        case 'cluster':
                    ...
                    if (sorter.isLabel) {
                        ...
                        switch (sorter.name) {
                            ...
                            case 'disks':
                                ...
                                break;
                            case 'group_id':
                                var networkGroup1 = node1.get('group_id'),
                                    networkGroup2 = node2.get('group_id');
                                result = networkGroup1 && networkGroup2 ?
                                        utils.natsort(this.props.networkGroups.get(networkGroup1).get('name'), this.props.networkGroups.get(networkGroup2).get('name'))
                                    :
                                        networkGroup1 ? -1 : networkGroup2 ? 1 : 0;
                                break;
                            case 'cluster':
                                var cluster1 = node1.get('cluster'),
                                    cluster2 = node2.get('cluster');
                                result = cluster1 && cluster2 ?
                                        utils.natsort(this.props.clusters.get(cluster1).get('name'), this.props.clusters.get(cluster2).get('name'))
                                    :
                                        cluster1 ? -1 : cluster2 ? 1 : 0;
                                break;
                            default:
            ...
            return (
                <div className='nodes-group'>
                    ...
                    <div className='row'>
                        {this.props.nodes.map(function(node) {
                            return <Node
                                {... _.pick(this.props, 'cluster', 'mode', 'viewMode', 'networkGroups', 'clusters')}
                                key={node.id}
                                node={node}
