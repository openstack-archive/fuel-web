...
function($, _, i18n, Backbone, React, utils, models, dispatcher, controls, dialogs, componentMixins, Node) {
    ...
    NodeListScreen = React.createClass({
        ...
        resetSorters: function() {
            this.updateSorting(_.map(this.props.defaultSorting, function(sorter) {return new Sorter(sorter);}));
        },
        changeSortingOrder: function(sorter) {
            var activeSorters = this.state.activeSorters,
                changedSorter = _.find(activeSorters, function(sortObject) {return sortObject.name == sorter.name;});
        ...
        removeDeletedLabelsFromActiveSortersAndFilters: function() {
            var sorters = _.filter(this.state.activeSorters, function(sorter) {
                return !sorter.isLabel || !_.all(this.props.nodes.getLabelValues(sorter.name), _.isUndefined);
            }, this);
                            ...
                            {_.map(attributes, function(attribute, index) {
                                return (
                                    <controls.Input
                                        ...
                                        onChange={_.partial(this.onChange, attribute)}
                                    />
                                );
                            }, this)}
