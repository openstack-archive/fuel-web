...
function($, _, i18n, Backbone, React, utils, models, dispatcher, controls, dialogs, componentMixins, Node) {
        ...
        render: function() {
            ...
            var nodesToManageLabels = selectedNodes.length ? selectedNodes : nodes,
                ...
                indeterminateLabels = _.filter(labelList, function(label) {
                    return nodesToManageLabels.any(function(node) {return _.isUndefined(node.get('labels')[label]);});
                });
    ...
    ManagementPanel = React.createClass({
        ...
        processLabelString: function(str) {
            // "=" is forbidden for label kay and value
            return _.trim(str).replace(/=/g, '');
        },
        addLabel: function() {
            var newLabelKey = this.processLabelString(this.refs['new-label-key'].getInputDOMNode().value);
            if (newLabelKey && !_.contains(_.union(this.props.labels, _.keys(this.state.newLabels)), newLabelKey)) {
                var newLabel = {},
                    newLabelValue = this.processLabelString(this.refs['new-label-value'].getInputDOMNode().value);
                newLabel[newLabelKey] = newLabelValue ? this.processLabelString(newLabelValue) : null;
                this.setState({
        ...
        applyLabels: function() {
            ...
            if (labels.length) {
                ...
                var nodes = new models.Nodes((this.props.nodes.length ? this.props.nodes : this.props.screenNodes)
                    .map(function(node) {
                        ...
                        _.each(labels, function(label) {
                            var labelState = this.refs[label + '-checkbox'].getInputDOMNode();

                            // rename label
                            if (labelState.checked || labelState.indeterminate) {
                                if (!_.isUndefined(nodeLabels[label])) {
                                    // rename label
                            ...
                            if (this.refs[label + '-value']) {
                                // FIXME: need to change label value if it was changed by user
                                if (!_.isUndefined(nodeLabels[label])) {
                ...
                Backbone.sync('update', nodes)
                    .done(_.bind(function() {
                        // fetch screen nodes
                        app.page.refs.tab.refs.screen.refs.screen.props.nodes.fetch().always(_.bind(function() {
                            this.toggleLabels();
                            this.setState({
                                actionInProgress: false,
                                newLabels: {}
                            });
                            // FIXME: forceUpdate should be removed from the code
                            app.page.forceUpdate();
                        }, this));
                    }, this))
                    .fail(_.bind(function(response) {
                        utils.showErrorDialog({
                                        ...
                                        <div className='forms-box form-inline' key={this.state.labelsKey}>
                                            {_.map(labelsToDisplay, function(label) {
                                                var nodes = this.props.nodes.length ? this.props.nodes : new models.Nodes(this.props.filteredNodes),
                                                    ...
                                                    allNodesHaveLabel = !nodes.any(function(node) {return _.isUndefined(node.get('labels')[label]);}),
                                                    value = isNewLabel ? this.state.newLabels[label] : (allNodesHaveLabel ? nodes.at(0).get('labels')[label] : '');
                                                ...
                                                return (
                                                    <div className='clearfix' key={label}>
                                                        <controls.Input
                                                            ...
                                                            ref={label + '-checkbox'}
                                                            name={label + 'delete'}
                                                            defaultChecked={isNewLabel || allNodesHaveLabel}
