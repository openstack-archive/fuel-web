...
function($, _, i18n, Backbone, React, utils, models, dispatcher, controls, dialogs, componentMixins, Node) {
    ...
    _.extend(Sorter, {
        fromObject: function(sorterObject, isLabel) {
            ...
            return Sorter(sorterName, sorterObject[sorterName], isLabel);
        }
    });
    ...
    _.extend(Filter, {
        fromObject: function(filters, isLabel) {
            return _.map(filters, function(values, name) {
                return Filter(name, values, isLabel);
            });
        },
    ...
    NodeListScreen = React.createClass({
        ...
        changeSortingOrder: function(sorter) {
            var activeSorters = this.state.activeSorters.map(function(sorter) {
                    return Sorter.fromObject(sorter.toObject(), sorter.isLabel);
                }),
        ...
        updateSorting: function(sorters, updateLabelsOnly) {
            ...
            if (this.props.mode != 'add') {
                ...
                if (!updateLabelsOnly) this.changeUISettings('sort', _.invoke(groupedSorters.false, 'toObject') || []);
        ...
        changeFilter: function(filter, values) {
            var activeFilters = _.flatten(this.state.activeFilters.map(function(filter) {
                    return Filter.fromObject(Filter.toObject([filter]), filter.isLabel);
                }));
            _.find(activeFilters, {name: filter.name, isLabel: filter.isLabel}).values = values;
                        ...
                        <controls.Popover toggle={this.props.toggle}>
                            <div className='clearfix'>
                                <controls.Input {...props}
                                    name='start'
                                    value={_.isEmpty(this.props.values) ? this.props.min : this.props.values[0]}
                                ...
                                <controls.Input {...props}
                                    name='end'
                                    value={_.isEmpty(this.props.values) ? this.props.max : this.props.values[1]}
