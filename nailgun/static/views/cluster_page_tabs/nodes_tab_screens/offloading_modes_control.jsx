...
function($, _, React, i18n) {
    ...
    var OffloadingModesControl = React.createClass({
        ...
        makeOffloadingModesExcerpt: function() {
            var icons = {
                true: 'glyphicon glyphicon-ok-sign text-success',
                false: 'glyphicon glyphicon-remove-sign text-danger',
                null: 'glyphicon glyphicon-question-sign'
            };
            return this.props.interface.get('offloading_modes').map(function(mode, index) {
                return <span>
                    {(index ? ', ' : '') + mode.name}
                    <i className={icons[mode.state]}></i>
                </span>
            });
        },
        renderChildModes: function(modes, level) {
            return modes.map((function(mode) {
                var lines = [
                    <tr key={mode.name}>
                        <td className={'level' + level}>{mode.name}</td>
