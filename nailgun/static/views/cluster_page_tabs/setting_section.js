      ...
      <div className={'setting-section setting-section-' + sectionName}>
        ...
        <div>
          ...
          {_.map(sortedSettings, (settingName) => {
            ...
            if (CustomControl) return this.renderCustomControl(_.extend(renderOptions, {CustomControl, path}));

            var render = setting.values ? this.renderRadioGroup : this.renderInput;
            return render(_.extend(renderOptions, {settingName}));
          })}
        </div>
      </div>
    );
