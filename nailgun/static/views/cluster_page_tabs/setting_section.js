...
var SettingSection = React.createClass({
  ...
  checkValues(values, path, currentValue, restriction) {
    ...
    var result = _.all(values, (value) => {
      ...
      return new Expression(restriction.condition, this.props.configModels, restriction)
    ...
    return _.compact(this.props.allocatedRoles.map((roleName) => {
      ...
      if (_.any(role.get('restrictions'), (restriction) => {
        ...
        if (_.contains(restriction.condition, 'settings:' + path) &&
          !(new Expression(restriction.condition, this.props.configModels,
  ...
  checkDependentSettings(sectionName, settingName) {
    ...
    var addDependentRestrictions = (setting, label) => {
      var result = _.filter(_.map(setting.restrictions, utils.expandRestriction),
          (restriction) => restriction.action == 'disable' && _.contains(restriction.condition,
            'settings:' + path)
        );
      if (result.length) {
    ...
    _.each(this.props.settings.attributes, (section, sectionName) => {
      ...
      _.each(section, (setting, settingName) => {
        // we support dependecies on checkboxes, toggleable setting groups,
  ...
  togglePlugin(pluginName, settingName, enabled) {
    ...
    if (enabled) {
      ...
      if (pluginMetadata.chosen_id !== initialVersion) this.onPluginVersionChange(pluginName,
        initialVersion);
    }
  },
  render() {
    ...
    var isGroupAlwaysEditable = isPlugin ?
      _.any(metadata.versions,
        (version) => version.metadata.always_editable) : metadata.always_editable;
    var isGroupDisabled = this.props.locked || (this.props.lockedCluster &&
      !isGroupAlwaysEditable) || processedGroupRestrictions.result;
      ...
      <div className={'setting-section setting-section-' + sectionName}>
        <h3>
          {metadata.toggleable ?
            ...
            />
          :
            <span className={'subtab-group-' + sectionName}>
              {sectionName == 'common' ?
        ...
        <div>
          ...
          {_.map(sortedSettings, (settingName) => {
            ...
            var isSettingDisabled = isGroupDisabled || (metadata.toggleable &&
              !metadata.enabled) || processedSettingRestrictions.result ||
              processedSettingDependencies.result;
            ...
            if (setting.values) {
              var values = _.chain(_.cloneDeep(setting.values))
                ...
                .compact()
                .value();
              if (setting.type == 'radio') return <RadioGroup
                {...this.props}
                key={settingKey}
                name={settingName}
                label={setting.label}
            ...
            var settingDescription = setting.description &&
              <span dangerouslySetInnerHTML={{__html:
                utils.urlify(_.escape(setting.description))}}
              />;
            return <Input
              ...
              debounce={setting.type == 'text' || setting.type == 'password' ||
