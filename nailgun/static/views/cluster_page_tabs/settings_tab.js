...
function($, _, i18n, React, utils, models, Expression, componentMixins, controls, customControls, settingsMixins) {
    ...
    var SettingsTab = React.createClass({
        ...
        applyChanges: function() {
            ...
            _.each(dataToSave, function(section, sectionName) {
                if (section.metadata.group == 'network') {
                    dataToSave[sectionName] = initialSettings[sectionName];
                } else if (!section.metadata.group) {
                    // TODO: Remove this else after settings yaml reconfiguration
                    _.each(section, function(subSetting, subSettingName) {
                        if (subSetting.group == 'network') dataToSave[sectionName][subSettingName] = initialSettings[sectionName][subSettingName];
                    });
                }
            });
