        ...
        render: function() {
            ...
            _.each(settings.attributes, function(section, sectionName) {
                ...
                if (!isHidden) {
                    ...
                    if (group) {
                        groupedSettings[settings.sanitizesGroup(group)][sectionName] = {invalid: hasErrors};
                    } else {
                        ...
                        _.each(settingGroups, function(settingGroup) {
                            var calculatedGroup = settings.sanitizesGroup(settingGroup),
                                pickedSettings = _.chain(section)
                                    .keys()
                                    .without('metadata')
                                    .filter(function(settingName) {
                                        return settings.sanitizesGroup(section[settingName].group) == calculatedGroup &&
                                            !this.checkRestrictions('hide', settings.makePath(sectionName, settingName)).result && section[settingName].type != 'hidden';
                                    }, this)
                                    .value(),
                                hasErrors = _.any(pickedSettings, function(settingName) {
                                    return (errors || {})[settings.makePath(sectionName, settingName)];
                                });
                            if (!_.isEmpty(pickedSettings)) groupedSettings[calculatedGroup][sectionName] = {settings: pickedSettings, invalid: hasErrors};
                        }, this);
                    }
                }
            }, this);
    ...
    var SettingSection = React.createClass({
        ...
        render: function() {
            var group = this.props.settings.get(this.props.sectionName),
                ...
                sectionSettings = this.props.groupData.settings || _.chain(_.keys(group))
                    .without('metadata')
                    .value();
            if (_.isEmpty(sectionSettings)) return null;
            var sortedSettings = _.sortBy(sectionSettings, function(settingName) {return group[settingName].weight;}),
