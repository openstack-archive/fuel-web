...
function($, _, i18n, React, utils, models, Expression, componentMixins, controls, customControls) {
    ...
    var SettingsTab = React.createClass({
        ...
        checkGroupIsConsistent: function(group) {
            return _.contains(this.props.cluster.get('settings').predefinedGroupList, group) ? group : 'other';
        },
        render: function() {
            var cluster = this.props.cluster,
                settings = cluster.get('settings'),
            ...
            _.each(settings.getGroupList(), function(group) {
            ...
            _.each(settings.attributes, function(section, sectionName) {
                ...
                if (!isHidden) {
                    ...
                    if (group) {
                        groupedSettings[this.checkGroupIsConsistent(group)][sectionName] = {invalid: hasErrors};
                    } else {
                        var subGroupsDefined = _.chain(section).pluck('group').unique().value();
                        // Settings with undefined groups and subgroups go to 'Other' group
                        ...
                        // Settings like 'Common' can be splitted to different groups
                            ...
                            _.each(subGroupsDefined, function(subSettingGroup) {
                                subSettingGroup = this.checkGroupIsConsistent(subSettingGroup);
                                var pickedSettings = _.chain(_.keys(section))
                                    .without('metadata')
                                    .map(function(settingName) {
                                        if ((section[settingName].group == subSettingGroup || subSettingGroup == 'other' && !section[settingName].group || this.checkGroupIsConsistent(section[settingName].group) == 'other') &&
                                            !this.checkRestrictions('hide', settings.makePath(sectionName, settingName)).result) return settingName;
                                    }, this)
                                    .compact()
                                    .sortBy(function(settingName) {return section[settingName].weight;})
                                    .value();
                                if (!_.isEmpty(pickedSettings)) groupedSettings[subSettingGroup][sectionName] = {settings: pickedSettings};
                            }, this)
                        }
                    }
                }
                            ...
                            <div className={'col-xs-10 forms-box ' + groupName} key={groupName}>
                                {_.map(sortedSections, function(sectionName) {
                                    return <SettingSection
                                        ...
                                        cluster={this.props.cluster}
                                        groupName={groupName}
                                        groupData={selectedGroup[sectionName]}
        ...
        render: function() {
            return (
                <div className='col-xs-2'>
                    ...
                    {
                        this.props.settings.getGroupList().map(function(groupName) {
                            var settings = this.props.groupedSettings[groupName];
                            if (_.isEmpty(settings)) return null;
                            var hasErrors = _.any(_.pluck(settings, 'invalid'));
                            return (
                                <li
                                    key={groupName}
                                    role='presentation'
    ...
    var SettingSection = React.createClass({
        ...
        render: function() {
            var group = this.props.settings.get(this.props.sectionName),
                ...
                sortedSettings = this.props.groupData.settings || _.chain(_.keys(group))
                    .without('metadata')
