    ...
    var SettingsTab = React.createClass({
        ...
        getInitialState: function() {
            ...
            return {
                ...
                groupsList: settings.getGroupList(),
        ...
        checkGroupIsConsistent: function(group) {
            return _.contains(this.state.groupsList, group) ? group : 'other';
        },
        render: function() {
            var cluster = this.props.cluster,
                settings = cluster.get('settings'),
            ...
            _.each(settings.attributes, function(section, sectionName) {
                var group = section.metadata.group,
                    isHidden = this.checkRestrictions('hide', settings.makePath(sectionName, 'metadata')).result,
                    hasErrors = invalidSections[sectionName]
                if (group) {
                    group = this.checkGroupIsConsistent(group);
                    if (!groupedSettings[group]) groupedSettings[group] = {};
                    groupedSettings[group][sectionName] = {hidden: isHidden, invalid: hasErrors};
                } else {
                    ...
                    if (_.isEmpty(subGroupsDefined)) {
                        ...
                        groupedSettings.other[sectionName] = {hidden: isHidden, invalid: hasErrors};
                        ...
                        _.each(subGroupsDefined, function(group) {
                            ...
                            var pickedSettings = _.chain(_.keys(section))
                                ...
                                .map(function(settingName) {
                                    if ((section[settingName].group == group || group == 'other' && !section[settingName].group) &&
                                ...
                                .value();
                            if (!_.isEmpty(pickedSettings)) groupedSettings[group][sectionName] = {pickedSettings: pickedSettings};
                        }, this)
                    }
                }
            }, this);
            // Additional check: 'other' group can arrive
            return (
                        ...
                        return (
                            <div className={'col-xs-10 forms-box ' + groupName}>
                                {_.map(sortedSections, function(sectionName) {
                                    if (selectedGroup[sectionName].hidden) return null;
                ...
                <div className='col-xs-2'>
                    <CSSTransitionGroup component='ul' transitionName='subtab-item' className='nav nav-pills nav-stacked'>
                    {
                        this.props.groupsList.map(function(groupName) {
                            ...
                            if (_.isEmpty(settings) || _.all(_.pluck(settings, 'hidden'))) return null;
    ...
    var SettingSection = React.createClass({
        ...
        render: function() {
            var group = this.props.settings.get(this.props.sectionName),
                ...
                sortedSettings = this.props.groupData.pickedSettings || _.chain(_.keys(group))
                    .without('metadata')
