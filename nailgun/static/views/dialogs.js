...
export var NailgunUnavailabilityDialog = React.createClass({
  ...
  componentDidMount() {
    $(ReactDOM.findDOMNode(this)).on('shown.bs.modal', () =>
      $(ReactDOM.findDOMNode(this.refs['retry-button'])).focus());
  },
  startCountdown() {
...
export var DiscardNodeChangesDialog = React.createClass({
  ...
  renderFooter() {
    return ([
      ...
      <button key='discard' className='btn btn-danger'
        disabled={this.state.actionInProgress} onClick={this.discardNodeChanges}>
...
export var ProvisionVMsDialog = React.createClass({
  ...
  renderFooter() {
    return ([
      <button key='cancel' className='btn btn-default' onClick={this.close}
        disabled={this.state.actionInProgress}>{i18n('common.cancel_button')}</button>,
      <button key='provision' className='btn btn-success'
        disabled={this.state.actionInProgress} onClick={this.startProvisioning}>
...
export var StopDeploymentDialog = React.createClass({
  ...
  renderFooter() {
    return ([
      ...
      <button key='deploy' className='btn stop-deployment-btn btn-danger'
        disabled={this.state.actionInProgress} onClick={this.stopDeployment}>
...
export var ResetEnvironmentDialog = React.createClass({
  ...
  renderFooter() {
    return ([
      <button key='cancel' className='btn btn-default' disabled={this.state.actionInProgress}
        onClick={this.close}>
        {i18n('common.cancel_button')}
      </button>,
      <button
          ...
          {_.map(groups, (group, groupIndex) => {
            ...
            return (
              <div className='panel panel-default' key={group}>
                <div className='panel-heading' role='tab' id={'heading' + group}
                  onClick={this.toggle.bind(this, groupIndex)}
                >
                  <div className='panel-title'>
                    <div data-parent='#accordion' aria-expanded='true'
                ...
                <div className='panel-collapse collapse' role='tabpanel'
                  aria-labelledby={'heading' + group} ref={'togglable_' + groupIndex}>
                        ...
                        {_.map(groupEntries, (entry, entryIndex) => {
                          return (
                            <div className='nested-object'
                              key={'entry_' + groupIndex + entryIndex}>
                              {_.map(utils.sortEntryProperties(entry, sortOrder[group]),
                                (propertyName) => {
                                  if (!_.isObject(entry[propertyName])) {
                        ...
                        {!_.isEmpty(subEntries) &&
                          <div>
                            {_.map(subEntries, (subentry, subentrysIndex) => {
                              return (
                                <div className='nested-object' key={'subentries_' + groupIndex +
                                  subentrysIndex}>
      ...
      <div>
        {this.props.renderActionButtons && this.props.node.get('cluster') &&
          <div className='btn-group' role='group'>
            <button className='btn btn-default btn-edit-disks'
            ...
            <button className='btn btn-default btn-edit-networks'
...
export var DeleteNodesDialog = React.createClass({
  ...
  renderFooter() {
    return [
      <button key='cancel' className='btn btn-default'
        onClick={this.close}>{i18n('common.cancel_button')}
      </button>,
      <button key='delete' className='btn btn-danger btn-delete'
...
export var RegistrationDialog = React.createClass({
  ...
  renderFooter() {
    ...
    if (!this.state.loading) buttons.push(
      <button key='apply' className='btn btn-success' disabled={this.state.actionInProgress ||
          this.state.connectionError} onClick={this.validateRegistrationForm}>
...
export var RetrievePasswordDialog = React.createClass({
  ...
  renderFooter() {
    ...
    if (!this.state.loading) buttons.push(
      <button key='apply' className='btn btn-success' disabled={this.state.actionInProgress ||
          this.state.connectionError} onClick={this.retrievePassword}>
