...
function($, _, i18n, Backbone, React, utils, models, dispatcher, controls, componentMixins) {
    ...
    dialogs.NailgunUnavailabilityDialog = React.createClass({
        ...
        componentDidMount() {
            $(this.refs['retry-button'].getDOMNode()).focus();
        },
        startCountdown() {
        ...
        countdown: function() {
            ...
            currentDelay--;
            if (!currentDelay) {
                this.setState({currentDelay, actionInProgress: true}, this.reinitializeUI);
            } else {
                this.setState({currentDelay}, this.startCountdown);
            }
        },
        reinitializeUI() {
        ...
        retryNow() {
            ...
            this.setState({
                currentDelay: 0,
                currentDelayInterval: this.props.retryDelayIntervals[0],
            ...
            return (
                <div>
                    ...
                    {this.state.currentDelay ?
                        <p>{i18n('dialog.nailgun_unavailability.retry_delay_message', {count: this.state.currentDelay})}</p>
                    :
