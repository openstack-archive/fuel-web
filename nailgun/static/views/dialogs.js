...
export var ShowNodeInfoDialog = React.createClass({
  ...
  getInterfaceIp(networkName) {
    ...
    var networkData = _.find(node.get('network_data'),
      (network) => network.name === networkName
    );
    if (!networkData) return i18n('common.not_available');
    var interfaceData = _.find(node.get('meta').interfaces, (ifc) => ifc.name === networkData.dev);
