...
function($, _, i18n, Backbone, React, utils, models, dispatcher, controls, componentMixins) {
    ...
    dialogs.DiscardSettingsChangesDialog = React.createClass({
        ...
        renderBody: function() {
            var message = this.props.forbidDiscard ? 'no_discard_message' : this.props.isSavingPossible ? 'default_message' : 'no_saving_message';
            return (
                <div className='text-danger dismiss-settings-dialog'>
                    {this.renderImportantLabel()}
                    {i18n('dialog.dismiss_settings.' + message)}
                </div>
            );
        },
        renderFooter: function() {
            var buttons = [
                ...
                <button
                    ...
                    disabled={this.state.actionInProgress || this.props.forbidDiscard}
                >
