...
function($, _, i18n, React, Backbone, utils, models, componentMixins, dialogs, controls) {
            ...
            return (
                <div className='wizard-compute-pane'>
                    <ComponentCheckboxGroup
                        ...
                        onChange={this.props.onChange}
                    />
                    {this.hasErrors() && <div className='alert alert-warning'>{i18n('dialog.create_cluster_wizard.compute.empty_choise')}</div>}
                </div>
            );
        }
    });
    ...
    var Network = React.createClass({
        ...
        onChange: function(name, value) {
            ...
            if (!component.isML2Driver() && component.id != this.constructor.ml2core) {
            ...
            return (
                <div className='wizard-storage-pane'>
                    <div className='row'>
                        ...
                        <p><big>{i18n('dialog.create_cluster_wizard.storage.block')}</big></p>
                        {this.renderSection(this.components, 'block', this.props.onChange)}
                        </div>
                        <div className='col-xs-6'>
                            <p><big>{i18n('dialog.create_cluster_wizard.storage.object')}</big></p>
                            {this.renderSection(this.components, 'object', this.props.onChange)}
                        </div>
                    </div>
                    <div className='row'>
                        <div className='col-xs-6'>
                            <p><big>{i18n('dialog.create_cluster_wizard.storage.image')}</big></p>
                            {this.renderSection(this.components, 'image', this.props.onChange)}
                        </div>
                        <div className='col-xs-6'>
                            <p><big>{i18n('dialog.create_cluster_wizard.storage.ephemeral')}</big></p>
