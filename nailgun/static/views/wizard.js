...
var Network = React.createClass({
  ...
  componentWillMount() {
    var groups = _.groupBy(this.components, (component) => component.isML2Driver() ? 'ml2' : 'monolithic');
    this.monolithic = groups.monolithic;
    this.ml2 = groups.ml2;
  },
  updateRestrictions() {
    this.processRestrictions(this.monolithic, this.constructor.panesForRestrictions);
    this.processCompatible(this.props.allComponents, this.monolithic, this.constructor.panesForRestrictions, this.monolithic);
    this.selectActiveComponent(this.monolithic);

    this.processRestrictions(this.ml2, this.constructor.panesForRestrictions);
    this.processCompatible(this.props.allComponents, this.ml2, this.constructor.panesForRestrictions);
  },
  onChange(name, value) {
...
var Storage = React.createClass({
  ...
  updateRestrictions() {
    ...
    _.each(['block', 'object', 'image', 'ephemeral'], (subtype) => {
      var sectionComponents = _.filter(components, (component) => component.get('subtype') == subtype);
