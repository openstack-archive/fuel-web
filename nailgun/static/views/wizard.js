...
var ClusterWizardPanesMixin = {
  componentWillMount() {
    if (this.props.allComponents) {
      this.components = this.props.allComponents.getComponentsByType(this.constructor.componentType,
        {sorted: true});
    ...
    _.each(paneComponents, (component) => {
      ...
      _.each(incompatibles, (incompatible) => {
        ...
        var isInStopList = _.find(stopList, (component) =>
          component.id == incompatible.component.id);
...
var NameAndRelease = React.createClass({
  ...
  isValid() {
    ...
    var [name, cluster, clusters] = [wizard.get('name'), wizard.get('cluster'),
      wizard.get('clusters')];
  ...
  render() {
    ...
    var connectivityAlert = i18n('dialog.create_cluster_wizard.name_release.' + os +
      '_connectivity_alert');
    return (
  ...
  checkVCenter(allComponents) {
    ...
    if (!hasCompatibleBackends) {
      var vCenter = _.find(allComponents.models, (component) =>
        component.id == this.constructor.vCenterPath);
      vCenter.set({
        disabled: true,
...
var Network = React.createClass({
  ...
  renderMonolithicDriverControls() {
    ...
    if (!hasMl2) {
      monolithic = _.filter(monolithic,
        (component) => component.id != this.constructor.ml2CorePath);
    }
    this.processRestrictions(monolithic, this.constructor.panesForRestrictions);
    this.processCompatible(this.props.allComponents, monolithic,
      this.constructor.panesForRestrictions, monolithic);
...
var Storage = React.createClass({
  ...
  renderSection(components, type) {
    ...
    this.processRestrictions(sectionComponents, this.constructor.panesForRestrictions, (isRadio ?
      sectionComponents : []));
  ...
  render() {
    ...
    this.processCompatible(this.props.allComponents, this.components,
  ...
  onChange(name, value) {
    ...
    switch (name) {
      ...
      default:
        ...
        var panesToRestore = this.getListOfTypesToRestore(this.state.activePaneIndex,
  ...
  renderFooter() {
    ...
    return (
      <div className='wizard-footer'>
        <button className={utils.classNames('btn btn-default pull-left',
          {disabled: actionInProgress})} data-dismiss='modal'>
          {i18n('common.cancel_button')}
        </button>
        <button
          className={utils.classNames('btn btn-default prev-pane-btn',
        ...
        {this.state.nextVisible &&
          <button
            className={utils.classNames('btn btn-default btn-success next-pane-btn',
        ...
        {this.state.createVisible &&
          <button
            className={utils.classNames('btn btn-default btn-success finish-btn',
