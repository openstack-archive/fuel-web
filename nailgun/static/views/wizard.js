...
function($, _, i18n, React, Backbone, utils, models, componentMixins, dialogs, controls) {
    ...
    var Network = React.createClass({
        ...
        renderMonolithicControls: function() {
            ...
            if (!hasMl2) {
                monolith = _.filter(monolith, (component) => component.id != this.constructor.ml2CorePath);
            }
            this.processRestrictions(monolith, this.constructor.panesForRestrictions);
            this.processCompatible(this.props.allComponents, monolith, this.constructor.panesForRestrictions, monolith);
            this.selectActiveComponent(monolith);
            return (
                <ComponentRadioGroup
                    groupName='network'
                    components={monolith}
                ...
                <div className='wizard-storage-pane'>
                    <div className='row'>
                        <div className='col-xs-6'>
                            <div className='pane-title'>{i18n('dialog.create_cluster_wizard.storage.block')}</div>
                            {this.renderSection(this.components, 'block', this.props.onChange)}
                        </div>
                        <div className='col-xs-6'>
                            <div className='pane-title'>{i18n('dialog.create_cluster_wizard.storage.object')}</div>
                            {this.renderSection(this.components, 'object', this.props.onChange)}
                        </div>
                    </div>
                    <div className='row'>
                        <div className='col-xs-6'>
                            <div className='pane-title'>{i18n('dialog.create_cluster_wizard.storage.image')}</div>
                            {this.renderSection(this.components, 'image', this.props.onChange)}
                        </div>
                        <div className='col-xs-6'>
                            <div className='pane-title'>{i18n('dialog.create_cluster_wizard.storage.ephemeral')}</div>
                    ...
                    <div className='wizard-steps-box'>
                        ...
                        {!this.components &&
                            <div className='pane-content col-xs-9 pane-progress-bar'>
                                <div className='pane-title'>{i18n('dialog.create_cluster_wizard.loading_components')}</div>
