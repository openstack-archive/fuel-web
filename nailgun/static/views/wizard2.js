...
function($, _, i18n, React, Backbone, utils, models, componentMixins, dialogs, controls) {
    ...
    class ComponentModel extends models.BaseModel {
        ...
        isMl2Driver() {
            return !!this.id.match(/^network:neutron:core:ml2:(.*)$/);
        }
    }
    ...
    var Network = React.createClass({
        ...
        getInitialState: function() {
            return {
                activeComponent: 'xxx.xxx.xxx.xxx'
            }
        },
        onChange: function(name, value) {
            ...
            this.props.onChange(this.currentSelection, false);
        ...
        processNetworks: function(networks) {
            ...
            if (result.ml2.length > 0) {
                var ml2 = new ComponentModel({
                    type: 'network',
                    name: 'network:neutron:ml2',
                    label: 'Neutron ML2',
                    description: 'Neutron ML2'
                });
                result.monolith.push(ml2);
            }
            return result;
        },
        render: function() {
                ...
                <div className='wizard-network-pane'>
                    {
                        _.map(networks.monolith, (network) => {
                            var isEnabled = !!network.get('enabled');
                            if (isEnabled) {
                                this.currentSelection = network.id;
                            }
                            return (
                                <controls.Input
                                    type='radio'
                                    ...
                                    onChange={() => this.onChange(network.id, true)}
                                />
                            );
                        })
                    }
        ...
        onChange: function(name, value) {
            ...
            switch (name) {
                ...
                case 'release':
                    ...
                    break;
                default:
                    console.log('No default handler yet');
                    break;
            }
