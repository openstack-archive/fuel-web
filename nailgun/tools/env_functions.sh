...
cleanup_database_role() {
    ...
    psql -h ${DB_ROOT_HOST} -p ${DB_ROOT_PORT} -U ${DB_ROOT} -c "DROP ROLE IF EXISTS ${NAILGUN_DB_USER}"
}

cleanup_database() {
    # requires pgpass
    echo "Terminating database activities for ${NAILGUN_DB} if there are any"
    psql -h ${DB_ROOT_HOST} -p ${DB_ROOT_PORT} -U ${DB_ROOT} \
    ...
    psql -h ${DB_ROOT_HOST} -p ${DB_ROOT_PORT} -U ${DB_ROOT} -c "DROP DATABASE IF EXISTS ${NAILGUN_DB}"
}

prepare_database_role() {
    # requires pgpass
    echo "Trying to find out if role ${NAILGUN_DB_USER} exists"
    local roles=$(psql -h ${DB_ROOT_HOST} -p ${DB_ROOT_PORT} -U ${DB_ROOT} -t -c "SELECT 'HERE' from pg_roles where rolname='${NAILGUN_DB_USER}'")
    if [[ ${roles} == *HERE ]]; then
        echo "Role ${NAILGUN_DB_USER} exists. Setting password ${NAILGUN_DB_PW}"
        psql -h ${DB_ROOT_HOST} -p ${DB_ROOT_PORT} -U ${DB_ROOT} -c "ALTER ROLE ${NAILGUN_DB_USER} WITH SUPERUSER LOGIN PASSWORD '${NAILGUN_DB_PW}'"
    else
        echo "Creating role ${NAILGUN_DB_USER} with password ${NAILGUN_DB_PW}"
        psql -h ${DB_ROOT_HOST} -p ${DB_ROOT_PORT} -U ${DB_ROOT} -c "CREATE ROLE ${NAILGUN_DB_USER} WITH SUPERUSER LOGIN PASSWORD '${NAILGUN_DB_PW}'"
    fi
}
...
prepare_database() {
    ...
    local databases=$(psql -h ${DB_ROOT_HOST} -p ${DB_ROOT_PORT} -U ${DB_ROOT} -t -c "SELECT 'HERE' from pg_database where datname='${NAILGUN_DB}'")
    if [[ ${databases} == *HERE ]]; then
        echo "Database ${NAILGUN_DB} exists"
    else
        echo "Creating database ${NAILGUN_DB}"
        psql -h ${DB_ROOT_HOST} -p ${DB_ROOT_PORT} -U ${DB_ROOT} -c "CREATE DATABASE ${NAILGUN_DB} OWNER ${NAILGUN_DB_USER}"
    fi
}
