...
module Naily
  class Dispatcher
    ...
    def stop_deploy_task(data, service_data)
      ...
      task_uuid = data['args']['task_uuid']
      nodes = data['args']['nodes']
      tasks_queue = service_data[:tasks_queue]
      return unless task_in_queue?(target_task_uuid, tasks_queue)

      reporter = Naily::Reporter.new(@producer, data['respond_to'], task_uuid)
      result = nil

      Naily.logger.debug("Cancel task #{target_task_uuid}. Start")
      if target_task_uuid == tasks_queue.current_task_id
        ...
        tasks_queue.replace_task(target_task_uuid, new_task_data)
      end
      report_result(result, reporter)
    end
