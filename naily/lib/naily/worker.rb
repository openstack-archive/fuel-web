  ...
  private
    ...
    def run_server
      ...
      AMQP.connect(connection_options) do |connection|
        ...
        @cancel_channel = create_channel(@connection, prefetch=false)
        @cancel_exchange = @cancel_channel.topic(Naily.config.broker_cancel_queue, :auto_delete => true)

        @producer = Naily::Producer.new(@exchange)
        @delegate = Naily.config.delegate || Naily::Dispatcher.new(@producer)
        @server = Naily::Server.new(@channel, @exchange, @delegate, @producer, @cancel_channel, @cancel_exchange)
