...
class TestMulticastVerification(unittest.TestCase):
    ...
    def test_multicast_verification(self):
        ...
        self.assertEqual(info_node_112, ["112", "113"])
        self.assertEqual(info_node_113, ["112", "113"])


class TestFullMulticastVerification(unittest.TestCase):

    def setUp(self):
        self.config = {"uid": "112", "group": "225.0.0.250",
                       "port": 8890, 'timeout': 3}
        self.mchecker = multicast_api.MulticastChecker(**self.config)
        self.api = api.Api('multicast', self.config)

    def test_multicast_verification_with_detach(self):

        listen_response = self.api.listen()
        self.assertEqual(listen_response, consts.READY)

        sender_response = self.api.send()
        self.assertEqual(sender_response, consts.READY)

        get_info_response = self.api.get_info()
        self.assertEqual(get_info_response, '["112"]')
