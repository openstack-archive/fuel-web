    ...
    def setup(self):
        ...
        if self.is_present is False:
            rc, out, err = execute(['ip', 'a', 'add', self.addr,
                                    'dev', self.iface])
            if rc:
                msg = 'Cant add address {0} to {1}. Err: {2}'.format(
...
class Route(object):
    ...
    def setup(self):
        ...
        if rc:
            msg = 'Cant add default gateway {0} on iface {1}. Err: {2}'.format(
                self.gateway, self.iface, err)
            raise CommandFailed(msg)
