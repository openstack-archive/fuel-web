...
class CephRolePlugin(StoragePlugin):
    ...
    def process_cluster_attributes(self, cluster_id, attributes):
        if 'storage' not in attributes["editable"]:
            return attributes
    ...
    def process_volumes(self, volumes, disk_id, node_id):
        check_node_role = self.storage.search_records(
            ...
            with self.storage as storage:
                for volume in volumes:
                    if volume.get("plugin_name") == self.plugin_name:
                        existing = storage.search_records(
                            "volume",
                            node_id=node_id,
                            disk_id=disk_id
                        )
                        for e in existing:
                            storage.drop_record(e.id)
                        storage.add_record(
                            "volume",
                            {
                                "node_id": node_id,
