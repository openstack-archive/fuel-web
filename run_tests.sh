...
function run_nailgun_tests {
  ...
  # run tests
  tox -epy27 -- -vv $testropts $TESTS --xunit-file $NAILGUN_XUNIT || result=1
  result=$?
  popd >> /dev/null
  return $result
}

function run_nailgun_flake8 {
  pushd $ROOT/nailgun >> /dev/null
  tox -epep8 || result=1
  result=$?
  popd >> /dev/null
...
function run_cli_tests {
  ...
  if [ $pid -ne 0 ]; then
    ...
    # run tests
    tox -epy27 -- -vv $testropts $TESTS --xunit-file $FUELCLIENT_XUNIT || result=1
    result=$?
    popd >> /dev/null
...
function run_upgrade_system_tests {
  ...
  if [ $# -ne 0 ]; then
    ...
    tox -epy27 -- -vv $testropts $DOWNLOADER_TESTS --xunit-file $FUELUPGRADEDOWNLOADER_XUNIT || result=1
    popd >> /dev/null
...
function run_shotgun_tests {
  local TESTS="$ROOT/shotgun/shotgun/tests/"
