...
function usage {
  ...
  echo "  -N, --no-nailgun            Don't run NAILGUN unit and integration tests"
  echo "  -p, --performance           Run NAILGUN performance tests"
  echo "  -P, --no-performance        Don't run NAILGUN performance tests" 
  echo "  -f, --flake8                Run FLAKE8 and HACKING compliance check"
  echo "  -F, --no-flake8             Don't run static code checks"
...
function process_options {
  for arg in $@; do
    case "$arg" in
      ...
      -p|--performance) performance_tests=1;;
      -P|--no-performance) no_performance_tests=1;;
      ...
      -f|--flake8) flake8_checks=1;;
      -F|--no-flake8) no_flake8_checks=1;;
