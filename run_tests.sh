...
function run_tests {
  ...
  if [[ $nailgun_tests -eq 0 && $webui_tests -eq 0 && $cli_tests -eq 0 \
    ...
    if [ $no_nailgun_tests -ne 1 ]; then
      nailgun_tests=1
    fi

    if [ $no_webui_tests -ne 1 ]; then
      webui_tests=1
    fi

    if [ $no_cli_tests -ne 1 ]; then
      cli_tests=1
    fi

    if [ $no_flake8_checks -ne 1 ]; then
      flake8_checks=1
    fi

    if [ $no_jslint_checks -ne 1 ]; then
      jslint_checks=1
    fi
  fi
...
function run_nailgun_tests {
  local TESTS_DIR="$ROOT/nailgun/nailgun/test"
  ...
  # make tests
  local output=$($TESTRTESTS -vv $testropts $testrargs $TESTS_DIR 3>&1 1>&2 2>&3 | tee /dev/stderr)
  local result=$?
...
function run_webui_tests {
  ...
  local UI_TESTS=$ROOT/nailgun/ui_tests
  ...
  if [ $pid -ne 0 ]; then
    for testcase in $UI_TESTS/test_*.js; do
      dropdb
      syncdb true

      ${CASPERJS} test --includes="$UI_TESTS/helpers.js" --fail-fast "$testcase"
      if [ $? -ne 0 ]; then
        result=1
        break
      fi
...
function run_cli_tests {
  ...
  local TESTS_DIR=$ROOT/fuelclient/tests
  ...
  if [ $pid -ne 0 ]; then
    ...
    ${TESTRTESTS} -vv $testropts $testrargs $TESTS_DIR
    if [ $? -ne 0 ]; then
      result=1
      break
    fi
...
function run_server {
  ...
  local pid=`ps aux | grep "$RUN_SERVER" | grep -v grep | awk '{ print $2 }'`
  if [ -n "$pid" ]; then
    kill $pid
    sleep 5
  fi
