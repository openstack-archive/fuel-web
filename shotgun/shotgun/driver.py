...
class File(Driver):
    ...
    def snapshot(self):
        ...
        if self.exclude:
            utils.remove_matched_files(self.target_path, self.exclude)


class Dir(File):
    def __init__(self, data, conf):
        super(Dir, self).__init__(data, conf)
        self.exclude_subdirs = self.data.get('exclude_subdirs', [])
    ...
    def snapshot(self):
        ...
        self.path IS /var/log/somedir
        self.target_path IS /target/host.domain.tld/var/log
        self.full_dst_path IS /target/host.domain.tld/var/log/somedir
        self.exclude_subdirs IS ['subdir1', '/subdir2/something/']
        """
        super(Dir, self).snapshot()

        if self.full_dst_path:
            utils.remove_sub_dirs(self.full_dst_path, self.exclude_subdirs)
