...
class PuppetAction(action.Action):
    ...
    def puppet_resources(self):
        ...
        to view report and profiling data intercatively
        :rtype: dict
    ...
    def run(self):
        ...
        log.debug(
            "Task '%s' with command '%s' returned code '%s' out: '%s' err: '%s'",
            self.task.name,
            self.command,
            self.exit_code,
            self.stdout,
    ...
    def filter_useless_resources(cls, resource_title):
        """
        Filter function. Used to remove
        useless resources from the report
    ...
    def puppet_options(self):
        """
        Puppet options from global configuration
        or from task configuration
        :rtype: str
