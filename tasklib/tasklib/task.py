...
class Task(object):
    ...
    def __init__(self, task_name, config):
        self.config = config
        self.name = task_name
        self.dir = os.path.join(config['library_dir'], self.name)
        self.file = os.path.join(self.dir, config['task_file'])
        self.pid_dir = os.path.join(self.config['pid_dir'], self.name)
        self.report_dir = os.path.join(self.config['report_dir'], self.name)
